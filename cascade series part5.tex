...
The DESI DR2 BAO fit gives $\chi^2/n = 1.68$ vs.\ Planck $\Lambda$CDM at $\chi^2/n = 1.90$.
\end{abstract}
...
\textbf{Step 2} ([4], item~8; [2], Remark~6.5). The Bott partition classifies cascade layers
...
With the cascade's parameter set, $\ell_A = 297.6$. Deviation from Planck ($301.6$): $-1.34\%$.
\end{theorem}

\begin{remark}[Sign and magnitude]
The $-1.34\%$ deviation is negative, placing $\ell_A$ in the descent-dependent population
(mean $-1.24\%$). A single subleading correction---the Gram eigenvalue deficit---closes
all descent-dependent deviations simultaneously; $\ell_A$ requires a $+1.3\%$ shift,
consistent with the universal correction.
Note: $\ell_A = 297.6$ was computed with the previous cascade parameter set
$(H_0 = 70.65, \Omega_m = 0.31150)$; recomputation with $(H_0 = 71.05, \Omega_m = 1/\pi)$
is a revision item.
\end{remark}
...
\begin{remark}[Note on numerical values]
The $\chi^2$ values and individual BAO pulls below were computed with the previous cascade
parameter set $(H_0 = 70.65, \Omega_m = 0.31150, r_d = 142.3~{\rm Mpc})$. Recomputation
with $(H_0 = 71.05, \Omega_m = 1/\pi, r_d \approx 141~{\rm Mpc})$ is a revision item.
The qualitative conclusion---that the cascade's smaller $r_d$ gives a competitive fit to
DESI with $w = -1$---is robust.
\end{remark}
...
\hline
$z_{\rm eff}$ & Type & DESI DR2 & $\sigma$ & Cascade & Pull$_{\rm cas}$ \\
\hline
...
\end{tabular}
\end{center}
\noindent$^\dagger$: The $z = 0.510$, $D_H/r_d$ bin is a $3.23\sigma$ outlier shared by both
models; see Remark~\ref{rem:outlier}.
...
$\chi^2/n = 1.68$, compared to Planck $\Lambda$CDM at $\chi^2/n = 1.90$
($\Delta\chi^2 = 2.87$ in the cascade's favour). The apparent preference for dynamical
...
The bin $z_{\rm eff} = 0.510$, $D_H/r_d$, shows a $3.23\sigma$ pull in the cascade and
$3.60\sigma$ in Planck $\Lambda$CDM. This single bin contributes $\approx 9.0$ of the
cascade's total $\chi^2 = 21.8$, and $\approx 11.6$ of Planck's total 24.7. Without it,
...
\item \textbf{The $\chi^2$ recomputation.} The BAO comparison table was computed with
$(H_0 = 70.65, \Omega_m = 0.31150)$; updated values with $(H_0 = 71.05, \Omega_m = 1/\pi)$
are a revision item.
\end{itemize}
...
\begin{enumerate}
\item \textbf{Recompute DESI $\chi^2$ with updated parameters.} Full recomputation with
$(H_0 = 71.05, \Omega_m = 1/\pi, r_d \approx 141~{\rm Mpc})$ required.
...
\textit{Descent-dependent quantities} carry negative deviations: $\alpha_s$ ($-1.7\%$), $v$
($-2.2\%$), mass ratios, $m_\tau$ ($-1.2\%$), $m_W$ ($-0.6\%$), $\Omega_m^{\rm Bott} = 0.31150$
($-1.1\%$), $\ell_A$ ($-1.3\%$). \textit{Geometric quantities} carry positive deviations:
$m_H/m_W$ ($+0.8\%$), $\sin^2\theta_W$ ($+0.6\%$), $\theta_C$ ($+1.7\%$), $\Omega_b$ ($+2.8\%$),
and the leading-order lapse identity $\Omega_m = 1/\pi$ ($+1.1\%$), $\Omega_r$ ($+0.1\%$),
$T_{\rm CMB}$ ($+0.2\%$). The $1/\pi$ identity is a single-step algebraic result (geometric);
the Bott fraction is the multi-layer descent result (descent-dependent). They are successive
approximations. $\Lambda$ at $+0.04\%$ belongs to neither population (threshold quantity).
