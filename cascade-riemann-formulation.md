...
"source-selection flags" — now gated with per-token adjudication, the
per-item locations gated round 95: sources, constants, and dynamics, not
labels]** (the lone "19" is "Wall 1964"; A2's
...
state space is the lattice; all four of the kernel's T1 definitions are
anchored verbatim (the α anchor added round 95) with their positive
argument image an exhibit of the anchored offsets (round-94 F3 replaced
conjuncts that could not fail), while **both faces of the given evaluate
Γ_ℝ at negative arguments** (the d = 7 mirror weight gated) — no axiom
...
multiplicity 3 and per-leg occupancy remain instantiation. *(Net-state, Theorem 1w of the
review paper: the multiplicity is entailed given the pairing-act plus this theorem's own
root–unit identity — μ₆ is the A₂ root system by rank-2 classification, N(N−1) = 6 forcing
N = 3 — so the instantiation residue narrows to per-leg occupancy only.)*
...
| 4 | S5 lemma: root-frame projection | **derived** (T8, Addendum 36): frame = trace-dual lattice (inverse different of ℚ(ζ₃)); value = 30° rotation of ℤ[ω], unique to disc −3 | colour count 3 and per-leg occupancy remain instantiation *(net-state, T1w of the review paper: the count is entailed given the pairing-act plus T8's root–unit identity — μ₆ = A₂, N(N−1) = 6 ⟹ N = 3; residue narrows to per-leg occupancy)* |
