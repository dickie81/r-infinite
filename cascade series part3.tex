...
II [2] & Complex state space, propagator $e^{-iHt}$ & Gleason (Born rule) \\
...
classification ([2], Remark 6.5) establishes the partition topologically, and the fermion
...
background cosmological observables. The density fractions $\Omega_\Lambda = 1 - \Omega_m = 0.68850$, $\Omega_m = 0.31150$
(from the Bott partition of cascade layers; see [3] and Corollary 4.8a of [1]), and $\Omega_b =
1/(2\pi^2)$ follow from the cascade's sphere-area structure. The Hubble constant $H_0 = 70.65$~km/s/Mpc follows from the lapse-corrected Friedmann equation with $N(4) = 3\pi/8$, sitting
between the Planck measurement (67.4) and the SH0ES value (73.0). These parameters
predict a sound horizon $r_d = 142.3$~Mpc. The apparent DESI preference for $w \neq -1$ is
a zero-parameter consequence of imposing Planck's sound horizon $r_d = 147.6$~Mpc on a
universe where $r_d = 142.3$~Mpc: forcing the wrong ruler onto cascade distances produces
apparent $w \approx -0.78$, matching DESI's BAO+CMB-only report of $-0.76 \pm 0.11$ within $1\sigma$.
With the cascade's ruler, DESI BAO is consistent with $w = -1$ throughout.
\end{remark}
...
\hline
Complex QM, Born rule, $\hbar$ & Complex structure, propagator & Gleason, [2] \\
...
three inputs: the propagator phase classification ([2], Remark 6.5), which establishes the
...
\item \textbf{Unitary dynamics.} The Schr\"odinger equation ([2]) preserves norms. Each slicing step
is a unitary operation on the Hilbert space.
