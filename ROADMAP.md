...
**Near-term sub-target — close the m_μ/m_e residual via cascade-native 1-loop QED.** Part IVb's `oq:mu-e-residual` documents a +0.132% leading-order discrepancy on m_μ/m_e (cascade 206.50 vs PDG 206.7683). Numerical observation: α_em/(2π) ≈ 0.116% at the cascade-derived α_em = 1/137.028, and the cascade structural prefactor at the chirality-selection-rule m−k=0 slot is exactly 1/(N(0)·Γ(1/2)²) = 1/(2π) per Remark `rem:two-pi-prefactor-identification`. If the cascade's higher-loop machinery produces a +α_em/(2π) shift on m_μ/m_e at the m=1, k=1 slot (closed photon loop + chirality-projected external mass insertion), it closes ~88% of the residual. The remaining ~0.016% would sit at α_em²/π² level, consistent with 2-loop in the structural-prefactor reading. Caveats: in standard QED the on-shell mass-ratio is RG-invariant (both leptons have same anomalous dimension) so the symmetric 1-loop self-energy cancels in the ratio; the cascade's chirality-selection-rule could give an asymmetric correction if the per-Dirac-layer structure attaches to the descent path (m_μ descends from d=21 vs m_e from d=13, crossing one extra Dirac layer at d=21). Concrete cascade-native test: compute the m=1, k=1 slot's contribution to m_μ/m_e under (a) symmetric (cancels), (b) per-Dirac-layer-crossed (asymmetric, gives one extra α_em/(2π)), (c) per-source α(d_f) (asymmetric, gives wrong magnitude/sign). Reading (b) is the natural structural candidate. Tool: `tools/research/cascade_mu_e_radiative_attempt.py`.
