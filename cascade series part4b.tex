...
\begin{theorem}[Topological obstruction factor]
The fermion-to-scalar propagator ratio at each Dirac layer is $1/(2\sqrt{\pi})$, independent of $d$. The numerical evidence is decisive (0.13\% match, with $R'(d)$ excluded at 61--69\%); the structural decomposition below identifies the two factors; a derivation from the Dirac operator spectrum on the cascade geometry remains open (Section 11, Question 1).
\end{theorem}

The factor decomposes as: (a) $\sqrt{\pi} = \Gamma(\tfrac{1}{2})$: the cascade's unique dimension-independent constant (Paper I, Theorem 3.1), forced by orthogonality. (b) $2 = \chi(S^{2n})$: the Euler characteristic of any even-dimensional sphere, equal to the number of chirality channels at the Dirac junction.
...
\begin{remark}[Universality from the data]
...
The cascade contains both irreversibility (slicing suppresses information about the integrated-out direction) and unitarity (the Schr\"odinger equation from [2]). These describe different levels: the full geometry is unitary; every junction looks irreversible from below. There is no paradox because there was never a sharp boundary---only asymptotic compactification.

\section{The Hubble Tension and Sound Horizon}

The cascade's geometric parameters give a Hubble constant $H_0 = 70.65$~km/s/Mpc from the lapse-corrected Friedmann equation with $N(4) = 3\pi/8$, sitting between the Planck measurement (67.4) and the SH0ES distance-ladder value (73.0). The Hubble time $1/H_0 = 13.84$~Gyr matches the observed age of the universe to $0.3\%$.

The matter fraction $\Omega_m = 0.31150$ is derived from the sphere-area fraction of non-trivial-phase cascade layers ($d \bmod 8 \in \{5, 6\}$) in the full cascade tower, using the Bott partition of [4] and the sphere-area primacy of [1], Corollary 4.8a. The baryon fraction $\Omega_b = 1/(2\pi^2) = 0.0507$ follows from the observer's boundary area $\Omega_3 = 2\pi^2$.

The cascade parameter triple $(H_0 = 70.65, \Omega_m = 0.31150, \Omega_b = 0.0507)$ predicts a sound horizon $r_d = 142.3$~Mpc from first principles, 3.6\% smaller than the Planck-inferred $r_d = 147.6$~Mpc. With the cascade's ruler, the DESI DR2 BAO observations are consistent with $w = -1$ throughout (established in [3], Theorem 6.1). The apparent DESI preference for $w \neq -1$ arises when Planck's $r_d = 147.6$~Mpc is imposed as a prior: forcing the wrong ruler onto cascade distances produces apparent $w \approx -0.78$, matching DESI's BAO+CMB-only report of $-0.76 \pm 0.11$ within $1\sigma$, as a zero-parameter consequence.
...
Hubble tension & $H_0 = 70.65$, $r_d = 142.3$~Mpc & --- & 1 \\
\hline
\end{tabular}
\end{center}
...
\item Derive why the topological obstruction is exactly $\Gamma(\tfrac{1}{2})$ from the Dirac operator spectrum.
\end{itemize}

\section{Open Questions}

\begin{enumerate}
\item Rigour of the topological obstruction factor. A fully rigorous derivation would compute the fermion propagator on the cascade geometry at the hairy ball zero from first principles, confirming that exactly $\Gamma(\tfrac{1}{2})$ emerges without $R'(d)$.
...
\textbf{Two-population systematic.} The cascade's predictions separate into two populations with distinct error signatures. \textit{Descent-dependent} quantities---those computed via the cascade potential $\Phi(d)$ or lapse $N(d)$---carry uniformly negative deviations: $\alpha_s$ ($-1.7\%$), $v$ ($-2.2\%$), $\Omega_m$ ($-1.1\%$), $m_\tau/m_\mu$ ($-1.7\%$), $m_\mu/m_e$ ($-0.13\%$), $m_\tau$ ($-1.2\%$), $m_W$ ($-0.6\%$). All seven are negative, with mean $-1.24\%$ and scatter $0.71\%$; the probability of 7/7 same sign by chance is $(1/2)^7 = 0.8\%$. \textit{Geometric} quantities---determined by sphere topology, geodesics, or Bott structure---carry positive deviations: $m_H/m_W$ ($+0.8\%$), $\sin^2\theta_W$ ($+0.6\%$), $\theta_C$ ($+1.7\%$), $\Omega_b$ ($+2.8\%$). All four are positive. The clean sign separation identifies the subleading cascade descent from $d_2 = 217$ to $d = 4$ as the source of the dominant systematic: it affects the descent-dependent quantities uniformly and leaves the geometric quantities untouched. The single quantity $\Lambda$ at $+0.04\%$ is indistinguishable from zero and belongs to neither population (it is a threshold quantity, not a descended one).

\textbf{Tier 1: Theorem-level results.} (a) The geometric-topological factorization, with $R'(d)$ excluded at 61--69\%. (b) $1/\alpha_{\text{GUT}} = 4\pi/N(12)^2 = 25.02$ from the Gamma function. (c) Adams' theorem gives $N_c = 3$ and $\dim\,\text{U}(1) = 1$. (d) The hairy ball theorem gives SU(3) $\times$ U(1) unbroken, SU(2) broken. (e) Three generations from Bott periodicity + the $d_1 = 19$ phase transition. (f) $w = -1$ exactly, with GB corrections vanishing by two independent mechanisms ([3]).
