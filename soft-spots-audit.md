...
### SP-21. Theorem 10.2 proof conflates "Lorentzian signature" with "oscillatory unitary evolution" — **Structural** — ✅ **FIXED (acknowledged as cascade-motivated Wick rotation)**
...
### SP-22. §4.4 asserts unit lapse $g_{tt}=-1$ while §4.2 gives cascade lapse $N(4)=3\pi/8$ — **Structural** — ✅ **FIXED (cascade-lapse vs metric-lapse distinction articulated)**
...
| ~~SP-21~~ | ~~Lorentzian-sig = oscillatory-evolution "by definition"~~ | Part III | ✅ Fixed (Wick rotation acknowledged; remark `rem:wick-rotation-cascade`) |
| ~~SP-22~~ | ~~Unit-lapse vs cascade-lapse $N(4)$ tension~~ | Part III | ✅ Fixed (Remark `rem:cascade-vs-metric-lapse`) |
...
- **Load-bearing items by claim:**
  ...
  - ~~SP-21 underwrites Lorentzian signature.~~ ✅ Closed: Wick rotation acknowledged as the Euclidean-to-Lorentzian map, cascade-motivated by the forced precession; Theorem 10.2 proof rewritten honestly, Remark `rem:wick-rotation-cascade` explains the cascade-QFT distinction in motivation.
...
### Commit (SP-22: cascade lapse vs metric lapse distinction articulated)

**SP-22 — Closed.** Part III §4.2 introduced the cascade lapse $N(d) = \sqrt{\pi}\,R(d)$ with $N(4) = 3\pi/8 \approx 1.178$; §4.4 wrote the 4D observer's metric as $ds^2 = -dt^2 + a(t)^2\,d\sigma_3^2$ with metric lapse $g_{tt} = -1$. Both objects used the word "lapse" but refer to different mathematical quantities. The reconciliation wasn't stated.

Resolution: the two "lapses" are **distinct objects** that happen to share a word. Cascade lapse $N(d)$ is a dimensionless ratio of adjacent unit-ball volumes, indexed by cascade dimension. ADM/metric lapse is a spacetime function converting coordinate time to proper time. The 4D observer's metric uses unit metric lapse by the standard FRW choice of proper-time coordinate; the cascade lapse doesn't enter the 4D metric at all.

Changes to `src/cascade-series-part3.tex`:

1. **§4.2 renamed** "The lapse function" → "The cascade lapse function", with a clarifying paragraph appended: *"The word 'lapse' is used here in its cascade-internal sense: $N(d)$ is a dimensionless ratio ... It is not the lapse function of an ADM metric foliation."* Forward reference to Remark `rem:cascade-vs-metric-lapse`.

2. **§4.4 clarified.** The FRW metric is now labelled as using unit *metric* lapse (observer's proper-time coordinate). An explicit sentence notes that the cascade lapse $N(4) = 3\pi/8$ does not enter the metric, with forward reference to the new remark.

3. **New Remark `rem:cascade-vs-metric-lapse`** added at the end of §4.4. The remark:
   - Distinguishes cascade lapse (dimensionless, layer-indexed, about content compression) from metric lapse (spacetime function, about time-coordinate conversion).
   - Notes the 4D metric's unit lapse is a proper-time coordinate choice.
   - Documents the retracted Paper V attempt that conflated the two (identifying $N(4)$ as the ADM lapse of a cascade-time frame and using $N(4)^2$ as a frame-conversion factor). Cites Paper V Section 6's "Why the observer bridge and not a descent lapse" retraction explicitly.
   - Preserves Paper II's identification of cascade slicing with physical time: the cascade coordinate $d$ maps to the observer's proper time $t$, but the cascade lapse $N(d)$ does not map to the metric lapse of $t$. *"They share a word, not a referent."*

**What this buys.**

- Internal tension between §4.2 and §4.4 is resolved by an explicit distinction rather than left implicit.
- Paper V's retracted derivation is now cited from within Part III, making the fix traceable across papers. A reader who wonders "why didn't the cascade lapse enter the 4D metric?" gets a direct answer with a cross-reference.
- The word "lapse" is now disambiguated at first use in Part III (§4.2's paragraph), preventing the conflation that led to Paper V's retracted approach.
- Paper II's identification of cascade slicing with physical time is preserved; what's corrected is the identification of cascade *lapse* with metric *lapse*, which never followed from Paper II.

**What this does not claim.**

The remark doesn't introduce new mathematical content; the 4D metric is exactly as before, and the cascade lapse $N(d)$ is exactly as before. What changes is the textual relationship between the two in Part III — from silent conflation (with downstream retraction in Paper V) to explicit distinction (stated in Part III where the tension first arises).

Cost: ~50 lines added to Part III §4 (one clarifying paragraph in §4.2, one in §4.4, one full Remark after §4.4). Zero numerical predictions change; zero theorems modified; Paper V's existing retraction remark is cited but not altered.

### Commit (SP-21: Lorentzian signature via cascade-motivated Wick rotation)

**SP-21 — Closed.** Theorem 10.2's proof previously opened: *"Lorentzian signature means $g_{tt}<0$. This is equivalent, **by definition**, to time evolution being generated by a self-adjoint Hamiltonian..."*. The "by definition" is wrong — metric signature is a property of the metric tensor, independent of whether a Hamiltonian exists. The following Remark compounded this with *"not through analytic continuation of the metric"*, denying Wick rotation while performing exactly the sign flip ($+dx^2 \to -dt^2$) that *is* Wick rotation.

Resolution: acknowledge the Wick rotation honestly, then explain what distinguishes the cascade's Wick rotation from the standard QFT one (**motivation**, not the operation).

Changes to `src/cascade-series-part3.tex`:

1. **Theorem 10.2 retitled** "Lorentzian signature from orthogonality" → "Lorentzian signature from the cascade" (label `thm:lorentzian`).

2. **Theorem 10.2 proof fully rewritten.** Three explicit steps:
   - Step 1: Cascade propagator is oscillatory in real $x$ (from forced precession, Paper II).
   - Step 2: Oscillatory evolution is the Lorentzian regime (Euclidean regime is $e^{-\tau H}$, thermal partition function). Identification of $x$ with observer's proper time therefore requires Lorentzian sign.
   - Step 3: The identification is Wick rotation $x = it$, converting $+dx^2$ to $-dt^2$. Acknowledged explicitly.

3. **Remark `rem:wick-rotation-cascade`** replaces the old "Euclidean geometry, Lorentzian physics" remark. Makes four points:
   - The Euclidean-to-Lorentzian map **is** Wick rotation; prior drafts denying this are retracted.
   - **Standard QFT Wick rotation**: mathematical trick to make Lorentzian path integrals converge. No physical content attached to the rotation itself.
   - **Cascade Wick rotation**: the cascade is *built* Euclidean; Lorentzian observer metric arises via the physical identification hypothesis; the rotation preserves oscillatory propagator as oscillatory unitary time evolution.
   - Three consequences: (a) cascade does not avoid Wick rotation, it motivates it; (b) the cascade's Euclidean geometry is the starting structure, not a computational device; (c) §14's Lorentzian Gauss–Codazzi analysis uses the Wick-rotated metric, identified with the cascade's Euclidean slicing through $x = it$.

4. **Abstract line 45–46** updated from *"The signature $(-,+,+,+)$ follows without semiclassics or analytic continuation"* to *"The Euclidean-to-Lorentzian map is Wick rotation ($x\leftrightarrow it$), motivated cascade-internally by the forced precession rather than imposed as a mathematical trick; see Theorem `thm:lorentzian` and Remark `rem:wick-rotation-cascade`."*

5. **§14 supplement line 857** updated from *"We install the Lorentzian cascade metric directly from Theorem 10.2, without analytic continuation"* to *"We install the Lorentzian cascade metric directly from Theorem `thm:lorentzian`: under the Wick-rotation identification $x = it$, the cascade's Euclidean slicing metric maps to the Lorentzian form below."*

6. **Labels added** to `lem:spectral-gap` and `def:hyp` so the new remark/proof can reference them cleanly.

**What this buys.**

- Apparent denial of Wick rotation is replaced by an honest acknowledgement, eliminating a target a reviewer would correctly flag.
- The cascade's claim — that Wick rotation in its context is physically motivated rather than mathematically imposed — is preserved and clarified, now distinguishing motivation from operation.
- Theorem 10.2's proof, Theorem 9.4's third characterisation (which uses $a(t) = \sqrt{1-t^2}$ as a Lorentzian scale factor), and §14's Gauss–Codazzi analysis are all unified under the Wick-rotation framing; no internal inconsistency remains between these three invocations.
- Paper V's retracted "lapse-corrected Friedmann" derivation (which conflated cascade and metric lapses, SP-22) and this SP-21 fix jointly clean up the Euclidean-vs-Lorentzian accounting across the series.

**What this does not claim.**

The cascade's motivation for Wick rotation (forced precession + identification hypothesis) is cascade-internal, but the Wick rotation *operation* is the same as the standard QFT one. Readers who reject Wick rotation on physical grounds would reject the cascade's identification hypothesis too; the fix does not change the strength of that identification, only removes the false claim that Wick rotation is avoided.
