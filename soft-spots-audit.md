...
| SP-12 | New `rem:sp12-status` in Part I makes the forcing chain for "observer's host = $d_V = 5$" explicit: four-step argument from empirical 4D observer + Part III foliation + dimensional matching ($d-2=3$) forcing $d=5$, with cascade confirmation via volume max. Identification no longer depends on Cover Sheet's thought experiment as premise | `dc3393c` |
