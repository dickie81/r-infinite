...
`ZetaNoZeroInUnitInterval`: classical (`(1 − 2^{1−σ})ζ(σ) = Σ(−1)^{n+1}n^{−σ} > 0`), but Mathlib
only has `ζ(s) ≠ 0` for `Re s > 1`.

`rh_of_simple_ground_states`: the chain stated with the zero-swap lemma plugged in. Its inputs are
(a), eventual simplicity, the swap realisation (Paley–Wiener, named in ZeroSwap.lean) and the named
input above.
-/
...
/-- **Named input**: `ζ` has no zero in `(0, 1)`. Classical; not in Mathlib. -/
