#!/usr/bin/env python3
"""Round 96 (+ round 110: 'chi:-4', 'chi:-3' twisted sets): zero sets from a chosen prime set Q: theta(t)/pi + 1 + S_Q(t) = k - 1/2 (t < 1000),
...
for spec in sys.argv[1:]:
    ...
    if kind == "chi":   # round 110: Dirichlet-twisted S over primes <= 101; p = -4 or -3 (Kronecker symbol)
        from sympy import jacobi_symbol
        def chi(n, D=p):
            if D == -4: return 0 if n % 2 == 0 else (1 if n % 4 == 1 else -1)
            if D == -3: return 0 if n % 3 == 0 else (1 if n % 3 == 1 else -1)
        S = np.zeros_like(t)
        for qq in primerange(2, 102):
            c = chi(qq)
            if c: S -= np.imag(np.log(1 - c*qq**(-0.5 - 1j*t)))/np.pi
        ...
        Z = [z for z in Z if z >= 10]; name = "chi" + str(-p); json.dump(Z, open(f"pzeros_{name}.json", "w"))
        print(spec, "zeros", len(Z), "down-crossings", int(np.sum(np.diff(k)[idx] < 0))); continue
