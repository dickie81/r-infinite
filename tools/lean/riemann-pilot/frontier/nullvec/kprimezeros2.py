...
S_Q(t) = -(1/pi) sum_{p in Q} Im log(1 - p^{-1/2 - it}). Specs: 'only:p' or 'minus:p' (primes <= 101 without p).
...
for spec in sys.argv[1:]:
    kind, p = spec.split(":"); p = int(p)
    Q = [p] if kind == "only" else [q for q in primerange(2, 102) if q != p]
    S = np.zeros_like(t)
    ...
    name = spec.replace(":", "_"); json.dump(Z, open(f"pzeros_{name}.json", "w")); m = min(len(Z), len(Zt))
