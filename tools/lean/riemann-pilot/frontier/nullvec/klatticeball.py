#!/usr/bin/env python3
"""Round 92: zeros of the discretised-d-ball zeta Xi_d(s) = s(s - d/2) Lambda_d(s), Lambda_d = pi^-s Gamma(s) sum' |m|^-2s over Z^d,
...
d, T = int(sys.argv[1]), float(sys.argv[2]); mp.mp.dps = int(25 + 0.75*T)
U, NP = mp.log(40), 16
gl = GaussLegendre(mp.mp); nodes = []
for p in range(NP):
    nodes += gl.get_nodes(U*p/NP, U*(p + 1)/NP, 6, mp.mp.prec)
W = [(u, w*(mp.jtheta(3, 0, mp.exp(-mp.pi*mp.exp(u)))**d - 1)) for u, w in nodes]
h = mp.mpf(d)/2
def Lam(s):
...
if len(sys.argv) > 3:   # validation against closed forms
    for s in (mp.mpc(0.7, 3), mp.mpc(h/2, 14), mp.mpc(h + 0.3, 25)):
        ...
        ref = {1: 2*mp.zeta(2*s), 2: 4*mp.zeta(s)*mp.dirichlet(s, [0, 1, 0, -1]),
               ...
               8: 16*(1 - mp.power(2, 1 - s) + mp.power(2, 4 - 2*s))*mp.zeta(s)*mp.zeta(s - 3)}[d]*pre
