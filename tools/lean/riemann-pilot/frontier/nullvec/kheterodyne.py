#!/usr/bin/env python3
"""Round 107: score PREREG_heterodyne.md. Model kernels M1 (smooth) and M2 (smooth + one wave beyond the edge) from the
...
for i, xv in enumerate(x):
    ...
    A = np.abs((np.exp(1j*np.outer(kap, g))*e) @ o); k = kap[np.argmax(A)]
...
json.dump(out, open("kheterodyne_results.json", "w"), indent=1)
