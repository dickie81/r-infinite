#!/usr/bin/env python3
"""Round 111: score PREREG_Lchi4.md (as amended): single-prime check (p = 3, chi = -1, L kernel), L3, L2."""
...
def lx(pat, key):
    ...
    return r
T = lx("rL_true_*.jsonl", 0); Bs = lx("rL_base_*.jsonl", 0); K = lx("rL106_*.jsonl", 0)
...
q = np.array(json.load(open("Lquant.json"))); W = np.array([K[v]["w"] for v in x]); q = q[:W.shape[1]]
Z = np.array(json.load(open("Lzeros.json")))[:W.shape[1]]
rho = np.log(4*q/(2*np.pi))/(2*np.pi)
...
def chi(p): return 0 if p == 2 else (1 if p % 4 == 1 else -1)
...
out["per_prime"] = per
single = abs(per[3][0][0] - 4.14) <= 0.5; L3 = all(abs(w - 9.47) > 0.5 for w, _ in out["L_chain_lines"][:3])
L2 = [(w, abs(w - 4.14) <= 0.5) for w, _ in out["L_chain_lines"][:2]]
...
print("single-prime check (p=3 strongest within 0.5 of 4.14):", single, "| L3 (no 9.47 in top 3):", L3, "| L2:", L2)
json.dump(out, open("kLscore_results.json", "w"), indent=1)
