#!/usr/bin/env python3
"""Round 97: score PREREG_resonator.md from r97_*.jsonl."""
...
for N in ("N1", "N2"):
    for s in (1, 2, 3, 4):
        w, Y, r = spectrum(f"r97_pzeros_{N}_s{s}.jsonl")
        out[f"{N}_s{s}"] = dict(peaks=peaks(w, Y), rms_ratio=round(float(r.std()/rt.std()), 3), corr_true=round(float(np.corrcoef(r, rt)[0, 1]), 3),
                                **{str(t): present(w, Y, t)[0] for t in (9.42, 4.97, 16.75)})
cnt = lambda N, t: sum(out[f"{N}_s{s}"][str(t)] for s in (1, 2, 3, 4))
out["RES"] = bool(cnt("N1", 9.42) >= 3 and (cnt("N1", 4.97) >= 3 or cnt("N1", 16.75) >= 3)); out["ARITH"] = bool(cnt("N1", 9.42) <= 1)
out["counts"] = {N: {t: cnt(N, t) for t in (9.42, 4.97, 16.75)} for N in ("N1", "N2")}
