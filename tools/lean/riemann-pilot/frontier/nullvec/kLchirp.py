...
qc = 4
R = {}
for f in glob.glob("rL106_*.jsonl"):
    for l in open(f): o = json.loads(l); R[o["x"]] = o
x = np.array(sorted(R)); assert len(x) == 226
q = np.array(json.load(open("Lquant.json")))[:len(R[x[0]]["w"])]; rho = np.log(qc*q/(2*np.pi))/(2*np.pi); spacing = 1/rho
re = []
for v in x:
...
PARS = np.array(PARS); np.save("Lchirp_params.npy", np.column_stack([x, PARS]))
...
json.dump(dict(lam=lam, r_e_mean=float(re.mean()), r_e=re.tolist(), chirp=[c0, c1, c2], beta_prime=bp, beta_segments=seg, variance_captured=float(np.mean(FRAC)), predictions=preds), open("Lpredictions.json", "w"), indent=1)
