#!/usr/bin/env python3
"""Round 91: score PREREG_resonance.md. Usage: kresonance.py [out.json]"""
...
x0, l0 = zs("true"); x1, l1 = zs("smooth"); assert len(x0) == len(x1) == 451 and np.allclose(x0, x1)
x = x0; d = np.log(x)
m = {}
...
R = bool(abs(p1 - p0) <= res_w and line(r1, p1)["passes"])
out = dict(validation_corr=round(V, 4), V=bool(V >= 0.9), peak_true=round(p0, 3), peak_smooth=round(p1, 3), peak_measured=round(peak(rm), 3),
           ...
           R=R, A=bool(not L1["passes"]), rms_true=float(r0.std()), rms_smooth=float(r1.std()), rms_ratio=float(r1.std()/r0.std()),
...
np.savez("kresonance_series.npz", x=x, true=l0, smooth=l1, measured=lm, r_true=r0, r_smooth=r1, r_meas=rm)
