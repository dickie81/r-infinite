...
import os
mp.mp.dps = 20; q = int(os.environ.get('LQ', '4')); CHAR = {4: [0, 1, 0, -1], 3: [0, 1, -1], 8: [0, 1, 0, 1, 0, -1, 0, -1], 5: [0, 1, -1, -1, 1], 12: [0, 1, 0, 0, 0, -1, 0, -1, 0, 0, 0, 1]}[q]   # round 115: chi_-8 (odd); round 116: chi_5, chi_12 (even)
GA = mp.mpf(os.environ.get('LGA', '0.75'))   # round 116: Gamma shift, 3/4 odd, 1/4 even   # round 112: conductor via LQ
def theta(t): return t/2*mp.log(q/mp.pi) + mp.im(mp.loggamma(GA + 1j*t/2))
def Z(t):
    t = mp.mpf(t); v = mp.exp(1j*theta(t))*mp.dirichlet(mp.mpc(0.5, t), CHAR)
    return v
