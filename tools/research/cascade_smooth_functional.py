...
a weight sqrt(2D/T). Consequences, given T^s the least value of F^s and T_u in the convex bracket (2T_0/e, 2T_0) (both computed at the
states): min F_k = F_k^s(T^s) + Osc_inf + O(ln T/sqrt T); |T_u - T^s| = O(T^{1/4} (ln T)^{1/2}); the rung's exponent from the smooth count and the holes
...
times the smooth count's ln(r/2 pi) (for D <= T/100, T >= 80), so the Binet bound is at most (3/2)(1 + 8D/T)/(T^2 ln(T/2 pi)) times the smooth count's integral
plus the cut's tail below T - 3D (chi <= Phi(-3) there); at the cells the Binet bound is at most 1.0x times (3/2)/(T^2 ln(T/2 pi)) of the
smooth count's integral (gated); and the pole term E_pole = 2 ghat(i/2)^2 chi(i/2), |E_pole| <= 8 a e^a e^{-(T^2 - 1/4)/(2 D^2)}: after the
...
with _mp.workdps(60):
    ...
    # the shoulder factor: (T/r)^2 ln(T/2pi)/ln(r/2pi) <= 1 + 8D/T on [T - 3D, T] at T = 87.4, 200, 1000 with D = 1, and at 100 heights over [80, 5000] with D = T/100 (the largest D the block allows; the factor's excess over 1 grows with D/T)
    ...
    ok &= all((T/r)**2*math.log(T/TWO_PI)/math.log(r/TWO_PI) <= 1 + 8*(T/100)/T for T in np.linspace(80.0, 5000.0, 100) for r in np.linspace(T - 3*(T/100), T, 601))
...
ok &= _sci(S_LEFT.split("at most ")[1]) == _r1(max(left)) and _sci(S_LEFT.split("bound at most ")[1]) == _r1(max(lbnd)) and _nums(S_LEFT.split("itself at most ")[1].split(" times")[0]) == [ce(max(ebr), 3)]
