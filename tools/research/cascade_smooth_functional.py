...
gate(f"g5 the exterior tail by the explicit formula (the cut chi = Phi((r - T)/D) - Phi((r - T')/D), D = 1, the grid h = 0.05, the shells to e^(delta + 7.5)): 2 sum ghat^2 chi = (1/pi) int ghat^2 chi ln(r/2pi) dr + E_arch - 2 sum Lambda(n) n^-1/2 f_chi(ln n) within {_r1u(max(res_g)):.1e} relative at the five ground states over the windows [T_1, 6980], [1, 1.25], [1.25, 2], [2, 4], [4, 6980/T_1] T_1 (gated 1e-5; the windows away from the cut at the edge within {_r1u(max(res_far)):.1e}, gated 1e-9 -- the residual at the edge is the stored arb profile's 1e-4 rounding at the dodged zeros the cut still weighs) and within {_r1u(max(res_r)):.1e} at the 40 rungs over [T_1, 6980] and [T_1, 1.25 T_1] (gated 1e-9); the product form against the stored profile beyond the edge within {_r1u(pfm):.1e} at the 45 states (gated 1e-4); the leftover after the shells, E_arch/tail at most {_r1u(max(left)):.1e} (gated 1e-5), below its Binet bound (3/2pi) int ghat^2 chi r^-2 at every window (gated), the bound at most {_r1u(max(lbnd)):.1e} of the tail (gated 2e-4) and at most {ce(max(ebr), 3):.3f} times (3/2)/(T^2 ln(T/2pi)) of the smooth count's integral over the 105 windows -- 25 at the ground states, 80 at the rungs -- (gated 1.1, and below 1 + 8D/T at each: the shoulder factor); the pole term below e^{math.ceil(lpole):d} of the tail (gated e^-1000); the smooth count's integral over the actual exterior leakage: " + ", ".join(f"{x:.2f}" for x in ratio_all) + f" over the whole exterior (gated [1.8, 3.0]), " + ", ".join(f"{x:.2f}" for x in ratio_near) + f" in the near quarter [T_1, 1.25 T_1] (gated [1.9, 4.5]), " + ", ".join(f"{x:.2f}" for x in ratio_mid) + " on [1.25, 2] T_1 (gated [1.3, 3.6]), " + ", ".join(f"{x:.2f}" for x in ratio_2) + " on [2, 4] T_1 (gated [1.2, 2.0]), " + ", ".join(f"{x:.3f}" for x in ratio_far) + f" beyond 4 T_1 (gated [0.99, 1.2]); the largest shell over the whole exterior n = " + ", ".join(str(t) for t in tops) + f" (gated among 2, 3, 5, 7), in the near quarter n = " + ", ".join(str(t) for t in tops_near) + f"; at the 40 rungs smooth/tail runs {min(rr_all):.2f}-{max(rr_all):.2f} over the whole exterior (gated within [1.5, 4.0]) and {min(rr_near):.2f}-{max(rr_near):.2f} in the near quarter (gated within [1.5, 8.0])", ok)
...
ok &= _nums(S_MIN.split("sits ")[1].split(" nats")[0]) == [round(abs(x), 4) for x in cfd] and all(cfd[i] < cfd[i + 1] for i in range(4)) and _nums(S_MIN.split("within ")[1].split(" nats")[0]) == [ce(abs(min(cfd_r)), 2)]
