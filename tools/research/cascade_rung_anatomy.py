...
tau_eff = sqrt((k - 1)/(-Dkappa_k)) between 1.17 and 1.42 T_1 over the safely deep rungs; |Dkappa_2| falling across the cells (the correction vanishing
...
state's T_1 (the edge at the hole tolerance 0.2; 1bu's T_D at 0.05 lies below it at every cell, 2.9-8.5%); (0b) at delta = 2.3 the rungs' dodging
zeros against the ground state's by zeta zero (within 1e-12 up to gamma = 50-70 at rungs 2-5, 1e-5 below gamma = 40 at rung 8,
the displacement growing toward the edge to 0.08-0.18) and rung 9's touching at gamma_1 (no sign change, the complex pair from
...
factor 10 at every rung; (3) tau_eff/T_1 in [1.1, 1.5]; (4) |Dkappa_2| falling across the cells; the interior mass
...
PAPER_NEEDLES = [
    ...
    {'g': 'g7', 's': 'shared with the ground state’s within 10⁻¹² up to γ = 69.5, 65.1, 59.3, 49.8 at rungs 2–5 and within 2.7 × 10⁻⁶ below γ = 40 at rung 8, the difference growing toward the rung’s edge to 0.11, 0.11, 0.16, 0.18, 0.08, 0.11, 0.09 at rungs 2–8 (δ = 2.3)', 'form': 'ws'},
...
ok &= all(0.05 <= SH[k]["max"] <= 0.2 for k in SH)                                    # the displacement grows toward the edge to 0.08-0.18
def _shared_sentence():
    return ("shared with the ground state’s within 10⁻¹² up to γ = " + ", ".join(f"{SH[k]['first']:.1f}" for k in (2, 3, 4, 5)) + " at rungs 2–5 and within " + _sci_str(SH[8]["low40"]) +
            " below γ = 40 at rung 8, the difference growing toward the rung’s edge to " + ", ".join(f"{SH[k]['max']:.2f}" for k in range(2, 9)) + " at rungs 2–8 (δ = 2.3)")
def _sci_str(v):
...
gate("g0b the shared dodging zeros at delta = 2.3 (rung k's against the ground state's, by zeta zero): within 1e-12 up to gamma = " + ", ".join(f"{SH[k]['first']:.1f}" for k in (2, 3, 4, 5)) + " at rungs 2-5, within 1e-5 below gamma = 40 at rung 8, the displacement growing toward the edge to " + ", ".join(f"{SH[k]['max']:.2f}" for k in range(2, 9)) + f" at rungs 2-8; rung 9's touching at gamma_1: ghat_9 rises to {T9['max']:.2e} at r = {_x:.3f} with no sign change, the complex pair {_x:.3f} +- {_y:.3f}i whose 2 Re(tau^-2) is the sum rule's remainder to four digits", ok)
...
gate("g3 tau_eff = sqrt((k - 1)/(-Dkappa_k)) between 1.1 and 1.5 T_1 at every safely deep rung (observed 1.17-1.42) -- " + "; ".join(f"{c}: " + ", ".join(f"{L['tau_eff']/CEN[c]['rungs'][0]['edge']:.2f}" for L in in_law(c)) for c in ORDER), ok)
...
S_SHARED = 'shared with the ground state’s within 10⁻¹² up to γ = 69.5, 65.1, 59.3, 49.8 at rungs 2–5 and within 2.7 × 10⁻⁶ below γ = 40 at rung 8, the difference growing toward the rung’s edge to 0.11, 0.11, 0.16, 0.18, 0.08, 0.11, 0.09 at rungs 2–8 (δ = 2.3)'
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'shared with the ground state’s within 10⁻¹² up to γ = 69.5, 65.1, 59.3, 49.8 at rungs 2–5 and within 2.7 × 10⁻⁶ below γ = 40 at rung 8, the difference growing toward the rung’s edge to 0.11, 0.11, 0.16, 0.18, 0.08, 0.11, 0.09 at rungs 2–8 (δ = 2.3)', 'ws')
...
ok &= [d['s'] for d in paper_needles.declared(PAPER_NEEDLES) if d.get('g') == 'g7'] == [S_SAFE, S_LOCATED, S_T1, S_EDGES, S_SHARED, S_ACCT, S_SHALLOW, S_ACC, S_DK23, S_TAU, S_RATIO, S_MASS, S_DEVE, S_DEVR, S_DEVM, S_WORST, S_EXT, S_DK2]
_re = __import__("re")
    ...
    for c in ORDER:
        ...
        if len(un) == 1:
            ...
            if t >= R_ext: at = f"|τ| = {t:.0f}, beyond the region {R_ext:.0f}"
                ...
                j = round(t/(math.pi/a_)); at = f"|τ| = {t:.0f}, {abs(t - j*math.pi/a_):.3f} below the sinc zero ω_{j} inside one census cell"
        else:
