...
excluded by computation at the rungs) is the rung's wall and its minimum the rung's exponent:
...
the hole count (from under 6% at one hole to 4-15% at the top rung; correlation +0.5 with m at t = 3.7, -0.2 with 1bx's onset shortfall within two standard errors of zero) --
...
PAPER_NEEDLES = [
    ...
    {'g': 'g6', 's': 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count — the first at t = 3.7, the two negatives within two standard errors of zero (a null correlation’s standard error is 0.16 at n = 40)', 'form': 'ws'},
...
n_agree = sum(1 for c in ORDER if abs(SL[c][2]) <= 0.02)                                          # the one-hole gap within 2% (round 344 F344-8)
ok &= 0.45 <= r_m <= 0.6 and -0.3 <= r_sh <= -0.1 and r_ms <= 0 and abs(r_ms) <= 2*SE and n_agree == 3 and faster == 4   # t(r_m) >= 3 and |r_sh| <= 2 SE are implied by the bounds on r_m and r_sh at n = 40 (round 344 F344-4), so not gated twice and SL["d2.0"][0] > SL["d2.0"][1] and exc.min() >= -0.03 and 0.14 <= exc.max() <= 0.16
...
S_EXCESS = 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count — the first at t = 3.7, the two negatives within two standard errors of zero (a null correlation’s standard error is 0.16 at n = 40)'
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count — the first at t = 3.7, the two negatives within two standard errors of zero (a null correlation’s standard error is 0.16 at n = 40)', 'ws')
...
ok &= [d['s'] for d in paper_needles.declared(PAPER_NEEDLES) if d.get('g') == 'g6'] == [S_GROUND, S_MARGIN, S_END, S_RUNGS, S_STICKY, S_EXP, S_CONT, S_DISP, S_TAIL, S_BOTTOM, S_EXCESS, S_SLOPES, S_AGREE]
_re = __import__("re")
...
ok &= _ints(S_AGREE.split("within 2% at ")[1]) == [n_agree] and _ints(S_AGREE.split("within ")[1].split("%")[0]) == [2]
...
ok &= _nums(S_EXCESS.split("the first at t = ")[1].split(",")[0]) == [round(tstat(r_m), 1)] and _nums(S_EXCESS.split("standard error is ")[1]) == [round(SE, 2)] and abs(r_sh) <= 2*SE and abs(r_ms) <= 2*SE and _ints(S_EXCESS.split("over the ")[1].split(" rungs")[0]) == [len(W)]
