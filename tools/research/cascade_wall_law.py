...
PAPER_NEEDLES = [
    ...
    {'g': 'g6', 's': 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count — the first at t = r√(n − 2)/√(1 − r²) = 3.7, the two negatives within two standard errors of zero (a null correlation’s standard error is 0.16 at n = 40)', 'form': 'ws'},
...
ok &= SL["d2.0"][0] > SL["d2.0"][1] and exc.min() >= -0.03 and 0.14 <= exc.max() <= 0.16
...
S_EXCESS = 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count — the first at t = r√(n − 2)/√(1 − r²) = 3.7, the two negatives within two standard errors of zero (a null correlation’s standard error is 0.16 at n = 40)'
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count — the first at t = r√(n − 2)/√(1 − r²) = 3.7, the two negatives within two standard errors of zero (a null correlation’s standard error is 0.16 at n = 40)', 'ws')
...
ok &= _nums(S_EXCESS.split("√(1 − r²) = ")[1].split(",")[0]) == [round(tstat(r_m), 1)] and _ints(S_EXCESS.split("r√(n − ")[1].split(")")[0]) == [2] and _nums(S_EXCESS.split("standard error is ")[1]) == [round(SE, 2)] and abs(r_sh) <= 2*SE and abs(r_ms) <= 2*SE and _ints(S_EXCESS.split("over the ")[1].split(" rungs")[0]) == [len(W)]
