...
median: the minimum is flat; the residual grows with the hole count); the continuum law within 3.9% rms of T_u and 3.3% of T_w; the displacement law at every dodged zero
...
the hole count (from under 6% at one hole to 4-15% at the top rung; correlation +0.5 with m at 3.7 standard errors, -0.2 with 1bx's onset shortfall within two) --
...
PAPER_NEEDLES = [
    ...
    {'g': 'g6', 's': 'the domain’s left endpoint T = h_max exceeds the minimum by at least 16.2 nats at the 40 rungs', 'form': 'ws'},
    ...
    {'g': 'g6', 's': 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count — the first 3.7 standard errors from zero, the two negatives within two (a null correlation’s standard error is 0.16 at n = 40)', 'form': 'ws'},
...
tstat = lambda r: r*math.sqrt(len(exc) - 2)/math.sqrt(1 - r*r); SE = 1/math.sqrt(len(exc) - 3)                                 # t statistics and the standard error of a null correlation at n = 40 (round 343 F343-1)
SL = {}
for c in ORDER:
...
ok &= 0.45 <= r_m <= 0.6 and -0.3 <= r_sh <= -0.1 and r_ms <= 0 and tstat(r_m) >= 3 and abs(r_sh) <= 2*SE and abs(r_ms) <= 2*SE and faster == 4 and SL["d2.0"][0] > SL["d2.0"][1] and exc.min() >= -0.03 and 0.14 <= exc.max() <= 0.16
...
S_END = 'the domain’s left endpoint T = h_max exceeds the minimum by at least 16.2 nats at the 40 rungs'
...
S_EXCESS = 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count — the first 3.7 standard errors from zero, the two negatives within two (a null correlation’s standard error is 0.16 at n = 40)'
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'the domain’s left endpoint T = h_max exceeds the minimum by at least 16.2 nats at the 40 rungs', 'ws')
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count — the first 3.7 standard errors from zero, the two negatives within two (a null correlation’s standard error is 0.16 at n = 40)', 'ws')
...
ok &= [d['s'] for d in paper_needles.declared(PAPER_NEEDLES) if d.get('g') == 'g6'] == [S_GROUND, S_MARGIN, S_END, S_RUNGS, S_STICKY, S_EXP, S_CONT, S_DISP, S_TAIL, S_BOTTOM, S_EXCESS, S_SLOPES]
_re = __import__("re")
...
ok &= _nums(S_END) == [round(endgap, 1)] and _ints(S_END.split("at the ")[1]) == [len(W)]
...
_m = _nums(S_EXCESS.split(" — the first")[0]); ok &= _m == [round(100*exc.min(), 1), round(100*exc.max(), 1), round(r_m, 2), round(r_sh, 2), round(r_ms, 2)] and r_ms < 0
ok &= _nums(S_EXCESS.split("the first ")[1].split(" standard")[0]) == [round(tstat(r_m), 1)] and _nums(S_EXCESS.split("standard error is ")[1]) == [round(SE, 2)] and abs(r_sh) <= 2*SE and abs(r_ms) <= 2*SE and _ints(S_EXCESS.split("over the ")[1].split(" rungs")[0]) == [len(W)]
