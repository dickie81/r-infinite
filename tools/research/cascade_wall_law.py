...
read through 1bx's balayage; its minimiser T_u(k) (a zeta zero: F_k is concave between zeros) is the rung's wall and its minimum the rung's exponent:
...
census edge T_k and 1bx's sharp wall T_w at 35 of 40, the exponent within half a nat at all 40 (a half-nat window spanning about a
tenth of T_u: the minimum is flat); the continuum law within 3.9% rms of T_u and 3.3% of T_w; the displacement law at every dodged zero
...
the hole count (from about nothing at one hole to 4-15% at the top rung; correlation +0.5 with m, -0.2 with 1bx's onset shortfall) --
...
PAPER_NEEDLES = [
    ...
    {'g': 'g6', 's': 'uT₀/T_u averages 1.028 with rms deviation from 1 of 0.039 over [0.988, 1.116] at the 40 rungs, uT₀/T_w 0.986 with 0.033 over [0.916, 1.044]; its per-hole descent is 4.43, 4.62, 4.67, 4.50, 4.27 at the five cells; and for one hole uT₀/T₁ = 1.015, 0.999, 0.988, 1.001, 0.993', 'form': 'ws'},
    ...
    {'g': 'g6', 's': 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count', 'form': 'ws'},
...
# so F_k is concave on every open spacing and its infimum on a closed spacing is an endpoint -- witnessed by second differences on 61 points per spacing
...
ok &= STICKY == [("d2.3", 2), ("d3.0", 2), ("d3.0", 3)] and len(PAIRS) == 16 and ABOVE == [("d2.3", 2), ("d3.0", 3), ("d3.5", 2)] and BELOW == [("d3.5", 2), ("d3.5", 3), ("d3.5", 5)] and below_one and bu_match == 39 and len(set(ABOVE) | set(BELOW)) == 5
ok &= 0.45 <= r_m <= 0.6 and -0.3 <= r_sh <= -0.1 and r_ms <= 0 and faster == 4 and SL["d2.0"][0] > SL["d2.0"][1] and exc.min() >= -0.03 and 0.14 <= exc.max() <= 0.16
...
gate(f"g2 the unlocking height T_u = argmin F_k at the 40 safely deep rungs: between the census edge T_k and 1bx's sharp wall T_w at {between} of 40 (gated 35), the exceptions above T_w at {ABOVE} and below T_k by one zero at {BELOW} (gated exactly these; five rungs in all, delta = 3.5's rung 2 in both sets); T_u/T_k mean {float(np.mean(ruk)):.3f}, rms deviation from 1 {rms(ruk):.3f}, range [{min(ruk):.3f}, {max(ruk):.3f}] (gated mean in [1.02, 1.03], rms <= 0.04, range within [0.99, 1.09]); T_u/T_w mean {float(np.mean(ruw)):.3f}, rms {rms(ruw):.3f}, range [{min(ruw):.3f}, {max(ruw):.3f}] (gated mean in [0.95, 0.97], rms <= 0.06, range within [0.87, 1.03]); T_u = the ground's minimiser at {STICKY} (gated exactly these); T_u shared with the rung below at {len(PAIRS)} rungs (gated 16); T_u equals Theorem 1bu(iv)'s k-level minimiser (the ladder checkpoint's edge) at {bu_match} of 40 (gated 39); the sharp wall's excess T_w/T_u - 1 from {100*exc.min():+.1f}% to {100*exc.max():+.1f}% (gated >= -3%, max in [14%, 16%]), correlating {r_m:+.3f} with the hole count (gated [0.45, 0.6]) and {r_sh:+.3f} with 1bx's onset shortfall at T_1 (gated [-0.3, -0.1]), the shortfall itself correlating {r_ms:+.3f} with the hole count (gated <= 0); per cell dT_u/dm, dT_w/dm, the excess at one hole and at the top rung: " + "; ".join(f"{c}: {SL[c][0]:+.2f}, {SL[c][1]:+.2f}, {100*SL[c][2]:+.1f}%, {100*SL[c][3]:+.1f}%" for c in ORDER) + f" (the functional's wall descending faster at {faster} of 5 cells, gated 4, the exception delta = 2 with three rungs; the gap growing to >= 4% at the four cells with six or more rungs)", ok)
...
S_CONT = 'uT₀/T_u averages 1.028 with rms deviation from 1 of 0.039 over [0.988, 1.116] at the 40 rungs, uT₀/T_w 0.986 with 0.033 over [0.916, 1.044]; its per-hole descent is 4.43, 4.62, 4.67, 4.50, 4.27 at the five cells; and for one hole uT₀/T₁ = 1.015, 0.999, 0.988, 1.001, 0.993'
...
S_EXCESS = 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count'
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'uT₀/T_u averages 1.028 with rms deviation from 1 of 0.039 over [0.988, 1.116] at the 40 rungs, uT₀/T_w 0.986 with 0.033 over [0.916, 1.044]; its per-hole descent is 4.43, 4.62, 4.67, 4.50, 4.27 at the five cells; and for one hole uT₀/T₁ = 1.015, 0.999, 0.988, 1.001, 0.993', 'ws')
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'T_w/T_u − 1 runs from −2.8% to +14.8% over the 40 rungs, correlating +0.52 with the hole count and −0.21 with 1bx’s onset shortfall at T₁, which itself correlates −0.11 with the hole count', 'ws')
...
ok &= [d['s'] for d in paper_needles.declared(PAPER_NEEDLES) if d.get('g') == 'g6'] == [S_GROUND, S_MARGIN, S_RUNGS, S_STICKY, S_EXP, S_CONT, S_DISP, S_TAIL, S_BOTTOM, S_EXCESS, S_SLOPES]
_re = __import__("re")
...
ok &= _ints(S_RUNGS.split("stored minimiser at ")[1]) == [bu_match, len(W)]
...
_m = _nums(S_EXCESS); ok &= _m == [round(100*exc.min(), 1), round(100*exc.max(), 1), round(r_m, 2), round(r_sh, 2), round(r_ms, 2)] and r_ms < 0 and _ints(S_EXCESS.split("over the ")[1].split(" rungs")[0]) == [len(W)]
...
_m = _nums(S_CONT.split("descent is ")[1].split(" at the five")[0]); ok &= _m == [round(-CSL[c], 2) for c in ORDER]
