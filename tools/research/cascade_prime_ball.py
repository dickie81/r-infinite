...
six balls re-derived live from the stored shifts with their signs --
...
the smooth-density tail with its oscillation resolved -- round-305 F305-1)
on the C_c^inf bump within 1e-13 of the true form and on the K = 48 Gram's
approximate minimiser within [5e-8, 1e-6]; the archimedean-only candidate rejected by a
...
(eta_lo is where the witnessing vector -- g_1, or the re-minimised one -- fails to witness, not where positivity holds); no
...
PAPER_NEEDLES = [
    ...
    {'g': 'g7', 's': "the zero side (the 6700 zeros as doubles, mpmath at 40 digits, the smooth-density tail with its oscillation resolved — floating point, not balls) agrees with the true form's Rayleigh quotient on the C_c^∞ bump below 3×10⁻¹⁶ and on the K = 48 Gram's approximate minimiser to 2.1×10⁻⁷", 'form': 'ws'},
...
for c in ORDER:
    ...
    if KCELLS[c]["two_sided"]:
        ...
        _d, _K, _prec = KE[c]["delta"], KE[c]["K"], KE[c]["prec"]
        _G, _N, _ = gram(_d, _K, _prec)
        with ctx.workprec(_prec):
            _a = arb(_d)/2
            for _r, _sg in ((m, -1), (q, 1)):
                for _eta, _neg in ((_r["eta_lo"], False), (_r["eta_hi"], True)):
                    _Ge = perturbed_gram(_G, 2, _sg*_eta, _K, _a, _prec)
                    _c, _ = _minim(_Ge, _N, _prec)
                    _rq = rayleigh(_Ge, _N, [arb(x) for x in _c], _prec)
                    ok &= (_rq.upper() < 0) if _neg else (_rq.lower() > 0)
    else:
        ok &= two is None
gate("g3 the two-sided windows for p = 2 at delta <= 2.0 (re-minimised, relative width 1e-2): both crossings bracketed with certified signs, the downward one within 2 percent below the fixed-vector eta_hi (its upper end cannot exceed it), the six balls re-derived live with the stored signs, the upward/downward asymmetry >= 100 ("
...
rz = compare(G1, zeros(1.0, 48, 400), N1, vecs, 400)   # the tail with its oscillation resolved (round-305 F305-1)
ok &= rz["bump"]["rel"] <= 1e-13 and 5e-8 <= rz["minimiser"]["rel"] <= 1e-6
...
gate("g6 the bench calibrations LIVE at delta = 1.0 (floating point, relative Rayleigh-quotient deviations): the zero side (tail resolved) on the bump within 1e-13 (" + f"{rz['bump']['rel']:.1e}" + ") and on the K = 48 minimiser in [5e-8, 1e-6] ("
...
S_BENCH = "the zero side (the 6700 zeros as doubles, mpmath at 40 digits, the smooth-density tail with its oscillation resolved — floating point, not balls) agrees with the true form's Rayleigh quotient on the C_c^∞ bump below 3×10⁻¹⁶ and on the K = 48 Gram's approximate minimiser to 2.1×10⁻⁷"
...
# the bench sentences: the two zero-side agreements (stated at or above the live value: the bump's 'below 3e-16' within a factor 2 of a
# noise-floor quantity, the minimiser's within 10 percent), the archimedean
...
ok &= len(_b) == 2 and rz["bump"]["rel"] <= _b[0] <= 2*rz["bump"]["rel"] and rz["minimiser"]["rel"] <= _b[1] <= 1.1*rz["minimiser"]["rel"]
...
gate("g7 the paper's own numbers: the seven table rows and the two (iii) sentences present as declared needles, tied to the declared entries and parsed back -- "
     ...
     "two-sided windows; the ratio range outward; e^(-N) with N <= -ln eta_2(3.5); the delta = 1.0 shifts outward; the asymmetries rounded down within 10 percent; the bench sentences' six values (two agreements, four balls) against the live gates and the stored delta = 1.0 balls", ok)
