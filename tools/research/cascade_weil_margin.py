...
Gates (all exit-gated; any failure exits 1):
  ...
  g2  the margin ladder and the pole anatomy (float64 prime-side form,
      ...
      balance along the delta = 4 extremal pinned (arch -2.415, pole
      +6.795, primes -4.379, each +-0.02) -- the pole of zeta fighting
      the primes and the archimedean drag to a draw.
  ...
  g4  the rate law (mp, dps 60; basis (1-x^2)^m x^j; K = 1000 verified
      zeros as labeled classical data; the zero-side section minimum
      is a rigorous lower bound on the K-zero form's margin -- the
      truncation omits only nonnegative terms): pinned margins (rel
      1e-3): (0.9, m3, n2) 1.574e-3, (0.9, m3, n12) 5.221e-5,
      (1.4, m3, n12) 3.338e-10, (2.2, m3, n10) 2.156e-13,
      (2.2, m5, n12) 1.932e-15; the fitted per-dimension rates c(delta)
      in windows c(0.9) in (0.05, 0.2), c(1.4) in (0.45, 0.65),
      c(2.2) in (0.70, 0.85) for BOTH basis families with
  ...
  g5  the phase change at the dodging horizon: at delta = 0.9 the
      ...
      n = 20); the horizon arithmetic 2 pi e^delta = 15.46 / 25.48 /
      56.71 at delta = 0.9 / 1.4 / 2.2 (constants-only conjuncts
      pinning the published numbers -- nature stated per the 1av-g2
      precedent; the 0.5% proximity of 2 pi e^{2.2} to gamma_12 is
      NOTED in the block, not leaned on, and not gated).
  g6  the two-sided pins (SECTION margins, not the support-class
      ...
      V/r^{m+1} with V from the ACTUAL minimizer's (m+1)-th
      derivative, mp quadrature) pins the (0.9, m3, n20) section's
      full-zero-set margin essentially exactly (bracket ends equal at
      four digits; floor 2.896e-5 gated) and the (1.4, m3, n20)
      section at 1.905e-12 (adapted tail ~6e-17, gated < 1e-12); the
      ceiling reads the beyond-K zeros' contributions as the
      nonnegative |fhat|^2 form (the labeled-classical-data clause).
...
ok &= -1.0 < l05np < -0.85
ok &= -17.5 < l50np < -16.0
ok &= abs(arch_c - (-2.415)) < 0.02 and abs(pole_c - 6.795) < 0.02 and abs(pr_c - (-4.379)) < 0.02
print(f"  g2 floor {l05:.6f}; prime-2 drop {l07:.6f}; no-pole {l05np:.3f}/{l50np:.2f}; "
      f"balance arch {arch_c:.3f} + pole {pole_c:.3f} + primes {pr_c:.3f} = "
      f"{arch_c+pole_c+pr_c:.6f}", flush=True)
gate("g2 the margin ladder and the pole anatomy: the unconditional floor, "
     "the 27x prime-2 drop, pole-carried positivity, the balance at the "
     "extremal", ok)
...
import math
ok &= abs(2*math.pi*math.exp(0.9) - 15.46) < 0.01
ok &= abs(2*math.pi*math.exp(1.4) - 25.48) < 0.01
ok &= abs(2*math.pi*math.exp(2.2) - 56.71) < 0.01
print(f"  g5 saturation ratio at 0.9: {mp.nstr(sat, 4)}; continuation factor "
      f"at 1.4: {mp.nstr(cont, 4)}; horizons 15.46/25.48/56.71", flush=True)
...
def adapted_tail(delta, m, n):
    ...
    a = mpf(delta)/2
    def fmder(x):
        # |d^{m+1}/dt^{m+1} f| at t = a x, via mp diff of the polynomial form
        def f(t):
            xx = t/a
            return sum(coefs[j]*(1 - xx*xx)**m*xx**j for j in range(n))
        return abs(mpdiff(f, a*x, m + 1))
    V = mpquad(fmder, [-1, 1])*a        # int over t = a * int over x
    gK = ZK[-1]
...
ok &= t14 < mpf("1e-12")
print(f"  g6 adapted tails: 0.9 bracket [{mp.nstr(lo09,4)}, {mp.nstr(hi09,4)}]; "
