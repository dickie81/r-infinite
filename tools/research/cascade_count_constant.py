...
least-squares slope of B - I against ln T on each list within 0.05 (zeta, T in
[42, 7000]), 0.2 (characters), 0.15 (Delta) of 4 c_L; a wrong constant (7/8
...
0.01. (5) mangle probes; (6) the paper's numbers parsed back; (7) the chain
...
SLOPE_BAND = {"zeta": 0.05, "chi_-3": 0.2, "chi_-4": 0.2, "chi_8": 0.2, "Delta": 0.15}
...
gate("g2 the slopes of B - I against ln T on the lists: " + ", ".join(f"{f}: {slopes[f]:+.3f} (4c_L = {float(EXACT[f]):+.1f}, band {SLOPE_BAND[f]})" for f in FORDER) + "; the constants 1 and 3/4 in place of 7/8 fail zeta's band", ok)
...
gate(f"g3 the constants: zeta's residual B - I - (7/2) ln T over T in [42, 7000] in [{res.min():.3f}, {res.max():.3f}] (width <= 1.0: the S(T) fluctuation), mean {res.mean():.3f} within 0.1 of the closed formula's C_zeta = {Cf['zeta']:.3f} (parts {parts['zeta'][0]:.3f}, {parts['zeta'][1]:.3f}, {parts['zeta'][2]:.3f}); "
     + "C_L by formula: " + ", ".join(f"{f}: {Cf[f]:.3f}" for f in FORDER), ok)
...
gate("g4 the 1bm consequence: 2 s_delta(2T_0) + 4 pi e^delta = B(2T_0) - I(2T_0) = " + ", ".join(f"{dvals[c]:.2f}" for c in ORDER) + " against (7/2) ln(4 pi e^delta) + C_zeta within 0.5 (deviations " + ", ".join(f"{dev[c]:+.2f}" for c in ORDER)
     + f"); 1bm(v)'s residual c(delta) reproduced within 0.01 (" + ", ".join(f"{cvals[c]:.2f}" for c in ORDER) + f"), its least-squares slope against ln T_0 {cslope:.3f} in [1.1, 1.5]", ok)
...
ok &= paper_needles.needle(PAPER_NEEDLES, '0.56', 'ws')
ok &= paper_needles.needle(PAPER_NEEDLES, '2.89, 2.24, 1.22 for χ₋₃, χ₋₄, χ₈', 'ws')
ok &= paper_needles.needle(PAPER_NEEDLES, '0.4', 'ws')
ok &= paper_needles.needle(PAPER_NEEDLES, '1.28', 'ws')
ok &= [d['s'] for d in paper_needles.declared(PAPER_NEEDLES) if d.get('g') == 'g6'] == [S_SLOPES, S_RESZ, S_CZ, S_CZPARTS, S_CD, S_CCHI, S_DEV, S_CSLOPE]
...
ok &= abs(_num(S_CZ) - Cf["zeta"]) <= 5e-3 + 1e-9 and abs(_num(S_CD) - Cf["Delta"]) <= 5e-3 + 1e-9
...
ok &= max(abs(dev[c]) for c in ORDER) <= _num(S_DEV) < max(abs(dev[c]) for c in ORDER) + 0.1 + 1e-9                     # outward 0.1
ok &= abs(_num(S_CSLOPE) - cslope) <= 5e-3 + 1e-9
gate("g6 the paper's numbers parsed back from the declared needles: the five slopes (nearest 0.01), zeta's residual band (outward), C_zeta, its three parts, C_Delta and the characters' constants (nearest 0.01), the seven-cell deviation bound (outward 0.1), the residual's slope", ok)
