...
VERIFICATION (12 gates, exit-gated).
  ...
  V5 -- g9 the honest partials retained in the paper (the
       ...
       (this script backticked; "101 scripts cited in place";
       "Theorems 1i–1by" -- the census advances with each landing
...
PAPER_NEEDLES = [
    {'s': '101 scripts cited in place', 'form': 'plain', 'min': 1},
    {'s': 'The ladder is generated by the single constant Γ(½)', 'form': 'plain', 'min': 1},
    {'s': 'Theorems 1i–1by', 'form': 'plain', 'min': 1},
    {'s': '`cascade_riemann_selection.py`', 'form': 'plain', 'min': 1},
    {'s': "entered the source set by an external route (Adams' theorem and the Bott mirror,", 'form': 'plain', 'min': 1},
    {'s': 'it is NOT offered as a derivation of the integer label 7', 'form': 'plain', 'min': 1},
    {'s': 'no uniform rounding rule produces {5, 7, 19, 217} from the feature set', 'form': 'plain', 'min': 1},
    {'s': 'one condition, three consecutive tower arguments', 'form': 'plain', 'min': 1},
    {'s': 'single root on x > 0', 'form': 'plain', 'min': 1},
    {'s': 'struck round 159 F1', 'form': 'plain', 'min': 2, 'max': 2},
    {'s': 'the Amplitude feature AT the balance point, the Observer feature at exactly −1 from it', 'form': 'plain', 'min': 1},
    {'s': 'the record has ONE sink', 'form': 'plain', 'min': 1},
    {'s': 'the sink cannot source', 'form': 'plain', 'min': 1},
    {'s': 'the ζ-native content is at the CONTINUOUS features', 'form': 'plain', 'min': 1},
]
ok = (paper_needles.needle(PAPER_NEEDLES, "entered the source set by an external route (Adams' theorem and the Bott mirror,", 'plain'))
ok &= (paper_needles.needle(PAPER_NEEDLES, 'no uniform rounding rule produces {5, 7, 19, 217} from the feature set', 'plain'))
ok &= paper_needles.needle(PAPER_NEEDLES, 'the sink cannot source', 'plain')
...
ok = paper_needles.needle(PAPER_NEEDLES, 'one condition, three consecutive tower arguments', 'plain')
ok &= (paper_needles.needle(PAPER_NEEDLES, 'the Amplitude feature AT the balance point, the Observer feature at exactly −1 from it', 'plain'))
ok &= paper_needles.needle(PAPER_NEEDLES, 'The ladder is generated by the single constant Γ(½)', 'plain')
ok &= paper_needles.needle(PAPER_NEEDLES, 'it is NOT offered as a derivation of the integer label 7', 'plain')
ok &= paper_needles.needle(PAPER_NEEDLES, 'the ζ-native content is at the CONTINUOUS features', 'plain')
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'struck round 159 F1', 'plain')
ok &= paper_needles.needle(PAPER_NEEDLES, 'the record has ONE sink', 'plain')
ok &= paper_needles.needle(PAPER_NEEDLES, 'single root on x > 0', 'plain')
...
ok = paper_needles.needle(PAPER_NEEDLES, '`cascade_riemann_selection.py`', 'plain')
ok &= paper_needles.needle(PAPER_NEEDLES, '101 scripts cited in place', 'plain')
ok &= paper_needles.needle(PAPER_NEEDLES, 'Theorems 1i–1by', 'plain')
