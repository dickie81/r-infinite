...
dyadic-exclusive.  SLOGAN (structural, exact): Q_2(i) =
Q_2(gamma^2) -- what the order-8 clock cannot see is what the
field generated by the clock's square norms away.  GRADING:
...
ever reads the two coordinates stays open.  Sharpened falsifier:
any future finite-place derivation touching colour at p = 2 must
factor through exactly this subgroup.
...
def main():
    ...
    ok8d &= (-3) % 8 == 5 and hilbert(2, -3, 2) == -1
