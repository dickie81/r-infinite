...
def main():
    ...
    print()
    print("=" * 78)
    print("HONEST PARAMETER COUNT")
    print("=" * 78)
    print()
    print("Cascade structural commitment: ZERO free parameters.")
    print()
    print("Parameters in each phase:")
    print("  Phase 1 non-hybrid:  3 params (width, offset, weight) GLOBAL")
    print("  Phase 2 partial:     2 + 2 + 1 = 5 params (window for growth, window for")
    print("                       descent, single global weight)")
    print("  Phase 5 full hybrid: 3 params per generation x 3 gens = 9 params total")
    print("  Phase 6 same-weight: 1 + 2*3 = 7 params (single weight + 6 window choices)")
    print()
    print("THE CASCADE PROBLEM:")
    print()
    print("Phase 5's joint err of 0.415 comes at the cost of 9 free parameters.")
    print("This is NOT a cascade closure -- it's curve-fitting to residuals.")
    print()
    print("The cascade's existing alpha(d*)/chi^k correction-family closures (Part IVb)")
    print("have ZERO free parameters because Part IVb's source-selection rule (cascade")
    print("internal: P, L, G flags + structural d* layer assignment per observable")
    print("type) PICKS the (d*, k) cascade-natively without fitting.")
    print()
    print("For Bott averaging to be a real closure mechanism, we'd need a similar")
    print("structural selection rule that picks (window, weight) per generation from")
    print("cascade primitives -- without fitting to data.")
    print()
    print("WHAT THIS SEARCH ESTABLISHES:")
    print()
    print("(1) NEGATIVE: No parameter-free Bott-averaging scheme combats all three")
    print("    lepton residuals.  The structural-hybrid scheme (forward-2 for growth,")
    print("    centered-3 for descent, uniform weight) gives joint err 1.65 -- not a")
    print("    precision closure.")
    print()
    print("(2) PARTIAL POSITIVE for tau: with Omega^2 weighting at width 2 starting")
    print("    at d_g=5, the tau correction comes within 2.8% relative error.  This")
    print("    is essentially exact closure of the leading-order tau residual, but")
    print("    requires a SPECIFIC (Omega^2, [5,6]) choice that's not yet derived")
    print("    from cascade structure.  If a cascade-internal reason for Omega^2")
    print("    weighting at the growth-region generation can be found, this becomes")
    print("    a real closure.  Otherwise it's parameter introduction.")
    print()
    print("(3) NEGATIVE for mu, e: best parameter-light schemes give 25-33% rel err.")
    print("    Higher-precision closure requires per-gen-specific weighting that has")
    print("    no current cascade derivation.")
    print()
    print("HONEST VERDICT:")
    print()
    print("The systematic search demonstrates that the cascade's leading-order lepton")
    print("mass residuals cannot be cleanly closed by a UNIFORM Bott-averaging scheme.")
    print("Per-particle parameter introduction can fit the residuals but doesn't")
    print("constitute a cascade-internal mechanism.")
    print()
    print("The tau-specific result (Omega^2-weighted [5,6] at 2.8% rel err) is the")
    print("only structurally suggestive partial finding.  Whether it derives from")
    print("cascade primitives or is coincidence at the 2.8% level remains open.")
    print()
    print("The cascade's alpha(d*)/chi^k correction-family (Part IVb) remains the")
    print("zero-parameter route to precision closure.  Bott averaging is structurally")
    print("interesting but does not supply a competing parameter-free mechanism.")
