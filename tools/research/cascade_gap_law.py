...
zeta zero), and T_w, Theorem 1bx's sharp wall (the wall at which the balayaged hole pairs' closed form equals the observed Delta kappa_k, the R^-3 moment of the outer counting deficit n_k - n_1). Shares: s_ext the
...
(1.7 nats rms); that whole reading's exponential is the rung-leakage-weighted mean of its two parts' (a common factor times the mediant of the
two window ratios of the ground's share to the rung's), so it lies between them at every rung: the near quarter [T_1, 1.25 T_1) alone reads a
wall 2.8% below T_w on average (rms from T_w 4.7%), the field beyond one 1.2% above (rms from T_w 1.5%), the near reading the higher
at 39 of 40. The envelope itself, read by the estimator A(gamma)^2 = ghat(gamma)^2 + ghat(mid)^2 (a zero displaced by d
spacings gives ghat(gamma) = A sin(pi d), ghat(mid) = A cos(pi d)), is not lifted uniformly (each rung's figure the mean of ln A_k^2/A_1^2 over a window's zeros): in the leakage's units (the lift
squared against the constant squared) its lift over the ground state's, net of the origin ratio (V's convention, 1bx(ii)), minus B's
constant runs a third of a nat low on [T_1, 1.25 T_1) and a sixth high on [1.25, 2) T_1 -- 1bx's onset lag in the envelope, and a far-field overshoot; the deficit's log-moment (exact from the census: V(infinity) = 2[sum_{O_1} ln tau -
...
PAPER_NEEDLES = [
    ...
    {'g': 'g6', 's': 'the near quarter [T₁, 1.25T₁) alone reads +0.39 ± 0.47 nats, a wall 2.8% below T_w on average (rms deviation from T_w 4.7%), and the field beyond −0.20 ± 0.25, a wall 1.2% above (rms from T_w 1.5%), the near reading the higher at 39 of the 40 rungs', 'form': 'ws'},
    {'g': 'g6', 's': 'in the leakage’s units, the constant squared less the envelope lift squared net of the origin ratio squared (V’s convention, 1bx(ii)) runs +0.34 ± 0.48 nats on [T₁, 1.25T₁), −0.16 ± 0.32 on [1.25T₁, 1.5T₁) and −0.18 ± 0.26 on [1.5T₁, 2T₁) (means and rms over the 40 rungs)', 'form': 'ws'},
...
rn = [w["r_near"] for w in ALL]; rf = [w["r_far"] for w in ALL]; tn = [w["T_near"]/w["Tw"] for w in ALL]; tf = [w["T_far"]/w["Tw"] for w in ALL]
n_ord = sum(1 for w in ALL if w["r_near"] > w["r_far"]); n_between = sum(1 for w in ALL if min(w["r_near"], w["r_far"]) - 1e-12 <= w["res_B"] <= max(w["r_near"], w["r_far"]) + 1e-12)
...
ok &= 0.25 <= float(np.mean(rn)) <= 0.55 and -0.3 <= float(np.mean(rf)) <= -0.1 and 0.95 <= float(np.mean(tn)) <= 0.99 and 1.0 <= float(np.mean(tf)) <= 1.03 and n_ord >= 36 and n_between == 40 and rms1(tn) <= 0.07 and rms1(tf) <= 0.03
gate(f"g1 the exterior reads the sharp wall: the rung's exterior leakage beyond T_1 against the ground state's (the ground's own share beyond its edge S_1(1) = " + ", ".join(f"{x:.3f}" for x in s1one) + f" at the five cells, gated within [0.95, 0.99]) less 2 ln|ghat_k(0)/ghat_1(0)| minus cost(T_w): mean {float(np.mean(rB)):+.3f} (gated |.| <= 0.05), rms {rms(rB):.3f} (gated 0.15), largest {max(abs(x) for x in rB):.2f} (gated 0.35) at the 40 rungs; with T_u in place of T_w mean {float(np.mean(rBu)):+.2f} (gated <= -0.7), rms {rms(rBu):.2f} (gated >= 1.2); the envelope lift over the ground state's (the estimator ghat(gamma)^2 + ghat(mid)^2), net of the origin ratio, minus B's constant by sub-window (each rung's figure the mean of ln A_k^2/A_1^2 over the window's zeros; value less formula in this label, in the leakage's units ln A^2; the needle states the negative), [1, 1.25) T_1: mean {float(np.mean(sub[0])):+.3f} rms {rms(sub[0]):.3f} (gated mean in [-0.5, -0.2]); [1.25, 1.5) T_1: {float(np.mean(sub[1])):+.3f}, {rms(sub[1]):.3f} (gated [0.05, 0.3]); [1.5, 2) T_1: {float(np.mean(sub[2])):+.3f}, {rms(sub[2]):.3f} (gated [0.05, 0.3]); the windows hold {min(min(x) for x in subn)}-{max(max(x) for x in subn)} zeros (gated >= 5); the exterior reading by leakage window (formula less value): the near quarter [T_1, 1.25 T_1) mean {float(np.mean(rn)):+.3f} rms {rms(rn):.3f} (gated mean in [0.25, 0.55]), the field beyond {float(np.mean(rf)):+.3f}, {rms(rf):.3f} (gated [-0.3, -0.1]); the wall each names alone, T_near/T_w mean {float(np.mean(tn)):.4f} (gated [0.95, 0.99]) rms deviation {rms1(tn):.4f} (gated 0.07), T_far/T_w mean {float(np.mean(tf)):.4f} (gated [1.0, 1.03]) rms {rms1(tf):.4f} (gated 0.03); the whole's exponential the rung-leakage-weighted mean of the two windows' (the common factor times the mediant): between them at {n_between} of 40 (gated 40), the near reading above the far at {n_ord} of 40 (gated >= 36); the log-moment wall (cost(T) = 2 V(infinity), V(infinity) = 2[sum_(O_1) ln tau - sum_(O_k) ln tau - sum_h ln h] from the census sets): T_log > T_w at {tl_above} of 40 (gated >= 36), T_log/T_w mean {float(np.mean(tl)):.4f} (gated [1.005, 1.025]), standard deviation {tl_sd:.4f} (gated 0.02), range [{min(tl):.4f}, {max(tl):.4f}] (gated within [0.98, 1.05])", ok)
...
ssone = [w["s_ss"] for w in ONE]; nz12 = [w["N_zone"] for w in ALL if w["m"] <= 2]; gap1 = [w["Tw"]/w["Tu"] - 1 for w in ONE]   # the one-hole gaps T_w/T_u - 1 (1by(v)'s figures, from the same checkpoints)
...
S_WIN = 'the near quarter [T₁, 1.25T₁) alone reads +0.39 ± 0.47 nats, a wall 2.8% below T_w on average (rms deviation from T_w 4.7%), and the field beyond −0.20 ± 0.25, a wall 1.2% above (rms from T_w 1.5%), the near reading the higher at 39 of the 40 rungs'
S_LIFT = 'in the leakage’s units, the constant squared less the envelope lift squared net of the origin ratio squared (V’s convention, 1bx(ii)) runs +0.34 ± 0.48 nats on [T₁, 1.25T₁), −0.16 ± 0.32 on [1.25T₁, 1.5T₁) and −0.18 ± 0.26 on [1.5T₁, 2T₁) (means and rms over the 40 rungs)'
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'the near quarter [T₁, 1.25T₁) alone reads +0.39 ± 0.47 nats, a wall 2.8% below T_w on average (rms deviation from T_w 4.7%), and the field beyond −0.20 ± 0.25, a wall 1.2% above (rms from T_w 1.5%), the near reading the higher at 39 of the 40 rungs', 'ws')
ok &= paper_needles.needle(PAPER_NEEDLES, 'in the leakage’s units, the constant squared less the envelope lift squared net of the origin ratio squared (V’s convention, 1bx(ii)) runs +0.34 ± 0.48 nats on [T₁, 1.25T₁), −0.16 ± 0.32 on [1.25T₁, 1.5T₁) and −0.18 ± 0.26 on [1.5T₁, 2T₁) (means and rms over the 40 rungs)', 'ws')
...
ok &= _nums(S_WIN.split("alone reads ")[1].split(" nats")[0]) == [round(float(np.mean(rn)), 2), round(rms(rn), 2)] and _nums(S_WIN.split("a wall ")[1].split("% below")[0]) == [round(100*(1 - float(np.mean(tn))), 1)] and _nums(S_WIN.split("deviation from T_w ")[1].split("%")[0]) == [round(100*rms1(tn), 1)] and _nums(S_WIN.split("beyond ")[1].split(", a wall")[0]) == [round(float(np.mean(rf)), 2), round(rms(rf), 2)] and _nums(S_WIN.split("a wall ")[2].split("% above")[0]) == [round(100*(float(np.mean(tf)) - 1), 1)] and _nums(S_WIN.split("(rms from T_w ")[1].split("%")[0]) == [round(100*rms1(tf), 1)] and _ints(S_WIN.split("higher at ")[1].split(" of the")[0]) == [n_ord]
...
ok &= _nums(S_EPS1.split("residual is ")[1].split(" nats")[0]) == [round(x, 2) for x in eone] and _nums(S_EPS1.split("slopes ")[1].split("%")[0]) == [round(100*min(abs(x)/4 for x in eone), 1), round(100*max(abs(x)/4 for x in eone), 1)] and _nums(S_EPS1.split("gaps of ")[1]) == [round(100*min(gap1), 1), round(100*max(gap1), 1)]
