...
(1.7 nats rms); that whole's exponential is the rung-leakage-weighted mean of its two parts' (the mediant of the two window ratios),
so it lies between them at every rung: the near quarter [T_1, 1.25 T_1) alone reads a wall 2.8% below T_w on average (rms from T_w
4.7%), the field beyond one 1.2% above (rms 1.5%), the near reading the higher at 39 of 40. The envelope itself, read by the estimator A(gamma)^2 = ghat(gamma)^2 + ghat(mid)^2 (a zero displaced by d
spacings gives ghat(gamma) = A sin(pi d), ghat(mid) = A cos(pi d)), is not lifted uniformly: in the leakage's units (the lift
squared against the constant squared) its lift over the ground state's minus B's constant runs a third of a nat low on
[T_1, 1.25 T_1) and a sixth high on [1.25, 2) T_1 -- 1bx's onset lag in the envelope, and a far-field overshoot; the deficit's log-moment (exact from the census: V(infinity) = 2[sum_{O_1} ln tau -
...
PAPER_NEEDLES = [
    ...
    {'g': 'g6', 's': 'in the leakage’s units, the lift squared against the constant squared, the constant less the envelope lift over the ground state’s runs +0.34 ± 0.48 nats on [T₁, 1.25T₁), −0.16 ± 0.32 on [1.25T₁, 1.5T₁) and −0.18 ± 0.26 on [1.5T₁, 2T₁) (means and rms over the 40 rungs)', 'form': 'ws'},
...
S_LIFT = 'in the leakage’s units, the lift squared against the constant squared, the constant less the envelope lift over the ground state’s runs +0.34 ± 0.48 nats on [T₁, 1.25T₁), −0.16 ± 0.32 on [1.25T₁, 1.5T₁) and −0.18 ± 0.26 on [1.5T₁, 2T₁) (means and rms over the 40 rungs)'
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'in the leakage’s units, the lift squared against the constant squared, the constant less the envelope lift over the ground state’s runs +0.34 ± 0.48 nats on [T₁, 1.25T₁), −0.16 ± 0.32 on [1.25T₁, 1.5T₁) and −0.18 ± 0.26 on [1.5T₁, 2T₁) (means and rms over the 40 rungs)', 'ws')
