...
zeta zero), and T_w, Theorem 1bx's sharp wall (the R^-2-moment wall of the outer count deficit n_1 - n_k). (i) The exterior
reads the sharp wall: with the envelope estimator A(gamma)^2 = ghat(gamma)^2 + ghat(mid)^2 (a zero displaced by d spacings from
gamma gives ghat(gamma) = A sin(pi d), ghat(mid) = A cos(pi d)), the rung's exterior envelope lift over the ground state's on
[T_1, 1.5 T_1] equals Hypothesis B's constant 2 ln|ghat_k(0)/ghat_1(0)| + 4 sum_h ln(1/sigma_h(T_w)) at T_w, not at T_u; and the
deficit's log-moment (exact from the census: V(infinity) = 2[sum_{O_1} ln tau - sum_{O_k} ln tau - sum_h ln h], the value at infinity
of ln|ghat_k/ghat_1| - ln|ghat_k(0)/ghat_1(0)|) is B's at a wall 1.5% above T_w on average (4% at worst) -- the harmonic measure's log-mean per
hole is ln((1 + s_h) T), s_h = (1 - sigma_h^2)/(1 + sigma_h^2), witnessed in g0. (ii) The leakage budget (an identity): with the
exponent read twice -- Theorem 1by's functional at T_u (the total leakage) and Hypothesis B on the exterior at T_w (the exterior
leakage, a share s_ext of the total, the tail beyond the 6700-zero list included) -- the hole pairs' constant at the two walls
differs by the unlocking cost less the zone's share:
        cost(T_w) - cost(T_u) = U - ln(1/s_ext) + epsilon,   U = F_1(T_u) - F_1(T_1u),   cost(T) = 4 sum_h ln(1/sigma_h(T)),
epsilon the difference of the two readings' residuals (each within a third of a nat), the norm ratio exactly 1 (the eigenvectors
N-normalised). (iii) The share's law: the rung's leakage profile is the ground state's attached at T_u -- the exterior share is
the ground state's own share beyond T_1/T_u times its edge, s_ext = S_1(T_1/T_u) -- so T_w follows from the ground state alone:
T_u and U (the functional, with the rung's holes its only rung input), S_1 (the ground's leakage profile), no rung leakage consulted; to under one percent rms at the thirty rungs with three
or more holes, and not at one or two holes, where the zone is at most nine zeros wide. (iv) The leading law: F_1' = ln(T/2T_0) in the
continuum (witnessed), so U = (T_1u - T_u)^2/(2 T_1u) to leading order about the ground state's minimiser T_1u and cost' = 4m/T give
ln(T_w/T_u) = (T_1u - T_u)/(2 T_1u) - ln(1/s_ext)/(4m): the sharp wall is the geometric mean sqrt(T_u T_1u) reduced by the zone's share,
s_ext^{1/(4m)} -- 1bx's "near the geometric mean" derived, and the gap's growth with the hole count is the unlocking cost's
(T_1u - T_u ~ 4m per Theorem 1by(iii)); the continuum form of U is the law's shape, not its figure, where few zeros are unlocked. (v) The zone's
anatomy: the zone's zeros are half-held (sin^2(pi d) averages a half per zeta zero), the observed deficit at T_1 lies below the sharp
profile at T_u at every rung with three or more holes, and the zone carries under a fifth of -Delta kappa_k.

THE GATES. (0) the profiles' integrity (every state's polished eigenvalue against the census, the pencil residuals, the list's
capture) and the algebra witnessed at 60 digits (the pair deficit's two moments, the continuum slope); (1) the exterior lift at
T_w against T_u and the log-moment wall; (2) the identity, its residuals, T_w recovered from T_u, U and the measured share;
(3) the share's law and T_w from the ground state alone; (4) the leading law; (5) the zone's anatomy; (6) the paper's numbers
parsed back; (7) the chain obligation; (8) the needles and census.
...
PAPER_NEEDLES = [
    ...
    {'g': 'g6', 's': 'the rung’s exterior envelope lift over the ground state’s on [T₁, 1.5T₁] equals 2 ln|ĝ_k(0)/ĝ₁(0)| + 4Σ_h ln(1/σ_h(T_w)) within −0.09 ± 0.19 nats (mean, rms) at the 40 rungs, against 1.7 nats rms with T_u in place of T_w', 'form': 'ws'},
    {'g': 'g6', 's': 'the log-moment wall lies 1.5% above T_w (T_log/T_w averages 1.015 with rms deviation from 1 of 0.019 over [0.987, 1.040])', 'form': 'ws'},
    {'g': 'g6', 's': 'cost(T_w) − cost(T_u) = U − ln(1/s_ext) within a residual of mean −0.09 and rms 0.21 nats over [−0.57, +0.27] at the 40 rungs (the two readings’ residuals within [−0.16, +0.43] and ±0.29 nats; the norm ratio 1)', 'form': 'ws'},
    {'g': 'g6', 's': 'T_w recovered from T_u, U and the measured share within 1.3% rms at the 40 rungs and 0.9% at the 30 with three or more holes (T_id/T_w over [0.958, 1.023])', 'form': 'ws'},
    ...
    {'g': 'g6', 's': 'T_w from the ground state alone lies within 0.9% rms of 1bx’s at the 30 rungs with three or more holes (T_ss/T_w averages 1.0011 over [0.975, 1.023]) and within 2.3% at all 40, the worst δ = 2.6’s rung 2 at −11.3%', 'form': 'ws'},
    ...
    {'g': 'g6', 's': '√(T_uT_u(1))/T_w averages 1.016 with rms deviation from 1 of 0.023; reduced by the share, s_ext^{1/(4m)}√(T_uT_u(1))/T_w averages 0.997 with 0.017 (0.011 at the 30 rungs with three or more holes)', 'form': 'ws'},
    ...
    {'g': 'g6', 's': 'sin²(πd) averages 0.48 per zeta zero of the zone, with standard deviation 0.14 over the 39 rungs with a zone; the observed deficit at T₁ lies below the sharp profile at T_u at all 30 rungs with three or more holes, by 0.88 zeros on average', 'form': 'ws'},
...
for c in ORDER:
    ...
    G1[c] = {"T1": T1, "T1u": T1u, "a": a, "delta": S["delta"], "T0": S["T0"], "j1": j1, "S1": S1, "below1": float(cum1[int(np.searchsorted(ZS, T1)) - 1]), "n_mid": nm}
    for r, p in zip(S["rungs"][1:ns + 1], P["rungs"][1:ns + 1]):
        ...
        res_g3 = c2 + Fk(Tu, a, holes) - F1(T1u, a) - dl; res_B = c2 + cost(holes, Tw) + math.log(1/s_ext) + lnN - dl; eps = gapc - (U - math.log(1/s_ext) - lnN)
        res_1by = 2*math.log(abs(r["g0"]/g["g0"])) + Fk(Tu, a, holes) - F1(T1u, a) - (r["ln_lam"] - g["ln_lam"])    # Theorem 1by's g3 residual from the census
        win = (ZS >= T1) & (ZS < 1.5*T1) & (Ek > -1e8) & (E1 > -1e8); n_win = int(win.sum())
        lift = float(np.mean((Ek - E1)[win])); liftB_w = c2 + cost(holes, Tw); liftB_u = c2 + cost(holes, Tu)
        T_id = solve_cost(holes, cost(holes, Tu) + U - math.log(1/s_ext) - lnN, 50*T1)
        s_ss = S1(T1/Tu); T_ss = solve_cost(holes, cost(holes, Tu) + U - math.log(1/s_ss) - lnN, 50*T1)
        gm0 = math.sqrt(Tu*T1u); gm = gm0*math.exp(-(math.log(1/s_ext) + lnN)/(4*m))
        ...
        U_cont = (T1u - Tu)**2/(2*T1u)
        W[(c, k)] = dict(k=k, m=m, holes=holes, Tw=Tw, Tu=Tu, T1=T1, T1u=T1u, s_below=s_below, s_zone=s_zone, s_ext=s_ext, lnN=lnN, c2=c2, dl=dl, U=U, gapc=gapc,
                         res_g3=res_g3, res_B=res_B, eps=eps, res_1by=res_1by, lift=lift, liftB_w=liftB_w, liftB_u=liftB_u, n_win=n_win, T_id=T_id, s_ss=s_ss, T_ss=T_ss, gm0=gm0, gm=gm,
                         Vinf=Vinf, T_log=T_log, Dk_c=Dk_c, Dk=L[k]["Dkappa"], mom_zone=mom_zone, mom_ext=mom_ext, N_zone=N_zone, S_zone=S_zone, D_T1=D_T1, sharp_u=sharp_u, U_cont=U_cont)
...
dw = [w["lift"] - w["liftB_w"] for w in ALL]; du = [w["lift"] - w["liftB_u"] for w in ALL]; nwin = [w["n_win"] for w in ALL]
tl = [w["T_log"]/w["Tw"] for w in ALL]
ok = abs(float(np.mean(dw))) <= 0.15 and rms(dw) <= 0.25 and max(abs(x) for x in dw) <= 0.6 and min(nwin) >= 5
ok &= rms(du) >= 1.2 and float(np.mean(du)) >= 0.7
ok &= 1.005 <= float(np.mean(tl)) <= 1.025 and rms1(tl) <= 0.03 and min(tl) >= 0.98 and max(tl) <= 1.05
gate(f"g1 the exterior reads the sharp wall: the rung's envelope lift over the ground state's on [T_1, 1.5 T_1] ({min(nwin)}-{max(nwin)} zeros, gated >= 5; the estimator ghat(gamma)^2 + ghat(mid)^2) minus Hypothesis B's constant 2 ln|ghat_k(0)/ghat_1(0)| + cost(T_w): mean {float(np.mean(dw)):+.3f} (gated |.| <= 0.15), rms {rms(dw):.3f} (gated 0.25), largest {max(abs(x) for x in dw):.2f} (gated 0.6) at the 40 rungs; with T_u in place of T_w the rms is {rms(du):.2f} (gated >= 1.2) and the mean {float(np.mean(du)):+.2f} (gated >= 0.7); the log-moment wall (cost(T) = 2 V(infinity), V(infinity) = 2[sum_(O_1) ln tau - sum_(O_k) ln tau - sum_h ln h] from the census sets): T_log/T_w mean {float(np.mean(tl)):.4f} (gated [1.005, 1.025]), rms deviation from 1 {rms1(tl):.4f} (gated 0.03), range [{min(tl):.4f}, {max(tl):.4f}] (gated within [0.98, 1.05])", ok)

# ---------------------------------------------------------------- g2: the leakage budget (the identity) and T_w recovered from T_u, U and the measured share
lnN_max = max(abs(w["lnN"]) for w in ALL); eps = [w["eps"] for w in ALL]; r3 = [w["res_g3"] for w in ALL]; rB = [w["res_B"] for w in ALL]
d3 = max(abs(w["res_g3"] - w["res_1by"]) for w in ALL)
tid = [w["T_id"]/w["Tw"] for w in ALL]; tid_hm = [w["T_id"]/w["Tw"] for w in HM]
sext = [w["s_ext"] for w in ALL]; sone = [w["s_ext"] for w in ONE]; sbel = [w["s_below"] for w in ALL]
ok = lnN_max <= 1e-9 and abs(float(np.mean(eps))) <= 0.15 and rms(eps) <= 0.3 and max(abs(x) for x in eps) <= 0.7
ok &= d3 <= 1e-6 and min(r3) >= -0.2 and max(r3) <= 0.5 and max(abs(x) for x in rB) <= 0.4
ok &= abs(float(np.mean(tid)) - 1) <= 0.005 and rms1(tid) <= 0.02 and rms1(tid_hm) <= 0.012 and min(tid) >= 0.95 and max(tid) <= 1.03
ok &= min(sone) >= 0.9 and min(sext) >= 0.4 and max(sext) <= 0.95 and max(sbel) <= 0.12
gate(f"g2 the leakage budget cost(T_w) - cost(T_u) = U - ln(1/s_ext) at the 40 rungs (the norm ratio ln(N_1/N_k) within {lnN_max:.1e} of 0, gated 1e-9): the residual epsilon mean {float(np.mean(eps)):+.3f} (gated |.| <= 0.15), rms {rms(eps):.3f} (gated 0.3), largest {max(abs(x) for x in eps):.2f} (gated 0.7); its two readings -- Theorem 1by's functional at T_u, residual within [{min(r3):+.2f}, {max(r3):+.2f}] (gated within [-0.2, 0.5]; equal to 1by's census-based residual within {d3:.1e}, gated 1e-6), and Hypothesis B on the exterior at T_w with the share, residual within {max(abs(x) for x in rB):.2f} (gated 0.4); the exterior share {min(sext):.3f}-{max(sext):.3f} (gated within [0.4, 0.95]), {min(sone):.3f} and above at one hole (gated >= 0.9), the share below T_u at most {max(sbel):.3f} (gated 0.12); T_w recovered from T_u, U and the measured share: T_id/T_w mean {float(np.mean(tid)):.4f} (gated within 0.005 of 1), rms deviation {rms1(tid):.4f} (gated 0.02), range [{min(tid):.4f}, {max(tid):.4f}] (gated within [0.95, 1.03]); at the 30 rungs with three or more holes rms {rms1(tid_hm):.4f} (gated 0.012)", ok)
...
tss = [w["T_ss"]/w["Tw"] for w in ALL]; tss_hm = [w["T_ss"]/w["Tw"] for w in HM]
...
ok &= abs(float(np.mean(tss_hm)) - 1) <= 0.004 and rms1(tss_hm) <= 0.012 and min(tss_hm) >= 0.965 and max(tss_hm) <= 1.035
ok &= rms1(tss) <= 0.03 and WORST[1:] == ("d2.6", 2) and WORST[0] <= -0.10 and len(TSS1) == 5
ok &= min(ssone) <= 0.7 and max(ssone) >= 0.95 and max(nz12) <= 9 and len(nz12) == 10
gate(f"g3 the share's law s_ext = S_1(T_1/T_u) (the ground state's share beyond T_1/T_u times its edge): ln(S_1/s_ext) mean {float(np.mean(lss)):+.3f} (gated |.| <= 0.1), rms {rms(lss):.3f} (gated 0.15), range [{min(lss):+.3f}, {max(lss):+.3f}] (gated within [-0.4, 0.3]) at the 40 rungs, rms {rms(lss_hm):.3f} at the 30 with three or more holes (gated 0.13); T_w from the ground state alone (T_u, U, S_1): T_ss/T_w at the 30 rungs with three or more holes mean {float(np.mean(tss_hm)):.4f} (gated within 0.004 of 1), rms deviation {rms1(tss_hm):.4f} (gated 0.012), range [{min(tss_hm):.4f}, {max(tss_hm):.4f}] (gated within [0.965, 1.035]); at all 40 rms {rms1(tss):.4f} (gated 0.03), the worst {WORST[1]} rung {WORST[2]} at {100*WORST[0]:+.1f}% (gated delta = 2.6 rung 2, <= -10%); the one-hole rungs: " + ", ".join(f"{c} {x:.3f}" for c, k, x in TSS1) + f"; at one hole S_1(T_1/T_u) runs {min(ssone):.3f}-{max(ssone):.3f} (gated min <= 0.7, max >= 0.95) against the shares {min(sone):.3f}-{max(sone):.3f}, the zone at one or two holes {min(nz12)}-{max(nz12)} zeros wide over {len(nz12)} rungs (gated <= 9; 10)", ok)

# ---------------------------------------------------------------- g4: the leading law -- the geometric mean reduced by the share
...
lead = [math.log(w["Tw"]/w["Tu"]) - ((w["T1u"] - w["Tu"])/(2*w["T1u"]) - math.log(1/w["s_ext"])/(4*w["m"])) for w in ALL]
ok = 1.01 <= float(np.mean(g0r)) <= 1.025 and rms1(g0r) <= 0.03 and abs(float(np.mean(gr)) - 1) <= 0.01 and rms1(gr) <= 0.02 and rms1(gr_hm) <= 0.012
ok &= float(np.mean(gr)) < float(np.mean(g0r)) and rms1(gr) < rms1(g0r)
ok &= 0.2 <= min(uc_hm) and max(uc_hm) <= 1.35 and len(uc_hm) == 30 and len(uc_lo) == 7
ok &= rms(lead) <= 0.025
gate(f"g4 the leading law ln(T_w/T_u) = (T_1u - T_u)/(2 T_1u) - ln(1/s_ext)/(4m): sqrt(T_u T_1u)/T_w mean {float(np.mean(g0r)):.4f} (gated [1.01, 1.025]), rms deviation {rms1(g0r):.4f} (gated 0.03); reduced by the share, s_ext^(1/4m) sqrt(T_u T_1u)/T_w mean {float(np.mean(gr)):.4f} (gated within 0.01 of 1), rms {rms1(gr):.4f} (gated 0.02; the reduction lowering both the mean and the rms, gated), {rms1(gr_hm):.4f} at the 30 rungs with three or more holes (gated 0.012); the law's residual in ln rms {rms(lead):.4f} (gated 0.025); the unlocking cost against its continuum form U_c = (T_1u - T_u)^2/(2 T_1u): U/U_c mean {float(np.mean(uc_hm)):.3f}, range [{min(uc_hm):.3f}, {max(uc_hm):.3f}] at the rungs with three or more holes and [{min(uc_lo):.3f}, {max(uc_lo):.3f}] at those with one or two and a wall below T_1u (gated within [0.2, 1.35] at the 30 with three or more holes; 7 at one or two)", ok)

# ---------------------------------------------------------------- g5: the zone's anatomy
s2 = [w["S_zone"]/w["N_zone"] for w in ZN]; n_zn = len(ZN)
below = sum(1 for w in HM if w["D_T1"] < w["sharp_u"]); lag = [w["sharp_u"] - w["D_T1"] for w in HM]
mz = [w["mom_zone"] for w in ALL]; me = [w["mom_ext"] for w in ALL]; dkr = max(abs(w["Dk_c"]/w["Dk"] - 1) for w in ALL)
ok = n_zn == 39 and 0.40 <= float(np.mean(s2)) <= 0.60 and float(np.std(s2)) <= 0.2 and min(s2) >= 0.15 and max(s2) <= 0.9
ok &= below == 30 and min(lag) > 0 and 0.6 <= float(np.mean(lag)) <= 1.2
ok &= max(mz) <= 0.19 and min(me) >= 0.81 and dkr <= 1e-6
gate(f"g5 the zone's anatomy: sin^2(pi d) = ghat(gamma)^2/A(gamma)^2 averages {float(np.mean(s2)):.3f} per zeta zero of the zone (gated [0.40, 0.60]) with standard deviation {float(np.std(s2)):.3f} over the {n_zn} rungs with a zone (gated 39; <= 0.2), range [{min(s2):.2f}, {max(s2):.2f}] (gated within [0.15, 0.9]); the observed deficit at T_1 lies below the sharp profile at T_u at {below} of the 30 rungs with three or more holes (gated 30), by {float(np.mean(lag)):.2f} zeros on average (gated [0.6, 1.2]), range [{min(lag):.2f}, {max(lag):.2f}]; the zone [T_u, T_1) carries at most {max(mz):.3f} of -Delta kappa_k (gated 0.19) and the exterior beyond T_1 at least {min(me):.3f} (gated 0.81); Delta kappa_k from the census sets against rung_laws' within {dkr:.1e} (gated 1e-6: Theorem 1bx(i))", ok)
...
S_LIFT = 'the rung’s exterior envelope lift over the ground state’s on [T₁, 1.5T₁] equals 2 ln|ĝ_k(0)/ĝ₁(0)| + 4Σ_h ln(1/σ_h(T_w)) within −0.09 ± 0.19 nats (mean, rms) at the 40 rungs, against 1.7 nats rms with T_u in place of T_w'
S_LOG = 'the log-moment wall lies 1.5% above T_w (T_log/T_w averages 1.015 with rms deviation from 1 of 0.019 over [0.987, 1.040])'
S_ID = 'cost(T_w) − cost(T_u) = U − ln(1/s_ext) within a residual of mean −0.09 and rms 0.21 nats over [−0.57, +0.27] at the 40 rungs (the two readings’ residuals within [−0.16, +0.43] and ±0.29 nats; the norm ratio 1)'
S_TID = 'T_w recovered from T_u, U and the measured share within 1.3% rms at the 40 rungs and 0.9% at the 30 with three or more holes (T_id/T_w over [0.958, 1.023])'
...
S_TSS = 'T_w from the ground state alone lies within 0.9% rms of 1bx’s at the 30 rungs with three or more holes (T_ss/T_w averages 1.0011 over [0.975, 1.023]) and within 2.3% at all 40, the worst δ = 2.6’s rung 2 at −11.3%'
...
S_GM = '√(T_uT_u(1))/T_w averages 1.016 with rms deviation from 1 of 0.023; reduced by the share, s_ext^{1/(4m)}√(T_uT_u(1))/T_w averages 0.997 with 0.017 (0.011 at the 30 rungs with three or more holes)'
...
S_ZONE = 'sin²(πd) averages 0.48 per zeta zero of the zone, with standard deviation 0.14 over the 39 rungs with a zone; the observed deficit at T₁ lies below the sharp profile at T_u at all 30 rungs with three or more holes, by 0.88 zeros on average'
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'the rung’s exterior envelope lift over the ground state’s on [T₁, 1.5T₁] equals 2 ln|ĝ_k(0)/ĝ₁(0)| + 4Σ_h ln(1/σ_h(T_w)) within −0.09 ± 0.19 nats (mean, rms) at the 40 rungs, against 1.7 nats rms with T_u in place of T_w', 'ws')
ok &= paper_needles.needle(PAPER_NEEDLES, 'the log-moment wall lies 1.5% above T_w (T_log/T_w averages 1.015 with rms deviation from 1 of 0.019 over [0.987, 1.040])', 'ws')
ok &= paper_needles.needle(PAPER_NEEDLES, 'cost(T_w) − cost(T_u) = U − ln(1/s_ext) within a residual of mean −0.09 and rms 0.21 nats over [−0.57, +0.27] at the 40 rungs (the two readings’ residuals within [−0.16, +0.43] and ±0.29 nats; the norm ratio 1)', 'ws')
ok &= paper_needles.needle(PAPER_NEEDLES, 'T_w recovered from T_u, U and the measured share within 1.3% rms at the 40 rungs and 0.9% at the 30 with three or more holes (T_id/T_w over [0.958, 1.023])', 'ws')
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'T_w from the ground state alone lies within 0.9% rms of 1bx’s at the 30 rungs with three or more holes (T_ss/T_w averages 1.0011 over [0.975, 1.023]) and within 2.3% at all 40, the worst δ = 2.6’s rung 2 at −11.3%', 'ws')
...
ok &= paper_needles.needle(PAPER_NEEDLES, '√(T_uT_u(1))/T_w averages 1.016 with rms deviation from 1 of 0.023; reduced by the share, s_ext^{1/(4m)}√(T_uT_u(1))/T_w averages 0.997 with 0.017 (0.011 at the 30 rungs with three or more holes)', 'ws')
...
ok &= paper_needles.needle(PAPER_NEEDLES, 'sin²(πd) averages 0.48 per zeta zero of the zone, with standard deviation 0.14 over the 39 rungs with a zone; the observed deficit at T₁ lies below the sharp profile at T_u at all 30 rungs with three or more holes, by 0.88 zeros on average', 'ws')
ok &= paper_needles.needle(PAPER_NEEDLES, 'the zone [T_u, T₁) carries at most 18% of −Δκ_k and the exterior beyond T₁ at least 82%', 'ws')
ok &= [d['s'] for d in paper_needles.declared(PAPER_NEEDLES) if d.get('g') == 'g6'] == [S_PRO, S_LIFT, S_LOG, S_ID, S_TID, S_SS, S_TSS, S_ONE, S_GM, S_UC, S_ZONE, S_MOM]
_re = __import__("re")
...
ok &= _ints(S_PRO.split("at all ")[1].split(" states")[0]) == [n_pro] and _nums(S_PRO.split("carries ")[1]) == [round(100*cap_lo, 1), round(100*cap_hi, 1)] and resid <= -150 and d_lam <= 1e-8
ok &= _nums(S_LIFT.split("within ")[1].split(" nats (mean")[0]) == [round(float(np.mean(dw)), 2), round(rms(dw), 2)] and _nums(S_LIFT.split("against ")[1]) == [round(rms(du), 1)] and _ints(S_LIFT.split("at the ")[1].split(" rungs")[0]) == [len(ALL)]
ok &= _nums(S_LOG) == [round(100*(float(np.mean(tl)) - 1), 1), round(float(np.mean(tl)), 3), round(rms1(tl), 3), round(min(tl), 3), round(max(tl), 3)]
...
ok &= _nums(S_ID.split("residuals within ")[1].split(" nats")[0]) == [round(min(r3), 2), round(max(r3), 2), round(max(abs(x) for x in rB), 2)] and lnN_max <= 1e-9
...
ok &= _nums(S_TSS.split("within ")[1].split("% rms")[0]) == [round(100*rms1(tss_hm), 1)] and _nums(S_TSS.split("averages ")[1].split(")")[0]) == [round(float(np.mean(tss_hm)), 4), round(min(tss_hm), 3), round(max(tss_hm), 3)]
ok &= _nums(S_TSS.split("and within ")[1].split("%")[0]) == [round(100*rms1(tss), 1)] and _nums(S_TSS.split("rung 2 at ")[1].split("%")[0]) == [round(100*WORST[0], 1)] and WORST[1:] == ("d2.6", 2) and _ints(S_TSS.split("at the ")[1].split(" rungs")[0]) == [len(HM)]
...
ok &= _nums(S_ZONE) == [round(float(np.mean(s2)), 2), round(float(np.std(s2)), 2), round(float(np.mean(lag)), 2)] and _ints(S_ZONE.split("over the ")[1].split(" rungs")[0]) == [n_zn] and _ints(S_ZONE.split("at all ")[1].split(" rungs")[0]) == [below]
