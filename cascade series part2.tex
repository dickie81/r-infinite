...
The compactification results of~\cite{paperI}, Section~4 are integrated into the temporal
interpretation: each slicing step sends one direction's effective radius to
$R_{\rm eff}=1/\sqrt{d+3}$, giving concrete geometric content to the arrow of time as
asymptotic suppression rather than sharp elimination. The boundary dominance
$\Omega_{d-1}/V_d=d$ (Theorem~4.4 of~\cite{paperI}) confirms that the observer's state
...
Section~4 of~\cite{paperI} proves that each step of the slicing recurrence is an asymptotic
compactification: the integrated-out direction retains an effective radius
...
areas, not volumes, are the primary objects. Corollary~4.8a of~\cite{paperI} establishes
that sphere area is the only independent cascade quantity at each level: every other cascade
object is derived from sphere areas, and any identification mapping cascade content to energy
must do so with a universal constant.
...
\begin{theorem}[Boundary dominance; Theorem~4.4 of~\cite{paperI}]
...
The compactification results of~\cite{paperI}, Section~4 sharpen this interpretation. Each
slicing step does not annihilate the integrated-out direction; it suppresses it to an
effective radius $R_{\rm eff}(d)=1/\sqrt{d+3}$ (Theorem~4.2 of~\cite{paperI}). The weight
function $(1-x^2)^{d/2}$ remains positive on $(-1,1)$ for all finite $d$. Measurement in
the cascade is asymptotic suppression, not sharp projection: the complementary coordinate is
exponentially suppressed but never exactly zero. This is a geometric realisation of the fact
that quantum measurement does not destroy the unmeasured degree of freedom---it renders it
inaccessible at scale $R_{\rm eff}$.
...
\subsection{Forced orthogonality of consecutive slicing axes}

\begin{theorem}[Precession angle is forced]\label{thm:precession}
The angle $\alpha$ between consecutive slicing axes $e_{d+1}$ and $e_d$ is forced to equal
$\pi/2$ by the same orthogonality axiom that forces $\Gamma(\tfrac{1}{2})=\sqrt{\pi}$ in
the slicing recurrence. The two applications of the axiom are distinct steps of the same
derivation, not a circular reuse of one result to prove another.
\end{theorem}
\begin{proof}
The cascade's orthogonality axiom is: at each step, the slicing direction is perpendicular
to all previously integrated directions. This single axiom is applied twice in succession.

\emph{First application (forces $\sqrt{\pi}$)}: At step $d+1$, the slicing direction
$e_{d+1}$ is chosen perpendicular to the existing $d$-ball. The slicing integral is:
\[
V_{d+1} = V_d\int_{-1}^1(1-x^2)^{d/2}dx.
\]
The substitution $x=\cos\theta$, $\theta\in[0,\pi/2]$, maps the half-interval to a quarter
turn, producing $B(\tfrac{1}{2},(d+1)/2)/2$, and hence $\Gamma(\tfrac{1}{2})=\sqrt{\pi}$.
The forced angle here is between the slicing axis and the equatorial hyperplane: always
$\pi/2$ by the definition of slicing.

\emph{Second application (forces $\alpha=\pi/2$)}: After step $d+1$, the direction
$e_{d+1}$ has been consumed by the integral. The residual geometry is the $d$-ball, which
lies entirely in the hyperplane orthogonal to $e_{d+1}$. The next slicing direction $e_d$
must be chosen from within this $d$-ball---it cannot have any component along $e_{d+1}$,
because that direction no longer exists as a geometric direction in the residual space.
Therefore $e_d\perp e_{d+1}$, giving $\alpha=\pi/2$.

The two applications are sequential, not circular: the first forces $\sqrt{\pi}$ by noting
that the slicing axis is perpendicular to the equatorial plane; the second forces
$\alpha=\pi/2$ by noting that the next slicing axis is perpendicular to the current slicing
axis, because the current axis has been integrated out. Both conclusions follow from the
...
irreversibility of integrating out a direction. The compactification results of~\cite{paperI},
Section~4 give this identification concrete geometric content: each slicing step does not
eliminate a direction but asymptotically suppresses it, with an exact compactification radius
determined by the Beta function.

\subsection{The effective radius of each temporal step}

\begin{theorem}[Compactification radius; Theorem~4.2 of~\cite{paperI}]
\label{thm:reff}
...
The compactification is asymptotic: $R_{\rm eff}\to 0$ as $d\to\infty$, but
$R_{\rm eff}\neq 0$ at any finite $d$. The weight function $(1-x^2)^{d/2}$ is smooth and
positive on $(-1,1)$ for all finite $d$. There is no sharp boundary, no horizon, no
topology change---only exponential suppression.
...
exponentially localised near $x=0$ with Gaussian width $\sigma\approx 1/\sqrt{d}$ (Lemma
4.1 of~\cite{paperI}). From the perspective of the $d$-dimensional residual geometry, the
direction has been compactified.
...
This is made precise by boundary dominance: $\Omega_{d-1}/V_d=d$ (Theorem~4.4
...
The compactification interpretation (Section~8) sharpens the entanglement structure. The
fibre coordinate $x$ is compactified to effective radius $R_{\rm eff}=1/\sqrt{d+3}$, but
the base radius $\sqrt{1-x^2}$ depends on $x$ throughout the compactification region. The
base--fibre entanglement is generated by this $x$-dependent coupling and persists at all
scales above $R_{\rm eff}$. Below $R_{\rm eff}$, the fibre is effectively frozen and the
entanglement is inaccessible to a $d$-dimensional observer---but it is retained in the
...
Irreversibility is asymptotic & $R_{\rm eff}>0$; no sharp loss & 8.2 \\
Unitarity + apparent irreversibility & Global unitary; boundary retains info & 8.3 \\
...
(2) the assumption that the observer has access to 4 dimensions. Five rows (marked 2.3,
4.1, 8, 8.2, 8.3) draw on the compactification results of~\cite{paperI}, Section~4. The
phase classification row (6.5) is derived from the forced precession of this paper combined
...
\item Derives the Gaussian character of wavefunctions from projection.
...
\item Integrates the compactification results of~\cite{paperI}, Section~4 into the quantum
interpretation: each temporal step is an asymptotic compactification with
$R_{\rm eff}=1/\sqrt{d+3}$. The arrow of time, the nature of measurement, and the
