...
The compactification results of~\cite{paperI}, Corollary~3.2 are developed into a full
geometric interpretation: each slicing step sends one direction's effective radius to
$R_{\rm eff}=1/\sqrt{d+3}$ (proved here from the Beta function), giving concrete geometric
...
%%% INSERTION 4: Layered boundary dominance remark %%%
...
%%% INSERTION 1: Gaussian concentration lemma %%%
...
%%% INSERTION 2: Full R_eff proof (replacing citation-only version) %%%
...
$B(1/2,\,d/2+1)$. Using $\Gamma(x+1) = x\,\Gamma(x)$:
\[
\langle x^2\rangle = \frac{B(3/2,\,d/2+1)}{B(1/2,\,d/2+1)}
= \frac{\Gamma(3/2)\,\Gamma(d/2+1)}{\Gamma(d/2+5/2)}
\cdot\frac{\Gamma(d/2+3/2)}{\Gamma(1/2)\,\Gamma(d/2+1)}
= \frac{1}{d+3}.
\]
...
%%% INSERTION 3: Scale coincidence and erf(1/sqrt(2)) %%%
