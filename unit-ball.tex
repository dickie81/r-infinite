...
Thus the required suppression \(\rho_\Lambda \approx 10^{-120} M_{\rm Pl}^4\) emerges as a genuine output of the complete KK + winding spectrum of any UV-complete higher-dimensional theory once evaluated at the discrete saddle \((N=219, m=1)\). The same derivation extends verbatim to the full anisotropic case of §5.
...
\subsection{Dynamical Realization via Einstein-Gauss-Bonnet Geometric Frustration}
The Discretized Saddle Principle selects the final topology, but the actual time-dependent collapse process is realized dynamically through geometric frustration in Einstein-Gauss-Bonnet (EGB) gravity. In EGB (or higher Lovelock) theories the action contains quadratic curvature terms
\[
S = \int d^{4+N}x\sqrt{-g}\Bigl(R + \alpha R_{\mu\nu\rho\sigma}R^{\mu\nu\rho\sigma} + \cdots\Bigr).
\]
For a \((4+N)\)-dimensional spacetime the effective cosmological-constant equation admits a discriminant \(c_1^2 - 4c_0c_2 < 0\) in a broad region of parameter space. When this condition holds, no maximally symmetric \((4+N)\)-dimensional vacuum exists. The theory is geometrically frustrated: isotropic expansion of all directions is impossible. The system is forced into a warped product \(M^4 \times b(t)M^N\), where the external 4D scale factor \(a(t)\) expands while the internal scale factors \(b_i(t)\) contract. The effective Friedmann equations become
\[
\alpha + \beta y + \frac{6\beta x}{N(N-1)} + \gamma y^2 + \frac{12\gamma x y (N-2)(N-3)}{N(N-1)} = 0,
\]
with \(x = H_a^2 > 0\) and \(y = {}^{(N)}R / b^2 < 0\) (negative curvature on the compact manifold). As \(t\to\infty\), \(b(t)\to b_c\) (Planck-scale constant) for all but the marginal direction identified by the discrete Poisson-balance condition.

The positive mismatch \(\delta(N,m) > 0\) enters the perturbed EGB equations of motion as the explicit frustration gradient \(\partial U/\partial\lambda = +\delta\), preventing further collapse once the saddle \((N=219,m=1)\) is reached. Stability analysis shows that the damping coefficients grow as \(N^5\)--\(N^6\), making high-\(N\) configurations increasingly stable — precisely consistent with the exponential suppression of large-\(m\) saddles via concentration of measure. Thus the entire collapse history (from an initial high-dimensional phase down to the unique surviving dark direction at \(\sim 3\)--\(8\,\mu\)m) is a deterministic geometric process with no fine-tuning and no ad-hoc potential. The Epstein potential of the remaining radion then takes over for the final stabilization (see next subsection).
