...
first zeta zero with no zero of ĝ_k within the hole tolerance 0.2; 1bu's
T_D, at 0.05, lies below the ground state's T₁ (T₁ = 121.4 at δ = 2.3
against 1bu’s T_D = 111.0; T_D below T₁ by 2.9–8.5% over the five cells)
— inside T₁ and never outward with k (the rungs’ edges 121.4, 111.9,
111.0, 105.4, 103.7, 94.7, 87.4 for rungs 1–7 at δ = 2.3); below it one
dodging zero per zeta zero, shared with the ground state’s within 10⁻¹²
up to γ = 69.5, 65.1, 59.3, 49.8 at rungs 2–5 and within 2.7 × 10⁻⁶
below γ = 40 at rung 8, the difference growing toward the rung’s edge to
0.11, 0.11, 0.16, 0.18, 0.08, 0.11, 0.09 at rungs 2–8 (δ = 2.3); the k −
1 hole zeros of 1bu(iii); beyond it the exterior: n_dodge + (k − 1) +
...
the census, Δκ_k = [Σ_{E_k}τ⁻² − Σ_{E_1}τ⁻²] − Σ_freed γ⁻² over the zeta
zeros the ground state dodges and rung k does not, up to the dodging
displacements (the accounting residual within 2.7 × 10⁻⁶ at every safely
deep rung): the k − 1 hole pairs are paid for by the exterior, Δκ_k < 0
and falling with k (−Δκ_k = 3.49 × 10⁻⁵, 7.87 × 10⁻⁵, 1.21 × 10⁻⁴, 1.70
× 10⁻⁴, 2.20 × 10⁻⁴, 2.87 × 10⁻⁴ for rungs 2–7 at δ = 2.3), the
effective height of the exterior pair a hole replaces τ_eff := √((k −
1)/(−Δκ_k)) between 1.17 and 1.42 T₁ over the safely deep rungs of the
five cells (τ_eff = 1.40, 1.31, 1.30, 1.26, 1.24, 1.19 T₁ at δ = 2.3).
The quadratic law holds to r = 40: the ratio (−ln C_k(r)/r²)/Δκ_k is
within 2 × 10⁻³ of 1 at r = 5 and within 0.25 at r = 40 (the ratio
1.049, 1.053, 1.056, 1.058, 1.062, 1.069 at r = 40 for rungs 2–7 at δ =
2.3). *(iii) The bi-orthogonal law (computed, gated).* The rungs are
N-orthogonal, ∫ĝ_jĝ_k = 0, so the hole polynomial H_k is orthogonal to
H_j for every j < k under the pair weight ĝ₁²C_kC_j: k − 1 linear
conditions that determine H_k up to scale. On the interior [0, T₁/2]
(the interior carrying 1.000000, 1.000000, 1.000000, 1.000000, 1.000000
of the ground state’s mass), with the exact C_k the conditions reproduce
the censused holes (E within 1.2 × 10⁻⁸, 1.7 × 10⁻⁸, 2.0 × 10⁻⁸, 2.2 ×
10⁻⁸, 2.5 × 10⁻⁸, 2.3 × 10⁻⁸ for rungs 2–7 at δ = 2.3); with the
Gaussian truncation C_j = e^{−Δκ_j r²} and the recursion's own lower
...
re-balayage in one number per rung, is computed, not derived: that is
what remains of (c). *(v) What is proved, computed, not claimed.*
Proved: the factorisation's identity and the r² coefficient
(elementary). Computed, gated: the census (the counts of dodging and
hole zeros directly, the placed pairs' neighbours), the edges, the
shared dodging zeros' displacements, the curvature law's residuals, the
bi-orthogonal law's residuals against the naive nodes and the mangle,
τ_eff, the fall of Δκ₂ across the cells, rung 9's touching at δ = 2.3.
Not claimed: a derivation of Δκ_k from the prime side or from the
balayage; the O(r⁴) term's law; the law beyond the safely deep rungs
...
1. **The caster explained — a prime-side reason for the shape, valid at
   ...
   rule fixes — with Δκ_k computed, not derived; (d) a zero pair at fixed
   ...
   1bv(i)) with no zero computed, and (c)'s coefficients Δκ_k derived (the
   exterior's re-balayage, Theorem 1bw's one number per rung) — a finite
