...
(χ₋₃(2) = −1) — gated as a same-fact check, an identification the paper already
...
(structural, exact):* ℚ₂(i) = ℚ₂(γ²) — **what the order-8 clock cannot see is what
the field generated by the clock's square norms away**, with coordinates the colour
...
derivation touching colour at p = 2 must factor through exactly this subgroup — a
checkable constraint on any such claim, stopping-rule-gated as ever.
