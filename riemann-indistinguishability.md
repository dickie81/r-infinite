...
positive offsets ln λ_k − ln(1 − χ_{4k+2}) at every safely deep odd
...
within 1% for rungs 2–4 at δ ≥ 2); the first hole zero 6.702, 6.049, 5.710, 5.626, 5.568, 5.516, 5.477 falls toward the Ξ-limit 5.418; the limits are the nodes
...
primes term. *(vii) What is proved, computed, not claimed.* Proved:
...
The Riemann-side program of §3 (Theorems 1ba–1bv) stands at the wall it
...
1. **The prime-side square root.** The true form is unconditional on the
   ...
   in the odd sector the archimedean term and the prime 2 carry it and the
   pole subtracts; the same zero side Σ|ĝ(γ)|² sits under both. Done when: an
...
2. **The certified frontier.** Weil positivity is certified unconditionally,
   in interval arithmetic, in both sectors on [0, 1.3828125] (Theorem 1bl) and
   on the one-prime window beyond log 2 and the two-prime window beyond
   log 3 in the odd sector (Theorems 1bj, 1bk); at the seven cells to δ = 3.5 the true form's ground state is
   ...
   concentration and a verified head matrix) or by item 1. Bears on the wall
   directly: positivity at support δ is the wall at that support.

3. **The slack law's reduction.** The finite-δ formula of Theorem 1bm(v)
...
4. **Hypothesis D from the prime side.** The ground state's transform
   ...
   real-rootedness of item 5 at that support, would put every zero of Ξ
...
5. **The real-rooted ladder and Hurwitz's theorem.** In the cosine basis the
   ...
   uniform limit are limits of zeros: real-rootedness of M at every support,
   ...
   real-rootedness proved at one support beyond the cells from the form's
...
6. **The sector tower.** Theorem 1bv(ii): an odd probe's transform is irĜ,
   so the odd sector is the even form with the multiplier γ² on the zero
   side; a probe vanishing to order m at the origin gives the multiplier
...
7. **The cross-L-function test as a filter.** Theorem 1bq carries the true
   ...
   Theorem 1bs per function. Any prime-side argument for items 1–5 must
...
8. **The residual and its parity.** The k-level formula of Theorem 1bu(iv)
...
9. **The exterior weight.** Theorem 1bu(vi) splits c(δ) into an origin term,
   proved to converge to ln 0.7729, and an exterior weight — the peak's
   ...
   constant of the slack law and, with item 3, the whole finite-δ law. Done
   when: the slope derived. Bears on the wall through item 3.

10. **The operator-level closure.** Theorem 1bg closed the windowed explicit
