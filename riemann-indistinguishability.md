...
λ₄₀ = 30.4774; λ₅₀ = 43.5311; λ_B(89) = 99.8944; all gated; the
disclosure's "max step 0.031" is the session pass's value — the
committed M = 2048 instrument prints 0.061; provenance noted
round 198).
...
channel IS the trend to 0.03% (all gated — the trough location
n = 88, its value 0.208, and the second trough −1.03 at n = 172
were ungated at the landing; their conjuncts added round 198
F4, making the census label true). *(iv) The
...
land in code"); the committed instrument observes the BETTER
value below, reproduced six-plus independent ways across both
reports] **γ₁ recovered to 14.1347 (error < 5×10⁻⁶) with pole
modulus 1.000000** — position
...
822 usable samples resolve zeros 1–2 cleanly (γ₃ recovers at
higher model order — one reviewer's K = 24 run reached error
0.004–0.007 — but at the committed order it STRAINS, error 0.07
beside a spurious competitor; the two round-198 reviewers
...
session's fourth independent criticality (1as(iv)'s pure-phase
equivalence — amplitude pinned at its floor; 1as(viii)'s
line-as-edge ledger; the n = 1 scarcity; Λ = 0 — the first two
member labels resolved to their in-paper homes round 198 F7).
...
**Theorem 1ax (the heat-flow energy at criticality: the Lyapunov
identity for the de Bruijn–Newman flow, the pressure census with
matched-ensemble controls, and the second-law clause's corrected
form; `cascade_heatflow_energy.py`).** *(i) The commission and
the frame.* "Pursue the de Bruijn–Newman heat-flow energy along
the fill" — the candidate named at the 1aw narrowing. The frame,
stated first: **no proof is claimed, and none results** — the
deliverables are one classical-in-essence identity verified live
on the arithmetic, one new measured number with controls, a
criticality demonstration, and the corrected statement of the
program's P2 clause. *(ii) The flow and the identity.* The
de Bruijn–Newman deformation H_t(z) = ∫ e^(tu²) Φ(u) cos(zu) du
— Φ the same super-exponential kernel committed in this paper's
own moment instrument — moves its real zeros by the pairwise
dynamics ẋ_k = Σ_{j≠k} 2/(x_k − x_j); RH ⟺ Λ ≤ 0 (de
Bruijn–Newman) and Λ ≥ 0 (Rodgers–Tao), so RH says the
arithmetic sits at the flow's EXACT critical point. Along this
flow the log-gap Coulomb energy E = −Σ_{j<k} log|x_j − x_k|
satisfies the H-theorem **dE/dt = −½ Σ ẋ_k²** ≤ 0: three lines
of chain rule (dE/dt = −Σ_k ẋ_k · ½v_k with v_k = ẋ_k) — the
P2-shaped functional whose monotonicity is
algebra, not hope. Gated live on the windowed system of the
first ±60 zeros: the finite-difference dE/dt matches −½Σv² to
relative deviation 1.7×10⁻⁷ (the RK4-step scale). The structure
is classical in essence — the Calogero-type dynamics behind
Csordas–Smith–Varga's Lehmer-pair bounds and Rodgers–Tao's
Λ ≥ 0; the identity's live verification on the arithmetic
window and everything measured below are this block's additions.
*(iii) The pressure at criticality: ½Σv² = 183.46* for the
±60-zero window — the informational-pressure proposal made a
number: the dissipation rate the arithmetic configuration would
...
mechanism of the historical lower bounds on Λ. *(iv) The controls: the low
...
height the actual zeros are MORE rigid than random-matrix
statistics, consistent with the known extra
stiffness of low zeros; one window at one height, a height
survey the named follow-up. *(v) The criticality demonstration.*
Backward flow (t → −2×10⁻⁴): E rises strictly monotonically
(−31326.1167 → −31326.0800, all 20 steps) and the tightest gap
contracts strictly (0.8451 → 0.8443); forward flow the exact
reverse (E → −31326.1534, gap → 0.8459), all gated. Close pairs
are where the backward flow first breaks reality; RH is the
statement that the arithmetic configuration is
the last configuration the backward flow cannot improve. *(vi)
The program mapping, honestly.* P1: E is sourced by the zeros —
the fill's condensate (the 1as reading). P2 CORRECTED: the
monotone direction is heat time t (a theorem of the flow); fill
monotonicity stays refuted (1aw stands) — the fill's role is
not to descend an entropy gradient but to place the initial
data at the flow's critical point Λ = 0, where Rodgers–Tao
forbids the subcritical side. P3: the flow commutes with the FE
symmetry x ↔ −x; the equilibrium is the symmetric configuration
— a detailed-balance reading, structural only. P4: the
stability of the critical configuration (the Hessian of E, the
pair-repulsion form) is where this must meet Weil positivity —
the coupling remains unbuilt; the hardness is conserved at P4,
exactly as the 1av declaration predicted. *Honest scope:*
category (a) instruments throughout (classical zeros as data at
labeled steps, RK4 on the windowed pairwise dynamics, seeded
ensembles); the identity is classical in essence and its
derivation elementary; the windowed system approximates the
full flow (the omitted tail zeros and archimedean confinement
act as a slow drift common to configuration and controls); the
percentile is one window at one height; the flow relaxes AWAY
from the arithmetic and nothing here decides where Λ sits; no
RH leverage claimed anywhere. Check 7 clean (heat kernels,
entire-function zero dynamics, random-matrix ensembles as
CONTROLS only — classical mathematics; no semiclassical
procedure as a route to any cascade quantity). Check 8 clean
(no hypothesis input; the owner's commission supplied
direction, translated into pure mathematics before use).

**Theorem 1ay (the saddle and the arithmetic curvature: the block
Hessian of the heat-flow energy, the stability spectrum with
exact flow rates, the curvature census, and the explicit-formula
conversion of curvature into primes;
`cascade_saddle_curvature.py`).** *(i) The commission and the
frame.* "Attack P4 — the Hessian against the Weil form," then
"push further first." The frame, stated first: **no proof is
claimed, and none results** — the deliverables are one elementary
theorem seen clearly, measured spectra with controls, and an
identity-level conversion; the novelty grading is in the honest
scope. *(ii) The saddle theorem.* At any real (on-line)
configuration, the full Hessian of the 1ax energy in complexified
coordinates (in-line x, off-line y) is
**Hess E = [[L, 0], [0, −L]]**, where L is the weighted graph
Laplacian with weights 1/(x_j − x_k)² — forced by 2D harmonicity
of the log kernel (∂²/∂y² = −∂²/∂x² for −log|z|), FD-verified on
the ±60-zero window to 6×10⁻⁷ with the cross-block identically
zero (gated). Signature: (119 positive, 119 negative — exact
mirrors, 2 zero) of 240: the critical configuration is
a perfect saddle — every on-line stability direction has an
off-line escape twin at the SAME magnitude. This is the Weil
form's signature split (positive-definite subspaces from on-line
zeros, negative from off-line) realized as the geometry of the
configuration: **RH is the statement that the arithmetic occupies
only the stable sheet**. *(iii) The stability spectrum and the
exact rates.* spec(L): zero mode at 10⁻¹⁶ (translations); soft
edge λ₂ = 0.00413 (the slowest collective mode); stiff edge
λ_max = 3.1867, localized on the tightest pair (γ₃₄, γ₃₅) with
IPR 0.250 — the four-site antisymmetric structure (all gated).
The linearized dB–N flow relaxes off-line perturbations
mode-by-mode at rate 2λ_k: measured against the NONLINEAR flow,
ratio 1.0000 at the soft edge and 0.9998 at the stiff edge
(gated) — forward flow restores the line at exactly these rates,
backward amplifies at the same; de Bruijn's realification with
its complete rate spectrum. *(iv) The curvature census.*
...
10.5th percentile, near-GUE: the rigidity has a spectral address
— the stiff modes (close-pair geometry), not the soft collective
ones. The landscape triple at the arithmetic configuration is
now measured with controls: value E, slope (the 1ax pressure
183.46 = 2|∇E|²), curvature spec(L). *(v) The conversion: the
...
resolvents, and the spectral density admit the same treatment
via multiple sums, the named next rung). The hardness, conserved
and located: Weil positivity quantifies over ALL test functions
while each conversion is one test function at a time —
the quantifier over test functions is where the difficulty
lives. *Honest scope:* the novelty grading, stated plainly: the
saddle structure is an elementary consequence of harmonicity
(log-gas folklore territory); the conversion is Montgomery's
method applied to one more kernel; the flow, the constant, and
the signature split are classical (de Bruijn–Newman,
Rodgers–Tao, Weil) — what is plausibly new here is
measurement-grade only (the dissipation and curvature
percentiles with matched controls, the spectral address, the
instrument pipeline), and no literature sweep was run — "to this
record's knowledge" is bounded by training data; the saddle is
...
the Slepian-optimal basis and the 1/c march to 2πe^δ;
`cascade_prolate_horizon.py`).** *(i) The commission and the
frame.* "Explore prolate spheroidal wave functions" → "chase
...
reaches 5.151×10⁻¹ by 340 (pinned); the explicit-formula
identity on minimizers validates the instrument at τ₀ = 300 and
360 (ratios gated in (0.97, 1.0001) and (0.99, 1.0001) over 260
verified zeros, the deficit the positive beyond-260 tail).
*(iv) The march.* the bandwidth march: lift-off crossings
τ*(ε, c) at thresholds 10⁻⁶ and 10⁻³ for c = 40, 60, 90, 120
(dimension tracking the Shannon number), each pinned within
±1.5 and strictly increasing in c (gated): (158.87, 222.69),
...
capacity is bandwidth, not dimension count: at c = 40,
τ₀ = 200, the margin falls 0.131 → 1.307×10⁻⁴ from n = 20 to
n = 24 — the cliff sits at the plunge (ratio > 500, gated) —
then crawls (n = 24 → 32 within a factor 5, gated;
9.79×10⁻⁶ at n = 38, pinned): dimensions beyond the Shannon
number buy almost nothing; only widening c moves the edge.
*(vii) The reading.* The dodging horizon has passed from
heuristic (Theorem 1az), through a refuted naive prediction
(Theorem 1ba), to a measured limit with a finite-size law: the
wall's location is the counting horizon exactly, its
finite-bandwidth approach is Slepian economics, and the
instrument that measured it is the operator the spectral
literature independently ties to the zeros. In the
reach-vs-support dictionary (classical input: the
...
τ∞ windows are those of a four-point fit — the law's FORM
(pure 1/c) is assumed by the fit, supported by residuals, not
derived; the deep-threshold interior offset is measured, not
explained here; boundary-regime margins are magnitude-window
pins only, no balance decompositions gated (the round-210 F1
standing caution); the prolate modes do not vanish at the
support edges (1/r pointwise Fourier tails — the true contrast
with the cos basis's 1/r², per the round-213 F1 record); no
literature sweep beyond the named dictionary reading; no RH
leverage claimed anywhere. Check 7 clean (Slepian concentration
theory, the classical explicit formula, finite Hermitian
eigenproblems — no semiclassical procedure as a route to any
cascade quantity). Check 8 clean (no hypothesis input; the
owner's commissions supplied direction, translated into pure
mathematics before use).
...
against three nulls sharing one instrument built from the
certified 1bb section machinery — the 1bb basis, Gram, and
dimensions carrying the ZERO-SIDE quadratic form at δ = 4,
pole- and prime-free, the only form defined for all four
configurations; ~~the certified 1bb instrument (the
prolate-modulated zero-side margin~~ *(struck round-218 F4: the
certified 1bb margin is the Weil-side form — ARCH, pole pair,
primes — and the zero side appears in 1bb only as the identity
check on minimizers; the instrument here shares 1bb's section
machinery, not its gated form)* (the committed section
reproduces the session pins at c = 120 — 1.139×10⁻⁷ /
3.760×10⁻⁴ / 2.549×10⁻¹ at τ₀ = 260/300/360, rel 10⁻², gated;
~~certified-era pins~~ *struck round-218 F4: the values appear
on no certified-era surface — they are 1bc-session values of
the new zero-side form, attested by the banked research log,
the harvest, and the clean run*). *(ii) The comb null.* Against
...
exactly across a container restore; the ±0.086 is std/√10
across ten points sharing the zeros, the sections, and the
restarting seed ladder — a dispersion summary, not an
...
Slepian out-of-band leakage interpolated at the smooth in-band
count index, multiplied by the mean zero density log(τ₀/2π)/2π
— the model's absolute normalization; round-218 F8 corrected
the landing's "leakage × count" description to the committed
formula; no arithmetic): its lift-off crossings τ*(10⁻³, c) =
...
(Theorem 1bb), ~~but its APPROACH carries arithmetic rigidity
beyond counting — the section holds the boundary 36–57 beyond
what concentration alone explains~~ *(struck round-218 F3: the
count-matched rigid comb — zero fluctuation, no arithmetic
...
arc, closed in sign and pattern with the magnitude mismatch
stated.
...
the pure-concentration null ~~isolates arithmetic (the approach
to the horizon outruns counting)~~ *(struck round-218 F3:
locates the smooth model's floor — every discrete configuration
measured outruns it; see (vii))*. What the low-height
...
matches the COMB's in-band count only (not higher moments), and
at 2 of the 10 conditioned points — (60, 200) and (60, 280) —
the zeros' own in-band count differs from that target by 1
...
**Theorem 1bd (the archimedean comb — the prolate operator
carries the counting and none of the fluctuations;
`cascade_sonin_dirac.py`).** *(i) The commission and the object.*
"Land 1bd." The Connes–Moscovici prolate operator W_λ =
−∂ₓ(λ²−x²)∂ₓ + (2πλ)²x² (PNAS 119 (2022); deficiency (4,4)),
whose self-adjoint realization commutes with both the space and
Fourier cutoffs (their Theorem 1.6), so its negative spectrum lives
in the Sonin space {f : f = 0 on J = [−λ,λ] and f̂ = 0 on J}
~~(their Theorem 1.6)~~ *(struck round-222 F2: the commutation is
Theorem 1.6's; the Sonin-space localization of negative
eigenfunctions is their **Corollary 2.2** — PNAS numbering; the
arXiv version numbers these Thm 2.6 / Cor 3.2)* —
the canonical archimedean-place candidate for a spectral
realization of the zeros. No proof is claimed; the deliverable is
the measurement of which features of the zeros this operator
carries. *(ii) The instrument.* Outside-region Sonin compression
(the committed `sonin_outside.py`): basis supported on [λ, X] only
(f = 0 on J exact), the Fourier-vanishing constraint cut at the
Slepian knee — constraint ranks pin at the Landau count 2λ(X−λ)
(gated ±25; observed 345/572/1138 at X = 120/200/400 against
~~335.3/561.4/1127.1~~ **335.4/561.7/1127.4** *(round-222 F7: the
landing quoted hand-rounded digits; these are 2λ(X−λ) recomputed)*)
— the same Landau–Widom concentration object
Theorem 1bc's third null used, here on the constraint side. A
machine-precision cut over-constrains: the true eigenfunctions'
universal sin/cos(2πλx)/x tails carry O(1/X) Fourier residual on J
when truncated, so the numerically-null space at 1e−15 excludes
them (the research log records two failed routes: shooting
quantizes a different self-adjoint extension — the Sonin condition
is nonlocal — and a Hermite–Galerkin basis covers the 1/x tails
only as M^(−1/4)). *(iii) The counting side — carried ~~exactly~~ **to its constant
term** (round-222 F9: the verifier's own qualifier, restored here).* At
λ = √2 the C-M per-parity counting constant log 4 − 2 log λ equals
the zeros' log 2 — each parity's counting matches the zeros'
counting (algebra gated, g2) — and the instrument confirms it
λ-resolved: per-parity counts in (10, 240] climb 96/97 → 98/98 →
99/99 across X = 120/200/400 (gated exact, monotone, approaching
the zeros' 102 from below with the 1/X truncation deficit
*(qualified round-222 F9: 102 is the zeros' count in the window —
the gated facts are the monotone climb and the ≤ 102 ceiling over
the measured configs; the instrument's own X → ∞ constant is open
within O(1): Prop 3.2's semiclassical constant gives ≈ 105, while
C-M's refined counting, Theorem 5.1's −log(E/2π) correction, gives
102.0)*); the
shallow end lands on the zeros' region (the X = 120 odd ladder
starts at 13.23 against γ₁ = 14.13; the parities interlace, and the
per-configuration merged shallowest ladder entry sits within 2.5 of
γ₁ at every √2 configuration, gated); the affine dilation against the zeros
shrinks toward 1 with X (endpoints gated in (1.0, 1.05),
a(400) < a(120)); and the **−2 log λ constant law** is confirmed
with the truncation bias cancelled: the matched-X difference
c₀(λ=1) − c₀(λ=√2) = log 2 ± 0.08 (gated; both absolute fits below
their targets log 4 and log 2 in the truncation direction, gated).
*(iv) The fluctuation side — absent.* **The rigid-comb verdict**
(`cascade_sonin_dirac.py`, g5–g6): the spectrum's unfolded
increment sd is 0.016–0.020 at every √2 configuration (gated
(0.004, 0.05)) against the zeros' own 0.352 (gated (0.33, 0.37)) —
a factor ~20 quieter — and the increments are uncorrelated with
the zeros' (per-configuration |corr| < 0.06 gated, observed
≤ 0.015; mean gated < 0.03). *(v) The reading.* Theorem 1bc
measured the zeros' fluctuations as anomalously stiff for their
density (+0.710 over count-conditioned CUE) yet strictly looser
than a rigid comb; this measurement places the canonical
archimedean spectral candidate AT the comb end of that ordering:
W_sa reproduces the counting function — location, constant term,
λ-law — and none of the fluctuation content. On this instrument
the archimedean place carries the smooth counting; the arithmetic
fluctuations, the primes' contribution, are exactly what it lacks
— the complementary null to 1bc's price list: a spectral geometry
that hits the smooth side exactly still sits a full fluctuation
class away from the zeros. *Honest scope:* float64; one knee
convention (relative cut 3e−4; the rank plateau 343→345 across
3e−3 → 3e−4 at X = 120 is the knee's sharpness); the (10, 240]
ladder window excludes knee-leakage entries at √2 (tol-wandering,
below the √2 ladder's start ≥ 12); ~~the same numeric cut applied
uniformly at λ = 1~~ *(qualified round-222 F6: at λ = 1 the uniform
cut excludes one tol-STABLE low mode at s ≈ 8.0 — semiclassically
expected there, not leakage; keeping it moves the g7 difference
0.717 → 0.736, still in-window, so the constant law is cut-robust
and the λ = 1 exclusion is a disclosed uniform convention, not a
wander class)*; individual shallow eigenvalues wander ±1–1.5
across X (the truncation–knee interplay) — the pinned quantities
are ladder-level statistics, and the single individual-eigenvalue
pin is same-configuration determinism, not a truth claim; compute
is content-addressed on the instrument's bytes (ckpt_key; the
landing recomputed fresh); a prior-session harvest of the same
operator reproduced in phenomenon (rigid comb, decorrelation,
counting *(qualified round-222 F8: the prior harvest's counting was
merged-parity bookkeeping ≈ N; the committed instrument's — and
C-M Prop 3.2's — is per-parity ≈ N to the constant term; the
phenomenon that reproduces is counting-carried, the bookkeeping is
superseded)*, constant law) with its exact digits
instrument-dependent
and superseded by this committed instrument's gated values; C-M's
spectral-realization theorem — their Theorem 5.1, ~~a λ = 2, 2D
construction~~ *(struck round-222 F3: Section 5 of the source takes
λ = √2, the same λ as this instrument's headline configurations,
and the operator ²𝔇 — the name is "twice the Dirac operator 𝔇,"
not a dimension and not a λ — is a Darboux-doubled Dirac operator
on the half-line whose spectrum is ±2√μ over W′_sa's, the same
s = 2√(−ξ) map used here; "λ = 2" appears nowhere in the source)*
— makes a statement this measurement does not test (the imaginary
spectrum's correspondence with the zeros); what is measured here is
the counting/fluctuation content of W_sa's compression itself; no
RH leverage claimed. Check 7 clean
(spectral theory of an ODE operator plus Fourier concentration —
classical analysis; no semiclassical procedure invoked, and no
cascade quantity is derived: a null measurement on a literature
object). Check 8 clean (no hypothesis input; the owner's
commission supplied direction only).

**Theorem 1be (the fold — the stiffness excess derived from the
primes' budget, with the non-Gaussian remainder measured;
`cascade_prime_budget_fold.py`).** *(i) The commission and the
claim.* "Attack the fold" → "Harden it" → "Land it." Theorem 1bc
measured the zeros' dodging economics as +0.710 ± 0.086 decades
stiffer than count-conditioned CUE; this theorem derives that
excess, parameter-free, from second-order rigidity alone, and
...
consecutive zeros — two endpoints × uniform variance 1/12; CUE
...
observation *(net state: derived in Theorem 1bg — the proximity
is the arch/zero horizon mismatch and vanishes to < 0.2% under
matched horizons)*), and the √-law is an instrument-sensitivity
law for the dodging form. *(ii) Section-level Weil positivity, arithmetic
...
**Theorem 1bg (the floor arc — the Slepian floor law, the
conjugacy of the arithmetic build, and the operator-level closure
of the windowed explicit formula; `cascade_floor_closure.py`).**
*(i) The commissions.* Four attacks in sequence — "Attack the
floor" → "Attack the antisymmetry" → "Attack the partition" →
"Attack the defect" — each refuting part of its predecessor's
reading, with every refutation on the record in the committed
instruments' RESULT blocks. *(ii) The floor law.* On τ₀ ladders
at c = 60 and 120 (52 points), the truncated dodging margin obeys
...
0.255 at c = 120, gated g1) it re-concentrates. The certified
deep margins are quantitatively the plunge attenuation. *(iii)
The conjugacy.* The certified 1bb-lineage arithmetic build
satisfies Q_W ≈ conj(Q_Z) — by machine-exact parity algebra, the
zero form with its ordinates reflected about the section center —
at defect 0.2–0.7% (pinned, g6). Everything the arc first met as
mystery follows: the g4 isospectrality, w_W = conj(w_Z)
(conjugate overlap 1.0000, gated g7 — the plain overlap is
0.01–0.36), the antisymmetry T(w_Z) = −T(w_W), the pencil
symmetry; and m_W − m_Z = D(conj w_Z) closes to 15% at every
...
certified scalar is conjugation-blind and unaffected), and the
"reflection identity" is **the plain windowed explicit formula
seen through that** convention. The formula is window-local (the
odd-sector projection β(T) plateaus at −1 from T ≈ 400),
per-prime resolved (β_n = −1 within 1–3% for every prime power
of non-tiny norm — the windowed zero configuration reconstructs
each prime's operator contribution individually), and tail-free
(280 further ordinates, computed to γ ≈ 1013, project at 0.0000
*(round-234 F2: the landing text said 269 — the T = 1000
trajectory-endpoint count; the gated projection uses all 280)*;
gated g10 — the arc's own "+2/−1 window-partition" hypothesis
was refuted by this measurement and is retracted in the
instrument's RESULT). *(v) The defect closure and the derived
g4.* The 0.2–0.7% residual is **the horizon mismatch, entirely**:
the arch window (τ₀ ± 800) against the fixed ordinate coverage
(±653.6). The quadrature share is zero (NR/2 = base = 2NR to
every printed digit; the base-vs-NR/2 leg gated at < 10⁻⁵, g11); matching the arch horizon to the ordinate coverage
collapses the defect 40–70×, and with the 660-ordinate list at
matched ±1013 it is 2–3×10⁻⁵, τ₀-uniform (gated g11). The
certified 1bf g4 proximity — dm/m = +0.019/+0.047/+0.001 at the
code windows — becomes ≤ 10⁻³ in magnitude under matched
horizons (the ext-matched family gated |dm/m| < 2×10⁻³, g12): **the g4 ratio is a derived quantity, not
...
β ≈ 1.1's closed form and the ≈ 2-modes-per-zero price; the
π²/4 asymptote reading is excluded by measurement
(`floor_probe8.py`), and the law's intercepts flow under the dBN
surrogate dynamics while τ\* and the tight mean sit still
(`floor_probe9.py`))*; the operator closure verifies the
...
coincidence, now known to be windowing bookkeeping); the
remaining ~2×10⁻⁵ closure residual is unattributed; RH content
sits untouched in height-uniformity and in the sign of the bottom
eigenvalue at the Shannon crossing, where zero fluctuations
enter. Conventions flag for future operator-level work: the
as-built arithmetic is the conjugate of the formula operator.
Check 7 clean (Slepian–Landau–Widom concentration asymptotics,
finite eigenproblems, the unconditional explicit formula —
classical analysis; no cascade quantity derived, no semiclassical
procedure). Check 8 clean (no hypothesis input). Substrates
`floor_probe.py`, `floor_probe2.py`, `floor_probe3.py`,
`floor_probe4.py`, `floor_probe5.py`, `floor_landing.py`
committed, imported, and content-addressed into the verifier's
keys (the stage lives in `floor_landing.py`; family-level keying
localizes recomputation).

**Theorem 1bh (the height-uniformity arc — the aperture dial, the
out-of-sample τ\* law, the height-uniform floor, and the
arithmetic side at the critical windows through height 2340;
`cascade_height_uniformity.py`).** *(i) The dial.* At fixed
aperture the high heights are deeper into saturation, so the
hair-thin regime — where positivity is informative and fragile —
is **one band per aperture**, at τ\*(A) = 2π·e^(2A): **the
...
record). The tight mean Q(top) = ln(τ₀/2π) is
aperture-independent as the algebra demands (gated g3; the
scatter grows only as the band narrows — feature-local). *(ii)
The height-uniform floor.* Under the declared recipe the climb
rate is β = 1.09–1.37 decades per in-band zero at every aperture
(spread ≤ 1.35×, gated g4), and the deep margins at matched count
deficit are height-stable for A ≥ 2 (d ≈ 5.4: 1.14/1.10/0.65
×10⁻⁷ at heights 260/630/1580; d ≈ 8.4: 0.81/0.97 ×10⁻¹⁰ at
550/1350; gated g5): **the race curve is flat** — the critical
window's positivity information does not degrade over the
measured span. The A = 1.5 depression is resolved as
deterministic band geometry (gated g6, the committed gradient
test: mirrors can only raise margins — PSD; the smooth-counting
comb with no fluctuations reproduces the depression while the
constant-density comb sits ≥ 10⁴× higher; the local Nyquist line
falls inside the wide low-aperture band and the crossing smears)
— not a height effect, not arithmetic. *(iii) The arithmetic
side.* Under matched s-windows and **the pole rule** (the pole
pair included iff A·τ₀ ≤ 2200, within quadrature validity;
omitted where analytically negligible — a computed negligible
term is unbounded noise, demonstrated at |u| = 1.9 against a
superexponentially small truth, closure 4×10⁻² → 3.4×10⁻⁵ on
omission; gated g10), the windowed explicit formula closes
operator-wise at every critical-window point (10⁻⁵ class at
A = 3, ≤ 2×10⁻⁴ at A = 2.5; gated g7) and **m_W ≥ 0
arithmetic-only at all five points** (gated g8) — at height 1580
from 98 prime powers (primes to 403), hair-thin positive
(6.3×10⁻⁸) with dm/m ~~= 0.01%~~ **= 0.022%** *(round-238 F1:
the 0.01% was the instrument-era with-pole value; under the
landed pole rule — the pole omitted at A·τ₀ = 4740 — the gated
checkpoint gives −2.2×10⁻⁴)* and the null cancellation
PRIME/ARCH = −1 to four digits (measured 1–2×10⁻⁸; gated at
10⁻⁴, g9 — tightened from 10⁻³ at the round-238 sweep, F9): the
1bg closure and
derived-g4 chain hold out-of-sample in the aperture. *(iv) The
capability, stated with its caveat.* m_W at τ\*(A) is **an
arithmetic-only falsifier** of RH at height 2π·e^(2A): primes to
τ\*/2π plus a fixed-cost integral, no zero computations anywhere,
and a **certified-negative** value at any window — the exact
functional bounded below zero by an explicit error budget, not
a bare computed float *(round-238 F4: the arc's own pole
demonstration shows an uncontrolled computed term is unbounded
noise)* — disproves RH. Single-height
spot-checks are cheap and the flat race curve says the
sensitivity does not degrade over the measured span; *sweeping* a height range
multiplies by the core-local window count, and **no complexity
advantage over classical verification at accessible heights is
claimed**. *Honest scope:* c = 120 only; four apertures; heights
≤ 2780; a green probe certifies necessary-condition instances,
nothing more — **no RH leverage claimed** (the uniformity
STRUCTURE is measured, not a uniform theorem; the theorem-shaped
remainder — the height-stationarity of the matched-deficit margin
distribution — is a conjecture the instruments motivate but do
not prove); the deep-margin statistic is ladder-sampled, not an
extreme-value census; the run-1 full-window quadrature failure at
A = 3 (mirror ordinates at |s| ≈ 15,400 against ~2,400 validity)
is on the record with its s-window fix. Check 7 clean (Slepian
sections, Poisson summation, the unconditional explicit formula,
mpmath ordinates — classical; no semiclassical procedure). Check
8 clean (no hypothesis input). Substrates `height_uniformity.py`,
`height_anomaly.py`, `height_arith.py`, `height_residue.py`,
`height_landing.py` committed, ~~imported,~~ **imported or, for
`height_residue.py`, named in `height_landing.py`'s explicit
dependency tuple** *(round-284 F284-3: `height_residue.py` is
imported by nothing; it is in every key through that tuple)*, and content-addressed
...
**Theorem 1bi (the floor law derived — the leakage identity, the
constraint price, the rate composition, the discount law, and the
budget-drift race curve; `cascade_floor_theory.py`).** *(i) The
commissions.* "Point the certified surrogate at the Shannon
crossing" → "pursue the derivation" — the crossing-sign null
(`crossing_probe.py`: the Mertens budget's saturation pins the
in-band count at the crossing, so the pre-registered blind mass
does not exist — P1 refuted as the finding; the zeros
statistically typical of their second-order twin at all six
windows; the stiffness excess and its CUE gap confirmed and
growing across the crossing), then the derivation of the certified
1bg/1bh floor law from the section's own mathematics. *(ii) The
...
— the certified 1bg "≈ 2 modes per added zero" open remainder is
closed. *(iv) The rate composition.* β_comb = (edge advance) ×
(per-mode leakage decade-rate of the section's computed
concentration spectrum): 1.41 composed against 1.28 measured at
c = 120 — a 10% leading-order residual, the minimizer spreading
beyond a single edge mode (gated g3). The per-mode rate follows
...
−0.50/−0.60/−1.17 decades at deep/crossing/near-saturation at the
physical budget σ (gated g5, with the regime-dependent σ-form
pinned: ~σ^1.3 deep, ~σ² at the crossing), and the twin's
discount at (120, 260), −0.498 ± 0.03, brackets the certified
true-zero comb discount R(260) = −0.40 at +0.4 sd (gated g6).
*(vi) The budget-drift race curve.* With full certified coverage
and per-height budget variance σ²(T) = (ln ln(T/2π) + Mertens +
1)/π² − 1/6, the certified d ≈ 5.4 race triple sits at
...
The twin's ingredients are the finite-height faces of classical
theorems: Gaussianity from the central limit theorem over
log-independent prime phases (Selberg; unique factorization
through Kronecker–Weyl equidistribution), the budget from
Mertens' theorem, the saturation from the almost-periodicity of
the prime sum — so the measured twin-obedience transforms "why do
the zeros obey the null" into the quantitative pseudo-randomness
of the prime phases, which is RH's own territory; the twin
instruments are conspiracy detectors. Stated as the named frame,
not a derivation. *Honest scope:* the composition is
leading-order (10% residual); the discount law is measured, its
form not derived; the drift agreement is one realization per
height against ensembles of 80 (sd ≈ 0.26/point) — consistency at
z ≤ 1.6, not a sharp confirmation; c = 120 only for the drift;
the twin model's completeness is itself the measured 1be and
crossing-null result, not a theorem; the crossing probe's six
windows overlap heavily (~1–2 independent samples) —
**no RH leverage claimed**: the derivation prices the instrument
under the second-order null and says nothing about why the zeros
obey it. Check 7 clean (Slepian concentration spectra, Gaussian
displacement fields, finite eigenproblems, smooth counting —
classical; no semiclassics; no cascade quantity derived). Check 8
clean (no hypothesis input). Substrates `crossing_probe.py`,
`floor_theory.py`, `floor_theory2.py`, `floor_theory3.py`
committed, imported, and content-addressed into the verifier's
keys per the post-eighth-catch law (every producing file in every
key).

**Theorem 1bj (the one-prime window certified — interval-rigorous
Weil positivity beyond log 2; `cascade_oneprime_interval.py`).**
*(i) The object, and why it is the full functional.* The
semi-local one-prime Weil form at the real place plus the prime 2:
Q(g) = (1/2π)∫|ĝ|²W dt ± 2⟨χ,G⟩² per parity, with the explicit
kernel W(t) = 2θ′(t) − √2·log 2·cos(t·log 2) — the prime term
frequency-diagonal, the pole rank-one. For test support length
δ < log 3 the only prime power inside the autocorrelation window
...
and (0.015, 2.5), margins 2.485×10⁻³ and 3.015×10⁻³. The
certified margins: Temple ≥ 1.2947×10⁻³ (even 0.6931),
1.6888×10⁻⁴ (0.8), 1.0090×10⁻⁵ (0.9), 7.3567×10⁻⁷ (0.95),
2.6832×10⁻⁷ (1.0); odd ≥ 1.3729×10⁻³ (0.9), 2.2583×10⁻⁵
(1.05), 4.8520×10⁻⁶ (1.09); count margins from 4.114×10⁻¹ down
...
remainders; Binet's integral for Re ψ; Hurwitz-zeta derivative
majorants; no libm accuracy assumption anywhere); a cumsum slop
lemma folding the accumulation of every table into its
enclosure; signed-integrand table differences widened by
endpoint slivers (the corner hull is not an enclosure for
...
outright (the forward direction of Weil's criterion, carried
through the part-(i) identity); the certificates are
two-sided instruments that could have returned that negative,
and did not — the margins instead track the unrigorous float
predictions to within the enclosure widths. *(vi) Prior art,
stated honestly.* Yoshida (1992): positivity for sufficiently
small support, qualitative. Connes–Consani (Selecta 2021): the
archimedean place — positivity through δ = log 2, exactly the
threshold where the first prime enters and the explicit-record
prior to this theorem. Bombieri's variational line (Rend.
Lincei 2000; CPAM 2003) carries no explicit threshold in the
accessible record — the audit record notes the CPAM primary
text is paywalled and its survey-based reading carries that
...
support length has no known unconditional zero-localization
consequence, and the two-prime window [log 3, log 4) is future
work for the same machinery. Check 7 clean (Kato–Temple,
Birman–Schwinger, Weyl interlacing, Binet, Hurwitz zeta,
IEEE-754 interval arithmetic — classical throughout; no
...
Weil positivity beyond log 3 in the odd sector;
`cascade_twoprime_interval.py`).** *(i) The object.* The
semi-local two-prime Weil form at the real place plus the primes 2
and 3: Q(g) = (1/2π)∫|ĝ|²W₂₃ dt ± 2⟨χ,G⟩² per parity with the
kernel W₂₃(t) = 2θ′(t) − √2·log 2·cos(t·log 2) − (2 log 3/√3)·cos(t·log 3)
— the prime terms frequency-diagonal, the pole rank-one. For test
support length δ ∈ [log 3, log 4) the prime powers inside the
autocorrelation window are exactly log 2 and log 3 (log 4 = 2 log 2
enters only past log 4), so on that window the two-prime form is
the full functional on [log 3, log 4) — identically, as the
one-prime form is below log 3 (Theorem 1bj(i)). *(ii) The
theorem.* **The semi-local two-prime Weil form is positive in the
odd sector at support length 1.10, λ₁ ≥ 6.3135e-06, every ingredient
an interval enclosure**, and by domain nesting (restriction of a
positive quadratic form; extension by zero) the same bound holds
at every shorter odd support length — in particular **the margin
at the log 3 threshold does not vanish**: λ₁(log 3) ≥ 6.3135e-06 in
the odd sector, the rigorous odd-sector answer to the structural
question of whether the full form's ground-state margin touches
zero where a prime power enters (it is continuous there, and under
RH strictly positive at every finite support by Beurling sampling;
the certificate makes the odd-sector statement unconditional).
*(iii) The machine.* The premise ℓ₂ ≤ λ₂(T_odd) by the
pole-inclusive count: for the odd sector the pole is the negative
rank-one −2|χ⟩⟨χ|, χ = sinh(t/2), so T_odd − ν = max(W₂₃ − ν, β)(D)
− [qt(D) + 2|χ⟩⟨χ|] and Weyl monotonicity bounds #{T_odd < ν} by
the number of eigenvalues above β of the counting operator qt(D) +
2|χ⟩⟨χ| — into which the pole enters with a *positive* sign, so
its Simpson-frame Gram bordered by the closed-form pole column is a
genuine Gram and the verified eigenvalue enclosure certifies
λ₂(T_odd) ≥ ν* = 0.05 directly, with no interlacing loss and no
secular gate (the even sector's devices in Theorem 1bj(iii); here
the two-stage interlacing route would have capped ℓ₂ at the
pole-free λ₁ ≈ 0.015, below what the reconnaissance's harmonic and
harmonic+Gegenbauer trials can use at this cell): the certified row (ν, β) = (0.05, 2.0) on 4,616 nodes over
a support of length 87.0 to r = 435 (tail lemma at rmax = 600),
μ₂ ∈ [1.990865, 1.990868], operator-error bound 7.92×10⁻⁵, margin
9.053×10⁻³. The Temple side single-stage at ℓ₂ = ν*: the frozen
pure-harmonic entire trial (48 sines) through the two-shift
closed-form operator — the one-prime instrument generated by
asserted exact substitutions (the second shift in value and
derivative, the second kink log 3 − a, the (C₂ + C₃) sliver
envelope) — with n, M, S interval enclosures: ρ ∈ [1.4350e-05, 1.4395e-05], σ² ≤
4.018e-07, Temple ≥ 6.3135e-06. *(iv) The arithmetic.* As Theorem 1bj(iv),
with the second cosine in the kernel enclosure and C₃ log^k 3 in
the four derivative majorants. *(v) The falsification content.*
Because the two-prime form is the full functional on the window, a
certified negative eigenvalue at this cell would have disproved the
Riemann Hypothesis outright; the certificate is two-sided and
returned positive, tracking the float prediction (6.389e-06
float against 6.3135e-06 certified). *(vi) Prior art and honest
scope.* Connes–Consani's proven record stops at log 2 and their
statements past log 3 are numerical (zeta-cycles sec. 2.4: failure
when p = 3 is withheld; the two-prime restoration measured here in
A417 at seven to eight orders of magnitude); within the accessible
record this is the first explicit unconditional positivity
threshold beyond log 3 for any localisation of the Weil
functional. *Honest scope:* one odd cell; **the even sector is open
on the whole two-prime window** *(net state after Theorem 1bl: closed — both sectors certified to 1.3828125 by mechanism)* (Temple-hard throughout: σ/λ₁ ≈ 10⁴
and the even spectral gap collapses toward log 4, A417) and the odd
sector beyond ≈ 1.13 is float-infeasible for Temple; the
certificate is one discrete cell plus nesting; the trial is a
frozen float64 vector (a trial carries no rigor burden); **no
Riemann Hypothesis consequence is claimed** — window positivity at
any certifiable support length constrains zeros less than the
critical strip does (A419), and the two-prime window is where this
arc stops. Check 7 clean (Kato–Temple, Birman–Schwinger, Weyl
monotonicity, Binet, IEEE-754 interval arithmetic — classical;
...
probe, by Slepian concentration and a verified head matrix;
`cascade_slepian_mechanism.py`).** *(i) The object.* For a real
probe g of fixed parity supported on [−a, a] with Fourier transform
ĝ, the S-local Weil form Q(g) = (1/2π)∫|ĝ|²W dr ± 2⟨χ,g⟩², W(r) =
Re ψ(¼ + ir/2) − log π − Σ_{(lag,w)∈S} w cos(r·lag), is Weil's full
functional whenever 2a is below the next prime-power lag (Theorems
1bj(i), 1bk(i)). *(ii) The bound.* For any Ω with W_out := W_∞(Ω) −
Σw > 0 (W_∞ is increasing on r > 0, so W ≥ W_out on |r| ≥ Ω), and
for EVERY g, Q(g) ≥ ⟨g, Mg⟩ with M = A_in + W_out(I − K) ± 2χχᵀ, A_in
the band form on |r| ≤ Ω and K the Slepian concentration operator of
the pair (a, Ω) on L²[−a, a] (0 ≤ K ≤ I; eigenfunctions the prolate
spheroidal wave functions ψ_k, eigenvalues λ_k decreasing). Splitting
L²[−a, a] into the head span{ψ₀, …, ψ_NH} and its orthogonal
complement, the tail block of M is ≥ q_⊥ = W_out(1 − Λ_tail) −
M_WΛ_tail − [odd] 2‖P_⊥χ‖², the head–tail coupling is ≤ b = (M_W +
W_out)√Λ_tail + 2‖χ‖‖P_⊥χ‖ (M_W = sup_{|r|≤Ω}|W|, Λ_tail =
sup_{f⊥head}⟨f, Kf⟩), and λ_min(Q) ≥ λ_min([[λ_min(M_head), −b],
[−b, q_⊥]]): the uncertainty principle is the tail — a probe
orthogonal to the first NH + 1 prolates carries at most Λ_tail of its
mass onto the band, where the kernel can be negative, and the rest
...
prolates per parity) λ_min(Q) ≥ 5.7134e-13 (even) and ≥ 3.9537e-10
(odd)** — hence, by domain nesting, Weil's full functional is
positive for every even and every odd probe of support length ≤
1.3828125, within 0.0035 of log 4. *(iv) The machine.* Verified
...
trial, no Birman–Schwinger count, no spectral-gap premise, no zero of
ζ. The slack against the true margin is the excess W − W_out dropped
outside the band (recoverable by larger Ω at the price of larger
heads). Beyond log 4 the same instrument applies with the three-prime
kernel (W_out = W_∞(Ω) − C₂ − C₃ − C₄ > 0 needs Ω ≳ 120), but the
head's minimum eigenvalue at window q falls at the zero side's rate
(five decades between the two cells here), so the arc's ceiling is a
handful of windows; **no Riemann Hypothesis consequence is claimed**.
Check 7 clean (Slepian, Paley–Wiener, Bernstein ellipses, Binet,
Stirling, ball arithmetic — classical; no semiclassics; no cascade
quantity derived). Check 8 clean (no hypothesis input). Substrate
`slepian_arb_certificate.py` committed, imported, and
content-addressed into the verifier's keys
(`cascade_slepian_mechanism.py`, tower member 21, the top) by the
computed transitive import closure; `mechanism_stage0.py` and
`oneprime_slepian_certificate.py` — the float reconnaissance and the
double-precision Stage 1 of the mechanism proof — committed and
cited, imported by neither the instrument nor the verifier and in no
key.

**Theorem 1bm (the slack law — the zero side's ground state against
support, the horizon scaling of its exponent, and the constant 4π of
the reduced problem; `cascade_slack_law.py`).** *(i) The object.* For
a real even probe g on [−a, a] (δ = 2a) with ĝ(r) = ∫g e^{irt}dt, the
zero side of the explicit formula is Q(g) = Σ_γ|ĝ(γ)|² over the zeros
½ + iγ, and λ₁(δ) = min_{‖g‖=1} Q(g) is the ground state that Theorems
1bj–1bl bound from below at δ ≤ 1.3828125. Computed here on the model
"the first 6700 zeros (to height 6997, at least 20 horizons at every
cell) plus the smooth density (1/2π)log(r/2π) beyond", in the
orthonormal Legendre basis of the even sector truncated at Legendre
degree 2(2aT₀/π + 120), i.e. 2aT₀/π + 120 even modes, T₀ = 2πeᵟ the dodging horizon of Theorems 1az–1bb, every
Gram entry a ball (python-flint/ARB, 320–1400 bits), the eigenvalue by
flint's eigensolver, and the Rayleigh quotient of the returned vector
evaluated in balls — **a rigorous upper bound on the truncated model's
λ₁, certified to the ball's radius (≤ 10⁻⁶ relative at every cell)**;
the truncations are measured, not certified: the basis by paired cells
(δ = 2 at 35/80/120 extra modes: ln λ₁ = −67.303 / −67.323 / −67.332; δ = 3.5
at 120/300: −383.219 / −383.420), the list by shorter and longer lists (700 against
2000 zeros at δ = 3.5: 0.03 nats; 1000 against 2000 at 3.75: 0.06; the
6700-zero list against the 2000-zero list at δ = 3.75 and 4.0: 0.001 and
0.013), the density tail by 16×50 against 64×200 nodes at δ = 3 (0.35%). *(ii) The
law.* At the seven cells δ = 1.0, 1.3828125, 2.0, 2.3, 2.6, 3.0, 3.5:

| δ | ln λ₁ | f = −ln λ₁/eᵟ | −ln λ₁/T₀ |
|---|---|---|---|
| 1.0 | −13.884 | 5.108 | 0.813 |
| 1.3828125 | −27.765 | 6.966 | 1.109 |
| 2.0 | −67.332 | 9.112 | 1.450 |
| 2.3 | −98.330 | 9.858 | 1.569 |
| 2.6 | −140.777 | 10.456 | 1.664 |
| 3.0 | −221.942 | 11.050 | 1.759 |
| 3.5 | −383.219 | 11.572 | 1.842 |

f rises monotonically and R = f/δ falls from δ = 2 on (gated): the δeᵟ
form of the first reading (A438) was the finite-range description, and
**the exponent is twice the horizon** in the limit of the reduced
problem (iv) — a statement conjectural for λ₁ itself, see (vi) —
−ln λ₁/T₀ = 1.842 at δ = 3.5 against that limit 2. Beyond the cells, the
instrument gives −500.529 at δ = 3.75 and −651.008 at δ = 4.0 on the
2000-zero list (f = 11.771 and 11.924; −ln λ₁/T₀ = 1.874 and 1.898; the
δ = 3.75 value at 300 extra modes, the δ = 4.0 at 400 — against −496.128
and −637.893 at 120 modes: the basis converges slowly at large δ, and
the 400-mode value at δ = 4 is not itself shown converged) — computed,
cited from the record (A439, A442, A443), not gated: the δ = 4 cell
costs three and a half hours at 2200 bits.
*(iii) The ground state, by construction.* The minimiser is the
**dodging prolate**: the even probe of type a that vanishes at every
zero inside its band and, among such, maximises its band concentration
...
record's inputs, ln λ₁ to three decimals and the gaps to two), dodging exactly the
...
density contributing nothing (∫₀^∞ ln|1 − r²/t²|dt = 0), so in horizon
units r = T₀x the log-envelope is eᵟφ(x), φ(x) = ∫ln|1 − x²/y²| dρ(y),
with the zeta deviation from Nyquist dρ_N = ln y dy forced on the
dodging band and the probe's own zeros free beyond it (A440). The
reduction of the exponent to this equilibrium problem — ln λ₁ =
2eᵟ[sup_{x>X}φ − max φ] + O(log) — is conjectural: **the reduction is
not a theorem**; it is stated as five lemmas (near-vanishing is vanishing; the pole at the origin;
sampling on the exterior; Euler–Maclaurin for the designed zeros; the
discrete-to-continuum limit), each classical in kind and each checked
numerically (A442: the sampling lemma on the exact ground states, the
pole lemma on the balayage constant, the limit lemma on the formula's
continuum difference; the five jointly by the finite-δ offsets of (v)). *(iv) The theorem (the reduced
problem solved; A441).* The equilibrium problem is a balayage problem
on the doubly slit plane Ω = ℂ ∖ ((−∞, −X] ∪ [X, ∞)): taking the probe's
own zeros to be the balayage of −(ln|y| dy on [−X, X]) onto the
exterior makes the potential constant there, equal to s(X) =
∫g_Ω(t, 0) ln|t| dt with g_Ω the Green function with pole at the origin,
g_Ω(t, 0) = ln[(1 + √(1 − t²/X²))X/t] by the map w = ζ/(1 + √(1 − ζ²)).
With A = ∫₀¹ln((1 + √(1 − u²))/u)du = π/2 and B = ∫₀¹ln((1 +
√(1 − u²))/u) ln u du = −(π/2)(1 + ln 2), both by parts,

s(X) = 2X(A ln X + B) = πX(ln X − 1 − ln 2),  f(X) := −2s(X) = 2πX(1 + ln 2 − ln X),

maximal at X* = 2 with **f_∞ = 4π**: the value is optimal over every
zero distribution — real or complex, signed or not — for the functional
normalised at the origin (the potential's reference point x₀ = 0; its
...
X²)) with I(x) = ∫_{−X}^{X} √(X² − t²) ln|t|/(x − t) dt, the un-normalised
...
(a boundary optimum), where the edge density is finite and equal to
...
*Proof.* √(X² − t²) ln|t| is even and (x − t)⁻¹ + (x + t)⁻¹ =
2x/(x² − t²), so I(x) = 2x ∫₀^X √(X² − t²) ln t/(x² − t²) dt; since
...
with r = (xs − a)/b = −(x − s)/(x + s) (since 2xs − 2x² + X² = −(x − s)²
and X² = (x − s)(x + s)), |r| < 1, the series converging uniformly on
[0, π]; Euler's series ln sin(φ/2) = −ln 2 − Σ_{k≥1} cos(kφ)/k converges
in L²(0, π), so its Fourier coefficients are ∫₀^π ln sin(φ/2) cos kφ dφ =
−π/(2k) and ∫₀^π ln sin(φ/2) dφ = −π ln 2; integrating the product term
by term (the kernel's series uniform, ln sin integrable), L =
½ ∫₀^π ln sin(φ/2)/(a + b cos φ) dφ = (π/(2xs))[−ln 2 − Σ_{k≥1} r^k/k] =
(π/(2xs))[−ln 2 + ln(1 − r)] = (π/(2xs)) ln(x/(x + s)), as 1 − r =
2x/(x + s). Assembling, I(x) = 2x[(π/2) ln(X/2) − s²(π/(2xs)) ln X −
s²L] = πx ln(X/2) − πs ln(Xx/(x + s)), and τ(x) = −I(x)/(πs) =
ln(Xx/(x + s)) − (x/s) ln(X/2), which is the displayed form since
ln(Xx/(x + s)) = ln(2x/(x + s)) − ln(2/X). □ *Consequences.* For x > X,
x/s > 1 and 2x > x + s, so both terms are positive when X < 2 and the
second vanishes when X = 2: **τ(x) = ln(2x/(x + √(x² − 4))) > 0 on the
whole exterior at X = 2**, equal to ln 2 at the edge and tending to 0 at
infinity — the admissibility of the balayage, at X* = 2 and for every
X ≤ 2, is proved, and the reduced problem's maximiser is admissible by
theorem, not by census. For X > 2 the second term is −ln(X/2)(x/s − 1) →
−∞ as x → X⁺: inadmissible, as the sign of the edge coefficient
...
below it (there are 2eᵟ), twice Slepian's rate, with the probe at the
level e^{−T₀} at every zero beyond two horizons; and only the density
of the zeros enters, so a degree-d density gives 2d·T₀(d) with T₀(d) =
2πe^{δ/d}. *(v) The finite-δ formula (A442).* The same balayage with
the zeros left discrete gives ln λ₁ ≈ min_T [4Σ_{γ<T} ln((1 + √(1 −
γ²/T²))·T/γ) − 2aT]: its minimum sits at T ≈ 2T₀, its continuum limit
is −4πeᵟ (the difference 2s_δ(2T₀) + 4πeᵟ is 16.8, 17.8, 19.9, 21.2, 21.9, 23.5, 25.3 at the
cells, O(δ)), and the cells exceed it by c(δ) = 4.70, 5.21, 5.94,
6.39, 6.64, 7.30, 7.90 nats (the minimum located by a fine grid with
bounded refinement between zeros) — an O(δ) offset (the norm's width, the
sine factor, the count of zeros at the flat level) against exponents of
14 to 383, gated at 0 < c ≤ 3δ + 5 and increasing. *(vi) Cross-checks
and honest scope.* The δ = 1.0 value lies inside Theorem 1bj's Temple
enclosure ρ ∈ [9.2494, 9.4548]×10⁻⁷ and the δ = 1.3828125 value sits
above Theorem 1bl's certified even bound 5.7134×10⁻¹³ within a factor 2
— the mechanism's known band slack — both loaded at their keys. What is
...
values are exact for the listed zeros and their smooth count, and only
...
1 − χ(λ) ∼ (2¹⁴/3)√2π⁵ e^{−4πλ² + 9 log(λ)}", and, as the second of
the three indications they offer for the two missing steps of their
strategy, "(2) The extremely small numbers ε_λ that occur as
...
within 0.01, increasing): λ₁ tracks the prolate law to within 2.5
nats over the cells and 1 − χ₂ itself to within 2.6 (the function
...
Friedrichs extension of D*G_aD, G_a = P_aGP_a the compression to the
mean-zero subspace of the integral operator G with the
screw-function kernel g (Theorem 1.1), so that the form's kernel is
the distributional second derivative −g″ (his (2.9)–(2.11)); the lowest
...
∫_{−∞}^{∞} S_x(z) S̄_y(z) dz = g(x − y) − g(x) − g(−y) + g(0)" (his 𝔖;
the bar the complex conjugate, as printed and dropped by text
extraction). No
...
by the computed transitive import closure, with the 6700-zero list's
hash in every key; `slack_law.py`, `slack_law_mp.py`,
`prolate_rate.py`, `prolate_wkb.py`, `constrained_prolate.py` and the
`slack_law/` instruments (`discrete_balayage.py` among them) — the
reconnaissance, the family, the rate function, the finite-δ formula,
the equilibrium LPs, the balayage check, the explicit probes —
...
radius below 2^{−prec/2}, every lower end positive; the K2, K1 and k_λ
columns are rounded outward (each entry is at or above the ball's upper
end), the model and 1 − χ₂ columns to the nearest:

| δ | prime powers ≤ eᵟ | K1/K2 | certified ln λ₁ ≤ (K2) | at K1 | 1bm's model | CCM's k_λ | ln(1 − χ₂) |
|---|---|---|---|---|---|---|---|
| 1.0 | 1 | 70/120 | −13.882 | −13.878 | −13.884 | −13.669 | −15.522 |
...
9.3524×10⁻⁷ exceeds Theorem 1bj's certified Temple lower bound
2.6832×10⁻⁷ and lies inside 1bj's trial enclosure [9.2494,
9.4548]×10⁻⁷; the δ = 1.3828125 bound 8.8392×10⁻¹³ exceeds Theorem
1bl's certified 5.7134×10⁻¹³ (as 1bl states it) — the orderings an upper bound on the
...
cells (the ratio 2.0–5.4), the other η_p at least as fast between consecutive cells (gated), and
no limit is certified — so no truncation or model of the primes at any fixed
...
(floating point), the ratios η₂/λ₁, the asymmetries, and the other
primes' shifts falling at least as fast as η₂ between consecutive cells
(every prime present at both cells). Not claimed: any
...
**Theorem 1bp (the knife-edge is linear response — the certified shift
is the slack over the form's slope, the pins compose additively, and the
archimedean constant is pinned to exactly λ₁; `cascade_linear_response.py`).**
*(i) The slope.* For a prime p write the shells of p in Theorem 1bo's
Q^η as P_p(η)(g) = 2Σ_k (log p + η)e^{−u_k/2}f_g(u_k), u_k = k(log p + η)
< 2a, so that Q^η(g) = Q(g) + P_p(0)(g) − P_p(η)(g). At g₁ — 1bo's
certified minimiser, its stored coefficients — the derivative at η = 0
is the closed form dP_p/dη = 2Σ_k e^{−u_k/2}[(1 − u_k/2)f(u_k) +
u_k f′(u_k)], f the autocorrelation of g₁ and f′ its derivative, both
O(K) closed forms (`weil_linear_response.py`), and D_p :=
−(dP_p/dη)/‖g₁‖² is the first-order rise of g₁'s Rayleigh quotient per
unit upward shift of log p. Gated: the closed form against a central
difference of the shell sum in balls at every cell and prime (below
10⁻³⁰ relative) and against a central difference of the K × K shifted
Gram at δ = 1.0 and 2.0 (below 10⁻²⁰). *(ii) The law (computed,
gated).* At every (cell, prime) — 36 pairs — η_lin := λ₁/D_p lies
inside 1bo's certified bracket [η_lo, η_hi]: the certified crossing is
the first-order crossing to the bracket's width 10⁻³, η_lin/η_hi between 0.9993 and 1.0000 at the pairs (gated in [1 − 10⁻³, 1]). And
D_p > 0 at every pair: to first order g₁'s quotient rises for every
upward shift, which is why g₁ witnesses only downward crossings — the
upward crossings of 1bo's two-sided windows are 5×10²–3×10⁵ further out and
need the re-minimised vector. So the knife-edge of 1bo is the ratio of
two numbers each computed from g₁ alone, η_p = λ₁/D_p — the slack
divided by the slope of the form in log p — and 1bo's ratio η₂/λ₁ is
1/D₂ (gated to the bracket's width): D₂ = 0.1876, 0.3189, 0.4199, 0.4462, 0.4644, 0.4807, 0.4931 at the cells, rising through the cells
(gated increasing) as the ratio falls from 5.33 to 2.03; no limit is
claimed. *(iii) The dilation edge (certified bracket; the composition
computed, gated).* Rescale every shell at once, log p → (1 − ε)log p
for every p ≤ eᵟ in position and weight — the relative scale of the
finite places against the real place. At every cell the crossing of
g₁'s quotient is bracketed as in 1bo(ii), [ε_lo, ε_hi] of relative
width 10⁻³ with a negative ball at ε_hi, so the rescaled form is not
positive at ε_hi; the first-order composition ε_lin = λ₁/Σ_p(log p)D_p
lies inside the bracket at every cell, and the harmonic composition
ε_hi Σ_p log p/η_p is within 10⁻³ of 1 at every cell (0.9996–1.0002, gated): the pins compose additively — the slack
λ₁ is spent by the primes in proportion to (log p)D_p, and 1/ε = Σ_p
log p/η_p. The ε_hi ball is re-derived by the K × K path at δ = 1.0 and
2.0. Every ε_hi below is rounded outward, λ₁ is 1bn's certified upper
bound, the ratio column is the smallest over the cell's primes, rounded
down:

| δ | ln λ₁ ≤ | D₂ | min_p η_lin/η_hi | ε_hi: every log p rescaled by | ε_hi Σ log p/η_p |
|---|---|---|---|---|---|
| 1.0 | −13.882 | 0.1876 | 0.9993 | 7.194×10⁻⁶ | 0.9997 |
| 1.3828125 | −27.754 | 0.3189 | 0.9993 | 3.989×10⁻¹² | 1.0001 |
| 2.0 | −67.233 | 0.4199 | 0.9994 | 2.133×10⁻²⁹ | 1.0000 |
| 2.3 | −98.267 | 0.4462 | 0.9994 | 6.633×10⁻⁴³ | 1.0000 |
| 2.6 | −140.713 | 0.4644 | 0.9994 | 2.335×10⁻⁶¹ | 1.0000 |
| 3.0 | −221.899 | 0.4807 | 0.9994 | 1.236×10⁻⁹⁶ | 1.0000 |
| 3.5 | −383.282 | 0.4931 | 0.9996 | 9.794×10⁻¹⁶⁷ | 0.9999 |

*(iv) The archimedean pin (exact).* The constant ψ(¼) − log π enters
the Gram only on the diagonal, as (ψ(¼) − log π)N_kk (Theorem 1bn's
Gram; `weil_prime_gram.py`): for every vector the Rayleigh quotient of
the form with log π → log π + ε is the quotient minus ε exactly (gated
live at δ = 1.0 to the balls' width), so the ground state of the form
on [−a, a] with log π + ε is λ₁(δ) − ε: the shifted form is positive on
[−a, a] exactly when ε < λ₁(δ) — positivity, where it holds at ε = 0, is
lost at ε = λ₁(δ) upward and by no downward shift; that it holds at
ε = 0 is not certified here (no lower bound on λ₁ is claimed, 1bn(vi)).
Gated at every cell on the stored balls: g₁'s quotient negative at
ε = λ₁'s upper end, positive at −ε and at ε/2 — statements about g₁,
not lower bounds. The archimedean constant is thus pinned from above to
exactly the slack — at δ = 3.5, log π to within 3.487×10⁻¹⁶⁷ from above — while the primes
are pinned from below (1bo): the real place's constant and the finite
places' positions bound the form from opposite sides, each to the
precision λ₁(δ) up to the slope. *(v) What is proved, computed, not
claimed.* Proved: the archimedean identity of (iv) (a one-line
consequence of the Gram's form, gated to the balls' width); every ε_hi
of (iii) (a negative ball on an explicit vector). Computed, gated: the
law of (ii) (containment in every bracket, the ratio band), the sign
D_p > 0, the monotone D₂, the composition of (iii). Not claimed:
anything about the perturbed forms beyond the certified points — the
law is a statement about the fixed vector g₁'s quotient, whose
crossings are the certified ones (1bo's re-minimised crossings sit
within 1 percent of them at δ ≤ 2.0, gated there); any lower bound on a
window; anything about the zeros; a limit of D₂ — **no Riemann
Hypothesis consequence is claimed**. Check 7 clean (a derivative and a
Rayleigh quotient; no semiclassics; no cascade quantity). Check 8 clean
(Riemann-side; no hypothesis input). Substrate
`weil_linear_response.py` committed, imported, and content-addressed
into the verifier's keys by the computed transitive import closure.

**Theorem 1bq (the cross-L-function test — the true form of four other
L-functions at the seven cells: certified bounds, the finite-δ formula
on their own zeros, the knife-edge in both directions with the sign
rule, and the linear-response law transferred; `cascade_lfunction_test.py`).**
*(i) The forms and the Gram.* L(s, χ₋₃), L(s, χ₋₄), L(s, χ₈) — the real
primitive characters of conductors 3, 4, 8 (κ = 1, 1, 0) — and L(Δ, s),
Ramanujan's cusp form (degree 2, level 1, Γ_ℂ(s + 11/2)). `lfun_gram.py`
generalises Theorem 1bn's Gram: the Γ-factors ∏Γ_ℝ(s + κ_i) give the
archimedean kernel Σ_i e^{(½−κ_i)u}/sinh u with digamma and trigamma at
¼ + κ_i/2 + iω/2 and the same geometric tails; the constant is log q −
d log π + Σ_i ψ(¼ + κ_i/2); there is no pole; the prime side is
2Σ(log p)c(p^k)n^{−1/2}f(log n) with the local coefficients (χ(p)^k;
α^k + β^k for Δ, αβ = 1, α + β = τ(p)p^{−11/2}). With ζ's descriptor it
reproduces 1bn's Gram entrywise below 10⁻¹⁰⁰ (gated), and for each form
the prime-side Rayleigh quotient of the C_c^∞ bump equals the explicit
formula's zero side on the form's own zeros — `lfun_zeros.py`: the sign
changes of the form's Hardy function on the line, the count checked
against the smooth count of the argument principle and by a half-step
rescan, the reality of the completed function checked to 10⁻⁸ for the
characters (Δ's is real by construction, 2 Re Σ) — plus
the smooth-density tail, within 10⁻⁴ — gated at δ = 1.0 with 24 modes,
...
most the K1 value (gated), the pair within 0.1 in ln. *(iii) The
finite-δ formula on the form's own zeros (computed, gated).* 1bm(v)'s
ln λ₁ ≈ min_T[4Σ_{γ<T} ln((1 + √(1 − γ²/T²))T/γ) − 2aT] + c(δ) with the
zeros of L in place of ζ's: its continuum minimum is −2dT₀ (closed form,
...
at log(p)/2 (`lfun_cells.py`): exactly one direction carries the near
crossing — a certified negative ball at η_hi, a positive one at η_lo,
relative width 10⁻³ — and it is downward exactly when c(p) > 0 (χ(p) =
+1; τ(p) > 0), upward when c(p) < 0, at all 124 triples; the other
direction has no crossing below the cap — every sample of the doubling
search and of a 64-point log grid up to the cap itself a positive ball
...
linear response of the form to its own shell coefficients, in four
L-functions as in ζ. The smallest unramified prime's η_hi/λ₁ runs
χ₋₃: 1.3/1.2/1.4/1.4/1.5/1.5/1.6; χ₋₄: 2.0/1.6/1.6/1.6/1.6/1.5; χ₈: 2.2/1.3/1.3/1.4/1.4/1.4; Δ: 2.8/2.4/2.2/2.1/2.1/2.0/1.9 at the cells with an unramified prime. Every ln λ₁ below is the certified upper end (ceiling to
10⁻³), every c_L the computed offset (nearest 10⁻²):

| form | (q, d) | ln λ₁ ≤ at δ = 1.0 | 2.0 | 3.5 | c_L(2.0) | c_L(3.5) | triples | far crossings |
|---|---|---|---|---|---|---|---|---|
| χ₋₃ | (3, 1) | −3.855 | −21.079 | −126.292 | 5.60 | 7.28 | 30 | 4 |
| χ₋₄ | (4, 1) | −1.965 | −14.311 | −92.476 | 5.58 | 7.23 | 29 | 0 |
| χ₈ | (8, 1) | −1.025 | −6.991 | −46.223 | 5.37 | 6.81 | 29 | 0 |
| Δ | (1, 2) | −5.381 | −24.765 | −91.050 | 5.66 | 6.79 | 36 | 0 |

*(v) Context.* Kim et al. (arXiv 2607.24830, July 2026, "A numerical realization of
Suzuki's Weil-quadratic-form operator") compute the same forms'
low spectrum in the prime-free regime 2a < log 2 and report the
conductor and the degree in it (their R2, R2b); Suzuki (arXiv
2209.12832) is the operator framework; neither is an input here — the
cells are all inside the prime regime. *(vi) What is proved, computed,
not claimed.* Proved: every bound of (ii) and every near crossing of
(iv) (balls). Computed, gated in bands: the normalisation of (i), the
offsets of (iii), the law's containment and its deviations of (iv).
Not claimed: any lower bound on any λ₁ or window; anything about the
zeros beyond the double-precision lists (data, count-checked, not
certified); anything about the Generalised Riemann Hypothesis — **no
Riemann Hypothesis consequence is claimed**. Check 7 clean (Rayleigh–Ritz,
the explicit formula for L-functions, Hardy's Z — classical; no
semiclassics; no cascade quantity). Check 8 clean (Riemann-side; no
hypothesis input). Substrates `lfun_gram.py` and `lfun_cells.py`
committed, imported, and content-addressed into the verifier's keys by
the computed transitive import closure; `lfun_zeros.py` committed, its
lists read as data.

**Theorem 1br (the spectrum ladder — the low spectrum of the true form
is, rung by rung, the prolate leakage ladder of the finite Fourier
transform at the horizon on its +1 eigenspace, the order-0 prolate
absent; the pole term load-bearing; `cascade_spectrum_ladder.py`).**
*(i) The ladder (certified).* On the even sector of L²(−a, a) the form Q
is the pencil (G, N) of Theorem 1bn's Gram. For the k lowest approximate
eigenvectors v₁, …, v_k (flint's approximate eigensolver on the
midpoints) the projected pencil (RᵀGR, RᵀNR), R = [v₁ ⋯ v_k], is a k × k
pencil in balls, and by Courant–Fischer λ_k(Q) ≤ max_{v ∈ span R}
Q(v)/‖v‖² = the projected pencil's largest eigenvalue, whatever the
vectors are: the upper end of flint's certified enclosure of that
eigenvalue is a rigorous upper bound on λ_k. `weil_spectrum_ladder.py`
stores it at every cell for k = 1, …, m with m = 10, 12, 18, 22, 28,
42, 68 (K2 modes, 600–1300 bits; enclosure radii below 10⁻¹⁰⁰ relative;
the ladder monotone in k, the spans growing; the approximate
eigenvalues within 10⁻⁹ in ln — convergence, not certification); rung 1
is 1bo's λ₁; at δ = 1.0 the ten rungs are re-derived live, and at K = 96
the leakage rungs agree with the K = 120 ones within 0.05 in ln.
*(ii) The shadow (computed, gated).* Theorem 1bm's leakage 1 − χ₂ =
1 − √λ₄(c) is the order-4 prolate's at c = T₀ = 2πeᵟ (the paper's χ_j =
√λ_{2j}); here the whole even ladder ln(1 − √λ_n(c)), n = 0, 2, …, 4m,
at two Legendre cutoffs (agreeing to 10⁻⁹ on every leakage entry; the
order-4 entry is 1bn's at every cell). At every cell and every rung in
the leakage regime — ln(1 − χ_{2k}) < −1, 87 rungs over the seven
cells — the nearest even order to ln λ_k is 4k, the k-th prolate of
Fourier eigenvalue +1 beyond ψ₀; in the deep regime (ln(1 − χ_{2k}) <
...
nats at every rung and by at least 2 where ln(1 − χ_{2k}) < −20, and the
order-0 prolate (leakage −31.5 at δ = 1.0) appears in no ladder. The alternative convention c = aT₀ (which coincides with c = T₀ at
δ = 2.0, where a = 1) is rejected at δ = 1.0 and 1.3828125 — its map
k ↦ 4(k − 1) is a +1-eigenspace map with ψ₀ included, and its offsets
change sign within a cell (δ = 1.0: k1→0 k2→4, offsets +0.92, −1.46; δ = 1.3828125: k1→0 k2→4 k3→8, offsets +4.21, +0.38, −0.60) —
and at δ = 2.3, where its map is neither (k1→8 k2→12 k3→18 …, gated). Every ln λ_k below is the certified
upper end (ceiling to 10⁻²), every prolate entry the computed leakage:

| δ | deep rungs | ln λ₂ ≤ | ln(1 − χ₄) | ln λ₃ ≤ | ln(1 − χ₆) | rung-1 offset | offsets of rungs 2 on |
|---|---|---|---|---|---|---|---|
| 1.0 | 2 | −4.01 | −4.98 | −0.18 | −0.28 | 1.64 | [0.96, 0.97] |
| 1.3828125 | 3 | −15.54 | −16.73 | −5.87 | −6.81 | 1.79 | [0.93, 1.19] |
| 2.0 | 7 | −52.11 | −53.58 | −38.75 | −40.10 | 2.12 | [0.47, 1.47] |
| 2.3 | 9 | −81.77 | −83.34 | −67.05 | −68.42 | 2.18 | [0.59, 1.57] |
| 2.6 | 13 | −122.98 | −124.51 | −106.67 | −108.23 | 2.20 | [0.42, 1.56] |
| 3.0 | 19 | −202.48 | −204.21 | −184.56 | −186.19 | 2.39 | [0.53, 1.72] |
| 3.5 | 32 | −361.73 | −363.61 | −341.85 | −343.48 | 2.47 | [0.37, 1.87] |

*(iii) The pole is load-bearing (computed, gated live at δ = 1.0,
1.3828125, 2.0).* The pole term 2ĝ(i/2)² is rank one: G − G₀ = 2ppᵀ
entrywise in balls, G₀ the Gram without it (`lfun_gram.py`'s
generalised Gram with the pole switched off). Q₀ = Q − 2ĝ(i/2)² has a
...
Q₀ sit within 0.5 nats below the true ones, each nearest the same
order 4k: the ladder from rung 2 is cast without the pole, and rung 1 —
the slack law's λ₁ — lies between Q₀'s negative eigenvalue and its
second, the pole term lifting it to the order-4 leakage. *(iv) What is
proved, computed, not claimed.* Proved: every rung of (i)
(Courant–Fischer on a certified enclosure); the rank-one identity and
the negative balls of (iii). Computed, gated in bands: the
nearest-order map, the offsets, the alternatives' rejection, the
interlacing on approximate values, the pole-free gaps. Not claimed: any
lower bound on any eigenvalue; a mechanism for the selection of the +1
orders or for the offsets — the shadow is identified, the law of its
caster is not; anything about the zeros — **no Riemann Hypothesis
consequence is claimed**. Check 7 clean (Courant–Fischer, Weyl,
Slepian's prolates — classical; no semiclassics; no cascade quantity).
Check 8 clean (Riemann-side; no hypothesis input). Substrate
`weil_spectrum_ladder.py` committed, imported, and content-addressed
into the verifier's keys by the computed transitive import closure.

**Theorem 1bs (the count-constant theorem — the discrete-to-continuum
correction of the finite-δ formula is 4c_L ln T + C_L, c_L the constant
...
Riemann–von Mangoldt formula: 7/8 for ζ, κ/4 − 1/8 for χ, 11/4 for Δ
(Γ_ℂ(s + 11/2) = Γ_ℝ(s + 11/2)Γ_ℝ(s + 13/2)). Let B(T) = 4Σ_{0<γ<T}
arccosh(T/γ) be the balayage sum of 1bm(v)'s finite-δ formula (its
summand is ln((1 + √(1 − γ²/T²))T/γ) = arccosh(T/γ)) and I(T) =
4∫₀^T arccosh(T/r)n₀(r)dr = T[ln(q(T/2π)^d) − d(1 + ln 2)] its
continuum. Then, unconditionally,

  B(T) − I(T) = 4c_L ln T + C_L + o(1)  (T → ∞),

with 4c_L = 4[pole] + Σ_i(κ_i − ½) — 7/2, 1/2, 1/2, −1/2, 11 for ζ,
χ₋₃, χ₋₄, χ₈, Δ — and C_L = −4c_L ln(γ₁/2) − 4∫₀^{γ₁}N₀(r)/r dr +
4∫_{γ₁}^∞ S(r)/r dr, γ₁ the first zero. *(ii) Proof.* With φ(r) =
4 arccosh(T/r) (φ(T) = 0, φ(r) = 4 ln(2T/r) + O(r²/T²) as r → 0),
B − I = ∫₀^T φ d(N − N₀), and since (N − N₀)(r) → 0 as r → 0 while
...
S part is 4∫_{γ₁}^T S(r)/(r√(1 − r²/T²)) dr, which converges to
4∫_{γ₁}^∞ S(r)/r dr because ∫_{γ₁}^T S(r)/r dr = S₁(T)/T − S₁(γ₁)/γ₁
+ ∫_{γ₁}^T S₁(r)/r² dr with S₁(T) = ∫₀^T S = O(ln T) (Littlewood's
mean-value bound and its analogues), and the weight defect
∫_{γ₁}^T S(r)[(1 − r²/T²)^{−1/2} − 1]/r dr → 0 (on [T − √T, T] by
S = O(ln T) against the integrable √-singularity, on [γ₁, T − √T] by
parts against S₁). Adding the three pieces gives the statement. □
The classical inputs are the Riemann–von Mangoldt formula with its
...
Check 7 clean (Stieltjes, Riemann–von Mangoldt, Littlewood — classical;
no semiclassics; no cascade quantity). Check 8 clean (Riemann-side; no
hypothesis input). Substrate `balayage_count.py` committed, imported,
and content-addressed into the verifier's keys by the computed
transitive import closure; the zero lists read as data.
...
Weil's explicit formula applied to its autocorrelation reads Q(g) =
Σ_ρ ĝ(t_ρ)², t_ρ = (ρ − ½)/i over the nontrivial zeros, unconditionally
(ĝ(t) = ∫g(u)e^{itu}du; the pole term is ĝ(i/2)² + ĝ(−i/2)² = 2ĝ(i/2)²).
...
2(a + sinh a)², which is negative on [0.2, 1] by a certified interval
evaluation (the margin's lower ball end positive on 2000 subintervals,
gated) and for a ≥ 1 by the elementary bound 2(a + sinh a)² ≥
...
Hypothesis consequence is claimed**. Check 7 clean (the explicit
formula, Hadamard, Weyl — classical; no semiclassics; no cascade
quantity). Check 8 clean (Riemann-side; no hypothesis input). Substrate
`pole_witness.py` committed, imported, and content-addressed into the
...
4 arccosh(T/τ) = 4 ln(2T/τ) − τ²/T² + O(τ⁴/T⁴). At an edge T = XT₀
with X fixed, a pair at fixed height τ therefore costs 4 ln T₀ +
4 ln(2X/τ) + O(T₀⁻²) — four units of ln T₀, which in Fuchs' fixed-order
law ln(1 − λ_n(c)) = −2c + (n + ½)ln c + O(1) at c = T₀ (Theorem
1bm(vii)) are four prolate orders — while a pair at the Hermite scale
τ = x√T₀ costs 2 ln T₀ + O(1), two orders. The prolate's own zero pairs
sit at the Hermite scale: Slepian's fixed-order asymptotics ψ_n(c, t) ~
H_n(√c t)e^{−ct²/2} put its zeros at t_j = x_j/√c, hence the
...
double-precision zero list and of the bisection — gated; the
displacement growing
...
origin the excited states are Ξ times the orthogonal polynomials of
...
3.5 within one standard error, the residual's sign change by more than
two, and the standard errors' range; not modelled; δ = 2.3's six safely
deep rungs give the parities unequal mean ranks and its raw means
conflate trend and parity, so that cell is not used). *(v) What it says.* The +1 eigenspace and the four
...
prolate's two; a rung at the orders 2 mod 4 would need a pair at the
Hermite scale √T₀, which the fixed envelope does not offer; and rung 1
is the pole's four (Theorem 1bs). The ladder from rung 2 on is cast by
the hole zeros, the caster's law of Theorem 1br(iv) named: the
orthogonal polynomials of Ξ². *(vi) The anatomy of c(δ) (computed).*
Since ln λ₁ = ln 2Σ_γ ĝ₁(γ)² (the zero side on the 800 zeros known to
100 digits plus the far tail), c(δ) = ln λ₁ − F(T*) splits as
[ln ĝ₁(0)²] + [ln(2w_max/ĝ₁(0)²) − F(T*)] + [ln Σ_γ w_γ/w_max] — the
origin value, the peak's excess over the balayage level, the effective
...
the real-zero census within the dodging tolerance, the hole zeros
and their nodes, the k-level residuals and the parity's sign in the
formula's part, the anatomy. Not claimed: the
...
**Theorem 1bv (the odd sector — the Weil form on the odd probes of
the window is the even form with the multiplier γ² on the zero side,
...
ln π)‖g‖² + ∫₀^∞[f(0) − f(u)]K(u)du − 2ΣΛ(n)n^{−1/2}f(ln n), with f the
autocorrelation and K(u) = e^{u/2}/sinh u, exactly as for even g
(Theorem 1bn's form) except at the poles: ĝ(−r) = −ĝ(r) gives
H(±i/2) = −ĝ(i/2)², and ĝ(i/2) = ∫g e^{−t/2} = −∫g sinh(t/2) =: −p, so
**the pole term is −2p²**, negative, where the even sector's is
+2(∫g cosh(t/2))². In the sine basis φ_k = sin(ω_k t), k = 1..K − 1,
the autocorrelations are f_jk(u) = (−1)^{j+k}[ω_j sin(ω_k u) −
ω_k sin(ω_j u)]/(ω_j² − ω_k²) and f_kk(u) = [(2a − u)cos(ω_k u) +
sin(ω_k u)/ω_k]/2, the pole integrals p_k = Im[2 sinh((½ + iω_k)a)/(½ +
iω_k)], and ĝ_k(r) = i(−1)^k sin(ra)·2ω_k/(r² − ω_k²), so the sine
coefficients are the Nyquist samples ĝ(ω_k) = i a v_k. □ Self-test
(computed, gated): at δ = 1 and 2 the odd Gram gives 0.021912748475 and 0.000167771811 against the zero side 0.021912748475 and 0.000167771811 — the odd Gram's Q against 2Σ_γ|ĝ(γ)|²
over the 6700 zeros with the density tail, for the odd bump
t e^{−1/(1 − (t/a)²)}, within 10⁻¹² relative. *(ii) The reduction
(proved).* For odd g let G(s) = ∫_{|s|}^{a} g(t)dt: G is even,
G(±a) = 0, G′ = −g on (0, a), and integrating by parts ĝ(r) = i r Ĝ(r).
Hence Q(g) = Σ_γ γ²|Ĝ(γ)|² and ‖g‖ = ‖G′‖: **the odd sector is the
even form with the multiplier γ² on the zero side, over the even
G ∈ H¹₀(−a, a) with the Dirichlet norm**, λ₁^odd(δ) = min Σ_γ
γ²|Ĝ(γ)|²/‖G′‖². □ The multiplier at the dodging edge, T² ≈ 4T₀², is
worth 2 ln T₀ + O(1) in ln λ — the odd probe's one zero at the origin
costs two Fuchs orders, half a pair of Theorem 1bu(i); the O(1) is
computed, not derived. *(iii) The complete shadow (computed, gated).*
odd minus even 5.374, 6.514, 7.781, 8.487, 9.140, 9.995, 11.140 against 2 ln T₀ = 5.676, 6.441, 7.676, 8.276, 8.876, 9.676, 10.676 at the seven cells, the excess −0.302, +0.073, +0.105, +0.211, +0.264, +0.319, +0.464. The odd rungs sit at the prolate orders 4k + 2 with
positive offsets ln λ_k − ln(1 − χ_{2k+1}) at every safely deep odd
rung (its own leakage below −20; 0, 0, 3, 5, 9, 11, 11 safely deep odd rungs at the cells), within 1 of the even
sector's offsets at the orders 4k: the ground state’s offset against the order 6 +1.22, +1.49, +1.67, +1.77, +1.81, +2.02, +2.22 (the even sector’s against the order 4 +1.64, +1.79, +2.12, +2.18, +2.20, +2.39, +2.47), at δ = 3.5 the rungs 2–6 +1.61, +1.76, +1.43, +1.61, +1.37 against the even +1.87, +1.62, +1.74, +1.29, +1.74. **The even sector's
orders 4k and the odd sector's 4k + 2 together fill every even
prolate order from 4 on**: the orders 2 mod 4, which Theorem 1bu(v)
says the even sector cannot reach, are the odd sector's rungs — a rung
there is the odd probe's zero at the origin, not a pair at the Hermite
scale. The odd dodging edge lies inside or at the even one: the odd edge at 1.23, 1.50, 1.66, 1.77, 1.86, 1.84, 1.93 T₀ against the even 1.46, 1.63, 1.79, 1.77, 1.86, 1.90, 1.94.
*(iv) The odd limit shape (proved under D_odd).* Ĝ₁ = ĝ₁/(ir) is
even, real, entire of exponential type a and in L², so Theorem
1bu(ii)'s argument applies verbatim: under D_odd(δ) — the zero
multisets of Ĝ₁ and of Ξ coincide below T_D, ε(δ) → 0 — Ĝ₁(r)/Ĝ₁(0)
→ Ξ(r)/Ξ(0) locally uniformly with the bound 2R²ε, and with
∫ĝ₁² = ∫r²Ĝ₁² = 2π fixed and 1bu(ii)'s exterior hypothesis, Ĝ₁(0)² →
2πΞ(0)²/∫t²Ξ², ⟨r²⟩ under ĝ₁² → ∫t⁴Ξ²/∫t²Ξ² (∫t²Ξ² = 20.5076, ∫t⁴Ξ² = 602.05, 2πΞ(0)²/∫t²Ξ² = 0.07572, √(∫t⁴Ξ²/∫t²Ξ²) = 5.418), and the
curvature of Ĝ₁ at the origin → Σ_γ γ⁻². □ At the cells (computed,
gated): in the sine basis Ĝ₁ = 2 sin(ra)N(r²)/(rΠ_{0<k<K}(r² − ω_k²))
with N of degree K − 2, so its zeros are K − 2 designed pairs and the
sinc zeros — the census as in 1bu: all K − 2 designed pairs located at every cell, the sum rule closing within 10⁻¹⁶; D_odd holds within the
dodging tolerance as in 1bu; Ĝ₁(0)² = 0.0410, 0.0527, 0.0635, 0.0667, 0.0691, 0.0713, 0.0730 rising toward 0.0757; √⟨r²⟩ = 6.496, 6.023, 5.705, 5.624, 5.567, 5.516, 5.477 falling toward 5.418; the curvature 0.01441, 0.01755, 0.02027, 0.02103, 0.02158, 0.02210, 0.02250 rising toward 0.02310. *(v) The odd
hole zeros (computed, gated).* Every safely deep odd rung k has
exactly k − 1 hole zeros, at the nodes of the orthogonal polynomials
of the weight ĝ₁² = r²Ĝ₁²: at δ = 3.5 rungs 2–5 at 5.477; 4.217, 8.580; 3.682, 7.431, 11.492; 3.287, 6.598, 9.980, 16.389 against the nodes 5.477; 4.217, 8.580; 3.681, 7.429, 11.489; 3.286, 6.597, 9.978, 16.384 (gated
within 1% for rungs 2–4 at δ ≥ 2.3; rung 4 at δ = 2.0 deviates 1.26%); the first hole zero 6.702, 6.049, 5.710, 5.626, 5.568, 5.516, 5.477 falls toward the Ξ-limit 5.418; the limits are the nodes
of t²Ξ² — the Ξ-limit nodes of t²Ξ² 5.418; 4.171, 8.499; 3.622, 7.306, 11.232; 3.249, 6.527, 9.888, 16.086: the odd excited states are rΞ times the
orthogonal polynomials of t²Ξ², the odd Hermite ladder on the Riemann
Ξ function. *(vi) The odd anatomy (computed, gated).* Q = pole +
const + archimedean + primes, the pole −2p², the archimedean term
...
(i), (ii), (iv) under D_odd. Computed, gated: the self-test, the
complete shadow, the census and the limits at the cells, the hole
zeros, the anatomy and its Ξ-limit. Not claimed: the gap's O(1) (the
reduction's heuristic, not a theorem); that the odd excited states are
exactly the polynomial multiples; anything about the zeros — **no
Riemann Hypothesis consequence is claimed**. Check 7 clean (Weil's
explicit formula, integration by parts, Hadamard, Slepian's prolates,
Stieltjes — classical; no semiclassics; no cascade quantity). Check 8
clean (Riemann-side; no hypothesis input). Substrates `odd_sector.py`
(keyed producer, the seven cells and the self-test),
`weil_prime_gram_odd.py` and `xi_limit.py` committed, imported and
content-addressed into the verifier's keys; the even sector's
checkpoints through `ladder_caster.py` (Theorem 1bu's producer,
imported).

**Theorem 1bw (the excited states' exact anatomy — every designed zero
pair of every safely deep rung censused, the rungs factoring as the
ground state times the hole polynomial times an exterior correction that
is a Gaussian on the interior with the curvature difference as its
coefficient, and the hole polynomials the bi-orthogonal family that the
ground state and those coefficients determine;
`cascade_rung_anatomy.py`).** *(i) The census (computed, gated).* At δ =
2.0, 2.3, 2.6, 3.0, 3.5 and every safely deep rung k of Theorem 1bu (3,
6, 9, 11, 11 safely deep rungs at δ = 2, 2.3, 2.6, 3, 3.5) the transform
of rung k, ĝ_k = 2 sin(ra)M_k(r²)/(rΠ_{0<j<K}(r² − ω_j²)) with M_k of
degree K − 1, has every designed pair real: all 159, 259, 319, 399, 539
designed pairs located at every safely deep rung but rung 3 at δ = 3
(398 of 399, the last placed by the sum rule at |τ| = 1470, 0.048 below
the sinc zero ω_702 inside one census cell) and every rung at δ = 3.5
(538 of 539, the last placed by the sum rule at |τ| between 8688 and
14501, beyond the region 7741) — 1bu(ii)’s branch; the sum rule
Σ_designed τ⁻² + (a/π)²Σ_{j≥K} j⁻² = κ_k := −ĝ_k″(0)/(2ĝ_k(0)) closes
within 10⁻¹³ wherever every pair is located (Richardson about the exact
ĝ_k(0) = 2av₀, as in 1bu). Rung k has its own dodging edge T_k — the
first zeta zero with no zero of ĝ_k within the hole tolerance 0.2 —
inside the ground state's T₁ and never outward with k over the safely
deep rungs at every cell (the rungs’ edges 121.4, 111.9, 111.0, 105.4,
...
ln Π(1 − r²/τ²) is −Στ⁻², so the coefficient is the sum rules' — and, by
the census, Δκ_k = [Σ_{E_k}τ⁻² − Σ_{E_1}τ⁻²] − Σ_freed z⁻² over the
ground state's dodging zeros at the zeta zeros it dodges and rung k does
not, up to the dodging displacements (the accounting residual within 2.7
× 10⁻⁶ at every safely deep rung): the k − 1 hole pairs are paid for by
the exterior, Δκ_k < 0 and falling with k (−Δκ_k = 3.49 × 10⁻⁵, 7.87 ×
10⁻⁵, 1.21 × 10⁻⁴, 1.70 × 10⁻⁴, 2.20 × 10⁻⁴, 2.87 × 10⁻⁴ for rungs 2–7
at δ = 2.3), the effective height of the exterior pair a hole replaces
τ_eff := √((k − 1)/(−Δκ_k)) between 1.18 and 1.42 T₁ over the safely
deep rungs of the five cells (τ_eff = 1.40, 1.31, 1.30, 1.26, 1.24, 1.19
T₁ at δ = 2.3). The quadratic law holds to r = 40: the ratio (−ln
C_k(r)/r²)/Δκ_k is within 2 × 10⁻³ of 1 at r = 5 and within 0.25 at r =
40 (the ratio 1.049, 1.053, 1.056, 1.058, 1.062, 1.069 at r = 40 for
rungs 2–7 at δ = 2.3). *(iii) The bi-orthogonal law (computed, gated).*
The rungs are N-orthogonal, ∫ĝ_jĝ_k = 0, so the hole polynomial H_k is
orthogonal to H_j for every j < k under the pair weight ĝ₁²C_kC_j: k − 1
linear conditions that determine H_k up to scale. On the interior [0,
T₁/2] (the interior carrying 1.000000, 1.000000, 1.000000, 1.000000,
1.000000 of the ground state’s mass), with the exact C_k the conditions
reproduce the censused holes (E within 1.2 × 10⁻⁸, 1.7 × 10⁻⁸, 2.0 ×
10⁻⁸, 2.2 × 10⁻⁸, 2.5 × 10⁻⁸, 2.3 × 10⁻⁸ for rungs 2–7 at δ = 2.3); with
the Gaussian truncation C_j = e^{−Δκ_j r²} and the recursion's own lower
polynomials — the ground state and the numbers Δκ₂, …, Δκ_k alone — they
reproduce them to R within 6.1 × 10⁻⁷, 4.0 × 10⁻⁶, 2.7 × 10⁻⁵, 9.9 ×
10⁻⁵, 2.9 × 10⁻⁴, 9.6 × 10⁻⁴ against the naive nodes’ 3.6 × 10⁻⁴, 1.1 ×
10⁻³, 2.8 × 10⁻³, 6.9 × 10⁻³, 1.2 × 10⁻², 2.4 × 10⁻² for rungs 2–7 at δ
= 2.3; the mangle with every Δκ's sign flipped misses by ten times or
more (the mangle’s 7.2 × 10⁻⁴, 2.2 × 10⁻³, 5.5 × 10⁻³, 1.4 × 10⁻², 2.2 ×
10⁻², 4.9 × 10⁻² at δ = 2.3); over the five cells E within 3.4 × 10⁻⁸
and R within 1.2 × 10⁻³ at every safely deep rung at every cell; the
producer’s law extends beyond the safely deep rungs to 4, 7, 11, 12, 12
rungs, over which E is within 8.9 × 10⁻⁶ and R within 5.7 × 10⁻³. *(iv)
What it says.* The roadmap's (c) — the excited states as the ground
state times the orthogonal polynomials of its power spectrum, which
1bu(vii) declined to claim exact — is exact at the cells in this form:
the polynomials are orthogonal under the pair weights ĝ₁²C_kC_j, not
under ĝ₁², and the exterior's correction is, on the interior, the
Gaussian e^{−Δκ_k r²} whose coefficient the curvature sum rule fixes.
The correction falls toward the Ξ-limit — −Δκ₂ = 6.52 × 10⁻⁵, 3.49 ×
10⁻⁵, 1.80 × 10⁻⁵, 8.16 × 10⁻⁶, 3.20 × 10⁻⁶ at the five cells — where
the orthogonal polynomials of Ξ² are the law (1bu(iii)); at finite δ the
naive nodes miss by the correction's size. Δκ_k itself, the exterior's
re-balayage in one number per rung, is computed here, not derived:
Theorem 1bx derives its law — the hole pairs balayaged onto the
complement of a wall — with the wall the one number per rung that
remains of (c). *(v) What is proved, computed, not claimed.* Proved: the
factorisation's identity and the r² coefficient (elementary). Computed,
gated: the census (the counts of dodging and hole zeros directly, the
placed pairs' neighbours), the edges, the shared dodging zeros'
displacements, the curvature law's residuals, the bi-orthogonal law's
residuals against the naive nodes and the mangle, τ_eff, the fall of Δκ₂
across the cells, rung 9's touching at δ = 2.3. Not claimed here: a
derivation of Δκ_k or of the O(r⁴) term's law (both at Theorem 1bx, from
the balayage, the wall computed); the law beyond the safely deep rungs
(the producer's extension is reported, not gated as a law); the
complexity of rung 10's eight unlocated pairs; the odd sector's anatomy;
anything about the zeros — **no Riemann Hypothesis consequence is
claimed**. Check 7 clean (Plancherel, orthogonal polynomials,
Gauss–Legendre quadrature — classical; no semiclassics). Check 8 clean
(Riemann-side; no hypothesis input). Substrates `rung_anatomy.py`
(keyed: the census at the five cells) and `rung_laws.py` (keyed on the
census: the laws), importing `ladder_caster.py`'s instruments, committed
and content-addressed into the verifier's keys by the computed
transitive import closure.

**Theorem 1bx (the exterior's re-balayage — the law of Δκ_k derived: the
k − 1 hole pairs balayaged onto the complement of a wall, the Green
function of Theorem 1bm's doubly slit plane giving the exterior's
correction in closed form, Δκ_k = −Σ_h(1 + σ_h²)/(2T_w²) and the deficit
profile (2/π)arccos(T_w/R); one wall per rung, computed;
`cascade_rung_balayage.py`).** *(i) The object.* Theorem 1bw's Δκ_k is a
statement about the outer zero sets O_k (the dodging and exterior zeros
of rung k, the placed pair included) against the ground state's O₁: Δκ_k
= Σ_{O_k}τ⁻² − Σ_{O₁}τ⁻² = 2∫₀^∞ (n_k − n₁)(R) R⁻³ dR, the R⁻³ moment of
the outer counting deficit n_k − n₁ (an identity, by parts; the deficit
is 0 below the shared dodging zeros and −(k − 1) beyond the region).
Where the deficit sits is the law. *(ii) The balayage law (proved).* Let
E_T = {|τ| ≥ T} be the complement of a wall and Ω = ℂ ∖ E_T Theorem
1bm's doubly slit plane, ψ(z) = (z/T)/(1 + √(1 − z²/T²)) its map onto
the unit disk (ψ(0) = 0, ψ(±T) = ±1; 1bm(iv)), with Green function G(z,
w) = −ln|(ψ(z) − ψ(w))/(1 − ψ(w)ψ(z))|. *Hypothesis B (the
re-balayage):* O_k is O₁ with the hole pairs ±h balayaged onto E_T —
equivalently V := ln|H_kC_k| = ln|ĝ_k/ĝ₁| − ln|ĝ_k(0)/ĝ₁(0)| is harmonic
in Ω off the holes, where it has the logarithmic singularities of H_k,
and constant on E_T (the two transforms' envelopes proportional beyond
the wall). Then, with σ_h := ψ(h) ∈ (0, 1), V = −Σ_h[G(·, h) + G(·, −h)]
+ 2Σ_h ln(1/σ_h) (the constant V's value on E_T, fixed by V(0) = 0), and
ln C_k(R) = Σ_h[ln|1 − ψ(R)²/σ_h²| − ln(1 − σ_h²ψ(R)²) − ln|1 − R²/h²|]
on the interior, normalised C_k(0) = 1; expanding at R = 0 (ψ(R)² =
R²/(4T²) + O(R⁴)) and using 1/σ² − σ² = (1 + σ²)(1 − σ²)/σ² with h² =
4T²σ²/(1 + σ²)²,  **Δκ_k = −Σ_h (1 + σ_h²)/(2T²)**;  at h → 0 (σ_h →
h/2T) the pair costs exactly 1/(2T²), the interior correction is ln
C_k(R) = −2(k − 1) ln[(1 + √(1 − R²/T²))/2] = (k − 1)[R²/(2T²) +
3R⁴/(16T⁴) + …] — the O(r⁴) law 1bw declined to claim — and the pair
deficit on E_T is the harmonic measure seen from the origin, P(R) =
(2/π)arccos(T/R), whose R⁻³ moment 2∫_T^∞ P R⁻³dR =
(2/T²)∫₀¹(2/π)arccos(u) u du = 1/(2T²) closes the circle. Every step is
the Green function of Ω (Joukowski's map and the maximum principle, as
at 1bm(iv)); the algebra is witnessed numerically in g0. *(iii)
Hypothesis B at the cells (computed, gated).* Define the wall T_w(k) by
inverting the law on Theorem 1bw's Δκ_k with the censused holes (one
number per rung, replacing 1bw's one number per rung). Then everything
else follows without further input and is checked — three confirmations
and one departure: the exact ln C_k from the census products (rung
against ground state, at midpoints between zeta zeros) matches the
closed form across the interior — the exact ln C_k within 0.5%, 1.7%,
5.9% of the closed form at R = 0.3, 0.5, 0.7 T_w at every safely deep
rung, against 6.9% to 51% off at 0.5 T_w with the wall at T₁ for k ≥ 3;
the outer counting deficit (n_k − n₁)/(k − 1) on [1.5, 6]T₁ fitted by
...
within 1.8% at δ ≥ 3; the window’s upper end is the sensitive one — the
same fit to 8T₁ puts the profile’s wall 22% off at its worst few-hole
rung while δ ≥ 3 stays within 3.8%; 1bw's interior ratio at r = 40 is
the closed form's — the ratio at r = 40 within 0.0131 of the closed form
at every safely deep rung, its excess over 1 reaching 0.125, and within
0.0005 at δ ≥ 3; and the onset lags the harmonic measure near the wall —
over the 35 rungs with two or more holes the signed shortfall (observed
minus profile, per hole) averages 0.05 at T₁, 0.05 at 1.25 T₁, 0.03 at
1.5 T₁ and −0.006 at 2T₁, positive at 32 of the 35 at T₁ and at most
0.17 there; the deficit at T₁ is under a quarter of a zero at 7 of them
(δ = 2’s rung 3, δ = 2.3’s rung 3, δ = 3’s rungs 3–4, δ = 3.5’s rungs
3–5) against the profile’s 0.31–0.66 zeros; the 5 one-hole rungs, rung 2
of every cell, are reported apart — a one-hole rung’s per-hole count
moves by a whole zero at a time: shortfall 0.01, 0.20, 0.26, −0.12 at
T₁, 1.25, 1.5, 2 T₁, a swing of 0.38 between 1.5 and 2T₁ — and at T₁ the
profile itself predicts under a quarter of a zero there (0.00–0.20), as
observed (0.009–0.108) — a soft wall: the deficit sets in later than a
hard wall's inverse-square-root density, and has caught up by 2T₁ in the
mean. Rung 2's wall is the ground state's edge within 1.3% at δ ≤ 3 and
4.8% at δ = 3.5: T_w/T₁ = 1.002, 0.987, 0.998, 0.998, 0.952 for rung 2
at the five cells; −Δκ₂T₁² = 0.498, 0.513, 0.502, 0.502, 0.552 at δ = 2,
2.3, 2.6, 3, 3.5 against the law’s 0.500, 0.500, 0.500, 0.500, 0.500
...
sits inside rungs 3–6's and above rung 7's): T_w/T₁ between 0.833 and
1.002 over the 40 safely deep rungs of the five cells, T_w/T_k between
0.980 and 1.245, and T_w/√(T₁T_k) between 0.966 and 1.019 at the 35
rungs with k ≥ 3; strictly between T_k and T₁ at 37 of the 40 rungs,
inside T_k by up to 2.0% (δ = 3.5’s rungs 2–3) and beyond T₁ by 0.2% (δ
= 2’s rung 2) — near the geometric mean of the two edges for k ≥ 3, and
neither edge's law (T_w/T_k reaches 1.245, T_w/T₁ falls to 0.833). *(iv)
What it says.* The roadmap's (c) asked for Δκ_k derived: it is the
...
soft wall, seen also in the onset lag of the rungs with two or more
holes, whose position the census fixes and this block does not derive.
...
soft onset's; its law is Theorem 1bz's, the unlocking cost less the
zone's leakage share over the hole slopes. *(v) What is proved,
computed, not claimed.* Proved: the identity (i); the balayage law (ii)
under Hypothesis B — the closed form of ln C_k, Δκ_k, the deficit
profile and the r⁴ coefficient. Computed, gated: Hypothesis B with one
wall per rung — the interior profile at three heights and the r = 40
ratio at every safely deep rung of the five cells, the deficit profile's
wall at the thirty with three or more holes; the onset lag near the
wall, B's one departure, gated as such; rung 2's wall at T₁ and T₁ at
two horizons; the walls' brackets and monotonicity. Not claimed here:
the wall's law (Theorem 1by); Hypothesis B beyond the safely deep rungs
or at other supports; the odd sector; anything about the zeros — **no
Riemann Hypothesis consequence is claimed**. Check 7 clean (Joukowski's
map, Green's function, Poincaré–de la Vallée Poussin balayage —
classical, already listed at 1bm; no semiclassics). Check 8 clean
(Riemann-side; no hypothesis input). No new substrate: the verifier
computes from Theorem 1bw's census and laws checkpoints, Theorem 1bu's
ladder checkpoints (the safely deep counts) and Theorem 1bm's 6700-zero
list (data, its hash in every key), all reused at their keys.
...
a nat over the censused range; the continuum law u ln(2/u) =
...
norm's moment, and the leakage at a dodged zero, ĝ(γ_j)² = d_j²ĝ′(γ_j)²,
is quadratic in that field. Verified (g5) with the Gram and polished
eigenvectors recomputed in-process at δ = 2 and 2.3 (pencil residuals
below 10⁻¹⁵⁰; signed ĝ and ĝ′ at every listed zeta zero in arb; the
designed zero itself by Newton where the displacement is
float-resolvable; the leakage beyond the 6700-zero list entering M_j as
−(λ‖g‖² − 2Σ_list ĝ²), its O(γ_j²/γ_l²) part neglected): (own + M)/(λH)
= 1 within 0.0023 at all 295 dodged zeros of the 11 rungs (median
1.00002), the field ratio M/(λH) running from +4.8 to −6107, the leakage
beyond the 6700-zero list 1.9–2.7% of the total, and the ground state’s
ln|d| per bin of γ/T₁ (0–0.3, 0.3–0.5, 0.5–0.7, 0.7–0.8, 0.8–0.9, 0.9–1)
at δ = 2.3: −78.0, −51.9, −28.1, −13.5, −7.4, −2.5 — the residual's
floor is that neglected term, not the identity (the residual’s
correlation with the neglected term’s proxy (γ_j/γ₆₇₀₀)²|M_tail|/|λH_j|
...
here, not derived; its law is Theorem 1bz's — the two walls' leakage
budget: the hole pairs' constant at the sharp wall exceeds its value at
T_u by the unlocking cost less the zone's leakage share, whence the
geometric-mean law. *(vi) What is proved, computed, not claimed.*
Proved: the displacement law (i) — the stationarity identity and its
dodged-zero form; the minimiser of the unlocking functional is a zeta
zero or the domain's endpoint h_max (the concavity between zeros); the
continuum stationarity and its equality with the edge-coefficient
balance, within the reduced problem. Computed, gated: the endpoint
excluded at the forty safely deep rungs; the functional's minimisers
against the edges at the five cells and forty rungs, with their margins
and the minimum's flatness; the exponent at the forty rungs; the sharp
wall's excess against the hole count; the continuum law against T_u and
the sharp wall; the displacement law at 295 dodged zeros of eleven
rungs, to the tail approximation. Not claimed here: the law of the sharp
wall's excess over T_u (Theorem 1bz's); the reduction of 1bm(iii);
Hypothesis B beyond the safely deep rungs; the odd sector; anything
about the zeros — **no Riemann Hypothesis consequence is claimed**.
Check 7 clean (Hadamard's product, Rayleigh's quotient, partial
fractions, the balayage of 1bm — classical; no semiclassics). Check 8
...
Theorem 1bu's ladder checkpoints and its 800 zeros to 100 digits, and
Theorem 1bm's 6700-zero list, all reused at their keys, and recomputes
the Gram and polished eigenvectors at δ = 2 and 2.3 in-process — no
checkpoint, no producer.

**Theorem 1bz (the gap's law — the two walls' leakage budget: the sharp
wall is the wall the exterior reads, the unlocking height the wall the
...
Theorem 1bx's sharp wall — the wall at which the balayaged hole pairs'
closed form −Σ_h(1 + σ_h²)/(2T²) equals the observed Δκ_k, the R⁻³
moment of the outer counting deficit n_k − n₁ (1bx(i)). With the
polished states of every safely deep rung and its ground state at the
five cells (the keyed producer `leakage_profiles.py`: the pencil's
eigenvectors polished by inverse iteration at the working precision, ĝ
at every listed zeta zero and at the midpoints below 4T₀ in arb; the
polished eigenvalues equal the census’s within 10⁻⁸ at all 54 states of
the five cells, the pencil residuals lie below 10⁻¹⁵⁰, and the 6700-zero
list carries 96.1–99.7% of every state’s leakage), write s_ext for the
rung's share of its leakage λ_k‖g_k‖² at the zeta zeros beyond T₁ (the
tail beyond the list included), S₁(x) := 1 − (the ground state's leakage
at the zeta zeros below xT₁)/λ₁ for the ground state's share beyond xT₁
(S₁(1) = 0.96–0.98 at the five cells: the ground state leaks two to four
percent below its edge), and ŝ := s_ext/S₁(1) for the rung's exterior
share relative to the ground state's own. Hypothesis B says the rung's
exterior envelope is the ground state's lifted by the constant e^{2Σ_h
ln(1/σ_h(T_w))} times the origin ratio, so the two exterior leakages
stand in the ratio of the lifts squared — and they do, at the sharp wall
and not at the unlocking height (every residual in (i) and (ii) as the
formula less the value): ln(λ_k s_ext/(λ₁S₁(1))) − 2 ln|ĝ_k(0)/ĝ₁(0)|
equals 4Σ_h ln(1/σ_h(T_w)) within +0.01 ± 0.11 nats (mean, rms; 0.30 at
most) at the 40 rungs, against −1.09 ± 1.72 with T_u in place of T_w.
That whole reading's exponential is the rung-leakage-weighted mean of
its two parts' — each part's exponential a common factor times its
window's ratio of the ground state's share to the rung's, the mean that
factor times the two ratios' mediant — so it lies between them at every
rung; the parts differ: the near quarter [T₁, 1.25T₁) alone reads +0.39
± 0.47 nats, a wall 2.8% below T_w on average (rms deviation from T_w
4.7%), and the field beyond −0.20 ± 0.25, a wall 1.2% above (rms from
T_w 1.5%), the near reading the higher at 39 of the 40 rungs. The
...
ĝ(γ)²/A(γ)² = sin²πd the phase), is not lifted uniformly (each rung's
figure the mean of ln(A_k²/A₁²) over a window's zeta zeros): in the
leakage’s units, the constant squared less the envelope lift squared net
of the origin ratio squared (V’s convention, 1bx(ii)) runs +0.34 ± 0.48
nats on [T₁, 1.25T₁), −0.16 ± 0.32 on [1.25T₁, 1.5T₁) and −0.18 ± 0.26
on [1.5T₁, 2T₁) (means and rms over the 40 rungs) — the envelope rises
...
in those units beyond; the integrated reading above is what B asks of
...
moment, which weights the far field more than 1bx's R⁻³ moment does,
sees the same far-field excess as the envelope, a systematic offset of
the harmonic measure's profile and not noise; the wall it names is T_w
to 1.5% on average and 4% at worst. *(ii) The leakage budget (an
identity).* Read the rung's exponent twice. Theorem 1by reads it at the
unlocking height: ln λ_k − ln λ₁ = 2 ln|ĝ_k(0)/ĝ₁(0)| + F₁(T_u) +
cost(T_u) − F₁(T_u(1)) − r₁, with cost(T) := 4Σ_h ln(1/σ_h(T)) the hole
pairs' constant and r₁ 1by's residual (the formula less the value).
Hypothesis B reads the exterior leakage at the sharp wall: λ_k s_ext =
e^{cost(T_w)}(ĝ_k(0)/ĝ₁(0))² λ₁S₁(1), that is ln λ_k − ln λ₁ = 2
...
of −1.7% to +5.8% (T_w/T_u − 1 at those five rungs, 1by(v)'s one-hole
gaps, gated here); the law is the multi-hole law. *(iv) The leading law
...
forms; the zone's anatomy. Not claimed here: a derivation of the ground
state's leakage profile S₁ (a ground-state input, computed from the
polished ground state; Theorem 1ca reads it by the explicit formula —
the smooth count's integral of the transform squared less the prime
shells — given the transform); the law at one or two holes; Hypothesis B
beyond the safely deep rungs; the reduction of 1bm(iii); the odd sector;
anything about the zeros — **no Riemann Hypothesis consequence is
claimed**. Check 7 clean (Hadamard's product, Rayleigh's quotient, the
balayage of 1bm and 1bx and the harmonic measure's moments — classical;
no semiclassics). Check 8 clean (Riemann-side; no hypothesis input).
...
**Theorem 1ca (the smooth count in the unlocking functional — the zero
sum split exactly into Theorem 1bs's smooth count and a leftover
oscillation; the smooth wall's law with the count constant, T ln(2T₀/T)
= 4m + 7/2, within two percent rms of the unlocking height over the 40
rungs; the leftover's decay at the wall, O(ln T/√T), from von Mangoldt's
and Littlewood's bounds; and the exterior tail read by the explicit
formula — the smooth count's integral of the transform squared less the
prime shells, exactly, the leftover after the shells the Stirling
remainder; `cascade_smooth_functional.py`).** *(i) The split (an
identity).* Write the zero count as N(T) = N₀(T) + 7/8 + S(T) for T ≥
γ₁, N₀(T) = (T/2π)(ln(T/2π) − 1) Theorem 1bs's smooth count, 7/8 its
constant, S the oscillation (1bs's S: the classical S(T) = (1/π)arg ζ(½
+ iT) up to the Stirling remainder of the Riemann–von Mangoldt formula,
O(1/T)). Theorem 1by's unlocking functional F_k(T) = 4Σ_{γ<T}
arccosh(T/γ) − 2aT + 4Σ_h arccosh(T/h), by 1bs(ii)'s Stieltjes
integration of the zero sum against the count (B(T) − I(T) = ∫₀^T (N −
N₀)(r) w_T(r) dr, w_T(r) = 4/(r√(1 − r²/T²)), an identity), splits
exactly, for every T above the holes and γ₁: **F_k(T) = F_k^s(T) +
Osc(T)**, with the smooth functional F_k^s(T) := I(T) − 2aT + 4Σ_h
arccosh(T/h) + (7/2) arccosh(T/γ₁) − ∫₀^{γ₁} N₀(r) w_T(r) dr — I(T) =
T[ln(T/2π) − 1 − ln 2] the continuum of the zero sum, the constant's
term exact since (7/8)∫_{γ₁}^T w_T = (7/2) arccosh(T/γ₁), the piece
below γ₁ where N = 0 — and the leftover Osc(T) := ∫_{γ₁}^T S(r) w_T(r)
dr. F_k^s is the unlocking functional with N₀ + 7/8 in place of the
zeros; nothing else changes. *(ii) The smooth wall (derived).* F_k^s is
smooth, and with I′ − 2a = ln(T/4π) − 2a = ln(T/2T₀) (T₀ = 2πe^δ) it is
stationary where **ln(2T₀/T) = 4Σ_h (T² − h²)^{−1/2} + (7/2)(T² −
γ₁²)^{−1/2} + 4∫₀^{γ₁} N₀(r) r (T² − r²)^{−3/2} dr**, that is T
ln(2T₀/T) = 4m + 7/2 + [4Σ_h(T/√(T² − h²) − 1) + (7/2)(T/√(T² − γ₁²) −
...
worth seven eighths of a hole. The stationary minimum is unique, for
every hole set: T·F_k^s″ = 1 − T·G(T) with T·G(T) = 4Σ_h T²(T² −
h²)^{−3/2} + (7/2)T²(T² − γ₁²)^{−3/2} + 12T²∫₀^{γ₁} N₀(r) r (T² −
r²)^{−5/2} dr — the first two terms strictly decreasing, T·d/dT[T²(T² −
r²)^{−3/2}] = −T²(T² + 2r²)(T² − r²)^{−5/2}; the third negative and
rising toward 0 (N₀ < 0 below γ₁, as 2πe > γ₁), of the cusp's own order
at T = γ₁ with the coefficient 4N₀(γ₁) = −1.70 against the constant's
7/2, its rise T·d/dT = 12T²∫₀^{γ₁} |N₀| r (3T² + 2r²)(T² − r²)^{−7/2}
dr. The constant's fall alone exceeds that rise wherever R(T) :=
12∫₀^{γ₁} |N₀| r (3T² + 2r²)(T² − r²)^{−7/2} dr / [(7/2)(T² + 2γ₁²)(T² −
γ₁²)^{−5/2}] < 1, and the holes' terms only add to the fall; R < 1 on
all of (γ₁, ∞), proved: the weight's integral is elementary, and with
|N₀| ≤ 1 on [0, γ₁] (its minimum −1 at r = 2π) alone, R(T) ≤ R̄(x) :=
(8/7)[1 − (x − 1)^{5/2}/(x^{3/2}(x + 2))] with x = T²/γ₁², a bound
strictly decreasing in x (the fraction's log-derivative is (9x/2 +
3)/(x(x − 1)(x + 2)) > 0) and equal to 1 at x* = 2.274 (T = 1.508γ₁),
which decides T > 1.508γ₁; below it, split the integral at 0.85γ₁, where
|N₀| ≤ |N₀(0.85γ₁)| = 0.673 on [0.85γ₁, γ₁] (N₀ negative and increasing
there, 0.85γ₁ > 2π): R(T) ≤ R₂(x) := 0.327A(x) + 0.673R̄(x) with A(x) =
(8/7)((x − 1)/(x − s))^{5/2}[x + 2s − (x − s)^{5/2}x^{−3/2}]/(x + 2), s
= 0.85², the weight's integral cut at 0.85γ₁ over the constant's fall,
and R₂, an elementary function, is enclosed in interval arithmetic on
[1, 2.28] in 4000 subintervals (gated): R₂ is at most 0.86 on [1, 2.28],
while R̄(2.28) = 0.9991 < 1 covers the rest; the computed R itself, a
cross-check (gated), rises from 0.49 at the cusp to at most 0.69, at T =
1.39γ₁, and falls to 0.68 at 1.508γ₁ (at the cusp R → 4|N₀(γ₁)|/(7/2) =
0.49). So T·G is strictly decreasing from +∞ at the domain's left
endpoint max(h_max, γ₁), the cusp of the holes' or the constant's
arccosh, to 0 (seen directly too, on a fine grid at the 45 states,
gated), F_k^s′ has one minimum, and where that minimum is negative
F_k^s′ has exactly two zeros, since F_k^s′ → +∞ at both ends of the
domain (as ln T at infinity; at the cusp as 4(T² − h_max²)^{−1/2} when a
hole sits above γ₁, and as (7/2 + 4N₀(γ₁))(T² − γ₁²)^{−1/2} with 7/2 +
4N₀(γ₁) = 1.80 > 0 when the cusp is γ₁): a maximum just above that cusp
and the stationary minimum T^s. Whether T^s is also the least value of
F_k^s on its domain is a separate question — the endpoint's value
F_k^s(cusp) is finite and can lie below F_k^s(T^s) for some hole sets —
answered by computation: at the 45 states the maximum just above the
cusp lies at least 16.5 nats and the endpoint F_k^s(cusp) at least 15.9
nats above F_k^s(T^s), so T^s is the least value of F_k^s on its domain.
Here F_k^s′ = ln(T/2T₀) + (4m + 7/2)/T + B(T)/T with B the bracket
above; without the bracket the minimum would be ln((4m + 7/2)/2T₀) + 1,
negative exactly when 4m + 7/2 < 2T₀/e (the condition of 1by(iii)
...
the smooth count puts the ground state's wall below two horizons by
7/(4T₀). The minimum's value is F_k^s(T^s) = −2T₀ + 4Σ_h ln(4T₀/h) +
(7/2) ln(4T₀/γ₁) − 4∫₀^{γ₁} N₀/r dr − (4m + 7/2)²/(4T₀) + O((4m +
7/2)³/T₀²) + O((Σ_h h² + γ₁²)/T₀²), the piece below γ₁ being (2/π)γ₁(2 −
ln(γ₁/2π)) = 10.70 (1bs's second term of C_ζ). At the cells (computed,
gated): T^s/T_u averages 1.003 with rms deviation from 1 of 0.019 over
[0.969, 1.063] at the 40 rungs (1.001 with 0.015 at the 30 with three or
more holes), against 1.028 with 0.039 for 1by’s law without the
constant; the closed form T ln(2T₀/T) = 4m + 7/2 reproduces T^s within
0.14% rms, and T^s/T_w averages 0.962 over [0.875, 1.034]; the ground
...
within 1.23 nats of F_k^s(T^s). *(iii) The leftover oscillation (proved;
computed).* Let Osc_∞ := ∫_{γ₁}^∞ S(r)·4/r dr (convergent: 1bs's third
term of C_ζ, 0.19) and S₁(r) := ∫_{γ₁}^r S. Then for every Δ ∈ (0, T −
γ₁), **|Osc(T) − Osc_∞| ≤ 4 sup_{[T−Δ,T]}|S|·[arccosh(T/(T−Δ)) −
ln(T/(T−Δ))] + 8 sup_{[γ₁,T]}|S₁|·D_T(T−Δ) + 4|S₁(T)|/T + 4∫_T^∞ |S₁(r)|
r⁻² dr**, D_T(r) := (1/r)[(1 − r²/T²)^{−1/2} − 1] the weight defect.
Proof: Osc(T) − Osc_∞ = 4∫_{γ₁}^T S D_T dr − 4∫_T^∞ S/r dr; the last, by
parts, is 4S₁(T)/T − 4∫_T^∞ S₁/r² dr, at most the third term (which is
at most 8 sup_{[T,∞)}|S₁|/T when that supremum is finite — Littlewood's
bound alone does not make it so); on [T − Δ, T] the first, 4∫S D_T, is
bounded by 4 sup|S| times ∫_{T−Δ}^T D_T = arccosh(T/(T−Δ)) −
ln(T/(T−Δ)); on [γ₁, T − Δ], by parts against S₁ with D_T increasing
(D_T′ ≥ 0), it is at most 8 sup|S₁|·D_T(T−Δ). □ With S(t) = O(ln t) (von
Mangoldt) and Littlewood's |S₁(t)| ≤ C ln t (the classical inputs of
1bs) and Δ fixed, arccosh(T/(T−Δ)) − ln(T/(T−Δ)) = √(2Δ/T)(1 +
O(√(Δ/T))), D_T(T−Δ) = (2ΔT)^{−1/2}(1 + O(√(Δ/T))) and the third term is
at most 4C(2 ln T + 1)/T, so **Osc(T) = Osc_∞ + O(ln T/√T)**: the
leftover oscillation is the count's local fluctuation read through the
Green function's square-root cusp at the wall, with a weight √(2Δ/T) —
...
## 11. Roadmap: routes toward the Riemann Hypothesis

The Riemann-side program of §3 (through Theorem 1bv) stands at the wall it
named: Weil's positivity criterion — the true form Q(g) of Theorem 1bn
non-negative on every window, at every support — is equivalent to the
Riemann Hypothesis, and the program as yet asserts neither that it holds
nor that it fails. What
follows is the forward agenda: every item is a step that could bear on the
wall, stated with what is in hand, what would count as done, and exactly how
it touches the wall. None is a claim. The stopping rule of §10.4 concerns the
...
1. **The caster explained — a prime-side reason for the shape, valid at
   ...
   (computed, gated; Theorems 1bu(iii), 1bv(v)), the product form made exact
   at the cells by Theorem 1bw — the ground state times the polynomials
   orthogonal under the pair weights ĝ₁²C_kC_j, the exterior's correction
   C_k a Gaussian on the interior whose coefficient Δκ_k the curvature sum
   rule fixes — and Δκ_k derived by Theorem 1bx as the balayage of the hole
   pairs onto the complement of a wall (the Green function of 1bm's doubly
   slit plane, Hypothesis B verified at the cells up to a late onset near
   the wall), and the wall derived by Theorem 1by as the minimiser of the
   ...
   and the sharp wall's excess over it (4% on average, growing with the hole
   count) derived by Theorem 1bz as the two walls' leakage budget — the hole
   pairs' constant at the sharp wall exceeds its value at the unlocking
   ...
   read by Theorem 1ca as the explicit formula's — the smooth count's
   integral of the transform squared less the prime shells, the leftover
   after the shells the Stirling remainder — given the transform, which is
   (a) and (b); (d) a zero pair at fixed height costs four prolate orders
   ...
   residual derived (Theorem 1ca reads the ground state's leakage profile
   ...
   at each support (T_D(δ) unbounded, as Hypothesis D's ε(δ) → 0 forces), or
   by item 6's with vanishing tails — Weil positivity, the Riemann
...
2. **The prime-side square root.** The true form is unconditional on the
   ...
   the prime sum of Theorem 1bn — and its positivity on every window is the
   wall. A
   factorisation Q = A*A with A built from the primes alone would be
   positivity itself. In hand: the anatomy of the odd ground state at the cells and its
   Ξ-limit, and the even sector's Ξ-limit by the same rule (Theorem 1bv(vi))
   — at the limit the even sector's pole term carries the positivity and
...
3. **The certified frontier.** Weil positivity is certified unconditionally,
   in interval arithmetic, in both sectors on [0, 1.3828125] (Theorem 1bl) and,
   in the odd sector, on the one-prime window beyond log 2 and the
   two-prime window beyond log 3 (Theorems 1bj, 1bk); at the seven cells to δ = 3.5 the true form's ground state is
   certified from above only (Theorem 1bn). The spectrum ladder (Theorem 1br)
   and its caster (Theorem 1bu) say what the ground state looks like at every
   cell — the prolate shadow, the transform dodging every zero below the
   edge — so a certifier knows where to look. Done when: a certified lower
   bound λ₁(δ) > 0 at the next cell by the mechanism of Theorem 1bl (Slepian
   concentration and a verified head matrix) or by item 2. Bears on the wall
   directly: positivity at support δ is the wall at that support.

4. **The slack law's reduction.** The finite-δ formula of Theorem 1bm(v)
   reproduces the zero side's ground state at every cell and its reduction
   (1bm(vi)'s five lemmas) is conjectured; the count constant (Theorem 1bs)
   and the pair-cost lemma (Theorem 1bu(i)) are proved pieces of it. A proof
   of the reduction gives the law of λ₁(δ) from the zeros. Bears on the wall
   through its mirror: the same asymptotic derived from the prime side, with
   no zero entering, would be positivity at every support with the rate
...
5. **Hypothesis D from the prime side.** The ground state's transform
   ...
   and (b) of item 6, D being one witness for (a′).
...
6. **The real-rooted ladder and Hurwitz's theorem.** In the cosine basis the
   ...
   locally uniformly, and (b) the transform real-rooted at every support
   beyond some δ — in the cosine basis, M's roots real and positive — put
   Ξ's zeros on the real line, for any family of probes, the variational
   structure entering only through (a′) and (b) themselves; D at every
   support with vanishing tails is one sufficient condition for (a′), the
   zero-dependent one — Theorem 1bu(ii) verifies D at the seven cells within
   the dodging tolerance, the rate of its tails not proved. (The
   ...
   and so records that no zero of ζ enters any hypothesis. Its later rounds
   take (b) out of that hypothesis: `theoremD` proves, at every support and
   from the form alone, that the ground space — the probes on which the
   Rayleigh quotient of Q attains its minimum, whatever its multiplicity —
   is finite-dimensional and spanned by the chain w, Gw, …, G^{m−1}w of one
   element under the Green operator G of ∂² − ¼, and `topGS_cross` that the
   chain's top element has a transform vanishing only on ℝ ∪ iℝ, with no
   uniqueness or shape input; Hurwitz's theorem for closed sets and ζ ≠ 0 on
   (0, 1) are proved from Mathlib rather than assumed, so
   `rh_of_hypConv_top` derives the Riemann Hypothesis from (a′) alone,
   stated for that one explicit family of ground states along any sequence
   of supports. `rh_of_hypConvStrip_top` weakens (a′) further, to
   convergence on the strip |Im z| < ½ alone, and `rh_of_close_top` reaches
   that from an L² statement: the top ground state, up to a nonzero scalar,
   within o(e^{−ba}/√a) of a kernel family whose transforms converge on the
   strip to a nonzero multiple of Ξ, for every b < ½ — Riemann's Φ itself is
   such a family by his integral formula for Ξ, which the pilot proves from
   Mathlib's definitions on all of ℂ, so that `rh_of_close_RPhi` derives the
   Riemann Hypothesis from that closeness to Φ alone, along supports tending
   to infinity. The pilot reports, from its own scripts, the ground state's
   angle to the truncated Φ falling at a rate well beyond the threshold over
   the supports it measured, the asymptotic rate inferred and not certified,
   and judges the closeness bound presumably RH-strength: its chain is a
   reduction of the hypothesis to one rate — presumably a reformulation, not
   a proof — and this paper claims nothing from it.) Both (a′)'s witness D
   and (b) are computed at the cells and neither is proved; each is a
   statement about a prime-side variational problem. Done when: M's roots
   proved real and positive at one support beyond the cells from the form's
   structure; the family of M's as a Laguerre–Pólya approximation to Ξ
   understood. Ruled out for (b) at the cells: concavity, Pólya's shape
   ...
   positive for small real z (asymptotic to z∫₀^a g as z → 0) and at most 0
   at z = 2π/a, where sin 2π = 0 and ∫₀^a h(t) sin(2πt/a) dt = ∫₀^{a/2}
   [h(t) − h(a − t)] sin(2πt/a) dt ≤ 0. At every cell the first zero of ĝ₁
   is γ₁ within the dodging tolerance (Theorem 1bu(ii)) and 2π/a = 4π/δ < γ₁
   once δ > 4π/γ₁ = 0.889 (every cell has δ ≥ 1), so the ground state is
   concave at no cell (nonnegativity is not used: h ≥ 0 follows from
   evenness and concavity, and positivity at small z needs only ĝ₁(0) > 0,
   which fixes the ground state's sign): whatever carries (b) there is not
   concavity. Shape routes that survive the argument exist — the
   Laguerre–Pólya closure, products of real-rooted transforms, that is
   convolutions of concave pieces, whose first zero can sit above γ₁ (a tent
   on [−0.3, 0.3] convolved with a box on [−0.2, 0.2], at δ = 1, is
   real-rooted and not concave, with first zero 5π = 15.71) — and so do
   routes tied to the form: the Gram pencil's positivity or interlacing
   across δ, or a variational argument that a non-real zero raises Q. (The
   pilot proves Pólya's theorem for concave probes with no representation
   input, `realRooted_of_concaveOn`, and reports from its own scripts, not
   this paper's: the ground state in the paper's Gram concave only at small
   ...
   support it tested, the zeros it tracked of the ground state's transform,
   below a fixed height, descending monotonically in δ with the transform
   and its δ-derivative interlacing there, the pinned zeros resting on the
   zeta zeros from above — an observed structure whose proof would need the
   sign of the minimiser's δ-derivative at its zeros, not yet a mechanism.)
   Both hypotheses at every support are the wall; by the pilot's chain, (a′)
   alone, for its top ground state, is already the wall.
...
7. **The sector tower.** Theorem 1bv(ii): an odd probe's transform is irĜ,
   ...
   side; a probe whose transform vanishes to order m at the origin gives the multiplier
   γ^{2m} and m boundary conditions. The two sectors already fill every even
   prolate order from 4 on (Theorem 1bv(iii)); the tower's higher members
   would each carry the ground state's Ξ-limit with the weight t^{2m}Ξ² and
   an anatomy whose positivity is carried by different terms — the odd
   sector's by the primes. Done when: the m = 2 form's Gram, ladder and
   anatomy computed and the pattern of carriers across m stated. Bears on the
   wall: positivity at each m is equivalent to the wall, and the member whose
   positivity is carried by the primes alone is the natural target for a
   prime-side proof.
