...
orthonormal Legendre basis of the even sector truncated at 2(2aT₀/π +
120) modes, T₀ = 2πeᵟ the dodging horizon of Theorems 1az–1bb, every
...
**the exponent is twice the horizon** in the limit — −ln λ₁/T₀ = 1.842
at δ = 3.5, against the limit 2 of (iv). Beyond the cells, the
instrument gives −500.529 at δ = 3.75 and −651.008 at δ = 4.0 on the
2000-zero list (f = 11.771 and 11.924; −ln λ₁/T₀ = 1.874 and 1.896; the
...
1.38, 2, 2.3, 3 (`constrained_prolate.py`, A439), dodging exactly the
...
on the exact ground states (A442). *(iv) The theorem (the reduced
...
zero distribution — real or complex, signed or not — by the maximum
principle for the difference potential in Ω, and the balayage is
admissible (a genuine zero excess, density 0.75 per unit length at the
edge, positive on the whole exterior) for X ≤ 2.05, so the maximiser is
admissible; the exterior potential is exactly −2π and the interior
potential falls monotonically from 0 to −2π (gated live). So, *within
...
cells, O(δ)), and the cells exceed it by c(δ) = 4.69, 5.15, 5.90,
6.31, 6.53, 7.20, 7.87 nats — an O(δ) offset (the norm's width, the
...
proved: the closed form of the reduced problem (iv). What is computed:
...
`prolate_rate.py`, `prolate_wkb.py`, `constrained_prolate.py`,
`discrete_balayage.py` and the `slack_law/` instruments — the
