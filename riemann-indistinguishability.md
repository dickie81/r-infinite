...
40 rungs, against −1.09 ± 1.72 with T_u in place of T_w. That whole's
exponential is the rung-leakage-weighted mean of its two parts' (the
mediant of the two window ratios), so it lies between them at every
...
ĝ(γ)²/A(γ)² = sin²πd the phase), is not lifted uniformly: in the
leakage’s units, the lift squared against the constant squared, the
constant less the envelope lift over the ground state’s runs +0.34 ±
...
and not noise; the wall it names is T_w to 1.5% on average and 4% at
worst. *(ii) The leakage budget (an identity).* Read the rung's exponent
twice. Theorem 1by reads it at the unlocking height: ln λ_k − ln λ₁ = 2
