...
(398 of 399, the last placed by the sum rule at |τ| = 1470) and every
rung at δ = 3.5 (538 of 539, the last placed by the sum rule at |τ|
between 8688 and 14501) — 1bu(ii)’s branch; the sum rule Σ_designed τ⁻²
+ (a/π)²Σ_{j≥K} j⁻² = κ_k := −ĝ_k″(0)/(2ĝ_k(0)) closes within 10⁻¹³
wherever every pair is located (Richardson about the exact ĝ_k(0) =
2av₀, as in 1bu). Rung k has its own dodging edge T_k — the first zeta
zero with no zero of ĝ_k within 0.2 — inside the ground state's and
moving inward with k (the rungs’ edges 121.4, 111.9, 111.0, 105.4,
103.7, 94.7, 87.4 for rungs 1–7 at δ = 2.3); below it one dodging zero
per zeta zero, shared with the ground state's to 10⁻¹³ at rungs 2–5 and
to 10⁻⁶ at rung 8 (δ = 2.3), and the k − 1 hole zeros of 1bu(iii);
beyond it the exterior: n_dodge + (k − 1) + n_ext = K − 1 at every rung
(at δ = 2.3 38 + 0 + 221; 34 + 1 + 224; 33 + 2 + 224; 31 + 3 + 225 for
rungs 1–4). Beyond the safely deep rungs the census meets complex pairs:
at δ = 2.3 rungs 9 and 10 (ln λ = −5.14, −0.67) have 2 and 8 complex
pairs. *(ii) The factorisation and the curvature law (the identities
proved; computed, gated).* Write ĝ_k = ĝ₁·H_k·C_k with H_k = Π_holes(1 −
r²/h²), of degree 2(k − 1), and C_k the ratio of the remaining zero
products of rung k and the ground state (the sinc zeros cancel): exact
once every pair is located. Then ln C_k(r) = −Δκ_k r² + O(r⁴) with Δκ_k
= κ_k − κ₁ − Σ_h h⁻² — the r² coefficient of ln Π(1 − r²/τ²) is −Στ⁻²,
so the coefficient is the sum rules' — and, by the census, Δκ_k =
[Σ_{E_k}τ⁻² − Σ_{E_1}τ⁻²] − Σ_freed γ⁻² over the zeta zeros the ground
state dodges and rung k does not, up to the dodging displacements (the
accounting residual within 2.7 × 10⁻⁶ everywhere): the k − 1 hole pairs
are paid for by the exterior, Δκ_k < 0 and falling with k (−Δκ_k = 3.49
× 10⁻⁵, 7.87 × 10⁻⁵, 1.21 × 10⁻⁴, 1.70 × 10⁻⁴, 2.20 × 10⁻⁴, 2.87 × 10⁻⁴
for rungs 2–7 at δ = 2.3), the effective height of the exterior pair a
hole replaces τ_eff := √((k − 1)/(−Δκ_k)) between 1.0 and 1.6 T_D at
every rung in the law (τ_eff = 1.40, 1.31, 1.30, 1.26, 1.24, 1.19 T_D at
δ = 2.3). The quadratic law holds across the interior: the ratio (−ln
...
T_D/2] (the interior carrying 1.000000, 1.000000, 1.000000, 1.000000,
...
and R within 1.2 × 10⁻³ at every rung in the law at every cell (rungs in
the law 4, 7, 11, 12, 12). *(iv) What it says.* The roadmap's (c) — the
excited states as the ground state times the orthogonal polynomials of
its power spectrum, which 1bu(vii) declined to claim exact — is exact at
the cells in this form: the polynomials are orthogonal under the pair
weights ĝ₁²C_kC_j, not under ĝ₁², and the exterior's correction is, on
the interior, the Gaussian e^{−Δκ_k r²} whose coefficient the curvature
sum rule fixes. The correction vanishes toward the Ξ-limit — −Δκ₂ = 6.52
× 10⁻⁵, 3.49 × 10⁻⁵, 1.80 × 10⁻⁵, 8.16 × 10⁻⁶, 3.20 × 10⁻⁶ at the five
cells, falling — where the orthogonal polynomials of Ξ² are the law
(1bu(iii)); at finite δ the naive nodes miss by the correction's size.
Δκ_k itself, the exterior's re-balayage in one number per rung, is
computed, not derived: that is what remains of (c). *(v) What is proved,
computed, not claimed.* Proved: the factorisation's identity and the r²
coefficient (elementary). Computed, gated: the census and its
accounting, the edges, the shared dodging zeros, the curvature law's
residuals, the bi-orthogonal law's residuals against the naive nodes and
the mangle, τ_eff, the fall of Δκ₂ across the cells, the shallow rungs'
complex pairs. Not claimed: a derivation of Δκ_k from the prime side or
from the balayage; the O(r⁴) term's law; the law beyond the safely deep
rungs (the shallow rungs lose real-rootedness); the odd sector's
anatomy; anything about the zeros — **no Riemann Hypothesis consequence
is claimed**. Check 7 clean (Plancherel, orthogonal polynomials,
