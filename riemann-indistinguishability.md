...
40 rungs, against −1.09 ± 1.72 with T_u in place of T_w. That whole
reading's exponential is the rung-leakage-weighted mean of its two
parts' (the mediant of the two window ratios of the ground state's share
to the rung's), so it lies between them at every rung; the parts differ:
the near quarter [T₁, 1.25T₁) alone reads +0.39 ± 0.47 nats, a wall 2.8%
below T_w on average (rms deviation from T_w 4.7%), and the field beyond
−0.20 ± 0.25, a wall 1.2% above (rms from T_w 1.5%), the near reading
the higher at 39 of the 40 rungs. The envelope itself, read at a zeta
zero γ by A(γ)² := ĝ(γ)² + ĝ(γ̄)², γ̄ the midpoint to the next zero (a
zero of ĝ displaced by d spacings from γ gives ĝ(γ) = A sin πd and ĝ(γ̄)
= A cos πd, so A is the envelope and ĝ(γ)²/A(γ)² = sin²πd the phase), is
not lifted uniformly: in the leakage’s units, the constant squared less
the envelope lift squared net of the origin ratio squared (V’s
convention, 1bx(ii)) runs +0.34 ± 0.48 nats on [T₁, 1.25T₁), −0.16 ±
0.32 on [1.25T₁, 1.5T₁) and −0.18 ± 0.26 on [1.5T₁, 2T₁) (means and rms
over the 40 rungs) — the envelope rises through the first quarter above
T₁, 1bx's onset lag seen in the envelope rather than the counts, and
overshoots the constant by a sixth of a nat in those units beyond; the
integrated reading above is what B asks of the exterior and what the
identity of (ii) uses. The far-field excess is read at infinity too. V
:= ln|ĝ_k/ĝ₁| − ln|ĝ_k(0)/ĝ₁(0)| has, by Hadamard's product over the
census sets, V(∞) = 2[Σ_{O₁} ln τ − Σ_{O_k} ln τ − Σ_h ln h] exactly
...
measure's profile and not noise; the wall it names is T_w to 1.5% on
average and 4% at worst. *(ii) The leakage budget (an identity).* Read
the rung's exponent twice. Theorem 1by reads it at the unlocking height:
ln λ_k − ln λ₁ = 2 ln|ĝ_k(0)/ĝ₁(0)| + F₁(T_u) + cost(T_u) − F₁(T_u(1)) −
r₁, with cost(T) := 4Σ_h ln(1/σ_h(T)) the hole pairs' constant and r₁
1by's residual (the formula less the value). Hypothesis B reads the
exterior leakage at the sharp wall: λ_k s_ext =
