...
the wall). Then V = −Σ_h[G(·, h) + G(·, −h)], and with σ_h := ψ(h) ∈ (0,
1): ln C_k(R) = Σ_h[ln|1 − ψ(R)²/σ_h²| − ln(1 − σ_h²ψ(R)²) − ln|1 −
R²/h²|] on the interior, normalised C_k(0) = 1; expanding at R = 0
(ψ(R)² = R²/(4T²) + O(R⁴)) and using 1/σ² − σ² = (1 + σ²)(1 − σ²)/σ²
with h² = 4T²σ²/(1 + σ²)²,  **Δκ_k = −Σ_h (1 + σ_h²)/(2T²)**;  at h → 0
(σ_h → h/2T) the pair costs exactly 1/(2T²), the interior correction is
ln C_k(R) = −2(k − 1) ln[(1 + √(1 − R²/T²))/2] = (k − 1)[R²/(2T²) +
...
else follows without further input and is checked: the exact ln C_k from
the census products (rung against ground state, at midpoints between
zeta zeros) matches the closed form across the interior — the exact ln
...
[1.5, 6]T₁ fitted by −(2/π)arccos(T_w′/R) returns the same wall — the
profile’s wall within 5.0% of T_w at every safely deep rung with three
or more holes and within 1.8% at δ ≥ 3; 1bw's interior ratio at r = 40
is the closed form's — the ratio at r = 40 within 0.0131 of the closed
form at every safely deep rung, its excess over 1 reaching 0.125, and
within 0.0005 at δ ≥ 3; the deficit already present at T₁ is the
profile's — the deficit at T₁ within 0.19 per hole of
−(2/π)arccos(T_w/T₁) at every safely deep rung, 0.06 on average. Rung
2's wall is the ground state's edge: T_w/T₁ = 1.002, 0.987, 0.998,
...
exactly. The wall moves inward with k: T_w/T₁ between 0.833 and 1.002
over the 40 safely deep rungs of the five cells, T_w/T_k between 0.980
and 1.245, and T_w/√(T₁T_k) between 0.966 and 1.019 at the 35 rungs with
k ≥ 3 — between the rung's own edge T_k and T₁, near their geometric
mean for k ≥ 3, neither edge itself (T_w reaches 1.245 T_k, and falls to
0.833 T₁). *(iv) What it says.* The roadmap's (c) asked for Δκ_k
...
The one input is the wall T_w(k): for rung 2 it is T₁ (1bm's X* = 2),
for the higher rungs it lies between T_k and T₁ because the zeta zeros
between them are unlocked by the rung but still half-held — a soft wall
whose position the census fixes and this block does not derive. What
remains of (c) is the wall's law: the rung's leakage budget (its
...
profile and the r⁴ coefficient. Computed, gated: Hypothesis B at every
safely deep rung of the five cells with one wall per rung (the interior
profile at three heights, the deficit profile's wall, the r = 40 ratio,
the pre-edge deficit); rung 2's wall at T₁ and T₁ at two horizons; the
walls' brackets. Not claimed: the wall's law; Hypothesis B beyond the
...
verifier computes from Theorem 1bw's census and laws checkpoints and
Theorem 1bu's zero list, all reused at their keys.
...
1. **The caster explained — a prime-side reason for the shape, valid at
   ...
   slit plane, Hypothesis B verified at the cells), with the wall T_w(k)
   ...
   and (c)'s wall T_w(k) derived (Theorem 1bx's one number per rung: the
   rung's leakage budget against the cost of unlocking the zeta zeros
   between T_w and T₁) — a finite advance of the wall by T_D(δ). Bears on
   the wall: at every support this is the proof itself — (a) and (b)
   everywhere put Ξ's zeros on the line by item 6's step — Weil positivity,
