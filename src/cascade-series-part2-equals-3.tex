...
Paper~II~\cite{part2} derives quantum mechanics as the measurement theory of
a 4D observer in the cascade geometry. Paper~III~\cite{part3} derives general
...
\section{The Schism and Its Dissolution}\label{sec:the-schism-and-its-dissolution}
...
\section{Single-Source Structure}\label{sec:single-source-structure}
...
(Paper~II, Theorem~\xref{part2}{thm:propagator}), and
\item the source of Lorentzian signature: oscillatory character requires
$g_{tt} < 0$ (Paper~III, Theorem~\xref{part3}{thm:lorentzian}).
\end{enumerate}
\end{theorem}
...
\emph{mathematically imposed} (Paper~III~\cite{part3},
...
(Paper~III~\cite{part3}, Lemma~\xref{part3}{lem:cascade-spinor-id}), the
...
(Paper~III~\cite{part3}, \S\,Clifford classification); the
...
$d_f$ (Paper~IVb~\cite{part4b}, Theorem~\xref{part4b}{thm:obstruction});
the charge $e = \sqrt{\alpha(14)}$ from the cascade U(1) gauge
structure at the gauge-window boundary $d_g = 14$
(Paper~IVa~\cite{part4a}); and the gauge field $A_\mu$ from the
...
By Paper~III's Lemma~\xref{part3}{lem:cascade-spinor-id} together with
Theorem~\xref{part3}{thm:d4}, the cascade Hilbert space $\mathcal{H}$ at
...
Paper~IVb's Theorem~\xref{part4b}{thm:obstruction} and the Berezin partition
...
height $x \in [-1,1]$ per Paper~III~\cite{part3}~\S4.1; spinor
representation of $\mathrm{Spin}(1,3)$ on the cascade Hilbert space
per Lemma~\xref{part3}{lem:cascade-spinor-id}), the cascade fermion field
is a section
\[
...
By Lemma~\xref{part3}{lem:cascade-spinor-id}, the cascade Hilbert space
...
(Paper~IVb~\cite{part4b},
Remark~\xref{part4b}{rem:berezin-partition-derivation}) equals the
...
(Paper~IVb~\cite{part4b}, Section~\xref{part4b}{sec:fermion-action}, with
...
(Paper~IVb~\cite{part4b},
Theorem~\xref{part4b}{thm:obstruction}) and the Hamiltonian-form Dirac
...
(Lemma~\xref{part3}{lem:cascade-spinor-id}), the spatial derivatives
...
Lemma~\xref{part3}{lem:cascade-spinor-id}). This is the same posture
as Schur's lemma in Paper~III's
Lemma~\xref{part3}{lem:cascade-spinor-id}, Gleason's theorem in
...
of dimensions above the observer (Paper~VI~\cite{part6},
Definition~\xref{part6}{def:tower-time}).
...
(Paper~IVb~\cite{part4b}, Remark~\xref{part4b}{rem:action-uniqueness}),
...
lattice covariance (Paper~IVb~\cite{part4b},
Remark~\xref{part4b}{rem:marginal-greens}).
...
(Paper~VI~\cite{part6}, \S1.2 within-moment vs
...
$A$ (Paper~VI~\cite{part6},
Proposition~\xref{part6}{prop:time-evolution}) governs the genuine
...
tower-action is derived in Paper~IVb~\cite{part4b}'s
Remark~\xref{part4b}{rem:action-uniqueness} from lattice covariance;
...
defined in Paper~VI~\cite{part6}.
\end{remark}
...
Lemma~\xref{part3}{lem:cascade-spinor-id}, and the same posture
...
\section{Double Uniqueness}\label{sec:double-uniqueness}
...
cascade forces $d = 4$ (Paper~III, Section~\xref{part3}{sec:d-equals-4}) precisely where both
...
(Lorentzian cross-section radius, Paper~III, Theorem~\xref{part3}{rem:sp-19-inputs}).
...
\section{Bekenstein--Hawking Entropy Is Hidden Geometry}\label{sec:bekenstein-hawking-entropy-is-hidden-geo}
...
(Paper~0, Theorem~\xref{part0}{thm:boundary}~\cite{part0}, \xref{part0}{thm:boundary}):
$\Omega_{d-1}/V_d = d$, hence $V_d = \Omega_{d-1}/d$.

\emph{Step 2 (derived, cascade-content primitive).} By Paper~0, Corollary~\xref{part0}{cor:primary}, sphere area is the unique independent
...
$V_d = \Omega_{d-1}/d$ (Paper~0, Theorem~\xref{part0}{thm:boundary}~\cite{part0}). A Gamma-function
identity; no assumption.
\item \emph{Derived as a cascade primitive.} Content~$\equiv$~area and
volumes are derived: Paper~0, Corollary~\xref{part0}{cor:primary} (\xref{part0}{cor:primary}).
...
Corollary~3.2 of Paper~0 (\xref{part0}{cor:primary}): sphere area is the only
...
hierarchy (Paper~I, Theorem~\xref{part0}{thm:hierarchy}), seen from the other side: $\Lambda \sim
...
which is any $d\geq 1$ (Paper~0, Theorem~\xref{part0}{thm:boundary}). It is not restricted to the
...
structural commitments (Paper~I~\cite{part1}~\S3.2;
Paper~II\,=\,III~\S5--6; Paper~III~\cite{part3}~\S12) explicitly
...
discrete cascade action (Paper~IVb~\cite{part4b}, Remark~4.6) evaluated
...
\section{Resolution of the Standard Conflicts}\label{sec:resolution-of-the-standard-conflicts}
...
Mutual consistency & Gleason $+$ Lovelock: double uniqueness & 5 \\
No absolute scale & All predictions are ratios & 6 \\
UV dissolution & Finite tower; projection artefact & 6 \\
Gravity already quantum & Metric is state property on $S^{d-1}$ & 7 \\
$S = A/4$ & Boundary dominance at $d = 4$ & 8 \\
$T = 1/(8\pi M)$ & $S(M) + $ Birkhoff; no semiclassics & 8.5 \\
Hidden geometry $\sim 10^{120}$ & Cascade hierarchy $/\, 4$ & 8.6 \\
\hline
\end{tabular}
\end{center}

\noindent(The table's pointers were re-synchronised with the current
sectioning by content-matching each row to its section: the section
digit had drifted by one for every row from ``Mutual consistency''
onward, and the two subsection pointers additionally by one sub-index
--- external review rounds 73--74. An earlier version of this note
attributed the drift to the Dirac Descent section's insertion; the
repository history cannot support that causal claim, which is
withdrawn --- the re-synchronisation stands on the content-match
alone.)
...
Kaluza--Klein): the discrete cascade action of Paper~IVb~\cite{part4b}
...
Corollary~\xref{part0}{cor:primary}. The remaining step---that $S = A/d$ holds for any cascade
...
(Paper~II, Lemma~\xref{part2}{lem:gaussian}); (ii)~the decoherence factor
$D = \exp(-D x^2/4R_{\rm eff}^2)$ is Gaussian (Paper~II, Theorem~\xref{part2}{thm:reff}); (iii)~by the Araki--Woods theorem, Gaussian
...
Paper~I~\S3.2~\cite{part1}, Paper~II\,=\,III~\S5--6,
Paper~III~\S12~\cite{part3}). Candidate cascade-native routes: the
proposed discrete cascade action (Paper~IVb~\cite{part4b} Remark~4.6)
...
graviton particles (Paper~III, Section~\xref{part3}{sec:no-gravitons}). Computing the effective
...
\bibitem{part0} RTAC, \extlink{\cascadebase/cascade-series-part0.pdf}{\textit{The Cascade Series~--- Part~0: The
Gamma-Function Structure of the Cascade}}, March 2026.

\bibitem{part1} RTAC, \extlink{\cascadebase/cascade-series-part1.pdf}{\textit{The Cascade Series~--- Part~I: Scale Variance
from Orthogonality}}, March 2026.

\bibitem{part2} RTAC, \extlink{\cascadebase/cascade-series-part2.pdf}{\textit{The Cascade Series~--- Part~II: Quantum
Mechanics from the Cascade}}, March 2026.

\bibitem{part3} RTAC, \extlink{\cascadebase/cascade-series-part3.pdf}{\textit{The Cascade Series~--- Part~III: General
Relativity from the Cascade}}, March 2026.

\bibitem{part4a} RTAC, \extlink{\cascadebase/cascade-series-part4a.pdf}{\textit{The Cascade Series~--- Part~IVa:
Standard Model from the Cascade --- Gauge Group, Symmetry Breaking,
and Three Generations}}, March 2026.

\bibitem{part4b} RTAC, \extlink{\cascadebase/cascade-series-part4b.pdf}{\textit{The Cascade Series~--- Part~IVb:
Masses, Couplings, and Precision Predictions from the
Geometric-Topological Factorization}}, March 2026.

\bibitem{part6} RTAC, \extlink{\cascadebase/cascade-series-part6.pdf}{\textit{The Cascade Series~--- Part~VI:
