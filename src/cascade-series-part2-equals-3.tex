...
Definition~\texttt{def:tower-time}).
...
(Paper~IVb~\cite{paper4b}, Remark~\texttt{rem:action-uniqueness}),
...
Remark~\texttt{rem:marginal-greens}).
...
\begin{remark}[Closes Open Question~\ref{oq:tower-vs-4d-action}]
\label{rem:tower-vs-4d-resolved}
Open Question~\ref{oq:tower-vs-4d-action} asked whether the
cascade tower-action and the standard $4$D continuous action are
genuinely complementary (a), unifiable via a foliated total-space
action (b), or unifiable via the Wick rotation reading $\Delta\varphi(d) \leftrightarrow \partial_t\Phi$ (c).
...
Proposition~\texttt{prop:time-evolution}) governs the genuine
...
Remark~\texttt{rem:action-uniqueness} from lattice covariance;
...
Open Question~\ref{oq:tower-vs-4d-action} is closed by
reading~(a) with the structural justification above.
\end{remark}
...
\item\label{oq:tower-vs-4d-action} \textbf{[closed] Tower-action
vs.\ $4$D continuous action: genuinely complementary by design.}
Resolved in Lemma~\ref{lem:within-moment-action-decomposition}
and Remark~\ref{rem:tower-vs-4d-resolved}: reading~(a) genuinely
complementary, with the structural justification that the two
actions act on \emph{different fields} (cascade tower-scalar
$\varphi(d) = \ln\Omega_d$ on the layer index vs.\ $4$D-projected
$\Phi(x^\mu)$ on $M_4$) and govern \emph{different cascade-geometric
directions} (cross-layer vs.\ within-layer). Readings~(b) and (c)
were structurally incorrect: (b) requires a single field on which
both actions act, which does not exist; (c) confused the layer
index $d$ (cascade tower-depth) with a Wick-rotation of $4$D time
($4$D time arises from Wick-rotating the within-layer foliation
parameter $x \in [-1,1]$ per Paper~III~\S10, not from the layer
index). Paper~VI~\cite{paper6}'s within-moment vs.\ between-moments
factorization provides the structural framework: within-moment
cross-layer kinetics (tower-action) and within-moment within-layer
kinetics ($4$D action) are two distinct directions of the
within-moment cascade geometry, with between-moments dynamics
governed separately by Paper~VI's tower-growth operator $A$.
...
\bibitem{paper6} RTAC, \textit{The Cascade Series~--- Part~VI:
