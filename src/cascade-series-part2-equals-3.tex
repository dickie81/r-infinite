...
\item \emph{Step~3's covariance argument} is the standard Wigner
classification of relativistic wave equations on a complex spinor
representation. It is mathematically rigorous as standard physics
but is invoked rather than re-derived in cascade-lattice terms. A
cascade-native version would derive the first-order spatial
operator structure from the cascade lattice's discrete adjacency,
parallel to how Paper~III~\S4~\cite{paper3} derives the $4$D
continuous metric from the cascade slicing recurrence.
\item \emph{Step~4's mass identification.} The Berezin-mass
operator of Paper~IVb~\cite{paper4b}
(off-diagonal on $S^+\oplus S^-$) is identified with the
Hamiltonian-form $\beta$-mass; a small lemma making this
identification explicit in cascade-lattice terms would close a
one-paragraph gap.
