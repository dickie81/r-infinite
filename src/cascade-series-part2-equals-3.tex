...
\texttt{tools/btz\_cross\_check.py}. The substitution
...
The cross-check does not constitute an independent derivation of $G_3$
from the cascade's KK reduction---that remains an open question. What it
establishes is that boundary dominance is consistent with known 3D BH
physics, and that the $S = A/d$ formula is not an accident of the
observer's dimension $d=4$. Together with Theorem~\ref{thm:hawking}
...
\item Derive $G_3$ \emph{independently} from the cascade's KK reduction
to verify $G_3 = 3/4$ from first principles (Theorem~\ref{thm:Gd} gives
the matching condition; an independent derivation of $G_3$ from the
cascade's layer structure would be a stronger result).
...
\begin{enumerate}
\item \textbf{Hawking radiation spectrum.} Theorem~\ref{thm:hawking} derives
the temperature $T = 1/(8\pi M)$ from boundary dominance and Birkhoff's
theorem, with no semiclassical input. The thermal \emph{spectrum}
(Planckian with greybody factors) and the radiation \emph{mechanism}
(geometric content leaking through asymptotic compactification) remain
open: a geometric derivation of the spectral shape would complete the
connection.
\item \textbf{Independent derivation of $G_3$.} Theorem~\ref{thm:Gd}
gives $G_3 = 3/4$ from matching $S = A/d$ to standard Bekenstein--Hawking.
Theorem~\ref{thm:btz} verifies that standard BTZ thermodynamics is
self-consistent with this value. What remains open is an
\emph{independent} derivation of $G_3$ from the cascade's KK reduction
or layer-by-layer structure, which would confirm $G_3 = 3/4$ without
using the $S = A/d$ identification. This is the stronger structural
test gestured at in the previous open question.
