...
Paper~III~\S4 derives the FRW metric from the slicing recurrence
as a continuous object; the cascade lattice on the layer index $d$
is parallel to but not identical with the $4$D differential
structure on which $\gamma^\mu\partial_\mu$ acts. The explicit
bridge from ``discrete Laplacian on layer index'' to ``continuous
$\gamma^\mu\partial_\mu$ on $4$D Lorentzian manifold'' is implicit
in Paper~III but not written as a theorem. This is the substantive
residual gap; the other two are presentation gaps.
\end{enumerate}
Closing (c) would promote
Theorem~\ref{thm:cascade-dirac-descent} from
``Wigner classification on cascade-derived inputs'' to ``cascade
descent of fermion fields end-to-end,'' and would tighten the
cascade-native derivation of $g=2$
