...
(Lemma~\texttt{lem:cascade-spinor-id}), the spatial derivatives
...
Lemma~\texttt{lem:cascade-spinor-id}). This is the same posture
as Schur's lemma in Paper~III's
Lemma~\texttt{lem:cascade-spinor-id}, Gleason's theorem in
...
\item \emph{[partial closure]} Step~3's covariance argument is
the standard Wigner classification of relativistic wave equations
on a complex spinor representation, with two distinct contents:
($\alpha$) first-order spatial forced by covariance + first-order
time, and ($\beta$) form of the first-order operator as
$\gamma^\mu\partial_\mu$. Lemma~\ref{lem:4d-fermion-projection}
provides a complementary geometric derivation of ($\beta$) via
the spin-connection lift of the Levi-Civita connection on $M_4$,
using only cascade-derived inputs. The spin-connection route does
not handle ($\alpha$): selecting first-order over second-order
requires the cascade propagator's first-order time form plus
Lorentz covariance on the spinor representation, which is
Wigner's content~($\alpha$). For ($\alpha$), Wigner remains
load-bearing as a standard mathematical theorem applied to
cascade-derived inputs --- same posture as Schur in
Lemma~\texttt{lem:cascade-spinor-id}, Gleason and Lovelock in
Theorem~\ref{thm:double}. See Remark~\ref{rem:a-closed} for the
full status.
