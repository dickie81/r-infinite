...
(Paper~III~\cite{paper3}, Lemma~\texttt{lem:cascade-spinor-id}), the
...
$d_f$ (Paper~IVb~\cite{paper4b}, Theorem~\texttt{thm:obstruction});
...
By Paper~III's Lemma~\texttt{lem:cascade-spinor-id} together with
Theorem~\texttt{thm:d4}, the cascade Hilbert space $\mathcal{H}$ at
...
Paper~IVb's Theorem~\texttt{thm:obstruction} and the Berezin partition
...
per Lemma~\texttt{lem:cascade-spinor-id}), the cascade fermion field
is a section
\[
...
By Lemma~\texttt{lem:cascade-spinor-id}, the cascade Hilbert space
...
\begin{remark}[Closes soft spot (c1) of Open Question~\ref{oq:dirac-discrete-continuous}]
\label{rem:c1-closed}
Lemma~\ref{lem:4d-fermion-projection} establishes the
spinor-bundle structure on the $4$D Lorentzian manifold parallel
to Paper~III~\S4's metric derivation. This closes soft spot (c1)
of the original Open Question: the continuous $4$D structure on
which $\gamma^\mu\partial_\mu$ acts is now derived, not implicit.
The remaining open piece is the relation between the cascade
\emph{tower-action} on the layer index and the $4$D continuous
action on $M_4$ --- two complementary cascade-internal structures
that are not the same object. See Open
Question~\ref{oq:tower-vs-4d-action}.
\end{remark}
...
Remark~\texttt{rem:berezin-partition-derivation}) equals the
...
(Paper~IVb~\cite{paper4b}, Section~\texttt{sec:fermion-action}, with
...
(Paper~IVb~\cite{paper4b},
Theorem~\texttt{thm:obstruction}) and the Hamiltonian-form Dirac
mass agree both in cascade value and in operator action on the
chirality decomposition.
\end{proof}

\begin{remark}[Closes soft spot (b) of Open Question~\ref{oq:dirac-discrete-continuous}]\label{rem:b-closed}
...
chirality basis. Soft spot (b) is closed.
\end{remark}

\begin{remark}[Closes soft spot (a) of Open Question~\ref{oq:dirac-discrete-continuous}]\label{rem:a-closed}
Step~3 of Theorem~\ref{thm:cascade-dirac-descent} invokes Wigner's
classification of relativistic wave equations to pin
$\gamma^\mu\partial_\mu$ as the unique Lorentz-covariant
first-order operator. Soft spot (a) of the original Open Question
asked whether this could be re-derived in cascade-lattice terms.

Lemma~\ref{lem:4d-fermion-projection} provides a complementary
\emph{geometric} derivation of the same operator: the spin-connection
lift of the Levi-Civita connection on $M_4$ gives
$\gamma^\mu\nabla_\mu = \gamma^\mu\partial_\mu$ (in flat-frame
coordinates) as the unique first-order Dirac operator on the
spinor bundle $\mathcal{S}$. The geometric derivation uses only
cascade-derived inputs: the foliated $4$D manifold $M_4$
(Paper~III~\cite{paper3}~\S4) and the spinor representation
identification (Lemma~\texttt{lem:cascade-spinor-id}). It does not
invoke Wigner classification.

With Lemma~\ref{lem:4d-fermion-projection} in place, Step~3's
Wigner appeal is no longer load-bearing for the descent: the
geometric route reaches the same conclusion via spin-connection
geometry on cascade-derived inputs. Soft spot (a) is closed by
the existence of the geometric route, with Wigner's algebraic
route now serving as complementary context. The two routes agree
because they are two presentations of the same standard fact ---
the Dirac operator on a Lorentzian Spin-manifold is unique, by
either Wigner's covariance argument or by the unique torsion-free
metric-compatible spin connection.
\end{remark}
...
Lemma~\texttt{lem:cascade-spinor-id}, and the same posture
Theorem~\ref{thm:double} adopts when invoking Gleason and Lovelock.
Three soft spots are documented in
Open~Question~\ref{oq:dirac-discrete-continuous}.
\end{remark}
...
\begin{enumerate}
\item\label{oq:dirac-discrete-continuous} \textbf{Discrete-to-continuous
bridge for the cascade Dirac descent.}
Theorem~\ref{thm:cascade-dirac-descent} applies Wigner classification
to cascade-derived inputs to force $(i\gamma^\mu D_\mu - m_f)\psi = 0$
at $d=4$, parallel to how Theorem~\ref{thm:double} applies Gleason and
Lovelock. Three soft spots remain.
\begin{enumerate}[label=(\alph*),nosep]
\item \emph{[closed]} Step~3's covariance argument is the standard
Wigner classification of relativistic wave equations on a complex
spinor representation. The Wigner appeal is no longer load-bearing
for the descent: Lemma~\ref{lem:4d-fermion-projection} provides a
complementary geometric derivation of $\gamma^\mu\partial_\mu$
from the spin-connection lift of the Levi-Civita connection on
$M_4$, using only cascade-derived inputs. See
Remark~\ref{rem:a-closed}. The two routes (Wigner-algebraic and
spin-connection-geometric) agree because they present the same
standard mathematical fact: the Dirac operator on a Lorentzian
Spin-manifold is unique.
\item \emph{[closed]} Step~4's mass identification. The
Berezin-mass operator of Paper~IVb~\cite{paper4b} is identified
with the Hamiltonian-form $\beta$-mass in
Lemma~\ref{lem:berezin-equals-beta-mass}: both are
$\gamma^0\cdot m_f$ on the chirality basis. See
Remark~\ref{rem:b-closed}.
\item \emph{The discrete-to-continuous bridge for fermion fields,
revised.} Lemma~\ref{lem:4d-fermion-projection} establishes the
$4$D continuous spinor-bundle structure on which
$\gamma^\mu\partial_\mu$ acts, parallel to Paper~III~\S4's metric
derivation. The original soft spot (c) therefore splits into:
\begin{itemize}[nosep]
\item \emph{(c1) [closed]} The $4$D continuous structure on which
$\gamma^\mu\partial_\mu$ acts is now derived
(Lemma~\ref{lem:4d-fermion-projection}).
\item \emph{(c2) [open, reframed]} The relation between the
cascade tower-action
$\sum_d (2\alpha(d))^{-1}(\Delta\varphi(d))^2$ on the layer
index and the $4$D continuous action $\int |\partial\varphi|^2$
on $M_4$. These describe complementary aspects of cascade
physics (tower-action governs cross-layer descent, used in
chirality factor and per-leg primitive; $4$D action governs
within-layer dynamics on $M_4$). Whether they should be
derivable from a single underlying cascade-action principle, or
whether they are genuinely complementary by design, is a
separate question. See Open
Question~\ref{oq:tower-vs-4d-action}.
\end{itemize}
\end{enumerate}
With (c1) closed, the cascade-native derivation of $g=2$
(Paper~IVb~\cite{paper4b}, Corollary~\texttt{cor:g-equals-two})
rests on stated theorems end-to-end on the spinor-bundle side. The
remaining (a) and (b) are presentation gaps; the residual
substantive question is (c2), tracked separately below.

\item\label{oq:tower-vs-4d-action} \textbf{Tower-action vs.\ $4$D
continuous action: complementary or unifiable?}
The cascade has two action-like structures: the tower-action
$\sum_d (2\alpha(d))^{-1}(\Delta\varphi(d))^2$ on the layer index
(Paper~IVb~\cite{paper4b},
Remark~\texttt{rem:action-uniqueness}) and the standard $4$D
continuous action $\int |\partial\varphi|^2 + \ldots$ on the
foliated manifold $M_4$ (implicit in the standard physics that
$\gamma^\mu\partial_\mu$ generates after
Lemma~\ref{lem:4d-fermion-projection}). These describe different
physics: the tower-action governs cross-layer descent and supplies
the chirality factor and per-leg primitive in
Paper~IVb~\cite{paper4b}'s precision predictions; the $4$D action
governs within-layer dynamics on $M_4$. Three readings:
\begin{enumerate}[label=(\alph*),nosep]
\item \emph{Genuinely complementary}: the cascade has one action
per ``direction'' --- a tower-action across layers and a
continuous-manifold action within a layer --- and they should
not unify, just as a quantum field on a manifold has both a
phase-space symplectic structure and a path-integral measure
that are not the same object.
\item \emph{Unifiable via a higher-dimensional action principle}:
both are projections of a single action on the foliated total
space $B^{d+1}$, with the tower-action being the cross-section
sum and the $4$D action being the within-section integral.
\item \emph{Unifiable via the cascade Wick rotation}: the tower
direction $d$ is Wick-related to the $4$D time direction $t$
via the Paper~III~\S10 rotation, so the tower-action's
$\Delta\varphi(d)$ corresponds to a time-derivative
$\partial_t\varphi$ on $M_4$, with the cascade scalar action's
sum becoming a kinetic term in the $4$D continuous action.
\end{enumerate}
Reading~(c) is the most concrete cascade-native candidate; it
would derive the cascade tower-action's connection to standard
$4$D physics by interpreting $\Delta\varphi(d)$ as the
discrete-time derivative on a Wick-rotated foliation. Closing
this question --- in any of (a), (b), (c) --- would unify the
cascade's two action-like structures. Reading~(a) (genuinely
complementary) is itself a defensible answer if no unification
exists; the question is which of (a), (b), (c) holds.
