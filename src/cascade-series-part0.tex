...
\begin{proof}
\emph{Exhaustiveness of the enumeration.} A scalar $s$ is ``derivable from
$d_0$'' iff it is expressible as an analytic function of (i)~the values of
$p$ and its decomposition at $d_0$, (ii)~the higher derivatives $p^{(n)}(d_0)$,
(iii)~the integer $d_0$ itself, or (iv)~the recurrence's primitive
$\sqrt{\pi}$ (equivalently $c_1 = \tfrac{1}{2}\ln\pi$ in log-space). The
quadruple $((p,q,\text{const.}),\,(p^{(n)}),\,d_0,\,\sqrt{\pi})$ is the
\emph{complete input} available to any cascade-internal construction at
$d_0$: no other cascade object is defined without additional data
(Theorem~\ref{thm:irred} identifies $\sqrt{\pi}$ as the recurrence's unique
primitive; Corollary~\ref{cor:primary} identifies sphere areas as the
unique independent cascade quantities, and their values at~$d_0$ reduce to
(i) and (iv)). These four sources partition the input; the four classes
below exhaust them.
...
(Corollary~2.3 of Paper~IVb). Each is a Class~4 function of $c_1$
(equivalently $\sqrt{\pi}$) and fails the primitivity criterion~(b):
\begin{itemize}
...
(Part~IVb, Corollary~2.3) is a \emph{product} of the primitive
...
recurrence, and each reduces to Class~4 under the exhaustiveness argument
above.
\end{remark}
...
\begin{proof}
\emph{Mechanism enumeration.} A ``distinguished dimension'' is an integer
$d$ selected by a cascade-internal mechanism operating on the cascade
sequence $(\Omega_d)_{d\geq 0}$ or its log-ratio $p(d)$. Any such mechanism
applies one of four operator classes to a cascade-internal quantity:
\begin{itemize}
\item[(a)] \emph{Local extrema of a unimodal cascade sequence.}
\item[(b)] \emph{Zeros of a strictly monotonic cascade sequence.}
\item[(c)] \emph{Crossings of a monotonic cascade sequence with a
cascade-primitive reference value.}
\item[(d)] \emph{Higher-order invariants} (zeros of $p^{(n)}$ for $n\geq 1$,
inflection points of $\Omega_d$, etc.).
\end{itemize}
The classes~(a)--(d) exhaust cascade-internal dimension-selecting
mechanisms because any further construction is an analytic function of
(a)--(d) applied to the sequence, and Theorem~\ref{thm:c1unique}'s
primitivity criterion excludes iterated or composite constructions.

\emph{Classes~(a) and (b) yield two dimensions each.}
