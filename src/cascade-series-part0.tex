...
above as corollaries. That per-label principle is itself derivable
from one global requirement: the invariant, branch-swapped through
the functional equation $s \mapsto 1-s$ (every weight
$2/\Gamma_{\mathbb{R}}(d+1)$ replaced by the mirror layer's
$2/\Gamma_{\mathbb{R}}(-d)$), must remain finite and nonzero ---
\emph{mirror coherence}. Over the eight labelings the mirrored
invariant is infinite four times, zero three times, and
finite-nonzero uniquely at the supremum; and the fixed interior
landmark $d_V = 5$ carries nonzero mirror weight
$-15/(4\pi^3)$ while its even neighbours' vanish, so coherence is a
test the structure passes rather than a constraint imposed on a
choice. Mirror coherence is adopted, not derived: the open
question is now exactly whether that single global requirement
