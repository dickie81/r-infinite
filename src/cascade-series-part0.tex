...
is the unique labelling avoiding the trivial-zero mirror set. In
local form, (iii) reads: the infimum's twist points are poles of
the archimedean local functional equation's transfer coefficient
$\gamma_\infty(s) = \Gamma_{\mathbb{R}}(1-s)/\Gamma_{\mathbb{R}}(s)$
(Tate), while the supremum's are its regular points --- so a single
non-degeneracy principle, \emph{integer labels at regular points of
the local functional equation}, forces the supremum outright, with
the variational characterisation's output and all the reformulations
above as corollaries. That per-label principle has an exactly
equivalent reformulation on the invariant as a whole: the invariant,
branch-swapped through the functional equation $s \mapsto 1-s$
(every weight $2/\Gamma_{\mathbb{R}}(d+1)$ replaced by the mirror
layer's $2/\Gamma_{\mathbb{R}}(-d)$), must evaluate to a finite
nonzero value --- \emph{mirror coherence}. Since
$\Gamma_{\mathbb{R}}$ never vanishes, this holds exactly when every
label's mirror weight is nonzero, i.e.\ it is the per-label
principle restated, not a logically prior requirement. Over the
eight labelings the mirrored invariant evaluates finite-nonzero
uniquely at the supremum; it is zero three times, infinite once,
and an indeterminate $0\cdot\infty$ form (no exact value) three
times --- so the supremum's uniqueness is a statement about exact
values of the defining formula, not about regularised limits, one
of which is finite. (An earlier version of this parenthetical
stated ``infinite four times'' and framed coherence as a single
requirement one level up; both were corrected --- three of the four
are indeterminate forms, and the reformulation is an exact
equivalent.) The fixed interior landmark $d_V = 5$ carries nonzero
mirror weight $-15/(4\pi^3)$; since the mirror weight vanishes at
every even layer, this is the parity fact that $d_V$ is odd,
restated. Mirror coherence is adopted, not derived: the open
question is unchanged --- whether the principle, in either face,
follows from the cascade's axioms.) (Adjudicated for the committed
record: it does not follow from them as committed --- the axioms'
asserted content quantifies over the descent lattice and its
weights, while the principle constrains the weight function at
negative arguments, which no axiom governs; and each of its five
equivalent faces is either an extremal convention, an unbacked
parity rule, or an off-lattice condition. The question thereby
becomes foundational: whether the weight function's global
identity --- its continuation and functional equation --- is
axiom content. That choice has now been made --- the review
programme's round 98, by the owner's decision: the kernel is taken
as $\Gamma_{\mathbb{R}}$ \emph{entire} (the owner's phrase;
``entire'' in the sense of \emph{in its entirety} --- the function
itself is meromorphic, its poles load-bearing), the global function
with its functional-equation role, carrying mirror coherence as its
non-degeneracy clause, worded as the boundary-labelling
\emph{selector} --- and the labelling $(7,19,217)$ is thereby
forced, with the variational characterisation's output and all the
reformulations above following as corollaries; the variational
characterisation of the Evaluation theorem thereby becomes an exact
\emph{characterisation} rather than the primitive selector, its
argmax provably coinciding with the clause-selected labelling. The cost is recorded
in the open: the axiom enlarges from a lattice-value table to the
global function with one clause; the adoption adds no empirical
content and changes no number.)
\end{remark}
