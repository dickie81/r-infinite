...
layer---remains open. (Two exact reformulations narrow the question:
...
decreasing in the invariant, and (ii) the \emph{odd} member of each
straddling pair---the Euler-null sphere, $\chi(S^d) = 0$, the same
Euler characteristic whose value $\chi(S^{2n}) = 2$ weights the
potential-shift family above; indeed all four distinguished
dimensions $\{5, 7, 19, 217\}$ are odd. A principled derivation of
any one of these equivalents would settle the question; none is yet
derived.)
\end{remark}
