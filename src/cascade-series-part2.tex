...
amplitudes. The phase--obstruction lockstep (Corollary~\ref{cor:lockstep}) follows: the
propagator phase is imaginary if and only if $S^{d-1}$ carries a hairy ball zero, both
controlled by the parity of $d$. The cascade's classification has period~4; the Clifford
refinement to period~8 is established in Part~III.
(5)~Time is the cascade's slicing direction. The discrete propagator
...
(computed in \texttt{tools/cascade\_decoherence.py}).
\end{proof}
