...
cascade's orthogonal slicing direction yields a discrete propagator with forced phase $i$
per step; the Schr\"{o}dinger equation is the effective description reconstructed by a 4D
observer who cannot resolve individual cascade steps.
...
\subsection{Concentration at $\cos^2\theta$}
...
\textbf{Step 4} (uniqueness from concentration). For $d\gg 1$, the uniform distribution on
$S^{d-1}$ concentrates on the equatorial band $|z| < O(1/\sqrt{d})$
(Theorem~\ref{thm:gaussian}). The state $u$ at angle $\theta$ from $e$ has its
neighbourhood on the sphere concentrated near the level set $z = \cos\theta$. The
fraction of the total geometric content attributable to the measurement outcome $e$ is
the squared projection $\cos^2\theta$, because the sphere's measure in the polar
direction partitions as:
\[
1 = \cos^2\theta + \sin^2\theta = (u\cdot e)^2 + \|u - (u\cdot e)e\|^2,
\]
and concentration of measure forces this partition to be the unique assignment consistent
with additivity across orthogonal axes. For $k$ orthogonal axes $\{e_1,\ldots,e_k\}$:
\[
\sum_{j=1}^{k}(u\cdot e_j)^2 = 1,
\]
by Parseval's identity on $S^{d-1}$. No other function of $\theta$ satisfies this
constraint for all $k$ and all orientations simultaneously.
\end{proof}
...
\end{corollary}
\begin{proof}
Dividing~\eqref{eq:difference} by $\Delta t = N(d)$:
\[
\frac{\psi_{d-1}-\psi_d}{N(d)} = \frac{iN(d)-1}{N(d)}\,\psi_d
= \left(i - \frac{1}{N(d)}\right)\psi_d.
\]
Multiplying both sides by $-i$:
\[
i\,\frac{\psi_d - \psi_{d-1}}{N(d)} = \left(1 - \frac{i}{N(d)}\right)\psi_d
= \frac{1}{N(d)}\bigl(N(d) - i\bigr)\psi_d.
\]
In the slowly varying regime, replacing the difference quotient by $d\psi/dt$ and
writing $\mathcal{H} = \mathrm{Re}[(N-i)/N^2] = (1-N)/N^2$ (the imaginary part contributes
a decay/growth that is absorbed into the lapse normalisation), gives~\eqref{eq:schrodinger}.
\end{proof}
...
\begin{remark}[Cumulative decoherence over the cascade descent]
Each of the 213 slicing steps from $d=217$ to $d=4$ traces out one fibre, each with its
own decoherence length $\ell_{\rm dec}(d) = 2/\sqrt{d+3}$. The cumulative decoherence for
a state with angular separation $\Delta\theta$ depends on the orientation relative to
each fibre axis. In the worst case (separation aligned with the fibre at every step),
the total suppression is $\prod_d \exp(-\Delta\theta^2(d+3)/4)$, which is exponentially
complete for any $\Delta\theta > 0$. In the generic case (random orientation), each step
contributes a fraction $1/d$ of the total separation to the fibre direction, giving a
per-step factor $\exp(-\Delta\theta^2(d+3)/(4d^2))$. The cumulative product over the
full descent converges, leaving a finite residual coherence for small $\Delta\theta$.
The precise value depends on the cascade's orientation structure and is not computed here.
\end{remark}
...
\item \textbf{Cumulative decoherence over the full descent.}
Theorem~\ref{thm:decoherence} derives the per-step decoherence factor
$D = \exp(-\Delta x^2(d+3)/4)$ and identifies the decoherence length as $2R_{\rm eff}$.
The remaining question is the cumulative decoherence over all 213 steps: this requires the
orientation structure of the cascade's slicing axes (how the fibre direction at each step
is oriented relative to the previous ones). Whether the cumulative decoherence reproduces
the quantitative predictions of environment-induced decoherence~\cite{zurek} for specific
physical systems is open.
\end{enumerate}
