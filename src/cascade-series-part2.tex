...
$|\langle u, v\rangle_{\mathbb{C}}|^2$. The choice between the
$\mathbb{Z}_2$ gauge of Step~4 (real Born rule on $\mathbb{R}P^{d-1}$)
and the full $U(1)$ gauge (complex Born rule on
$\mathbb{C}P^{n-1}$) is structural: both are cascade-consistent;
the $U(1)$ choice additionally matches standard QM at the projective
state-space level.
\end{remark}
...
Remark~\ref{rem:U1-gauge-Born} and Remark~\ref{rem:U1-gauge-J} show that the cascade's
$J$-generated $U(1)$ commutes with the discrete propagator at every step
($[L(d),R_\phi]=0$). Treating this $U(1)$ as a gauge equivalence (rather than just a
dynamical symmetry) refines the projective state space from the $\mathbb{Z}_2$-quotient
$\mathbb{R}P^{d-1}$ of Step~4 to the $U(1)$-quotient $\mathbb{C}P^{n-1}$ via the Hopf
bundle, recovering the standard QM Bloch sphere and complex Born rule
$|\langle u,v\rangle_{\mathbb{C}}|^2$. Both gauge choices ($\mathbb{Z}_2$ and $U(1)$) are
cascade-consistent; the upgrade is structural, not strictly forced by Step~4 alone.
The question: is the $U(1)$ upgrade \emph{forced} by some additional cascade-internal
requirement (e.g.\ matching the dim-2 Born rule via compositionality, or unitarity of the
discrete propagator on the gauge-quotient state space), or is it one of two cascade-
permitted projective structures, distinguished only by the empirical fact that QM uses the
$U(1)$ choice? A forcing argument would close the gap noted in Remark~\ref{rem:sp15-status};
its absence leaves the gauge choice as a structural input.
\end{enumerate}
