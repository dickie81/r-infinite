...
% Note: in this paper's bibliography \bibitem{paperI} points at Part 0
...
matter-content path tensor product of Part~IVa, Section~\xref{paper4a}{sec:three-generations} reaches
...
Part~IVa, Section~\xref{paper4a}{sec:three-generations} as Wilson holonomies in the multiplicative
...
(Part~IVb~\cite{paper4b}, \emph{rem:fermion-gauge-coupling}:
...
Part~IVa, Section~\xref{paper4a}{sec:bott-mirror} Spin$(12)$ Dirac decomposition
respectively.
...
\medskip\noindent\emph{No torus / no Kaluza--Klein quotient.} The cascade's
``compactification'' is exclusively this metric attenuation. There is no global
torus topology $T^{213}$ (or any other $T^N$) underlying the cascade descent, nor
any Calabi--Yau-style compactification, nor any periodic identification of the
slicing axes. A torus is topologically distinct from $\mathbb{R}^N$ (the
identification $x \sim x + 2\pi R$ is a topology change, even when $R$ is
finite); the cascade refuses such identifications by the ``no topology change''
clause above. Equivalently, Paper~I~\cite{paperI}~\S3.2 commits explicitly:
``the cascade does not do Kaluza--Klein reduction.''  The cascade-native global
topology is a 1D lattice in the layer index $d$, with per-layer $S^{d-1}$
boundary in Part~0's slicing recurrence, and asymptotic metric attenuation along
each slicing axis --- no global compactification quotient at any finite or
infinite cascade depth.
...
the same quantity that drives the descent corrections in the Part~0.
\end{corollary}
...
eigenvalue deficit is the same quantity that drives the descent corrections in Part~0's
inter-layer-coupling section. The remaining question: whether the cascade's geometric decoherence, applied to
...
(the matter-content path tensor product of Part~IVa, Section~\xref{paper4a}{sec:three-generations}
...
\bibitem{paperI} RTAC, \extlink{\cascadebase/cascade-series-part0.pdf}{\emph{The Cascade Series --- Part 0: Scale Variance from
...
\bibitem{paper4a} RTAC, \extlink{\cascadebase/cascade-series-part4a.pdf}{\emph{The Cascade Series --- Part IVa: The Standard Model from the Cascade --- Gauge Group, Symmetry Breaking, and Three Generations from Bott Periodicity and Hairy Ball Zeros}}, March 2026.
\bibitem{paper4b} RTAC, \extlink{\cascadebase/cascade-series-part4b.pdf}{\emph{The Cascade Series --- Part IVb: The Standard
Model from the Cascade --- Masses, Couplings, and Precision Predictions from
the Geometric-Topological Factorization}}, March 2026.
\bibitem{paper6} RTAC, \extlink{\cascadebase/cascade-series-part6.pdf}{\emph{The Cascade Series --- Part VI: Tower Growth,
