...
\begin{remark}[Reading G: gauged path-states close the
$\mathbb{R}P^3$ vs $\mathbb{C}P^1$ gap]\label{rem:reading-G-born}
...
\]
Treating this $U(1)$ as a gauge equivalence (path-state Reading G,
verified in \texttt{tools/verifiers/cascade\_path\_state\_born\_rule.py})
reduces the projective state space from $\mathbb{R}P^{d-1}$ to
$\mathbb{C}P^{n-1}$ via the Hopf bundle $S^{2n-1} \to \mathbb{C}P^{n-1}$
($n = d/2$ for even~$d$).

Under Reading G, Step~4's Cauchy argument runs on the
$U(1)$-quotient of the complex simplex:
...
identified). Reading G upgrades the gauge to the full $U(1)$
generated by $J$, recovering standard QM. Both readings give the
same numerical Born rule on real bases; Reading G additionally
recovers the QM complex Born rule $|\langle u, v\rangle|^2$ as the
...
\begin{remark}[$U(1)$ generated by $J$ as gauge equivalence]\label{rem:reading-G-J}
...
Promoting it to a \emph{gauge equivalence} (Reading G,
Remark~\ref{rem:reading-G-born}) refines the projective state space
...
and the $U(1)$ gauge of Reading G (complex Born rule on
$\mathbb{C}P^{n-1}$) is structural: both are cascade-consistent;
Reading G additionally matches standard QM at the projective
state-space level. Audit item 3.4 (Hopf fibration $S^3 \to S^2$)
records this as the cascade-internal Bloch sphere construction.
\end{remark}
...
\]
the gauge-invariant content of the holonomy. Under Reading G
(Remark~\ref{rem:reading-G-J}), the modulus-squared is the physical
