...
\begin{remark}[Open structural questions on $d \geq 29$ matter content]\label{rem:bott-tower-open}
The treatment of the Bott Dirac layers $d \in \{29, 37, 45, \ldots, 213\}$ in this paper and Part~IVb~\cite{paperIVb} leaves four structural questions open. These do \emph{not} affect the closure of the three-charged-fermion-generation count or the precision predictions at $d=5, 13, 21$; they concern the cascade's matter-content story \emph{above} the third generation.

\begin{enumerate}[label=(OQ-T\arabic*),nosep]
\item \emph{Bott-tower stopping criterion.} Bott periodicity is infinite. The cascade descent contains 27 Dirac layers up to the Planck sink at $d_2=217$. The cascade text invokes only $d=29$ (as ``neutrino source mass''); higher Bott layers $d=37, 45, \ldots, 213$ are mentioned (``$\{5,13,21,29,\ldots\}$ is the cascade's Dirac-layer sequence'') but never assigned a role. Under the cascade fermion mass formula, $m_{37}\approx 0.2$~eV and $m_{45}\approx 30~\mu$eV --- both at cosmologically interesting scales. \emph{What stops the source assignment at $d=29$, or the particle assignment at $d=21$?} No cascade theorem currently provides a stopping criterion. Verifier: \texttt{tools/research/cascade\_bott\_tower\_beyond\_29.py}.
...
These four open questions form a coherent structural cluster: closing any one of them (especially OQ-T2) provides the path-tensor extension or termination argument that resolves the others. The cleanest closure path would be a Bott-replication argument with a derived cutoff at the third orbit, supplying both ``no path-tensor at $d \geq 29$'' (resolving charge) and ``no propagating fermion at $d \geq 29$'' (resolving particle status). The cascade text currently has none of these. Tier~4. Verifier including Gram-correction analysis: \texttt{tools/research/cascade\_gram\_bott\_tower.py}.
\end{remark}
