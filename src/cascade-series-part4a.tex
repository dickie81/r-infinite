...
\emph{the observer's hemispheric position on $S^4$ at $d=5$}.
Specifically, $S^4$ at the observer host has exactly two basins of
equal area (Poincar\'{e}--Hopf $\chi(S^4)=2$ + $\mathbb{Z}_2$ height-
function symmetry); the Gram first-order correction (Part~0
Supplement) preserves this $\mathbb{Z}_2$ basin equality strictly to
machine precision (verifier:
\texttt{tools/research/cascade\_s4\_antipodal\_test.py}; analytic:
slicing integrand $(1-x^2)^{d/2}$ is even in $x$ on $[-1,1]$). The
two basins host matter and antimatter respectively under the matter-
content reading; the observer occupies one, with the antipodal basin
outside causal contact. The sign anchor is therefore \emph{geometric
content}, not a free convention.

The cascade structurally predicts:
\begin{itemize}
\item Total cosmic baryon number $=0$ cascade-internally; observed
matter excess is hemispheric localization, not Sakharov-violation.
...
(falsifier; currently consistent with observation).
\end{itemize}
