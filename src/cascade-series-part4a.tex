...
% Note: \bibitem{paperI} in this paper's bibliography points at Part 0
% (cascade-series-part0), not Part I -- legacy bibliography naming.
\usepackage{xr-hyper}
\xrhyperdoc{paperI}{cascade-series-part0}
\xrhyperdoc{paperII}{cascade-series-part2}
\xrhyperdoc{paperIII}{cascade-series-part3}
\xrhyperdoc{paperIVb}{cascade-series-part4b}
...
is independently set to zero by the cascade's topological classification (Part~IVb, Section~\xref{paperIVb}{sec:gauge-couplings}), not by a leakage argument from the $d=13$ phase.
...
spectrum. The Y spectrum is closed in Part IVb~\cite{paperIVb}
...
Part~IVb~\cite{paperIVb}~$\mathit{thm{:}sector\text{-}fundamental\text{-}y}$
...
Part~IVb~\cite{paperIVb} Section on the Cascade Fermion Action
...
Part~IVb~\cite{paperIVb} Theorem on the sector-fundamental U(1)$_Y$ rule
...
Part~IVb~\cite{paperIVb}~$\mathit{thm{:}chirality\text{-}factorisation}$
articulated in Part~IVb~\cite{paperIVb} Remark on cascade CPT-balance
...
of cascade fermion + gauge invariance, Part~IVb~\cite{paperIVb}'s
...
in $(\mathbf{1},\mathbf{2})$ via Part~IVb~\cite{paperIVb}~$\mathit{thm{:}sector\text{-}fundamental\text{-}y}$:
...
denominators (Part~IVb~\cite{paperIVb}~$\mathit{thm{:}sector\text{-}fundamental\text{-}y}$),
...
(Part~IVb~\cite{paperIVb}~$\mathit{rem{:}cpt\text{-}balance\text{-}basins}$),
...
(\cite{paperI}, Theorem~7.1), and the
...
closed cascade-natively in Part IVb~\cite{paperIVb}
...
supplied in Part~IVb~\cite{paperIVb}~$\mathit{sec{:}fermion\text{-}action}$
...
Part~IVb~\cite{paperIVb}~$\mathit{sec{:}fermion\text{-}action}$
...
Bott periodicity is infinite. The cascade descent contains Bott Dirac layers ($d \bmod 8 = 5$) at $d = 5, 13, 21, 29, 37, \ldots$, with no cascade-internal termination of the replication; sphere areas decay super-exponentially past the Planck sink at $d_2=217$, but the Bott periodicity itself extends without bound. The first three layers ($d=5,13,21$) are the SM charged-fermion generations (Theorem~\ref{thm:generations}); the fourth ($d=29$) sources the heaviest cascade neutrino mass via Part~IVb~\cite{paperIVb}. The remaining infinitely many layers ($d=37, 45, 53, \ldots$) are cascade primitives at which the fermion mass formula extrapolates Bott-periodically to predict specific masses --- $m_{37}\approx 0.2$~eV, $m_{45}\approx 30~\mu$eV, and successively smaller scales.
...
\midrule
\emph{Time-coupled cascade modes} (Bott Dirac $d \in \{5, 13, 21\}$) & Dynamical vacuum (running couplings, anomalous moments) & $1/\alpha_{\rm em}$ screening (Part~IVb~\cite{paperIVb}~$\mathit{rem{:}per\text{-}leg\text{-}primitive\text{-}derivation}$), photon self-energy, $(g{-}2)$, $\theta_C$ amplitude descent, etc. \\[0.3em]
\emph{Time-decoupled cascade primitives} (Bott Dirac $d \geq 29$ plus the entire static cascade tower up to $d_2 = 217$) & Static vacuum / zero-point structure & Cosmological constant $\Lambda = I = (\Omega_{19}\Omega_{217})/\pi^3$ (Paper~I~\cite{paperI}); $\Omega_m$ via Bott partition; $m_{29}$ as cascade-descent source for the heaviest active neutrino (Part~IVb~\cite{paperIVb}). \\
\bottomrule
\end{tabular}
\end{center}
...
\item \emph{(R5-cardinality), the active particle-host orbit has cardinality $3$.} Bott shift on Dirac layers yields the infinite sequence $\{5,13,21,29,37,\ldots\}$. Restricted to layers with active particle-host status under Theorem~\ref{thm:generations}'s activation criterion, the active set is $\{5,13,21\}$: $d=5=d_V$ is the cascade lower endpoint (volume maximum, \cite{paperI}~Theorem~7.1); $d=13$ is the gauge-window Dirac (subcritical); $d=21$ is the first marginally-supercritical Dirac (active, $1.3$ transition widths past $d_1$); $d=29$ is the first deep-supercritical Dirac ($9.3$ widths past, $\sim 289\times$ charged-fermion suppression). The cardinality is fixed once two cascade primitives are fixed: the lower endpoint $d_V=5$ (one of Paper~I's distinguished critical points) and the activation cutoff width near $d_1=19$. The active orbit happens to be symmetric around $d=13$ (since $5+21 = 26 = 2 \cdot 13$), but this is an arithmetic consequence of $d_V = 5$ and Bott period $8$, not an additional reflection primitive.
\end{itemize}
...
Theorem~\ref{thm:forced-paths} derives the \emph{path} of every observable whose cascade home is a distinguished layer (generation or gauge boson). It does \emph{not} derive (a) the \emph{correction formula} to the leading-order $\exp(\Phi)$ on each path --- supplied in Part~IVb~\cite{paperIVb} as the $\alpha(d^*)/\chi^k$ correction family ($\mathit{rem{:}phase\text{-}family}$, with structural rules at $\mathit{prop{:}source\text{-}selection}$, $\mathit{thm{:}sign\text{-}rule}$, and $\mathit{thm{:}chirality\text{-}selection\text{-}rule}$); (b) the absolute mass scale, which enters via the electroweak VEV and ultimately the reduced Planck mass as a dimensional anchor (Part~IVb~\cite{paperIVb}~$\mathit{thm{:}mtau\text{-}abs\text{-}closure}$ closes $m_\tau = 1776.82$~MeV to $-0.31\sigma$ via $\delta\Phi_{\rm phase}=\alpha(19)/\chi$, with $M_{\mathrm{Pl,red}}$ as the dimensional anchor); (c) up-type quark mass ratios, which the paper classifies as Tier~4 pending a derivation of the chirality correction at the $\mathrm{SU}(3)$ layer. The theorem separates the path problem from the correction problem: with paths forced, any remaining precision deviation is isolated to the correction formula, not to the choice of cascade layers.
\end{remark}
...
established in this paper and Part~IVb~\cite{paperIVb}
...
volume maximum $d_V=5$ is unique (Paper~I~\cite{paperI},
...
(Paper~III~\cite{paperIII}). No selection.
\item \emph{Cascade tower itself}: forced by the Gamma function
structure of Part~0~\cite{paperI}. One cascade tower exists; no
choice.
...
(Part~IVb~\cite{paperIVb}~$\mathit{thm{:}chirality\text{-}factorisation}$ + Part~IVb~\cite{paperIVb}'s
...
\bibitem{paperI} [Author], \extlink{\cascadebase/cascade-series-part0.pdf}{\emph{Scale Variance from Orthogonality: How the Unit Ball
Generates $10^{120}$ Orders of Magnitude}}, March 2026.
\bibitem{paperII} [Author], \extlink{\cascadebase/cascade-series-part2.pdf}{\emph{Quantum Mechanics from the Cascade: Effective Theory of
a 4-Dimensional Observer in the Sphere-Area Geometry}}, March 2026.
\bibitem{paperIII} [Author], \extlink{\cascadebase/cascade-series-part3.pdf}{\emph{General Relativity from the Cascade: Lovelock Uniqueness
and the Cosmological Constant of a 4-Dimensional Observer}}, March 2026.
\bibitem{paperIVb} [Author], \extlink{\cascadebase/cascade-series-part4b.pdf}{\emph{The Standard Model from the Cascade: Part IVb---Masses,
