...
\medskip\noindent\textbf{Underlying principle: fundamental-or-trivial at $d=13$.}
The rep-dimension restriction $V_{13}\in\{\mathbf{1},\mathbf{2}\}$ is
the $d=13$ instance of a unified cascade-internal principle: matter at
each gauge-window layer transforms in either the \emph{fundamental} or
the \emph{trivial} representation of the gauge group at that layer.
The verifier
\texttt{tools/research/cascade\_fundamental\_rep\_lemma.py} reports the
status of this principle layer by layer:
\begin{itemize}
\item $d=12$ (SU(3)): closed structurally. The cascade space
  $\mathbb{R}^{12}=\mathbb{H}^3$ is the carrier of the fundamental;
  $N_c=3$ is the dimension of the $\mathbf{3}$
  (Remark~\ref{rem:su3-d7-algebra}, item ``fundamental representation
  dimension $N_c=3$ at $d=12$ via Adams' theorem'').
\item $d=14$ (U(1)): partially closed. The cascade complex structure
  $J$ on $\mathbb{R}^{14}=\mathbb{C}^7$ is the carrier of the
  fundamental; $\dim_{\mathbb{C}}=1$ is forced. Specific hypercharge
  values remain open (CLAUDE.md ``Known Quantitative Issues'',
  matter-rep / hypercharge gap).
\item $d=13$ (SU(2)): closed by the present remark.  Fundamental of
  SU(2) is $\mathbf{2}$, trivial is $\mathbf{1}$, hence
  $V_{13}\in\{\mathbf{1},\mathbf{2}\}$.
\end{itemize}
The argument splits by SU(3) sector: color-charged matter
($V_{12}=\mathbf{3}$) realises the principle through the Spin$(12)$
Dirac structure on $\mathbb{H}^3$; color-singlet matter
($V_{12}=\mathbf{1}$) realises it through path-tensor layer
independence. The two routes give the same conclusion through
distinct cascade structures.
