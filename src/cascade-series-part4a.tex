...
\item \emph{Eigenvalue conjecture (falsified).} A stronger reading
...
$\pm$-phase rather than a $Y$-graded phase. The SM hypercharge weights
are supplied by the Bott-period representation structure of
\S\S2.1--2.3 (this paper) and the chiral assignments of Theorem~6.1
of~\cite{paperII}, not by $J$ alone. This separation is
consistent with the reading that $J$ fixes the gauge \emph{group}
while the representation structure fixes the \emph{weights}.
...
to a closed identity plus a falsified eigenvalue claim. Numerical
...
\medskip\noindent\textbf{Single-$\mathbb{H}$-factor claim.}
Each matter unit at $d=13$ couples to the diagonal $\mathrm{SU}(2)_R$
via at most one $\mathbb{H}$ factor of $\mathbb{H}^3=\mathbb{R}^{12}$.

\emph{Cascade grounding.} A matter unit at $d=12$ is the cascade
content of one specific color (per the $\mathbf{3}=\mathbb{H}^3$
fundamental, Remark~\ref{rem:su3-d7-algebra}) or is color-singlet
(symmetric across the three $\mathbb{H}$ factors). In either case the
matter unit enters $V_{13}$ via one specific tensor factor of
Remark~\ref{rem:path-tensor}'s decomposition, and that factor contributes
one Spin$(4)$-Weyl piece (either Weyl$_+$ or Weyl$_-$). Multiple Weyl$_-$
contributions per matter unit would require multiple $\mathbb{H}$
factors active simultaneously, contradicting the single-tensor-factor
structure.

\emph{Consequence.} Under the diagonal $\mathrm{SU}(2)_R$, each
$\mathbb{H}$ factor's Weyl$_+$ piece is $\mathrm{SU}(2)_R$-singlet and
each Weyl$_-$ piece is $\mathrm{SU}(2)_R$-doublet
(verifier \texttt{tools/research/cascade\_route\_c\_d13\_dirac.py}). The
matter unit's $V_{13}$ has at most one Weyl$_-$ tensor factor,
restricting to:
\begin{itemize}
\item zero Weyl$_-$ factors $\Rightarrow$ $V_{13}=\mathbf{1}$ (singlet);
\item one Weyl$_-$ factor $\Rightarrow$ $V_{13}=\mathbf{2}$ (fundamental).
\end{itemize}
Higher reps are excluded:
the triplet $\mathbf{3}$ requires two Weyl$_-$ factors via Clebsch--Gordan
$\tfrac{1}{2}\otimes\tfrac{1}{2}=0\oplus 1$;
the quartet $\mathbf{4}$ requires three Weyl$_-$ factors via
$\tfrac{1}{2}\otimes\tfrac{1}{2}\otimes\tfrac{1}{2}=\tfrac{1}{2}\oplus
\tfrac{1}{2}\oplus\tfrac{3}{2}$. Neither configuration is consistent with
single-$\mathbb{H}$-factor matter.
...
\end{tabular}
\end{center}
The single-$\mathbb{H}$-factor restriction selects exactly the
$\{\text{singlet},\text{doublet}\}$ subset, excluding the 6 triplets and
1 quartet. Combined with the chirality split, this reproduces the SM
SU$(2)_L$ parity-violation pattern: $L$-handed states are doublets
($V_{13}=\mathbf{2}$), $R$-handed are singlets ($V_{13}=\mathbf{1}$).
The exclusion of triplet and quartet representations from the SM matter
spectrum is therefore cascade-internal. Numerical verifiers:
\texttt{tools/research/cascade\_route\_c\_d13\_dirac.py} (full
decomposition) and \texttt{tools/research/cascade\_rep\_selection\_gap1.py}
(decomposition by number of Weyl$_-$ tensor factors and the
single-$\mathbb{H}$-factor selection).

\medskip\noindent\emph{What this closes and what remains.}
The remark closes the rep-dimension question at $d=13$
($V_{13}\in\{\mathbf{1},\mathbf{2}\}$, no higher reps). It does not
address (a)~the specific multiplicities of $\mathrm{SU}(2)$ singlets
and doublets that match SM matter content per generation (4 doublets
$+$ 7 singlets per generation, vs.\ the cascade's 14 of each in the
$n_S\in\{0,1\}$ subset), and (b)~the specific SM hypercharge values at
$d=14$. Both remain open structural questions; see CLAUDE.md
``Known Quantitative Issues'' for the broader matter-rep gap.
\end{remark}
