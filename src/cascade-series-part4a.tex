...
The four sub-questions are downstream facets of the deeper (D1)/(D2) tension: each is a different angle on the same hidden $N_{\rm gen}=3$ input. Closing any one of them (especially OQ-T2) selects between Reading~A (cascade-predicted Bott tower of additional fermion generations at cascade-derived masses, with (D2)'s layer-independence requiring qualification at high $d$) and Reading~B (cascade-internal cutoff terminating the propagating Bott-replication at the third orbit, with higher Bott layers realised only as cascade primitives entering geometric quantities and cascade-descent source masses, not as propagating fermions). Each carries distinct experimental falsifiers: Reading~A is testable via KATRIN/TRISTAN at $d=29$ (sub-keV sterile neutrino with cascade-predicted mixing $|U_{e4}|^2 \approx 9 \times 10^{-5}$) and via cosmological $\Sigma m_\nu$ bounds at $d=37$ ($\approx 0.2$~eV); Reading~B requires a cascade theorem terminating the Bott-replication, currently absent. The over-screening framing in (D1)/(D2) above is conditional on Part~IVb's per-layer screening derivation being extended universally to every Bott Dirac layer (Part~IVb itself uses it for $N_{\rm gen}=3$ as an external input); a different cascade derivation of $1/\alpha_{\rm em}$ that does not assign one $2\pi$ contribution per layer would dissolve the tension at the cost of replacing Part~IVb's structural account of the closure. Tier~4. Verifier including Gram-correction analysis: \texttt{tools/research/cascade\_gram\_bott\_tower.py}.
\end{remark}
