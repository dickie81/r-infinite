...
\begin{remark}[Cascade Bott tower beyond $d=21$: predicted generations and open particle status]\label{rem:bott-tower-open}
Bott periodicity is infinite. The cascade descent contains 27 Dirac layers ($d \bmod 8 = 5$) up to the Planck sink at $d_2=217$. The first three ($d=5,13,21$) are the SM charged-fermion generations (Theorem~\ref{thm:generations}); the fourth ($d=29$) sources the heaviest cascade neutrino mass via Part~IVb~\cite{paperIVb}. The remaining 23 layers ($d=37, 45, \ldots, 213$) are cascade primitives at which the fermion mass formula extrapolates Bott-periodically to predict specific masses --- $m_{37}\approx 0.2$~eV, $m_{45}\approx 30~\mu$eV, and successively smaller scales out to the Planck sink.

\medskip\noindent\textbf{The cascade may therefore be predicting additional fermion generations beyond the SM three, at cascade-derived masses.} The Bott-replication structure that places three SM charged generations at $d\in\{5,13,21\}$ does not, in itself, terminate at $d=21$: the same Bott / Adams / path-tensor machinery applies at $d \geq 29$ in principle. Whether this represents (Reading~A) a genuine prediction of 24 additional fermion generations at the cascade-derived masses above, or (Reading~B) a stopping mechanism that reserves $d \geq 29$ for structural source masses without propagating fermion content, depends on four structural questions that are currently open. Both readings are testable; the predicted phenomenology differs sharply between them. None of OQ-T1--T4 below affects the closure of the three-charged-fermion-generation count or the precision predictions at $d=5, 13, 21$; all concern the cascade's matter-content story \emph{above} the third generation.
...
These four open questions form a coherent structural cluster: closing any one of them (especially OQ-T2) selects between Reading~A (cascade-predicted Bott tower of 24 additional fermion generations at cascade-derived masses) and Reading~B (cascade-internal cutoff at the third Bott orbit, with $d \geq 29$ realised only as structural source masses) and resolves the others. Each Reading carries distinct experimental falsifiers: Reading~A is testable via KATRIN/TRISTAN at $d=29$ (sub-keV sterile neutrino with cascade-predicted mixing $|U_{e4}|^2 \approx 9 \times 10^{-5}$) and via cosmological $\Sigma m_\nu$ bounds at $d=37$ ($\approx 0.2$~eV); Reading~B requires a derivation of a cascade-internal stopping mechanism, currently absent from the series. Tier~4. Verifier including Gram-correction analysis: \texttt{tools/research/cascade\_gram\_bott\_tower.py}.
\end{remark}
