...
$d=29$ is deep in the supercritical regime and absent.
...
invariant $I=1.0996\times 10^{-120}$ matching the cosmological constant~\cite{paperI}; the
...
II~\cite{paperII}  & Complex state space, propagator $e^{-iHt}$ & Gleason (Born rule) \\
...
\subsection{The self-dual crossing at $d=12$}

The cascade's lapse function $N(d)=\sqrt{\pi}\cdot\Gamma((d+1)/2)/\Gamma((d+2)/2)$ is the
compactification radius at dimension $d$.

\begin{theorem}[Self-dual crossing]\label{thm:self-dual}
$N(12)=10395\pi/46080=0.70870$. The self-dual radius $1/\sqrt{2}=0.70711$. The deviation
is $0.225\%$.
\end{theorem}

In Kaluza--Klein compactification on a circle of radius $R$, the low-energy theory contains
a $\mathrm{U}(1)$ gauge field from the metric component $g_{\mu 5}$. At the self-dual
radius $R=1/\sqrt{2}$ (in cascade units), winding and momentum modes are degenerate, and
the $\mathrm{U}(1)$ enhances to $\mathrm{SU}(2)$. This gauge enhancement mechanism arises
in the cascade from the Gamma function, not from strings.
...
12  & 0.70870 & 0.70711 & Just above self-dual radius \\
13  & 0.68198 & 0.70711 & Below self-dual \\
19  & 0.56755 & 0.70711 & First threshold $d_1$ \\
\bottomrule
\end{tabular}
\end{center}

The crossing occurs between $d=12$ and $d=13$: $N(12)=0.70870>1/\sqrt{2}$ (just above)
and $N(13)=0.68198<1/\sqrt{2}$ (below). The crossing therefore falls exactly at the first
layer of the second complex spinor window, which opens at $d=12$. This coincidence is
forced by the interplay between the Gamma function (which determines $N(d)$) and Bott
periodicity (which determines the complex windows). No free parameter adjusts the crossing
point.
...
representations to $d=12$ and $d=14$ and a non-chiral (vector-like) representation to
$d=13$. This matches the physical structure: $\mathrm{SU}(3)$ and $\mathrm{U}(1)$ couple
chirally to fermions, while $\mathrm{SU}(2)$ couples only to left-handed fermions
(vector-like pre-breaking, chiral after breaking).

The real phases ($+1$ at $d=12$, $-1$ at $d=14$) give CP-even couplings for $\mathrm{SU}(3)$
and $\mathrm{U}(1)$. The imaginary phase ($i$ at $d=13$) gives a CP-odd component to
$\mathrm{SU}(2)$, which is broken by the hairy ball zero and therefore does not generate a
physical CP violation in the strong sector (see~\cite{paperIVb}).

\subsection{Generator counting}

\begin{theorem}[Generator count]\label{thm:generators}
The number of cascade layers between $d_0=7$ and $d_1=19$ is exactly
$12=\dim(\mathrm{SU}(3))+\dim(\mathrm{SU}(2))+\dim(\mathrm{U}(1))=8+3+1$.
\end{theorem}

The total rank of $\mathrm{SU}(3)\times\mathrm{SU}(2)\times\mathrm{U}(1)$ is $2+1+1=4$,
equal to the observer's spacetime dimension. This is the Bott mirror at the level of Lie
algebra rank: the spacetime window $\{4,5,6\}$ has dimension count 3 (the spatial dimensions
on the $S^3$ horizon), and the gauge window's total rank equals the full spacetime dimension
$d=4$.

\subsection{$N_c=3$ from Adams' theorem}
...
\subsection{The $d_1=19$ phase transition as cutoff}
...
The number of observable fermion generations is exactly three.
\end{theorem}
\begin{proof}
Generation 3 ($d=5$) and Generation 2 ($d=13$) are subcritical: $p(d)<c_1$ at both layers,
so their propagator amplitudes from $d=4$ are $O(1)$ and unsuppressed.

Generation 1 ($d=21$) is barely supercritical: $p(21)-c_1=+0.031$, only 1.3 steps past
the threshold in units of the transition width $1/p'(d_1)\approx 39.5$. Its propagator
amplitude is suppressed but nonzero.

Generation 0 ($d=29$) is 9.3 steps past threshold; its amplitude relative to Generation 1
is suppressed by a factor of $\sim 289$, making it unobservable. The transition at $d_1=19$
---derived in~\cite{paperI} from the Gamma function alone---is the wall that kills the
fourth generation.

The transition width deserves comment. The derivative $p'(d)=\tfrac{1}{4}\psi^{(1)}
((d+1)/2)>0$ is the trigamma function, strictly positive everywhere. At $d_1=19$:
$p'(19)=\tfrac{1}{4}\psi^{(1)}(10)\approx 0.0253$. The transition width
$\Delta d\equiv 1/p'(d_1)\approx 39.5$ layers means the transition is smooth, not sharp.
Generation 1 at $d=21$ is only $\Delta d_1=(21-19.73)/39.5=0.032$ transition widths past
the threshold. Generation 0 at $d=29$ is 0.24 widths past---modest, but the exponential
nature of the supercritical regime makes even this small overshoot devastating:
$\exp(8\times(p(29)-c_1))=\exp(8\times 0.192)\approx 4.6$ additional suppression per extra
generation.
\end{proof}
...
\item There are exactly three fermion generations, from the intersection of Bott periodicity
(which selects generation candidates at $d\bmod 8=5$) and the $d_1=19$ phase transition
(which suppresses candidates above $d_1$).
...
structure (\cite{paperII}, Remark~6.5) and the Bott partition established in this paper.
By Corollary~4.8a of~\cite{paperI}, sphere areas are the only independent cascade
