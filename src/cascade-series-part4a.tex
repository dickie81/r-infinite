...
\item \emph{(R1) $Y_H = N_c \cdot Y_{Q_L}$}: open. Cascade-native
interpretation: ``$V_{12} = \mathbf{1}$ weight is the trace over
$V_{12} = \mathbf{3}$ of the $V_{12} = \mathbf{3}$ weight'' on
$\mathbb{H}^3 = \mathbb{R}^{12}$. Structural theorem not yet derived.
\item \emph{Smallest-magnitude principle}
($\lvert Y_{Q_L}\rvert = 1/6$, $\lvert Y_{L_L}\rvert = 1/2$):
well-defined but not yet derived.
\item \emph{Sign anchor}: reframed as observer's hemispheric position
on $S^4$ at $d=5$ (geometric content, parity with SM's $Q_e = -1$).
\end{itemize}
...
The remaining cascade-internal open piece is (R1); the sign anchor
is at parity with the SM's external observational anchor but with
explicit geometric content.
\end{remark}
