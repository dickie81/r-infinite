...
\begin{itemize}[nosep]
\item \emph{(R1-extension):} that the cascade's gauge structure does not establish at $d=20, 28, 36, \ldots$ (the Bott-orbital images of $d=12$ past $d_1$), even though Adams' formula gives $\rho(d)-1=3$ there. This is the cascade-internal claim that gauge structure is established only in the subcritical-or-marginal range; supercritical Adams images do not host new gauge groups. A precise statement and proof would come from the $d_1$ phase-transition's role in cascade activation: gauge groups are established by Adams at the unique pre-transition layer; post-transition images are formally Adams-equivalent but not active gauge windows. This claim is implicit in part4a but not stated as a theorem.
\item \emph{(R2-extension):} that ``one gauge window'' implies ``one Bott mirror axis''. The gauge-window's Dirac layer $d=13$ is the natural mirror centre; uniqueness of the gauge window gives uniqueness of the centre. This step requires showing that the cascade's Bott-mirror operation is anchored to gauge-window content (not to arbitrary layers). The path-tensor structure $V_{12}\otimes V_{13}\otimes V_{14}$ at the gauge window provides this anchoring: the mirror acts on path-tensor charge content, and the path-tensor lives only at the gauge window, so the mirror has only one centre.
\item \emph{(R3-cardinality):} that the Bott-mirror orbit through the gauge window has cardinality exactly $3$ (the centre plus one image either side at radius equal to one Bott period). This is the geometry of a Bott mirror operation at period $8$, applied with radius $8$ once on each side of the centre.
\end{itemize}
The first two are existing-theorem extensions that explicitly state structural claims the series has been using implicitly. The third is a counting fact about the Bott mirror at its natural period. None requires new physics or new structural ingredients beyond what Part~IVa already has; what's missing is the explicit assembly of these pieces into a single ``$N_{\rm gen}=3$ from cascade-internal Bott-mirror uniqueness'' theorem.
