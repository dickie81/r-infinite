...
\item \emph{(R1) $Y_H = N_c \cdot Y_{Q_L}$}, decomposed:
\begin{itemize}
\item \emph{(R1a) $|Y_H| = 1/2$}: closed (Higgs vacuum neutrality
+ SU(2) doublet structure: $Q_{\rm vac} = T_3^{\rm vac} + Y_H = 0$,
$T_3^{\rm vac}\in\{\pm 1/2\}$).
...
\end{itemize}
The cascade's claim of zero free parameters is now precise about
gauge groups, rep dimensions, and the structural form of Y values
including the sector-fundamental denominator. The remaining
cascade-internal open piece is (R1) — the trace identity
$Y_H=N_c\,Y_{Q_L}$ as a path-tensor color-trace statement on
$\mathbb{H}^3=\mathbb{R}^{12}$ — which fixes the integer numerator
on the $(\mathbf{1},\mathbf{2})$ sector lattice. The cascade has
