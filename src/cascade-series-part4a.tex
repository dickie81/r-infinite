...
\item \emph{(R1b) $|Y_{Q_L}| = 1/6$}: open, sharpened to a single
cascade-native principle (\emph{sector-fundamental U(1)$_Y$ rule}):
matter at the cascade gauge window occupies the fundamental U(1)$_Y$
rep of its $(V_{12},V_{13})$ sector, with sector unit
$1/(\dim V_{12}\cdot\dim V_{13})$. Verified for 5 of 6 SM Y values;
u$_R$ is the Yukawa-singlet outlier ($+2$ sector units, forced by
gauge invariance of $\bar Q_L\tilde H u_R$). Status:
austerity-respecting and SM-spectrum-selected, \emph{not}
austerity-forced; uniqueness theorem is the residual open
question, articulated as
...
The remaining cascade-internal open piece is the (R1b)
sector-fundamental uniqueness theorem; the cascade has \emph{zero
observer-frame freedom} (the observer at $d=4$ is forced by cascade
descent; spatial structure is forced by Adams; position within
basin is irrelevant by homogeneity; basin label is convention-
symmetric).
\end{remark}
