...
\medskip\noindent\textbf{Toward a derivation: the Bott-mirror reach argument.}
Reading C's required theorem reduces to a Bott-periodicity statement that the cascade has the ingredients for. The candidate derivation route:

\begin{enumerate}[label=(R\arabic*),nosep]
\item \emph{Bott periodicity 8 fixes the cascade's natural reach scale.} The Clifford classification repeats every 8 layers (Section~\ref{sec:breaking}, Bott periodicity). The Bott mirror pairs the spacetime window $\{4,5,6\}$ with the gauge window $\{12,13,14\}$ at exactly one Bott period separation (Section 2.1).
\item \emph{The gauge window's structural role is unique to its Bott orbit.} Theorem~\ref{thm:adams-unique} establishes that $\rho(d)-1=3$ holds at exactly $d=12$ in $[5, d_1=19]$; the gauge group structure is unique to this Bott orbit. There is no second cascade-internal layer in $[5, d_1]$ replicating the gauge window's $N_c=3$ Adams structure.
\item \emph{Path-tensor charge structure is fixed at the gauge window.} The path-tensor $V_{12}\otimes V_{13}\otimes V_{14}$ (Remark~\ref{rem:path-tensor}) lives at the three gauge-window layers. Charge-carrying matter content occupies this tensor product at the gauge window itself ($d=13$), and replicates to other Dirac layers via Bott-orbital mirroring of the cascade structure.
\item \emph{Bott-mirror reach is exactly one Bott period.} The Bott mirror pairs layers at separation 8 (one Bott period) and no further. Three Dirac layers fall within $\pm 1$ Bott period of the gauge window center $d_{\rm gw} = 13$: $d=5$ (one Bott period below, the spacetime-window Dirac), $d=13$ (the gauge-window Dirac itself), and $d=21$ (one Bott period above). Beyond $\pm 1$ period, the Clifford structure has shifted to the next Bott orbit out, and the path-tensor's charge-carrying replication does not extend.
\item \emph{Therefore charged-matter Bott-replication terminates at $\{d=5, 13, 21\}$.} Higher Bott Dirac layers ($d=29, 37, 45, \ldots$) are outside the Bott-mirror reach of the gauge window. They remain cascade primitives (sphere areas, mass-formula values, slicing data exist) but they don't carry path-tensor charge structure --- hence no propagating gauge-coupled fermion content, hence no dynamical observer-frame role, hence (by Part~II=III's time-emergence-from-propagator framing) no time-coupling.
\end{enumerate}

This route uses only Part~IVa primitives (Bott periodicity 8, Bott mirror, Adams uniqueness, path-tensor) plus Part~II=III's time-from-propagator identification. No semiclassics; cascade-lattice and Bott-classification ingredients only. The conclusion --- exactly three charged Bott Dirac layers, with higher Bott layers as static-vacuum primitives --- matches both the SM observation and the $1/\alpha_{\rm em}$ closure precision.

\medskip\noindent\emph{The remaining derivational gap.} Step~(R4) is the key. ``Bott-mirror reach is exactly one Bott period'' is plausible from the structure of Bott periodicity (period $=8$ is the natural scale; one period either side covers neighbouring Clifford-equivalent orbits) but is currently used implicitly in the series rather than stated as a theorem. Closing Reading~C via this route requires:
\begin{itemize}[nosep]
\item A precise statement of ``Bott-mirror reach'' as a structural property of the cascade's Clifford classification --- e.g., that path-tensor replication of charge-carrying matter content extends to Bott-orbital images of the gauge window within $\pm 1$ Bott period and not further.
\item A proof of this property from cascade primitives (Bott periodicity, Adams' theorem, path-tensor structure of Remark~\ref{rem:path-tensor}). The proof would likely show that the slicing-recurrence structure connecting consecutive Dirac layers' tensor factors is bounded in Bott-orbital depth.
\end{itemize}
Both the statement and the proof are within reach of Part~IVa's existing toolkit; neither is currently written. This is the single structural derivation that would close the (D1)/(D2) tension and Reading~C in one step.
