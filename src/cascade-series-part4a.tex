...
at $d=5$ --- a single binary topological invariant, not a position,
frame, or coordinate. Specifically, $S^4$ at the observer host has
exactly two basins of equal area (Poincar\'{e}--Hopf $\chi(S^4)=2$
$+$ $\mathbb{Z}_2$ height-function symmetry); each basin is
internally homogeneous (4-ball topology), with no preferred point
within it. Each observer in the matter basin is \emph{relationally
central} in their own frame: the basin's geometry is
observer-frame-dependent, with each observer at the height-function
maximum and the equator $S^3$ at maximum geodesic distance.
Different observers within the matter basin see qualitatively
identical physics; only the basin LABEL is observable, and that
label corresponds to a topological invariant of the basin
($\mathbb{Z}_2$ choice between matter and antimatter). The Gram
first-order correction (Part~0 Supplement) preserves the
$\mathbb{Z}_2$ basin equality strictly to machine precision
(verifier: \texttt{tools/research/cascade\_s4\_antipodal\_test.py};
analytic: slicing integrand $(1-x^2)^{d/2}$ is even in $x$ on
$[-1,1]$). The sign anchor is therefore \emph{one bit of
observational input} (basin label), at strict parity with the SM's
$Q_e=-1$ convention.

The cascade structurally predicts:
\begin{itemize}
\item Total cosmic baryon number $=0$ cascade-internally; observed
matter excess is the hemispheric reading (we are in the matter
basin), not Sakharov-violation.
...
beyond our cosmological horizon is in the antipodal basin
(antimatter), causally disconnected. (Falsifier; currently
...
($Y_{Q_L} \in (1/6)\mathbb{Z}$, $Y_{L_L} \in (1/2)\mathbb{Z}$):
closed (from $N_c = 3$ + SU(2) doublet + $J$ eigenvalues $\pm 1$).
\item \emph{Sign anchor}: reframed as basin label (matter or
antimatter) of $S^4$ at $d=5$ --- a single binary topological
invariant, geometric content, parity with SM's $Q_e = -1$.
\end{itemize}
...
sector-fundamental uniqueness theorem; the sign anchor is at parity
with the SM's external observational anchor but with explicit
geometric content (one bit, basin label).
\end{remark}
