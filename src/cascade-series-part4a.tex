...
\item \emph{Open:} the gauge-coupling extension. The cascade has gauge
groups at $d\in\{12,13,14\}$ (Theorem~\ref{thm:adams},
Theorem~\ref{thm:adams-unique}) but no derived coupling of the
fermion field to those gauge fields at the gauge-window layers ---
no cascade-lattice analogue of $\bar\psi\gamma^\mu A_\mu\psi$. This
extension is the new
Open Question~\texttt{oq:fermion-gauge-action} of
Part~IVb~\cite{paperIVb}, articulated specifically because the
Y-spectrum closure (and the $1/\alpha_{\mathrm{em}}$ screening of
\texttt{oq:alpha-em-screening}) require it.
\end{itemize}
The four anomaly conditions are therefore \emph{structurally
downstream} of Part~IVb's
Open Question~\texttt{oq:fermion-gauge-action} specifically (the
gauge-coupled fermion action), not of the broader
\texttt{oq:fermion-cascade-action} (which is now narrowed to the
mass identification $m(d)=\sqrt{\alpha(d)}$ and is largely closed
by the Berezin construction).

This narrows the Y-spectrum gap precisely: it is downstream of a
specific structurally-prior open question (the gauge-coupling
extension of the Berezin partition function), not shallowly open.
A productive attack on the Y-spectrum should first close
\texttt{oq:fermion-gauge-action}, then derive (I)--(IV) from the
resulting gauge-coupled action by standard cascade-internal
reasoning.
