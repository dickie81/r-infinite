...
consistency.
\end{itemize}
The two $\mathrm{U}(1)$s have distinct cascade-internal origins (Adams'
theorem on $S^{13}$ vs.\ forced precession giving $J$), distinct
domains (charged-state subset at $d=14$ vs.\ full state space),
distinct actions (differential vs.\ uniform), and distinct
observability (gauge boson vs.\ gauge-trivial). They are not the same
group viewed at different levels --- they are two cascade structures
both abstractly isomorphic to $\mathrm{U}(1)$. Numerical illustration:
\texttt{tools/research/projective\_u1\_vs\_hypercharge\_u1.py}.

\medskip
\noindent\emph{Speculative connection (research-grade, not load-bearing
on Part~IVa or Part~IVb).} The hypercharge generator at $d=14$ might
be the restriction of the global projective $J$ to the $d=14$ cascade
layer, with Adams' obstruction selecting $d=14$ as the layer where
$J$ manifests as a gauge boson rather than a kinematic gauge.
Verifying this would require deriving the hypercharge coupling $g_Y$
from cascade primitives at $d=14$ (e.g., from
$\alpha(14) = R(14)^2/4$, used downstream in Paper~IVb's gauge
coupling structure) and showing $J|_{d=14}$ reproduces the SM
hypercharge eigenvalues. Flagged here as an open structural question;
no Part~IVa or Part~IVb prediction depends on its resolution.
\end{remark}
