...
\begin{remark}[Reading~III: $\mathrm{SU}(3)$ algebra at $d_0=7$ via $G_2/\mathrm{SU}(3)$]\label{rem:reading-III}
...
\texttt{tools/verifiers/cascade\_d7\_su3\_bs\_closure.py}, Step~1)
...
$G_2$ structure on $S^6$ is the $d=7$ instance of audit
item~10.11.

Reading~III: the cascade's $\mathrm{SU}(3)$ is distributed across
two distinguished layers, both forced:
\begin{enumerate}
...
deficit at $d=12$ alone (3 vs.\ 8 generators) is the audit-flagged
gap that 10.11 fills.

This reading does not change any prediction of Sections~2--4. It
...
by (ii). Verifier
\texttt{tools/verifiers/cascade\_path\_tensor\_product.py} confirms
that the SM rep content $(Q_L: \mathbf{3}\otimes\mathbf{2}\otimes Y$,
etc.) gives 15 Weyl per generation exactly.
\end{remark}
