...
\medskip\noindent\textbf{Toward a derivation: the single-Bott-mirror-axis argument.}
Reading C's required theorem reduces to a uniqueness statement that the cascade already has the ingredients for. The candidate derivation route:

\begin{enumerate}[label=(R\arabic*),nosep]
\item \emph{Adams' uniqueness pins the gauge structure to one Bott orbit.} Theorem~\ref{thm:adams-unique} establishes that $\rho(d)-1=3$ holds at exactly $d=12$ in $[5, d_1=19]$; the gauge group structure is unique to this Bott orbit within the cascade's active subcritical-or-marginal range. Bott periodicity gives images at $d=20, 28, 36, \ldots$ formally, but those layers are past $d_1$, so the gauge structure does not establish there as an independent active gauge window --- it remains structurally rooted at $d=12$.
\item \emph{One gauge window $\Rightarrow$ one Bott mirror axis.} The cascade Bott mirror is the natural reflection at period $8$ in the layer index. Centred on the gauge-window Dirac layer $d=13$ (the SU(2) breaking layer, the only Dirac layer in the gauge window), the mirror maps $d \to 26 - d$ within radius $8$:
\[
d_V = 5 \;\longleftrightarrow\; d=13 \;\longleftrightarrow\; d_{\rm post}=21.
\]
The cascade has \emph{one} structurally distinguished mirror axis at $d=13$ because it has \emph{one} structurally distinguished gauge window (R1).
\item \emph{The mirror's orbit is exactly the SM 3 generations.} The single-axis Bott-mirror orbit through $d=13$ is the triple $\{5, 13, 21\}$ --- one Bott period below the gauge window's Dirac layer, the gauge-window Dirac itself, and one Bott period above. These are precisely the SM charged-fermion generation layers.
\item \emph{Higher Bott Dirac layers are outside the mirror orbit.} The next Dirac layer $d=29 = 13 + 16$ is two Bott periods from the mirror axis. A Bott mirror centred at $d=13$ with radius $8$ does not reach $d=29$. To include $d=29$ in a Bott-mirror orbit would require a \emph{second} mirror axis (e.g.\ centred at $d=21$ with radius $8$, mapping $d=13 \leftrightarrow d=29$) --- but the cascade has no second mirror axis to embed, by the uniqueness of step (R1).
\item \emph{Therefore charged-matter Bott-replication terminates at $\{5, 13, 21\}$.} Higher Bott Dirac layers ($d=29, 37, 45, \ldots$) are outside the cascade's unique Bott-mirror orbit. They remain cascade primitives (sphere areas, mass-formula values, slicing data) but they don't carry path-tensor charge structure --- hence no propagating gauge-coupled fermion content, hence no dynamical observer-frame role, hence (by Part~II=III's time-emergence-from-propagator framing) no time-coupling.
\end{enumerate}

This route uses only Part~IVa primitives (Bott periodicity 8, Bott mirror, Adams' uniqueness theorem) plus Part~II=III's time-from-propagator identification. No semiclassics; cascade-lattice and Bott-classification ingredients only. The conclusion --- exactly three charged Bott Dirac layers, with higher Bott layers as static-vacuum primitives --- matches both the SM observation and the $1/\alpha_{\rm em}$ closure precision exactly, with $N_{\rm gen}=3$ replaced by ``cardinality of the cascade's unique Bott-mirror orbit''.

\medskip\noindent\emph{What the route depends on.} Three structural assertions need to be cleanly stated and proved:
\begin{itemize}[nosep]
\item \emph{(R1-extension), follows from the uniqueness of $d_0=7$.} The cascade's $\mathrm{SU}(3)$ gauge structure at $d=12$ rests on \emph{two} ingredients (Remark~\ref{rem:su3-d7-algebra}): (i) Adams' theorem giving $\rho(12)-1=3$ nowhere-zero tangent fields on $S^{11}$, with $\mathbb{R}^{12}=\mathbb{H}^3$ realising the three colours as the three quaternionic factors; and (ii) the full $\mathfrak{su}(3)$ Lie-algebra closure supplied by the $G_2/\mathrm{SU}(3)$ homogeneous-space structure at the area maximum $d_0=7$. Adams alone is insufficient --- $\rho(d)-1=3$ at $S^{d-1}$ gives three vector fields that close as $\mathfrak{su}(2)$ via quaternionic right-mults, not the 8-dimensional $\mathfrak{su}(3)$ algebra. The full algebra requires the $G_2/\mathrm{SU}(3)$ homogeneous space at $S^6$, which lives at $d_0=7$. \emph{And $d_0=7$ is structurally unique:} Paper~I~\cite{paperI} Theorem~7.1 establishes $d_0$ as the cascade's unique area maximum; there is no second area-maximum-equivalent layer in the cascade tower. Higher Bott images of $d=12$ (the layers $d=20, 28, 36, \ldots$) have Adams $\rho(d)-1=3$ structurally, but they lack the corresponding algebra-closure layer ($d_0$ does not have a Bott replica past $d_1$ --- it is one of Paper~I's four distinguished critical points $\{d_V,d_0,d_1,d_2\}$, which are uniquely determined by the Gamma function). Therefore active $\mathrm{SU}(3)$ gauge structure exists only at $d=12$. The same uniqueness of distinguished cascade primitives argument applies to $\mathrm{SU}(2)$ at $d=13$ (anchored to the gauge window's Dirac layer, with Lefschetz obstruction on $S^{12}$) and $\mathrm{U}(1)$ at $d=14$ (anchored to the cascade complex structure $J$, single-occurrence per Paper~II~\cite{paperII}). This proves (R1-extension) from existing Part~IVa primitives plus Paper~I's uniqueness of $d_0$.
\item \emph{(R2-extension), follows from (R1-extension) plus path-tensor uniqueness.} The path-tensor structure $V_{12}\otimes V_{13}\otimes V_{14}$ at the gauge window (Remark~\ref{rem:path-tensor}) is the cascade's only charge-carrying tensor structure --- it lives at the unique gauge window of (R1-extension) and nowhere else. The Bott mirror is the natural reflection at period $8$ in the layer index; for it to ``act'' as a structural identification on charged matter, it must be anchored to a layer carrying path-tensor structure. The unique Dirac layer of the gauge window is $d=13$ (the SU(2) breaking layer, the only $d \bmod 8 = 5$ layer in $\{12,13,14\}$). Therefore the mirror has a unique centre at $d=13$.
\item \emph{(R3-cardinality), the orbit is closed at three layers by the cascade range.} A Bott mirror at axis $d=13$ acts as $d \mapsto 26 - d$. A reflection orbit must be closed: every element's image must also be in the orbit. The orbit $\{5, 13, 21\}$ is closed --- $d=5 \leftrightarrow d=21$, $d=13$ fixed, all three Dirac (residue $5 \bmod 8$). Extending the orbit further along the Bott-period direction would require including $d=29$ (Dirac, the next Bott image of $d=21$), but the closure condition then demands the inclusion of its mirror image $d = 26 - 29 = -3$ --- which lies below the cascade's lower endpoint $d_V = 5$ (the volume maximum, Paper~I~\cite{paperI}~Theorem~7.1) and does not exist in the cascade range $[d_V, d_2]$. Therefore $d=29$ cannot be added without breaking orbit closure, and the orbit has cascade-realised cardinality exactly $3$. The cardinality is forced by the placement of the lower bound $d_V$ relative to the gauge-window axis $d=13$: there is room for exactly one Bott-period image below $d=13$ (namely $d=5$, the cascade's volume maximum) and one above (namely $d=21$), giving three orbit elements.
\end{itemize}
With (R1-extension) derived from the uniqueness of $d_0=7$ (Paper~I) and the $\mathrm{SU}(3)$-algebra structure of Remark~\ref{rem:su3-d7-algebra}, (R2-extension) following from path-tensor uniqueness, and (R3-cardinality) a counting fact about Bott reflection at its natural period, the full Reading-C theorem assembles cleanly from existing Part~I and Part~IVa primitives. The cascade structurally predicts $N_{\rm gen}=3$ via ``cardinality of the Bott-mirror orbit through the unique gauge window'', not as an empirical input.
