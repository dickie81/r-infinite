...
cascade fermion action coupling matter to the U(1)$_Y$ gauge
field at $d=14$ --- and this is the existing open question
\texttt{oq:fermion-cascade-action} of Part~IVb (the cascade has
a derived scalar action $S[\varphi]=\sum_d(2\alpha(d))^{-1}(\Delta\varphi)^2$,
Part~IVb Remark~\texttt{rem:action-uniqueness}, but no derived
fermion analogue). The four anomaly conditions are therefore
\emph{structurally downstream} of \texttt{oq:fermion-cascade-action}:
they cannot be formulated cascade-natively until a fermion action
exists.

This narrows the Y-spectrum gap: it is not shallowly open but is
\emph{downstream of a deeper open question already on Part~IVb's
docket}. A productive attack on the Y spectrum should first close
\texttt{oq:fermion-cascade-action}, then derive (I)--(IV) from
that action by standard Fujikawa-style or cascade-internal
reasoning.
