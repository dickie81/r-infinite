...
in~\cite{paper2} Section~7.1 as one quantum of \emph{dimensional collapse}: each
...
Paper~V~\cite{paper5} documents an earlier derivation that conflated
...
Paper~I~\cite{paper1} boundary dominance), indexed by the cascade
...
An earlier version of Paper~V~\cite{paper5} attempted to identify
...
and is retracted in Paper~V Section~6 (``Why the observer bridge and
...
projection factor of Paper~I~\cite{paper1} §3.2, not $N(4)^2$. Under
...
Paper~II~\cite{paper2} Section~7.1 identifies the cascade's slicing
...
\section{Deriving \texorpdfstring{$d = 4$}{d = 4}}
...
From Theorem~6.1 of~\cite{paper2}: the orthogonality axiom forces the
...
From Paper~II~\cite{paper2} Theorems~6.1 and~7.1, the forced precession
...
Hilbert space $\mathcal{H}$ (Paper~II~\cite{paper2}). The inner product
...
\begin{itemize}[nosep]
\item The cascade does not \emph{avoid} Wick rotation; it \emph{motivates} it. Claims to the contrary in earlier drafts (and in this paper's abstract before revision, Paper~V Section~6's retracted framing, etc.) are retracted in favour of the honest statement.
...
\section{Scale Factors: \texorpdfstring{$G$ and $\hbar$}{G and hbar}}
...
\section{No Gravitons: Metric Degrees of Freedom without Second Quantisation}
...
(Paper~I, Theorem~\xref{paper1}{thm:reff}). These are geometric degrees of freedom of the
...
We install the Lorentzian cascade metric directly from
Theorem~\ref{thm:lorentzian}: under the Wick-rotation identification
$x = it$ (Remark~\ref{rem:wick-rotation-cascade}), the cascade's
Euclidean slicing metric $dx^2 + (1-x^2)\,d\sigma_{n-1}^2$ maps to the
Lorentzian form below. The $n$-dimensional Lorentzian cascade metric is:
\[
...
three inputs: the propagator phase classification ([2], Corollary 6.8; period-8 refinement in this paper, Section 9), which establishes the
...
generations; and the uniqueness of sphere areas as cascade content (Corollary 3.2 of [1]),
...
Paper~II\,=\,III shows this is unnecessary because both are projections of the
same cascade state.
...
\begin{enumerate}
\item \textbf{The Standard Model.} Resolved in Papers IVa--IVb: the gauge
group $\text{SU}(3) \times \text{SU}(2) \times \text{U}(1)$ follows from
Bott periodicity and Adams' theorem applied to the cascade's layer
structure; three fermion generations from the Bott partition cut off at
the $d_1=19$ phase transition; the mass spectrum from the
geometric-topological factorisation.
\item \textbf{Quantum gravity.} Resolved in Paper~II\,=\,III: the quantum and
gravitational projections share the same source, propagator, and state space.
Double uniqueness (Gleason $+$ Lovelock at $d=4$) proves mutual consistency.
The metric is a state property; gravity was always quantum in the cascade.
\item \textbf{Black holes.} Resolved in Paper~II\,=\,III: boundary dominance gives
$S = A/4$ at $d = 4$, $S = A/3$ at $d = 3$ (BTZ cross-check), and the
Hawking temperature $T = 1/(8\pi M)$ follows from Birkhoff's theorem with no
semiclassical input. Remaining open questions (thermal spectrum, Page curve,
Kerr/RN entropy, second route to $G_d = d/4$) are listed there.
\item \textbf{Newton's constant.} The cascade produces dimensionless quantities;
$G$ enters as a unit-matching constant from the embedding of the observer's
4D slice in the 217-dimensional cascade. A cascade-native derivation of $G$
would come from the discrete action of Paper~IVb Remark~4.6 evaluated with
gravitational couplings, or from a second structural identity giving $G_d = d/4$
directly (Paper~II=III Open Question~4). \emph{Not a Kaluza--Klein problem}:
the cascade explicitly refuses semiclassical reduction over compactified
dimensions (Paper~I \S3.2, Paper~II=III \S5--6, Paper~III \S12); the metric
is a state property, not an operator on a fixed background, so there is no
``KK mass spectrum'' to derive $G$ from. Cascade-native alternative: the
action-principle approach now available in Paper~IVb (Remark~4.8).
...
connecting the cascade's layer structure to the physical expansion history via
classical differential geometry (Gauss--Codazzi, not semiclassical reduction)
remains open.
\item \textbf{The Hubble tension.} Under Paper~I's locked-time 3D projection
and Paper~V's Friedmann derivation, the cascade's $H_0 = 66.78$~km/s/Mpc
is essentially equal to Planck's $67.4$ after the Part~0 Gram
first-order correction, and is \emph{incompatible} with SH0ES's $73.0$ at the
$\sim 8\%$ level. Earlier phrasing that the cascade sits ``between the two
principal measurements'' has been retracted (Paper~V \S6 remark); the
cascade picks the Planck side. The full cosmological derivation is carried
out in Paper~V.
\end{enumerate}

\begin{thebibliography}{12}

\bibitem{paper0pre} {[Author]}, \extlink{\cascadebase/cascade-series-prelude.pdf}{\textit{The Cascade Series --- Prelude: Why Nothing Has Structure}}, March 2026.
