...
obstruction. Two members of the family have been identified, closing three Standard
Model precision observables at experimental precision with zero fitted parameters:
\begin{itemize}
\item[$\bullet$] $\delta\Phi_{\mathrm{U}(1)}=\alpha(14)/\chi$ (sourced at the
$\mathrm{U}(1)$ gauge layer $d=14$) closes $\alpha_s(M_Z)=0.117917$ ($+0.019\sigma$)
and $m_\tau/m_\mu=16.81731$ ($+0.243\sigma$) in a single identity;
\item[$\bullet$] $\delta\Phi_{\mathrm{phase}}=\alpha(19)/\chi$ (sourced at Paper~I's
phase-transition threshold $d_1=19$) closes the $\tau$ absolute mass $m_\tau=1776.82$~MeV
at $-0.31\sigma$.
\end{itemize}
Both shifts are closed forms in $\Gamma$ function values. Falsification requires
completing an approximate derivation to full exactness and finding a result that does
not fit, or identifying an observable class in Parts~IV--V whose shift falls outside
the $\alpha(d^*)/\chi$ family.
