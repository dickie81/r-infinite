...
\item \emph{Analytic formula for $\epsilon(n, d_0)$.} Closed in cascade slicing ratios via the power-trace bound family $U_k = (\mathrm{tr}\,C^k)^{1/k}$ (Corollary~\ref{cor:lambda1-power-trace}): each $U_k$ is a closed-form sum of $R$-ratio products, and $U_k \downarrow \lambda_1$ at geometric rate $(\lambda_2/\lambda_1)^k$, reaching $\sim 10^{-12}$ at $k = 4$ and machine precision at $k = 6$ for cascade-physics paths. At $n = 2$ a single-equation exact form holds (Corollary~\ref{cor:lambda1-n2}). What remains genuinely open is whether a single-equation closed-form equality (rather than a convergent closed-form family) exists at $n \geq 3$: a radical-form solution for general $n$ is obstructed in principle at $n \geq 5$ by Abel--Ruffini for unstructured matrices, but the cascade matrix's Hankel structure leaves room for a non-radical equality (e.g.\ via special-function identities for the moment problem of $\mu_m$, identified by Remark~\ref{rem:lambda1-OP-factorisation} as a question in classical OP theory).
