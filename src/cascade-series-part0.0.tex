...
\end{equation}
where $R(d) = \Gamma((d+1)/2)/\Gamma((d+2)/2)$ is the cascade slicing ratio of Paper~0 \S2. Numerical verification at all tested $d$ values is in \texttt{tools/verifiers/gram\_compliance\_laplacian.py} (closed form to machine precision; the Laplacian identity of Corollary~\ref{cor:gram-laplacian} below to $\sim 10^{-7}$ relative error, the floating-point cancellation noise floor at large $d$).
\end{theorem}
