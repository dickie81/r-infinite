...
\begin{remark}[Relationship to the fitted $k_Q$]
The first-order correction~\eqref{eq:first-order} and the eigenvalue-deficit parametrisation~\eqref{eq:correction} are related by $k_Q^{(1)} = \sum(1-C^2)/\epsilon$. The fitted $k_Q$ values (2.11, 1.66, 2.67) include a second-order observable-dependent factor from the projection of the sensitivity profile onto the subdominant eigenvector $v_2$. Equation~\eqref{eq:first-order} is the universal (observable-independent) leading order; the residual variation is second order in the eigenvalue deficit.
\end{remark}
...
\begin{enumerate}[nosep]
\item \emph{Derive the second-order (observable-dependent) correction.} The first-order correction $\delta Q/Q_0 = \sum(1-C^2)$ (Theorem~\ref{thm:first-order}) is derived and reduces descent deviations from $\sim\!2\%$ to sub-$1\%$. The residual variation (the ratio of observed correction to first-order prediction, ranging from 1.1 to 1.9 across observables) reflects the observable's projection onto the subdominant eigenvector $v_2$, which is weighted toward the ends of the path. Deriving this second-order factor requires computing the projection of the observable's sensitivity profile $w(d) = p(d)$ onto $v_2$ and relating it to the correction via the eigenvalue $\lambda_2$. This is a linear algebra problem with Beta function entries---a computation, not a conceptual gap.
