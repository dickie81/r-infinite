...
The cascade's leading-order predictions treat each compactification step as independent. A quantity that depends on the cascade's geometry over a path of $n$ layers receives a correction from the inter-layer coupling. The eigenvalue deficit $\epsilon$ (Theorem~\ref{thm:eigenvalue}) measures the magnitude of this coupling, and the adjacent overlap deficits $1-C^2_{d,d+1}$ (Theorem~\ref{thm:overlap}) measure its per-pair structure. Section~\ref{sec:first-order} derives the multiplicative correction from the same compliance function $\alpha(d) = R(d)^2/4$ that drives the slicing mechanism (Remark~\ref{rem:gram-action-unification}).
...
The geometric correction $\delta\Phi = \sum(1-C^2_{d,d+1})$ (Theorem~\ref{thm:first-order}) is strictly positive, since each adjacent overlap satisfies $C_{d,d+1}<1$ by Cauchy--Schwarz. Applied multiplicatively as $Q = Q_0\cdot\exp(\delta\Phi)$ (Corollary~\ref{cor:exp-resum}), it always increases the predicted value. This produces the observed sign structure across all cascade predictions:
...
The independent-step approximation corresponds to the limit $R_{\mathrm{eff}} \to 0$ (complete compactification). The geometric correction $\delta\Phi = \sum(1-C^2)$ is the leading-order cost of $R_{\mathrm{eff}} > 0$---the imperfect folding of the cascade's own geometry.
