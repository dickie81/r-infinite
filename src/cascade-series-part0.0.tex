...
\begin{corollary}[Exponential resummation of the per-step corrections]\label{cor:exp-resum}
For a multiplicative propagator $Q_0 = \prod_k (\text{step}_k)$ traversing
the same path as in Theorem~\ref{thm:first-order}, the full (all-orders)
Gram-corrected observable is
\begin{equation}\label{eq:exp-form}
Q \;=\; Q_0\cdot\exp\!\Bigl(\sum_{k=0}^{n-2}\bigl(1-C_{d_k,d_{k+1}}^2\bigr)\Bigr),
\end{equation}
agreeing with Theorem~\ref{thm:first-order} to first order.
\end{corollary}

\begin{proof}
Because the cascade recurrence is first-order multiplicative
(Corollary~3.2 of Paper~0), the Gram-corrected step at layer $d_k$
receives a multiplicative factor $1 + (1 - C_{d_k,d_{k+1}}^2) +
O\bigl((1-C^2)^2\bigr)$, accumulating as a product across layers:
\[
Q \;=\; Q_0\prod_{k=0}^{n-2}\bigl(1 + (1 - C_{d_k,d_{k+1}}^2) + O((1-C^2)^2)\bigr).
\]
Each per-step deficit scales as $1 - C_{d,d+1}^2 \sim 1/(8d^2)$
(Theorem~\ref{thm:overlap}), so for every path in the Part~0 tower the
deficits are uniformly small. Under this smallness condition,
$\prod_k(1 + x_k) = \exp\bigl(\sum_k \ln(1 + x_k)\bigr)
= \exp\bigl(\sum_k x_k - \tfrac{1}{2}\sum_k x_k^2 + \cdots\bigr)$,
and the leading order in $x_k$ gives~\eqref{eq:exp-form}. Expanding the
exponential to first order returns
$Q/Q_0 = 1 + \sum(1-C^2) + O((1-C^2)^2)$, recovering
Theorem~\ref{thm:first-order}. For the six paths in the verification
table above, the first-order linear form and the exponential
resummation agree to $O(10^{-4})$ (numerically:
$1 + 0.02108 = 1.02108$ versus $\exp(0.02108) = 1.02130$ for the
longest path), which is below the reported observational precisions.
The exponential form is therefore the natural closed-form expression
of the same correction and is the form used downstream (Paper~I,
$\rho_\Lambda$ closure; Paper~IVb, the $v$ and $m_\tau$ absolute-mass
shifts).
\end{proof}
...
\item \emph{Analytic formula for $\epsilon(n, d_0)$.} The deficit is computable numerically from the Gram eigenvalue. The perturbation theory (Theorem~\ref{thm:pert}) gives an explicit first-order expression as a sum of Beta function ratios, with second-order correction bounded by $\|D\|^2$; an analytic closed form in terms of $n$, $d_0$, and the Beta function would extend the correction to all cascade paths without numerical eigenvalue computation.
...
The first-order correction (Theorem~\ref{thm:first-order}) is derived: it is a sum of Beta function ratios with no free parameters, reducing descent deviations from sub-$2\%$ to sub-$1\%$. The observable response factor (the subleading correction beyond the first-order Gram identity) is the remaining open problem at the level of the Gram-matrix framework of this supplement. With both the first-order Gram identity and the observable response factor derived, descent-dependent predictions would close to sub-$0.1\%$. Open Question~1 is the calculation that separates the two at the Gram-matrix level.
\end{remark}
