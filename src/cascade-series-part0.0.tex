...
For $n \geq 3$ no simple closed form for $\lambda_1$ in cascade slicing ratios exists. Numerical exploration of the dominant eigenvector $v_1$ across $(n, d_0)$ samples in the cascade-physics range shows that the rescaled coordinates $v_1[i]/\|x^i\|$ are not a geometric, rational, or recognisably analytic sequence: the ratios $v_1[i+1]/v_1[i]$ vary slowly with $i$ in a way controlled jointly by $n$ and $d_0$, with no closed-form generator (\texttt{tools/research/lambda1\_closed\_form\_search.py}). The closed-form expressions that do exist are therefore (i)~the trace identities $\mathrm{tr}\,C^k$ as polynomial sums of slicing ratios, (ii)~the $n = 2$ identity above, and (iii)~the upper bound of Theorem~\ref{thm:pert}, which is exact at $n = 2$ and tight (relative error $< 10^{-3}$) for all cascade-physics paths tested ($d_0 \geq 5$, $n \leq 200$): \texttt{tools/research/lambda1\_closed\_form\_bounds.py} verifies $\epsilon_{\mathrm{exact}} / \epsilon_{\mathrm{upper}} > 0.99$ uniformly. The first-order Beta-function-ratio formula of Theorem~\ref{thm:pert} is the analytic closed form requested by the open question on $\epsilon(n, d_0)$ below; what cannot be promoted to an equality for $n \geq 3$ is the eigenvalue itself, only its tight upper bound.
\end{remark}

\begin{remark}[Moment-problem reduction and orthogonal-polynomial factorisation]\label{rem:lambda1-OP-factorisation}
The cascade correlation matrix admits the structural identity
\[
C \;=\; A A^{\!\top}, \qquad A_{ik} \;=\; \frac{\langle p_k,\, x^i\rangle_{\mu_m}}{\|x^i\|_{\mu_m}},
\]
where $\{p_k\}_{k=0}^{n-1}$ is the orthonormal-polynomial family for the measure $d\mu_m(x) = x^m/\sqrt{1-x^2}\,dx$ on $[0,1]$ with $m = 2 d_0 + 1$, and $A$ is lower triangular. Hence $\lambda_k(C) = \sigma_k(A)^2$. The substitution $u = x^2$ identifies $\mu_m$ as a shifted Jacobi measure $u^{(m-1)/2}(1-u)^{-1/2}\,du$ with parameters $(\alpha,\beta) = ((m-1)/2,\,-1/2)$, so the closed-form question reduces to a problem in classical orthogonal-polynomial theory; the residual obstruction is that the monomial basis $\{x^i\}$ mixes the even and odd subspaces under $u = x^2$, so the OP family for $\mu_m$ in variable $x$ is not directly a Jacobi family but is determined by Gram--Schmidt of $\{1, x, x^2, \ldots\}$ against $\mu_m$ (\texttt{tools/research/lambda1\_jacobi\_route.py}). Setting the OP-basis Rayleigh test vector to $e_0$ (the constant polynomial $p_0$) gives the cascade-native lower bound
\[
\lambda_1(C) \;\geq\; \sum_{i=0}^{n-1} C_{0,i}^2 \;=\; \sum_{i=0}^{n-1} \frac{R(m+i)^2}{R(m)\, R(m+2i)},
\]
which is closed-form in slicing ratios but weaker than the perturbation-theory bound of Theorem~\ref{thm:pert} (the dominant right singular vector of $A$ is dominantly $e_0$ but with non-negligible higher OP components). The factorisation reframes the open question: a closed form for $\lambda_1$ at $n \geq 3$ would be a closed form for the operator norm of the OP-coefficient matrix of normalised monomials in a shifted-Jacobi-related measure---a question with established machinery (Christoffel--Darboux kernel asymptotics, Hankel-determinant formulas, Szeg\H{o}/Widom theory) that has so far yielded only asymptotic, not exact, results in this setting.
\end{remark}

\begin{remark}[Stieltjes recurrence: exact closed forms at $k = 0$, non-classical for $k \geq 1$]\label{rem:lambda1-stieltjes}
The OP family for $\mu_m$ is determined by its three-term recurrence $x p_k = b_k p_{k+1} + a_k p_k + b_{k-1} p_{k-1}$, equivalent to the Jacobi (multiplication-by-$x$) matrix $J_n = \mathrm{tridiag}(b_{k-1}, a_k, b_k)$. The cascade-native closed forms at $k = 0$ are
\[
a_0 \;=\; \frac{R(m+1)}{R(m)}, \qquad b_0^2 \;=\; \frac{R(m+2)}{R(m)} - \left(\frac{R(m+1)}{R(m)}\right)^{\!2} \;=\; \frac{R(m)R(m+2) - R(m+1)^2}{R(m)^2},
\]
verified to $\sim\!10^{-78}$ relative precision (\texttt{tools/research/lambda1\_recurrence\_asymptotics.py}). For $k \geq 1$ the Wallis-ratio hypothesis $a_k = R(m+2k+1)/R(m+2k)$ fails immediately, and no simple cascade-native closed form has been identified across $(d_0, k)$ samples: the Stieltjes algorithm at 80-digit precision converges through $k \approx 25$ before the moment matrix's exponential conditioning exhausts the precision, and the resulting $(a_k, b_k)$ sequences are smooth but match no classical OP family (Jacobi, Bessel, Hermite, Laguerre). This is the structural diagnostic for the cascade OP family being non-classical: classical families have rational closed-form recurrence coefficients, and the cascade measure does not. The asymptotic limits $a_k \to 1/2$, $b_k \to 1/4$ as $k \to \infty$ are the universal Geronimus--Szeg\H{o} limits for measures on $[0,1]$ with positive density on the open interval; numerical evidence further suggests the convergence is \emph{faster} than the generic $O(1/k)$ Bernstein--Szeg\H{o} rate ($(a_k - 1/2) \cdot k$ peaks and then decreases), reflecting additional regularity from the cascade weight's $\Gamma$-function structure. A formal characterisation of this faster-than-$O(1/k)$ rate is a sharp open question.
\end{remark}
...
\item \emph{Analytic formula for $\epsilon(n, d_0)$.} Partially closed. At $n = 2$ the dominant eigenvalue admits the exact closed form $\lambda_1 = 1 + R(2d_0+2)/\sqrt{R(2d_0+1)R(2d_0+3)}$ (Corollary~\ref{cor:lambda1-n2}), so $\epsilon(2, d_0)$ is closed-form in cascade slicing ratios. For $n \geq 3$ no simple closed-form equality exists (Remark~\ref{rem:lambda1-no-closed-form}); the perturbation-theory upper bound of Theorem~\ref{thm:pert} is exact at $n = 2$ and tight to $< 10^{-3}$ relative error for all cascade-physics paths tested, so the cascade correction is computable in closed form to that precision without numerical eigenvalue computation. Remark~\ref{rem:lambda1-OP-factorisation} sharpens the open part of the question: $\lambda_1$ is the squared operator norm of the orthonormal-polynomial expansion matrix of normalised monomials in $L^2([0,1], \mu_m)$, with $\mu_m$ a shifted Jacobi measure under $u = x^2$. Closed-form equality at $n \geq 3$ thus reduces to a question about the spectrum of a structured Hankel-related triangular matrix with entries explicit in $\Gamma$-function ratios; classical OP machinery (Christoffel--Darboux, Hankel determinants, Szeg\H{o}/Widom asymptotics) is available but has so far yielded only asymptotic, not exact, results in this setting. A radical-form solution for general $n$ is obstructed in principle at $n \geq 5$ by Abel--Ruffini for unstructured matrices; the cascade matrix's Hankel structure leaves room for a non-radical closed form (e.g.\ via special-function identities for the moment problem of $\mu_m$).
