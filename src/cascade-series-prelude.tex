...
% Cross-paper hyperlinking infrastructure (see cascade-series-part1.tex
% for the full commentary). Prelude has cross-paper prose refs but no
% bibliography of its own, so it gets the \xref/\xrhyperdoc machinery
% without bibitems for the cited papers -- the cite key isn't used.
\edef\xrhash{\string#}
\makeatletter
...
\usepackage{graphicx}
\usepackage{xr-hyper}
\xrhyperdoc{paper0}{cascade-series-part0}
...
tick of cosmic time. Part~II, Section~\xref{paperII}{subsec:time-id} makes the static-versus-
...
(Part~0, Theorem~\xref{paper0}{thm:irred}: the recurrence's irreducible residue is forced
by orthogonality on the cardinality-$d$ realisation).
\end{itemize}

The resolution mechanism itself is also forced given the tower: the
slicing recurrence is the unique first-order multiplicative recurrence
on the tower of unit balls (Part~0, Theorem~\xref{paper0}{thm:irred}).
