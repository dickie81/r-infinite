...
theory enters in Section~\ref{sec:integer-structure}; geometry in
...
half-circle) is a downstream consequence (Section~\ref{sec:numbers-to-geometry}):
...
in Section~\ref{sec:numbers-to-geometry}. The cascade's foundational
...
selected in Section~\ref{sec:integer-structure} already supplies a
continuum (the computable reals of $\mathrm{RCA}_0$ with the
monotone-limit closure of $\mathrm{MCT}$). Cumulation
(Section~\ref{sec:distinction-and-cumulation}) is silent on questions
of magnitude --- it records whether states are distinguishable, not
\emph{how} --- so the cumulated states need a continuous container
before any non-binary geometric relation between them can be
expressed. The question is what \emph{shape} that container takes.
...
3 & Analytic-limits foundation $\mathrm{RCA}_0 + \mathrm{MCT}$ for downstream content (sphere areas, ball volumes, $d\to\infty$ limits) & Minimum classical fragment, Section~\ref{sec:integer-structure} & (ii) \\
...
$d\to\infty$ content (Section~\ref{sec:integer-structure});
...
(Section~\ref{sec:integer-structure}; status box at the head of
