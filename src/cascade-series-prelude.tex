...
$\pi$ as a specific real from the integer structure alone, and the
vector-space realisation $\mathbb{R}^d$ forced by Hurwitz's theorem
under a minimum-strength clause of austerity; the inner product on
$\mathbb{R}^d$ inherits its angular constant from the same $\pi$, and
orthogonality between distinguishable states is recovered as the
extremal $\pi/2$ angular case forced by (B), (S), (L). Every step is forced by an austerity clause or by a classical
...
% ============================================================
\section{Distinction and Cumulation}\label{sec:distinction-implies-orthogonality}
% ============================================================
...
Section~\ref{sec:the-dimension-tower}; the geometric content (vector
spaces, inner products, orthogonality at $\pi/2$) is derived in
Section~\ref{sec:graded-extension}, after $\pi$ has been forced from
...
follow from this integer cardinality combined with the minimum
analytic-limits foundation forced by austerity clause~(ii). Number
theory enters in Section~\ref{sec:the-dimension-tower}; geometry in
Section~\ref{sec:graded-extension}. Cumulation by itself is purely
combinatorial.

% ============================================================
\section{The Integer Structure: Cardinality and \texorpdfstring{$\pi$}{pi}}\label{sec:the-dimension-tower}
% ============================================================
...
by clauses (i) and (ii) of austerity; (ii)~within the minimum
analytic-limits foundation that clause (ii) selects, the integer
structure of $\mathbb{N}$ forces $\pi$ as a specific real via the
Euler product on primes. Both results are number-theoretic; neither
imports any geometric content.
...
$d$ is derived in Section~\ref{sec:graded-extension}; this theorem
...
combinatorial structure of Section~\ref{sec:distinction-implies-orthogonality})
...
asymptotic limits as $d\to\infty$ --- is inherently analytic, with
content not expressible in $\mathrm{RCA}_0$ alone. Some analytic
foundation must be invoked. Principle~\ref{princ:austerity}
clause~(ii) (minimal strength) selects the minimum sufficient
foundation: the analytic-limits sub-theory of $\mathrm{ACA}_0$ over
$\mathrm{RCA}_0$, equivalently $\mathrm{RCA}_0 + \mathrm{MCT}$
\cite{simpson2009}, where $\mathrm{MCT}$ is the Monotone Convergence
Theorem. Stronger fragments ($\mathrm{ATR}_0$,
$\Pi^1_1\text{-}\mathrm{CA}_0$) exceed what the cascade requires;
weaker ones cannot support the slicing-recurrence integral;
alternative foundations (constructive, intuitionistic,
type-theoretic) import strictly more axioms. $\mathrm{MCT}$ cannot
be cascade-derived; it is the cascade's single irreducible
foundational commitment beyond Definition~\ref{def:axiom}.

\begin{theorem}[The integer structure forces $\pi$]\label{thm:pi-from-primes}
Within the foundation $\mathrm{RCA}_0 + \mathrm{MCT}$ on the
cumulation's integer structure $\mathbb{N}$, the Euler product over
primes forces the existence of $\pi$ as a specific real:
\[
\zeta(2) \;=\; \sum_{n=1}^{\infty} \frac{1}{n^2}
\;=\; \prod_{p\,\mathrm{prime}} \frac{1}{1 - p^{-2}}
\;=\; \frac{\pi^2}{6},
\qquad
\pi^2 \;=\; 6\,\prod_{p\,\mathrm{prime}}\frac{p^2}{p^2 - 1}.
\]
By uniqueness of $\pi$ in standard mathematics (a single positive
real with this property), the Euler-product $\pi$ is identified with
every other classical occurrence of the constant. The integer
structure of $\mathbb{N}$ alone, without any geometric input, forces
this specific real.
\end{theorem}

\begin{proof}
$\zeta(2) = \pi^2/6$ is Euler's 1734 solution to the Basel problem;
the Euler-product identity is the 1737 result that the same value
equals the product over primes (proof: expand each factor as a
geometric series and apply the fundamental theorem of arithmetic).
Both rest on convergence of the corresponding series and product,
which is provable within $\mathrm{RCA}_0 + \mathrm{MCT}$. Uniqueness
of $\pi$ as the constant with this property is elementary:
$\zeta(2)$ is a specific computable real, and $\pi^2 = 6\zeta(2)$
determines a unique positive square root.
\end{proof}

\begin{remark}[$\pi$ enters from number theory, not from geometry]
Theorem~\ref{thm:pi-from-primes} forces $\pi$ as a specific real via
the integer/prime structure of $\mathbb{N}$ within
$\mathrm{RCA}_0 + \mathrm{MCT}$. No vector space, no inner product,
no angle, no Euclidean structure has yet been introduced. The
identification of this Euler-product $\pi$ with the angular constant
($\pi/2$ for orthogonality, $2\pi$ for a full circle, $\pi$ for the
half-circle) is a downstream consequence (Section~\ref{sec:graded-extension}):
when geometry is derived, its $\pi$ is necessarily the same constant,
by uniqueness. The cascade's Euclidean geometry inherits $\pi$ from
number theory rather than supplying it.
\end{remark}
...
$\mathbb{R}^d$ in Section~\ref{sec:graded-extension}) indexed by
...
is forced by the Euler product on $\mathbb{N}$ within
$\mathrm{RCA}_0 + \mathrm{MCT}$ (Theorem~\ref{thm:pi-from-primes});
the choice of $\mathbb{R}^d$ as the geometric realisation is derived
in Section~\ref{sec:graded-extension}. The cascade's foundational
footprint stays at $\mathrm{RCA}_0$ for the cardinal tower's
existence and at $\mathrm{RCA}_0 + \mathrm{MCT}$ for the
classical-analysis content (integration, the Gamma function)
developed in subsequent sections.

% ============================================================
\section{From Numbers to Euclidean Geometry}\label{sec:graded-extension}
% ============================================================

Section~\ref{sec:the-dimension-tower} forced the cardinality
$d\in\mathbb{N}$ from cumulation and clauses~(i)--(ii) of austerity,
and forced $\pi$ as a specific real from the Euler product within
$\mathrm{RCA}_0 + \mathrm{MCT}$. Both results live on the integer
...
The analytic-limits foundation $\mathrm{RCA}_0 + \mathrm{MCT}$
selected in Section~\ref{sec:the-dimension-tower} already supplies a
continuum (the computable reals of $\mathrm{RCA}_0$ with the
monotone-limit closure of $\mathrm{MCT}$); the question is what
\emph{shape} that continuum takes when it carries the cumulated
states.
...
unit vectors ranges over $[0, \pi]$, with $0$ corresponding to
identical states, $\pi/2$ to orthogonal, $\pi$ to antipodal. The
specific value $\pi$ is the same one forced by
Theorem~\ref{thm:pi-from-primes} via the Euler product on
$\mathbb{N}$: there is only one $\pi$ in standard mathematics, and
the geometric angular extent inherits it from the number-theoretic
constant. No new analytic content is introduced.
...
Section~\ref{sec:distinction-implies-orthogonality} be realised as
...
gradation: between two cumulated states, the geometric overlap takes
an extremal value of graded sharing on the inner-product scale
$[-1, 1]$, not an intermediate one. The available extrema are $+1$
(identical), $0$ (orthogonal), and $-1$ (antipodal): $+1$ is excluded
because the states are distinct, $-1$ is excluded because it imposes
a privileged pairing structure (antipodal partners) that has no
analogue in (B) --- distinguishability is a relation between two
states, not a coupling that links one state to a unique opposite. The
extremal value compatible with all three of (B), (S), and (L) is $0$.
...
$\mathbb{N}$ (Section~\ref{sec:the-dimension-tower}), vector-space
...
3 & Analytic-limits foundation $\mathrm{RCA}_0 + \mathrm{MCT}$ for downstream content (sphere areas, ball volumes, $d\to\infty$ limits) & Minimum classical fragment, Section~\ref{sec:the-dimension-tower} & (ii) \\
4 & $\pi$ forced via Euler product $\zeta(2)=\pi^2/6$ on $\mathbb{N}$ & Theorem~\ref{thm:pi-from-primes} (number theory on integers, no geometry) & --- \\
...
the analytic continuum required for the cascade's downstream
$d\to\infty$ content (Section~\ref{sec:the-dimension-tower});
clause~(ii) of austerity selects MCT as the minimum-strength
foundational path among standard fragments (see status box and
...
analytic-limits foundation for the cascade's downstream content,
Section~\ref{sec:the-dimension-tower}); clause~(ii) at step~5
...
(Section~\ref{sec:the-dimension-tower}; status box at the head of
this prelude). MCT cannot be cascade-derived; it is the unique
foundational commitment beyond Definition~\ref{def:axiom}.
