...
Section~\ref{sec:foundational-commitments}).
...
% ============================================================
\section{Foundational Commitments}
\label{sec:foundational-commitments}
% ============================================================

The prelude's chain from Definition~\ref{def:axiom} ($0\neq 1$) to
the dimension tower $\{(\mathbb{R}^d, B^d)\}_{d\in\mathbb{N}}$ with
asymptotic upper edge $B^\infty$ rests on the following commitments,
and on no others.

\textbf{Sole pre-mathematical input.} Definition~\ref{def:axiom}:
$0 \neq 1$. Austerity (Principle~\ref{princ:austerity}) is the
operational content of this input's sole-input status, not a
separate axiom.

\textbf{Foundational footprint.} Pure $\mathrm{RCA}_0$: natural
numbers, $\Sigma^0_1$ induction, $\Delta^0_1$ comprehension,
computable functions on $\mathbb{N}$, computable reals. This is the
arithmetic content cumulation produces; the cascade imports nothing
on top of it. The Monotone Convergence Theorem and stronger analytic
principles are not foundational commitments of the cascade.

\textbf{Analytic content as computable processes.} Every quantity
the cascade names --- $\pi$, $\zeta(2)$, $V_d$, $\Omega_{d-1}$, the
slicing-recurrence integral, $\Gamma$ at half-integers, the
$d\to\infty$ behaviour, the cascade invariant $10^{-120}$ --- is a
computable sequence of rational approximations indexed by a
resolution parameter $N$. The cascade commits only to
$\mathrm{RCA}_0$-internal convergence statements ($\forall\varepsilon
\,\exists N$); it does not commit to a completed real at the limit.
At any finite resolution depth $N$ (the universe's current age in
Planck ticks, under the temporal reading of
Section~\ref{sec:nothing-unstable}) every cascade quantity has a
specific rational value $\pi_N, V_d^{(N)}, \ldots$ The ``exact
identities'' of standard mathematics are statements about shared
convergence targets as $N$ grows.

\textbf{Downstream bookkeeping.} Parts~0--VI use Gamma identities,
integrals, asymptotic expansions, and other classical analytic
machinery in closed-form style. This is shorthand for the
partial-computation form above, not a separate foundational
commitment. Any line of downstream content can be expanded into a
finite-$N$ statement on computable sequences without invoking MCT or
any stronger axiom.
