...
required. The cascade's \emph{predictions} (specific values for $V_d$,
$\Omega_{d-1}$, $\pi$, slicing-recurrence ratios, sphere areas,
concentration of measure on $B^d$) live one tier below the imported
axiom MCT: at $\mathrm{RCA}_0 + \mathrm{WKL}_0$ + specific limit
estimates for the specific computable sequences in play. MCT is needed
for the foundational \emph{closure} argument (the proof that the
structure is forced by foundations), not for predictive content. The
cascade series' load-bearing hypothesis --- that $B^\infty$ descended
to four dimensions is indistinguishable from our universe --- is
stated independently in the cover sheet and does not depend on the
arguments of this prelude. If any step of the prelude's chain is
disputed, the cascade continues to rest on the cover-sheet hypothesis
and the Gamma-function theorems of Part~0 onward.}}
\end{center}
...
local), graded extension to an angular continuum, the Euler product on
$\mathbb{N}$ that forces $\pi$ as the angular extent, and the
vector-space realisation $\mathbb{R}^d$ forced by Hurwitz's theorem
under a minimum-strength clause of austerity; orthogonality between
distinguishable states is recovered as the extremal $\pi/2$ angular
case. Every step is forced by an austerity clause or by a classical
theorem on integers and analysis, with the Monotone Convergence
Theorem as the chain's single irreducible imported axiom. The starting
structure carries a slicing recurrence with unique constant
$\sqrt{\pi}$, four distinguished dimensions ($5, 7, 19, 217$), and a
cascade invariant of $10^{-120}$ --- consequences of $0\neq 1$, not
impositions on it. The series that follows develops what these
consequences imply about the geometry of the world.
\end{abstract}
