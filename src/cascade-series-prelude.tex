...
One distinction gives two orthogonal directions and a two-dimensional
Euclidean space $\mathbb{R}^2$. But the existence of one distinction does
...
product, and the unit ball $B^d = \{x\in\mathbb{R}^d : |x|\leq 1\}$. The
proof below establishes the cardinal indexing $d\in\mathbb{N}$; the choice
of $\mathbb{R}^d$ as the continuous realisation medium is derived in
Section~\ref{sec:graded-extension}.
\end{theorem}
...
clause~(ii) \cite{friedman1975,simpson2009}.
...
to force its starting structure: the Euclidean inner product on each
$\mathbb{R}^d$ is forced by elementary linear algebra (the unique
positive-definite symmetric bilinear form on $\mathbb{R}^d$, up to
isomorphism, given a basis); the choice of $\mathbb{R}^d$ as the
realisation medium is derived in
Section~\ref{sec:graded-extension}; and the cardinal indexing of the
tower is forced by clauses~(i) and~(ii) of
...
or by a classical theorem on integers and analysis. The chain's
single irreducible imported axiom is the Monotone Convergence Theorem
(MCT) over $\mathrm{RCA}_0$, selected by clause~(ii) of austerity as
the minimum-strength foundational path among standard fragments
(Section~\ref{sec:graded-extension}; status box at the head of this
prelude). MCT cannot be cascade-derived; it is the unique
foundational commitment beyond Definition~\ref{def:axiom}.
