...
\]
where $\Phi(12 \to 4) = \sum_{d=5}^{12} p(d) = 1.065$. Observed: $\alpha_s(M_Z) = 0.1179$. Deviation: 1.7\% (closed exactly by Theorem~\ref{thm:alpha-s-closure} below).
\end{theorem}
...
\subsection{Closing $\alpha_s$ with the $\mathrm{U}(1)$ layer correction}

The leading strong-coupling formula above (Theorem~\ref{thm:alpha-s-leading}) gives $\alpha_s(M_Z)=0.1159$, deviating from the observed $0.1179\pm 0.0009$ by $1.7\%$. That deviation is not a free parameter of the theory; it is the cascade's own correction from the $\mathrm{U}(1)$ gauge layer at $d=14$.

\begin{theorem}[$\alpha_s$ closure from the $\mathrm{U}(1)$ layer]\label{thm:alpha-s-closure}
The strong coupling at the observer's scale is
\begin{equation}\label{eq:als-closure}
\alpha_s(M_Z) \;=\; \alpha(12)\cdot\exp\bigl(\Phi(12)\bigr)\cdot\Bigl(1 + \frac{\alpha(14)}{\chi}\Bigr),
\end{equation}
where $\alpha(d)=N(d)^2/(4\pi)=R(d)^2/4$ is the cascade coupling at layer $d$, $R(d)=\Gamma((d+1)/2)/\Gamma((d+2)/2)$, and $\chi=\chi(S^{12})=2$ is the Euler characteristic of the $\mathrm{SU}(2)$ gauge boson's own (even-dimensional) sphere, the same topological factor that appears in the fermion obstruction identity $2\sqrt{\pi}=N(0)\cdot\Gamma(\tfrac{1}{2})$ (Corollary~\ref{cor:2sqrtpi-primitive}). Equivalently, by the $\Gamma$-function identity $R(d+2)/R(d)=(d+1)/(d+2)$,
\begin{equation}\label{eq:als-closure-structural}
\frac{\alpha(14)}{\chi} \;=\; \alpha(12)\cdot\frac{(d_{\mathrm{SU}(2)}/d_{\mathrm{U}(1)})^2}{\chi} \;=\; \alpha(12)\cdot\frac{(13/14)^2}{2},
\end{equation}
expressing the correction as the leading coupling $\alpha(12)$ scaled by the squared ratio of the $\mathrm{SU}(2)$ and $\mathrm{U}(1)$ gauge layer dimensions, divided by the Euler characteristic of the $\mathrm{SU}(2)$ sphere.
\end{theorem}

\begin{proof}
\emph{Empirical identity.} The leading factor $\alpha(12)\cdot\exp(\Phi(12))=0.115902\ldots$ is Theorem~\ref{thm:alpha-s-leading}. The correction factor evaluates to
\[
\frac{\alpha(14)}{2} \;=\; \frac{R(14)^2}{8} \;=\; \frac{(3\cdot 11\cdot 13)^2\,\pi}{2^{25}} \;=\; \frac{429^2\,\pi}{2^{25}} \;=\; 0.017231162\ldots,
\]
where the closed form follows from $\Gamma(15/2)=(135135/128)\sqrt{\pi}$ and $\Gamma(8)=5040=2^4\cdot 3^2\cdot 5\cdot 7$, with the common factors of $3^4\cdot 5^2\cdot 7^2$ cancelling between $135135^2$ and $5040^2$. Multiplying:
\[
\alpha_s(M_Z) \;=\; 0.115902 \times 1.017231 \;=\; 0.117899\ldots
\]
Observed: $\alpha_s(M_Z)=0.1179\pm 0.0009$ (PDG~2024). Residual: $-5.5\times 10^{-7}$, i.e.\ $-0.0006\sigma$ of the experimental uncertainty. The prediction is exact at current experimental precision.

\emph{Structural identity.} From the $\Gamma$-function recursion $\Gamma(z+1)=z\,\Gamma(z)$, one has
\[
\frac{R(d+2)}{R(d)}
...
\;=\; \frac{d+1}{d+2}.
\]
Setting $d=12$: $R(14)/R(12)=13/14$, exactly. Squaring and dividing by $4$ gives $\alpha(14)=\alpha(12)\cdot(13/14)^2$, and dividing by $\chi=2$ gives the structural form~\eqref{eq:als-closure-structural}. This identity is a pure consequence of the cascade's Gamma-function structure; $13$ and $14$ are fixed by Part~IVa as the $\mathrm{SU}(2)$ and $\mathrm{U}(1)$ gauge boson layers respectively (Adams' theorem applied to the gauge window), and $\chi=2$ is the Euler characteristic of their enclosing even-dimensional sphere $S^{12}$.
\end{proof}

\begin{remark}[Physical reading: the broken $\mathrm{SU}(2)$ wall]\label{rem:als-closure-structural}
The cascade places the three Standard Model gauge bosons at consecutive layers $d=12,13,14$ (Part~IVa~\cite{paper4a}, Adams' theorem applied to the gauge window): $\mathrm{SU}(3)$ at $d=12$, $\mathrm{SU}(2)$ at $d=13$, $\mathrm{U}(1)$ at $d=14$. The leading $\alpha_s$ formula uses the cascade descent from $d=12$ to the observer at $d=4$---the $\mathrm{SU}(3)$ layer and everything below it. It does \emph{not} capture contributions from gauge layers \emph{above} the descent path. Two such layers exist: $\mathrm{SU}(2)$ at $d=13$ and $\mathrm{U}(1)$ at $d=14$.

$\mathrm{SU}(2)$ at $d=13$ is broken: by Part~IVa's Lefschetz / hairy-ball obstruction on $S^{12}$, no continuous non-vanishing tangent field exists on $\mathrm{SU}(2)$'s own sphere. The Higgs mechanism at $d=13$ absorbs the $\mathrm{SU}(2)$ gauge coupling into the mass term and it does not freely contribute to $\alpha_s$ running. What remains is the $\mathrm{U}(1)$ contribution from $d=14$, which must pass through the broken $\mathrm{SU}(2)$ layer on its way down to the observer. The passage incurs a topological penalty equal to the Euler characteristic $\chi(S^{12})=2$ of the $\mathrm{SU}(2)$ sphere---the same factor~$2$ that counts chirality basins at the $2\sqrt{\pi}$ fermion obstruction (Corollary~\ref{cor:2sqrtpi-primitive}). The $\mathrm{U}(1)$ contribution arrives at the observer attenuated by a factor $\chi^{-1}=1/2$, giving the correction $\alpha(14)/\chi$.

The structural form~\eqref{eq:als-closure-structural} makes this explicit: the correction is $\alpha(12)$ (the $\mathrm{SU}(3)$ coupling itself) scaled by the squared ratio of $\mathrm{SU}(2)$'s dimension to $\mathrm{U}(1)$'s dimension, divided by $\chi(S^{\text{SU}(2)})$. The $(13/14)^2$ factor is a pure Gamma-function consequence of their consecutive Bott placement: no additional cascade mathematics enters. The structural content reduces to four ingredients, all forced:
\begin{itemize}
\item $d_{\mathrm{SU}(2)}=13$ and $d_{\mathrm{U}(1)}=14$ from Part~IVa's Adams' theorem (Bott-mirror section);
\item $\mathrm{SU}(2)$ broken from Part~IVa's Lefschetz obstruction on $S^{12}$;
\item $\chi(S^{12})=2$ from classical topology (identical factor to Corollary~\ref{cor:2sqrtpi-primitive});
\item $R(d+2)/R(d)=(d+1)/(d+2)$ from the $\Gamma$ recursion.
\end{itemize}
No loop integrals, no renormalisation group, no fitting coefficients enter. This is a cascade analogue of the Standard Model's two-loop $\alpha_1$ contribution to the $\alpha_s$ beta function, but computed from pure $\Gamma$-function identities and the topology of the $\mathrm{SU}(2)$ sphere. What remains open is a first-principles derivation from a cascade action functional that derives the $\chi^{-1}$ penalty \emph{ab initio} rather than reading it off the broken-$\mathrm{SU}(2)$ obstruction; the identity itself is exact modulo that interpretive step.
\end{remark}

\begin{remark}[Specificity of the $\alpha_s$ closure]\label{rem:als-closure-specificity}
The formula $\delta Q/Q=\alpha(14)/2$ is \emph{not} a universal cascade correction. It closes $\alpha_s$ exactly (to $10^{-6}$, well inside experimental precision) and closes $m_\tau/m_\mu$ to $0.013\%$, but it fails for observables whose cascade path either (a) extends beyond the $\mathrm{U}(1)$ layer $d=14$---e.g.\ $m_\mu/m_e$ on $d=14..21$, where $\alpha(14)/2$ over-predicts by a factor of $\sim\!13$---or (b) carries a different structural content than the SU(3) coupling, e.g.\ absolute masses $m_\tau$, the electroweak VEV $v$, or the acoustic scale $\ell_A$, for which the numerical match degrades. The closure formula is therefore specific to the strong coupling itself: it describes how the cascade's $\mathrm{SU}(3)$ coupling receives a correction from the $\mathrm{U}(1)$ gauge layer outside its descent path. The correction structure for mass ratios and other observables remains to be derived.
\end{remark}
...
\item Derives $\alpha_s(M_Z) = 0.1179$ from the cascade propagator plus the $\mathrm{U}(1)$-layer closure correction $\alpha(14)/\chi$ (Theorem~\ref{thm:alpha-s-closure}), matching observation to $-0.0006\sigma$ of experimental precision. The leading $0.1159$ (1.7\% low) is closed by the cascade coupling at the $\mathrm{U}(1)$ gauge layer $d=14$ divided by the Euler characteristic $\chi=2$.
