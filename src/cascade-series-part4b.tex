...
\begin{enumerate}
\item \emph{Resolved.} Rigour of the topological obstruction factor. Theorem~\ref{thm:obstruction} derives $1/(2\sqrt{\pi})$ from two geometric obstructions at the hairy ball zero: the chirality decomposition ($\chi(S^{2n}) = 2$) and the quarter-turn obstruction ($\sqrt{\pi}$ consumed by the global tangent-frame failure). Corollary~\ref{cor:2sqrtpi-primitive} sharpens this to a pure-cascade identity: $2\sqrt{\pi} = N(0)\cdot\Gamma(\tfrac{1}{2})$, where $N(0) = 2$ is the cascade's zeroth slicing lapse and $\Gamma(\tfrac{1}{2}) = \sqrt{\pi}$ is the unique cascade constant. Remark~\ref{rem:no-dirac-route} shows that the alternative ``Dirac operator derivation'' is not just redundant but misdirected: the $\hat A$-genus of every sphere vanishes, so a Dirac-based obstruction would predict $0$ rather than $2\sqrt{\pi}$. The Euler characteristic is the unique nonzero constant-across-generations invariant consistent with the formula $(2\sqrt{\pi})^{n_D+1}$, and the cascade's $N(0) = \chi(S^0)$ gives it directly.
