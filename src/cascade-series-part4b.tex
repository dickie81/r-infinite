...
\]
predicts: $m_\tau = 1755$~MeV (obs 1777, 1.2\%), $m_\mu = 106.2$~MeV (obs 105.7, 0.47\%), $m_e = 0.514$~MeV (obs 0.511, 0.60\%). Combined with the gauge coupling predictions ($1/\alpha_{\text{GUT}} = 25.02$, $\alpha_s = 0.1159$, $\sin^2\theta_W = 0.2286$ from the boson topological obstruction at the Dirac layer $d=13$, $m_H = 125.82$~GeV), and the cosmological matter fraction $\Omega_m = 0.31150$ from the Bott partition of cascade layers (Papers I and IVa), the series produces ten independent and four derived precision predictions, all sub-3\%, with zero free parameters. No standard renormalisation group running is used; the Weinberg angle is derived directly from the geometry of $S^{12}$ with the boson-only quarter-turn factor $1/\sqrt{\pi}$ extracted from the topological obstruction theorem.

The cascade's predictions separate into two populations with distinct error signatures. Descent-dependent quantities ($\alpha_s$, $v$, $\Omega_m$, mass ratios, $m_\tau$, $m_W$, $\sin^2\theta_W$) carry uniformly negative deviations of $-0.13\%$ to $-2.20\%$ (8/8 negative). Geometric quantities ($m_H/m_W$, $\theta_C$, $\Omega_b$) carry positive deviations of $+0.56\%$ to $+2.76\%$ (3/3 positive). The sign separation identifies the subleading cascade descent as the source of the dominant systematic.
\end{abstract}
...
\begin{remark}[A family of $\alpha(d)/\chi$ shifts]\label{rem:phase-family}
Theorems~\ref{thm:alpha-s-closure} and~\ref{thm:mtau-abs-closure} together exhibit two independent cascade shifts of the same structural form $\alpha(d^*)/\chi$, sourced at two different distinguished cascade layers and acting on two different classes of observables:
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lccc}
\toprule
shift & source layer $d^*$ & role of $d^*$ & acts on \\
\midrule
$\delta\Phi_{\mathrm{U}(1)} = \alpha(14)/\chi$ & $d=14$ & $\mathrm{U}(1)$ gauge layer & $\alpha_s(M_Z)$, $m_\tau/m_\mu$ \\
$\delta\Phi_{\mathrm{phase}} = \alpha(19)/\chi$ & $d_1=19$ & phase-transition threshold & $m_\tau$ absolute \\
\bottomrule
\end{tabular}
\end{center}
Both shifts use the same topological denominator $\chi(S^{2n})=2$, identical to the factor in the $2\sqrt{\pi}$ fermion obstruction (Corollary~\ref{cor:2sqrtpi-primitive}). Both source layers are distinguished dimensions from Paper~I's four-dimension tower, and both shifts are closed forms in Gamma function values.

The structural reading of $\delta\Phi_{\mathrm{phase}}$ parallels that of $\delta\Phi_{\mathrm{U}(1)}$ in Remark~\ref{rem:als-closure-structural}: the phase-transition layer $d_1=19$ marks the crossover from subcritical to supercritical cascade geometry (Paper~I), and $\alpha(19)/\chi$ is the cascade coupling \emph{at that wall}. Absolute mass scales, which unlike gauge couplings and mass ratios carry the Planck-mass anchor $M_{\mathrm{Pl,red}}$ and the Higgs-VEV suppression factor $\exp(-\pi/\alpha(5))$, receive this second shift at $d_1$ rather than the U(1) gauge-layer shift that acts on pure-cascade-descent observables.

Consistency across the lepton chain: using $k_g\equiv\log(m_g^{\text{obs}}/m_g^{\text{lead}})/\delta\Phi_{\mathrm{U}(1)}$, the required absolute-mass shifts for the three charged leptons are $k_\tau=+0.7450$, $k_\mu=-0.2540$, $k_e=-0.3306$. Their differences reproduce the ratio closures exactly: $k_\tau - k_\mu = +1.0000$ (= $m_\tau/m_\mu$ ratio shift = $\delta\Phi_{\mathrm{U}(1)}/\delta\Phi_{\mathrm{U}(1)}$) and $k_\mu - k_e = +0.0765$ (= $m_\mu/m_e$ ratio shift). The base shift $k_\tau\cdot\delta\Phi_{\mathrm{U}(1)}=0.01284$ is closed by $\delta\Phi_{\mathrm{phase}}=0.01282$ within $m_\tau$'s experimental precision. $m_\mu$ and $m_e$ absolute masses inherit $k_\tau$ minus the ratio shifts and close below the paper's $\sim$0.1\% precision target but above their own (extraordinary) experimental precisions.

What remains open: (a) a first-principles derivation of the structural selection rule that assigns each observable class to its own $d^*$ source (gauge-descent $\to$ $d=14$; Planck-anchored absolute masses $\to$ $d_1=19$); (b) analogous $\alpha(d^*)/\chi$ shifts, if any, for $v$, $\ell_A$, $m_W$, and the other precision observables; (c) a full cascade action functional that generates the $\alpha(d^*)/\chi$ family ab initio rather than reading it off structural coincidences at distinguished layers. What is shown: two independent precision observables of different classes close within experimental precision using the same structural form at different distinguished cascade layers, with no fitted parameters---promoting the U(1) closure from a single identity to the first member of a family.
\end{remark}
...
\item Derives $m_\tau = 1776.82$~MeV absolute via a \emph{second} structural shift $\delta\Phi_{\mathrm{phase}}=\alpha(19)/\chi$ sourced at the phase-transition layer $d_1=19$ (Theorem~\ref{thm:mtau-abs-closure}), matching observation to $-0.31\sigma$ of experimental precision. The two shifts $\alpha(14)/\chi$ and $\alpha(19)/\chi$ exhibit the first two members of a cascade-structural family $\alpha(d^*)/\chi$ acting on different observable classes at different distinguished layers.
