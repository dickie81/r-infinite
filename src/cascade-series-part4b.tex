...
A structural family of cascade potential shifts $\delta\Phi = \alpha(d^*)/\chi^k$, sourced at distinguished cascade layers $d^*$ and weighted by the same Euler characteristic $\chi=\chi(S^{2n})=2$ that appears in the $2\sqrt{\pi}$ fermion obstruction, closes \emph{four} Standard Model precision observables within experimental precision in one structural form. The $\mathrm{U}(1)$-layer shift $\delta\Phi_{\mathrm{U}(1)}=\alpha(14)/\chi$ closes $\alpha_s(M_Z)=0.117917$ ($+0.019\sigma$) and $m_\tau/m_\mu=16.81731$ ($+0.243\sigma$) together. The phase-transition shift $\delta\Phi_{\mathrm{phase}}=\alpha(19)/\chi$, sourced at Paper~I's threshold $d_1=19$, closes the $\tau$ absolute mass $m_\tau=1776.82$~MeV ($-0.31\sigma$). The observer-host shift $\delta\Phi_{\mathrm{obs}}=\alpha(5)/\chi^3 = 8/(225\pi)$, sourced at the volume maximum $d_V=5$ with three factors of the Euler characteristic, closes $\sin^2\theta_W = 0.231226$ ($+0.40\sigma$). All three shifts are closed forms in $\Gamma$ function values. Combined with the remaining gauge coupling predictions ($1/\alpha_{\text{GUT}} = 25.02$, $m_H = 125.82$~GeV), the cosmological matter fraction $\Omega_m = 0.31150$ (Papers I and IVa), and additional sub-3\% predictions, the series produces four SM precision observables (a gauge coupling, a mass ratio, an absolute mass, and the Weinberg angle) closed within current experimental precision, with zero fitted parameters and no standard renormalisation group running.
...
\end{tabular}
\end{center}
Among invariants that are both constant and nonzero, $\chi = 2$ is the canonical choice. The $\hat A$-genus of any sphere vanishes identically ($S^{2n}$ is null-cobordant in spin-bordism, and its Pontryagin classes vanish above degree $0$), so the Atiyah--Singer index of a Dirac operator on a spin sphere is zero. A \emph{Dirac-operator derivation} of the obstruction factor would therefore predict $0$, not $2\sqrt{\pi}$. The cascade's formula $(2\sqrt{\pi})^{n_D+1}$ is structurally incompatible with any Dirac-index interpretation and uniquely consistent with the Euler characteristic; Corollary~\ref{cor:2sqrtpi-primitive} gives the factor directly from the cascade's own primitives.
\end{remark}
...
\]
where $\Phi(12 \to 4) = \sum_{d=5}^{12} p(d) = 1.065$. Observed: $\alpha_s(M_Z) = 0.1179$. Deviation: 1.7\% (closed to experimental precision by the universal $\mathrm{U}(1)$ shift of Theorem~\ref{thm:alpha-s-closure} below, together with $m_\tau/m_\mu$ in the same identity).
\end{theorem}
...
\subsection{The universal $\mathrm{U}(1)$-layer shift to the cascade potential}
...
\delta\Phi_{\mathrm{U}(1)} \;=\; \frac{R(14)^2}{8} \;=\; \alpha(12)\cdot\frac{(d_{\mathrm{SU}(2)}/d_{\mathrm{U}(1)})^2}{\chi} \;=\; \alpha(12)\cdot\frac{(13/14)^2}{2} \;=\; \frac{429^2\,\pi}{2^{25}} \;=\; 0.017231162\ldots.
\end{equation}
\end{theorem}
...
\subsection{A second shift: $\alpha(19)/\chi$ closes $m_\tau$ absolute at the phase-transition layer}
...
The resolution is that $m_\tau$ absolute receives a \emph{different} structural shift, of the same $\alpha(d)/\chi$ form but sourced at a different distinguished cascade layer---the phase-transition threshold $d_1=19$ of Paper~I~\cite{paperI}.
...
\end{equation}
sourced at the phase-transition layer $d_1=19$ (Paper~I~\cite{paperI}) and weighted by the same Euler characteristic $\chi=\chi(S^{2n})=2$ that appears in Theorem~\ref{thm:alpha-s-closure} and Corollary~\ref{cor:2sqrtpi-primitive}. Applied exponentially to the $\tau$ absolute-mass formula,
...
\begin{remark}[A family of $\alpha(d^*)/\chi^k$ shifts]\label{rem:phase-family}
Theorems~\ref{thm:alpha-s-closure}, \ref{thm:mtau-abs-closure}, and~\ref{thm:weinberg-closure} together exhibit three independent cascade shifts of the structural form $\alpha(d^*)/\chi^k$, sourced at three different distinguished cascade layers and acting on three different classes of observables:
\begin{center}
\renewcommand{\arraystretch}{1.35}
\begin{tabular}{lcccc}
\toprule
shift & source $d^*$ & $\chi$ power & role of $d^*$ & closes \\
\midrule
$\delta\Phi_{\mathrm{U}(1)} = \alpha(14)/\chi$ & $d=14$ & $\chi^1$ & $\mathrm{U}(1)$ gauge layer & $\alpha_s(M_Z)$, $m_\tau/m_\mu$ \\
$\delta\Phi_{\mathrm{phase}} = \alpha(19)/\chi$ & $d_1=19$ & $\chi^1$ & phase-transition threshold & $m_\tau$ absolute \\
$\delta\Phi_{\mathrm{obs}} = \alpha(5)/\chi^3$ & $d_V=5$ & $\chi^3$ & observer host (volume max) & $\sin^2\theta_W$ \\
\bottomrule
\end{tabular}
\end{center}
All three shifts use the same topological denominator $\chi(S^{2n})=2$ (the even-sphere Euler characteristic, identical to the factor in the $2\sqrt{\pi}$ fermion obstruction of Corollary~\ref{cor:2sqrtpi-primitive}). All three source layers are distinguished dimensions from Paper~I's four-dimension tower (volume maximum $d_V$, phase-transition threshold $d_1$, and the $\mathrm{U}(1)$ gauge layer forced by Adams' theorem in Part~IVa). All three are closed forms in Gamma function values. Four Standard Model precision observables (a gauge coupling, a mass ratio, an absolute mass, and the Weinberg angle) close within experimental precision with zero fitted parameters, using one common structural form.

The structural readings differ per shift:
\begin{itemize}
\item $\delta\Phi_{\mathrm{U}(1)}=\alpha(14)/\chi$: the $\mathrm{U}(1)$ gauge-layer contribution, passing through the broken $\mathrm{SU}(2)$ wall at $d=13$ and picking up $\chi^{-1}=\chi(S^{12})^{-1}$ (Remark~\ref{rem:als-closure-structural}).
\item $\delta\Phi_{\mathrm{phase}}=\alpha(19)/\chi$: the cascade coupling at the phase-transition wall $d_1=19$ itself, acting on Planck-anchored absolute masses (Theorem~\ref{thm:mtau-abs-closure}).
\item $\delta\Phi_{\mathrm{obs}}=\alpha(5)/\chi^3$: the cascade coupling at the observer's own host $d_V=5$, weighted by \emph{three} factors of the Euler characteristic---plausibly one per gauge boson layer $\{12,13,14\}$ crossed by the Weinberg mixing (Theorem~\ref{thm:weinberg-closure}). The $\chi^3$ exponent is what distinguishes $\sin^2\theta_W$'s closure from the two $\chi^1$ shifts above.
\end{itemize}

Consistency across the lepton chain: using $k_g\equiv\log(m_g^{\text{obs}}/m_g^{\text{lead}})/\delta\Phi_{\mathrm{U}(1)}$, the required absolute-mass shifts for the three charged leptons are $k_\tau=+0.7450$, $k_\mu=-0.2540$, $k_e=-0.3306$. Their differences reproduce the ratio closures exactly: $k_\tau - k_\mu = +1.0000$ (= $m_\tau/m_\mu$ ratio shift) and $k_\mu - k_e = +0.0765$ (= $m_\mu/m_e$ ratio shift). The base shift $k_\tau\cdot\delta\Phi_{\mathrm{U}(1)}=0.01284$ is closed by $\delta\Phi_{\mathrm{phase}}=0.01282$ within $m_\tau$'s experimental precision. $m_\mu$ and $m_e$ absolute masses inherit $k_\tau$ minus the ratio shifts and close below the paper's $\sim$0.1\% precision target but above their own (extraordinary) experimental precisions.

What remains open: (a) a first-principles derivation of the selection rule that assigns each observable class to its own $(d^*,\chi^k)$ pair (gauge-descent ratios $\to$ $(d{=}14,\chi^1)$; Planck-anchored absolute masses $\to$ $(d_1{=}19,\chi^1)$; the Weinberg angle $\to$ $(d_V{=}5,\chi^3)$); (b) analogous $\alpha(d^*)/\chi^k$ shifts, if any, for $v$, $\ell_A$, $m_W$, $\Omega_m$, and the other precision observables; (c) a full cascade action functional that generates the family ab initio. What is shown: three independent precision observables of three different classes close within experimental precision using the same structural form at three different distinguished cascade layers, with no fitted parameters---promoting the U(1) closure from a single identity to the first three members of a structural family.
\end{remark}
...
of~\cite{paperIVa}). The obstruction at $d=13$ is therefore consumed by
the symmetry breaking; it does not appear as a propagator factor for the
SU(2) coupling. Hence $g_2 = N(13)$ unmodified.

\textbf{Step 2: U(1) crosses the Dirac layer at $d=13$ during descent.}
...
$d=14$ to the observer at $d=4$ traverses the Dirac layer $d=13$ once.
A field passing through a Dirac layer encounters the topological
obstruction analysed in Theorem~\ref{thm:obstruction}. There the
...
\emph{by Bott periodicity} (Section~2 of~\cite{paperIVa}), so SU(2) is
...
With the cascade-derived $\sin^2\theta_W = 0.2286$:
\[
...
$N(13), N(14)\to\theta_W\to m_W\to m_H$ is now closed within the cascade
...
Combining $m_W$ above with $m_H/m_W = \pi/2$ from~\cite{paperIVa}, Theorem 3.3:
\[
...
\item \emph{Partially resolved.} Individual electroweak couplings and $1/\alpha_{\text{em}} = 137$. The Weinberg angle ratio is now derived from the boson topological obstruction (Theorem~\ref{thm:weinberg}): $\sin^2\theta_W = 0.2286$, deviation $-1.12\%$. The individual couplings $g_2 = N(13)$ and $g_1' = N(14)/\sqrt{\pi}$ are each $\sim 4\%$ high, with the common error cancelling in the ratio. Deriving $\alpha_{\text{em}} = 1/137$ separately would require the multi-step descent corrections to the absolute couplings, which are not computed.
