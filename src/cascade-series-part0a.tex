...
depth. The paper does not derive the distinguished dimensions, the
...
results. The paper develops this three-way agreement as a
\emph{saturation reading}: the cascade's prime-cascade and
dimensional-cascade routes converge on the same horizon-entropy
scale at the current cosmic depth, with $\Lambda$ staying fixed at
the Part~0 value. Whether the saturation epoch is anthropically
selected, structurally forced, or coincidental remains open. The
cascade's foundational footprint stays at pure $\mathrm{RCA}_0$.
\end{abstract}
...
scale). Reading~(b) (numerical coincidence at the current $N$)
remains a possible alternative if the match turns out to be
approximate rather than structurally forced.
