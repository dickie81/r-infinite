...
\begin{remark}[Clarification]\label{rem:paper2-71}
...
\begin{remark}[Clarification]\label{rem:paper2-73}
...
\begin{remark}[Clarification]\label{rem:paper2-86}
...
\]
grows monotonically with $N(t)$. Numerical evaluation using the Beta-function
adjacent-overlap asymptotic (Paper~0 Supplement~\cite{paper0supp}
Theorem~15.2, $1 - C^2_{d,d+1} \sim 1/(8d^2)$) gives:
\end{remark}
...
\begin{tabular}{cccc}
\toprule
Tick $N$ & Layers in tower & $\varepsilon(N)$ & Fraction of $\varepsilon(\infty)$ \\
\midrule
$7$   & 3   & $\sim 0.8\%$  & $13\%$ \\
$19$  & 15  & $\sim 3.0\%$  & $49\%$ \\
$50$  & 46  & $\sim 5.0\%$  & $82\%$ \\
$100$ & 96  & $\sim 5.6\%$  & $92\%$ \\
$217$ & 213 & $6.11\%$ (Paper~II) & $100\%$ \\
$\infty$ & $\infty$ & $6.11\%$ (saturated) & --- \\
\bottomrule
\end{tabular}
\end{center}
...
$\varepsilon(N) \lesssim 5.8\%$, slightly smaller than the saturated value.
The numerical impact on Part~V~\cite{paper5} observables is negligible ---
all Part~V observables (cosmological parameters, $H_0$, $r_d$, $t_0$) are set
at post-Big-Bang epochs where $\varepsilon(N) = \varepsilon(217) = 6.11\%$
to the precision required. Corollary~\ref{cor:pre-rec-gram} is a structural
refinement that becomes numerically relevant only for future cascade
observables set at specific pre-recombination epochs.
\end{corollary}
...
\item \textbf{Cascade-native $g_{\mathrm{eff}}$.} Count cascade propagating
modes at each tower layer: spinor dimensions at Dirac layers $d \in
\{5, 13, 21\}$, vector fields at gauge layers $d \in \{12, 13, 14\}$, scalar
at the $d = 13$ hairy-ball obstruction. The sum (weighted by chirality and
fermion/boson statistics) would replace the Standard Model $g_{\mathrm{eff}}
= 106.75$ used in Corollary~\ref{cor:TRH}. Expected to be of the same order
of magnitude; exact value open.
...
VEV $v$. If an instanton-like mechanism operates during the cascade's descent,
the uniform ``one tick per layer'' identification of Definition~\ref{def:tower-time}
may require refinement. Whether the cascade has a non-perturbative sector
relevant to cosmic history is open.
...
etc. Whether any of these produces a future phase transition in non-Λ
...
$T_{\mathrm{RH}} = 7 \times 10^{17}$~GeV (Corollary~\ref{cor:TRH}) & 5 &
  Uses SM $g_{\mathrm{eff}}$; cascade-native count open \\
