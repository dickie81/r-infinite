...
\midrule
0~\cite{part0} & Four distinguished dimensions, invariant $I$ & Theorem \\
I~\cite{part1} & $\rho_\Lambda/M_{\mathrm{Pl,red}}^4$ from observer frame & Tier 2 \\
II~\cite{part2} & QM from cascade; time = slicing direction & Tier 1--2 \\
III~\cite{part3} & GR, $d = 4$, Lorentzian signature & Tier 1 \\
II\,=\,III~\cite{part23} & Quantum gravity dissolution & Tier 1 \\
IVa--b~\cite{part4a,part4b} & SM group, masses, couplings & Tier 1--3 \\
V~\cite{part5} & Cosmological parameters today & Tier 2--3 \\
\textbf{VI (this)} & \textbf{Tower growth, cosmic history, inflation} & \textbf{Tier 5} \\
\bottomrule
\end{tabular}
\end{center}

\section{Time as Tower Growth}\label{sec:time-as-tower-growth}
...
Paper~II~\cite{part2}, Section~7.1 identifies time with the cascade's slicing
...
(Paper~II~\cite{part2}~\S2.3). The observer's 3-sphere is the boundary of
...
5D ambient geometry. Paper~I~\cite{part1}~\S3.2 uses the cover-sheet reading
...
Paper~I~\cite{part1} Theorem~3.1 and Paper~V~\cite{part5} Section~2, the
...
\section{The \texorpdfstring{$N = 217$}{N = 217} Big Bang}
\label{sec:bigbang}
...
\]
thermalises into the cascade's $w = 1/3$ metric content (Paper~III~\cite{part3}
...
\subsection{Cascade-native \texorpdfstring{$g_{\mathrm{eff}}$}{g\_eff}}\label{subsec:g_eff}
...
Following Part~IVa~\cite{part4a} \S4 ``The cascade obstruction map'':
...
assumption: Part~IVa~\cite{part4a} forces the gauge group
...
Paper~V~\cite{part5}, Section~5.8 asserts that the cascade has ``no
...
satisfies (Part~0~\cite{part0}, overlap deficit theorem):
\[
...
\begin{remark}
Paper~IVb's $\alpha(d^*)/\chi^k$ family (Part~IVb~\cite{part4b} Remark~4.6)
...
\subsection{Comparison to observed \texorpdfstring{$n_s$}{n\_s}}
...
classical via geometric decoherence (Paper~II~\cite{part2} Section~8.5) acting
...
\subsection{No cascade analogue of \texorpdfstring{$r = 16\epsilon$}{r = 16 epsilon}}
...
(Paper~II\,=\,III~\cite{part23} Theorem~6.1), not a quantum operator on
...
Einstein equation of Paper~III~\cite{part3}. They are not independent
...
$G_{\mu\nu} + \Lambda g_{\mu\nu} = 8\pi G\,T_{\mu\nu}$ is the unique
gravitational equation at $d = 4$ (Paper~III, Theorem~\xref{part3}{thm:d4}), so the metric
...
features into structural existence. Parts~IVa and~IVb~\cite{part4a,part4b}
...
\midrule
$N = 5$   & $d = 5$   & Gen~3 Dirac obstruction ($\tau$, $b$, $\nu_\tau$); observer host & Paper~IVa~\cite{part4a} \S4 \\
$N = 7$   & $d = 7$   & Area maximum $d_0$; decay-rate zero & Paper~0~\cite{part0} \S5 \\
...
Part~VI's dynamical reading clarifies three points in Paper~II~\cite{part2}
...
Beta-function Gram matrix of Part~0~\cite{part0} (Gram-as-Beta theorem).
\end{remark}
...
\texttt{tools/model\_checks/\allowbreak cascade\_decoherence\_\allowbreak vs\_tower\_\allowbreak height.py}; no approximation
...
the Part~0 first-order correction formula. For observables set
during Phase~C at $N \sim 100$--$200$, the appropriate correction is
$\varepsilon(N) \in [5.0\%, 6.0\%]$, slightly smaller than the saturated
value. The numerical impact on Part~V~\cite{part5} observables is
...
toward the pseudo-horizon'' is used by Paper~I~\cite{part1} Section~3.2 to
...
\begin{remark}[The unifying principle: time is the resolution rate of $B^\infty$]\label{rem:time-d-collapse}
The three time-pictures of the previous remark share a single structural
content: the cascade is the asymptotic resolution of the unresolved infinity
at $B^\infty$ (Cover sheet, ``One Infinity, Not Many''), and time is the
rate at which that resolution proceeds. Each Planck tick advances the
resolution by one cascade layer; equivalently, one more dimension of
$B^\infty$ collapses into a definite layer of the cascade. Under this
reading, time is not a coordinate but a counter: the number of dimensions
that have been resolved out of the unresolved infinite-dimensional starting
object into the observer's accessible cascade.

\medskip
\noindent\textbf{Tick = one $d$-collapse.} Definition~\ref{def:tower-time}
records the operational fact $N(t+\alpha\,t_{\mathrm{Pl,red}}) = N(t)+1$.
The unifying reading recognises this as the cascade's foundational time
quantum: the Planck tick is the rate at which one dimension of $B^\infty$
is resolved. The three vocabularies of the previous remark are then
consequences:
\begin{itemize}[nosep]
\item Paper~II's slicing direction is the cascade's intrinsic ordering of
dimensions to be resolved (the slicing recurrence picks one perpendicular
axis at each layer, so the order is total).
\item The cover sheet's asymptotic infall is the observer's relative
position with respect to the unresolved boundary $B^\infty$, which recedes
as resolution proceeds.
\item Part~VI's tower growth is the cumulative count of resolved layers.
\end{itemize}

\medskip
\noindent\textbf{Connection to the cascade's intrinsic angle.} Paper~II's
forced-precession theorem gives a $\pi/2$ angle between consecutive
slicing axes, statically. Under the present reading, the same $\pi/2$
acquires a dynamical meaning: the cascade rotates by a quarter-turn in
its own complex plane each time one dimension is resolved. The propagator
factor $i = e^{i\pi/2}$ appearing in Paper~II's discrete propagator
$L(d) = i\,N(d)$ is then ``one quarter-turn per $d$-collapse'', and the
cumulative phase $i^{N(t)-4}$ in $K(N(t), 4)$ is the total resolution
count modulo~4.

\medskip
\noindent\textbf{Consequence for the gauge structure of states.} Under
this dynamical reading, the per-tick phase $i$ is the cascade's intrinsic
counter of resolved dimensions. The $U(1)$ generated by $J$ (Paper~II's
$R_\phi := \cos\phi\,I + \sin\phi\,J$) acts on this counter; whether
$U(1)$ is a gauge equivalence or merely a dynamical symmetry (Paper~II's
``$U(1)$ gauge upgrade'' open question) is now seen at the cosmological
scale: under $\mathbb{Z}_2$ gauge the cumulative resolution phase would
be observable as a 2-cycle Planck-period oscillation in Born-rule
probabilities at every measurement, which is not seen in observation;
under $U(1)$ gauge the resolution phase is gauge-trivial and only the
real geometric factor $\prod_{j=4}^{N(t)-1} N(j)$ has cosmological
signature, matching what Part~VI already predicts. The Part~VI tick
reading thus narrows Paper~II's open question to a hypothesis-level
forcing: $\mathbb{Z}_2$ would predict an observable Planck-period
oscillation absent from the cascade's existing cosmological predictions
and absent from observation. Verifier
\texttt{tools/verifiers/u1\_gauge\_closure\_verifier.py} (V5).

\medskip
\noindent\textbf{The universe as the in-progress resolution.} Under the
unifying principle, the universe is not ``a place where infinity was
avoided'' but the asymptotic completion of $B^\infty$ caught partway
through its resolution. The Big Bang at $N=217$
(\S\ref{sec:bigbang} below, see Theorem~\ref{thm:reheating}) marks the
resolution reaching the $\Omega_{217}$ floor; pre-Big-Bang phases A--C
are stages of the resolution prior to that threshold; the post-Big-Bang
phase D continues the asymptotic resolution toward $B^\infty$ at
$N\to\infty$. In this language the cosmological constant is the floor of
the resolution at the observer's depth, and time dilation is the local
rate at which resolution proceeds along the observer's worldline.
\end{remark}
...
\item \textbf{Non-perturbative sector during inflation.} Paper~IVb~\cite{part4b}
uses $\exp(-\pi/\alpha(5))$ as a non-perturbative factor in the electroweak
VEV $v$. \emph{Partially resolved} by Paper~IVb, Remark~\xref{part4b}{rem:v-closure}: the factor
...
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}>{\raggedright}p{6.5cm} c >{\raggedright\arraybackslash}p{6cm}@{}}
\toprule
...
$P_\delta(d) \sim 1/(8d^2)$ (Prop~\ref{prop:dspectrum}) & 4 &
  Direct consequence of Part~0 overlap deficit theorem \\
...
\bibitem{part0} [Author], \extlink{\cascadebase/cascade-series-part0.pdf}{\textit{The Cascade Series --- Part~0: Scale Variance from Orthogonality}}, March 2026.
...
\bibitem{part2} [Author], \extlink{\cascadebase/cascade-series-part2.pdf}{\textit{The Cascade Series --- Part~II: Quantum Mechanics from the Cascade}}, March 2026.

\bibitem{part3} [Author], \extlink{\cascadebase/cascade-series-part3.pdf}{\textit{The Cascade Series --- Part~III: General Relativity from the Cascade}}, March 2026.

\bibitem{part23} [Author], \extlink{\cascadebase/cascade-series-part2-equals-3.pdf}{\textit{The Cascade Series --- Part II\,=\,III: Quantum Gravity without Quantising Gravity}}, March 2026.

\bibitem{part4a} [Author], \extlink{\cascadebase/cascade-series-part4a.pdf}{\textit{The Cascade Series --- Part IVa: Standard Model from the Cascade}}, March 2026.

\bibitem{part4b} [Author], \extlink{\cascadebase/cascade-series-part4b.pdf}{\textit{The Cascade Series --- Part IVb: Masses, Couplings, and Precision Predictions}}, March 2026.

\bibitem{part5} [Author], \extlink{\cascadebase/cascade-series-part5.pdf}{\textit{The Cascade Series --- Part V: Cosmology from the Cascade}}, March 2026.
