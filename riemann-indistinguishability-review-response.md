...
# Round 265: convergence test on the round-264 sweep — NOT CONVERGED: 0 majors + 4 minors; the architecture held, four enforcement claims did not; swept; round 266 owed

**Held under attack:** the reach-wide closure (64 files / 28 surfaces recomputed with the driver's own member_reach; no undeclared paper reader in any reach; no spawn path outside it); all seven pure-mirror declarations exact multiset matches to their inline compares; the windows sum-split strictly stronger; every round-264 clause probe reproduced; covers() directionality sound; the f-string sanction genuinely print-evidence at all nine sites; the cascade_tower drift assert both directions; batteries at expected censuses (0 live + 19 cached, TOWER PASS). **Findings (all swept, 33834ef):** F265-1 — chained comparisons skipped before the complex-node branch: the "unharvestable is a hard failure" universal false, a blind (unused) channel back to the stale-PASS class; now flagged. F265-2 — var_forms prefix-anchored: a suffixed read-transform was mislabeled instead of unmapped; now $-anchored, docstring re-sworn. F265-3 — the quarter_square conversion dropped the old raw bolded-heading conjunct (an un-bolding edit passed the tower); restored as a raw max=1 pin, probed. F265-4 — schema validation crashed on non-dict entries; completed for every malformed shape. Post-sweep battery: full live tower **19 live PASS + 0 cached + 0 FAIL (19/19)**, every key rotated.

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m swept. Convergence round 266 next.**

# Round 266: convergence test on the round-265 sweep — NOT CONVERGED: 0 majors + 3 minors + 2 cosmetics; two fixes held completely, two universals fell; swept; round 267 owed

**Held under attack:** F265-1 completely (16-shape harvest battery, zero unflagged paper-touching compares; the `not (s in paper)` inversion noted and fail-safe); F265-3 completely (both heading pins verified live, un-bolding probe trips, the straddle argument re-verified, entailment of the old gate on every constructed edit); the whole-arc stale-PASS construction attempt found no paper-edit-only counterexample; the regression suite's ten designed diagnostics reproduced; quantifier census exact (64/28; 19 live post-sweep battery corroborated by cache stamps). **Findings (all swept, af40962):** F266-1 — the F265-4 completion validated inside check() only; covers() crashed uncaught on the same malformed declarations (fail-safe exit 1, wrong contract); one shared schema predicate now serves both callers, driver-probed to the designed banner. F266-2 — the $-anchor closed only the suffix channel: an INFIX mutation inside the greedy open(PAPER…) span mapped as clean (demonstrated precheck-green/member-FAIL); F266-3 — the line-scan read docstring content, admitting a phantom mapping. Both closed by rewriting var_forms to exact AST-shape matching (string content never scanned; any extra call layer fails the match; probed end-to-end). F266-4 (cosmetic) — the "(trailing comment allowed)" prose imprecision, retired by the AST rewrite. F266-5 (cosmetic) — degenerate always-pass entry shapes now rejected at the schema; the laundering path was already proven closed. Post-sweep battery: full live tower **19 live PASS + 0 cached + 0 FAIL (19/19)**, every key rotated.

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m → 266: 0M+3m+2c swept. Convergence round 267 next.**

# Round 267: convergence test on the round-266 sweep — NOT CONVERGED: 1 MAJOR + 3 minors + 2 cosmetics; the round-266 fixes held, but a committed function-wrapped read was a live stale-PASS channel; swept; round 268 owed

**Held under attack:** the round-266 fixes in full (16-shape driver-path fuzz, zero uncaught tracebacks; infix and docstring-phantom channels closed; all six styles map; fail-safe near-misses verified; the regression suite's designed diagnostics reproduced; all 28 surfaces valid under the tightened schema; quantifier census exact; batteries corroborated). **Findings (all swept, 06bc157):** F267-1 MAJOR — `cascade_type_counting.py` read the paper through a function-wrapped `paper_for_g2()` (a return, not an Assign) with two g2 needles declared nowhere: invisible to every clause, and a one-word paper edit yielded TOWER PASS (19/19) from cache against a live member-chain FAIL — the arc's central contract falsified on committed code. Swept by rerouting g2 through the declared surface AND adding clause (v): every Load of the name PAPER must sit inside a var_forms-recognized creation assignment, so any read outside a declared surface is flagged at the read itself (with clause (iv), no route to the paper's bytes escapes). F267-2 — five binding shapes were unmapped and unflagged (an AnnAssign silently dropped a whole surface from the census); the creation census is now var_forms' alone, clause (v) polices the rest. F267-3 — the inner open() was never examined (encoding-swap divergence); pinned exactly. F267-4 — the shape rules trusted the name norm (rogue-body mislabel); the canonical definition is now part of the matched shape. F267-5/6 (cosmetic) — the min-0-no-max always-pass shape rejected; entry keys whitelisted. Post-sweep battery: full live tower **19 live PASS + 0 cached + 0 FAIL (19/19)**, every key rotated.

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m → 266: 0M+3m+2c → 267: 1M+3m+2c swept. Convergence round 268 next.**

# Round 268: convergence test on the round-267 sweep — NOT CONVERGED: 1 MAJOR + 2 minors + 3 cosmetics; clause (v) keyed creations on names, not nodes; swept; round 269 owed

**Held under attack:** the round-267 fixes on every probed shape (the g2 reroute count-identical under both transforms; the one-word-edit scenario trips; function-wrapped/encoding/norm-def/AnnAssign probes trip; the escape-surface classification — ImportFrom reads become declared readers, computed spellings outside tripwire scope; docstring-only edits cannot flip verdicts; reach and battery censuses exact). **Findings (all swept, 12ba0f4):** F268-1 MAJOR — the creation census keyed on target NAMES, so a one-line rebind of a mapped paper variable was itself sanctioned as a creation while vf kept the stale form: the reviewer committed a truncation rebind green and then demonstrated the full paper-edit-only stale cached TOWER PASS against a live member FAIL. Swept: creations keyed on var_forms-recognized Assign NODES (var_form_nodes), plus a REBIND CLAUSE flagging every Store of a paper variable outside its recognized creation; double-mapped names dropped from vf. F268-2 — check() calls sanctioned only on the declared surface (bare PAPER_NEEDLES or a comprehension over it); an undeclared-literal check now fails. F268-3 — any non-FunctionDef binding of norm refuses norm-based mappings (the lambda-shadow camouflage closes). F268-4 (cosmetic) — the closure sentence re-sworn to tripwire scope. F268-5 (cosmetic) — seq entries reject dead min/max. F268-6 (cosmetic) — the g2 observed-count evidence restored. Post-sweep battery: full live tower **19 live PASS + 0 cached + 0 FAIL (19/19)**, every key rotated.

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m → 266: 0M+3m+2c → 267: 1M+3m+2c → 268: 1M+2m+3c swept. Convergence round 269 next.**

# Round 269: convergence test on the round-268 sweep — NOT CONVERGED: 3 MAJORS + 1 minor + 1 cosmetic; both argument positions of the new clause, and a dotted-import reach escape; swept; round 270 owed

**Held under attack:** the round-268 fixes on every named shape (twelve rebind-blind-spot probes caught; the double-binding drop cannot go invisible — the dup assigns trip clauses iii/v; norm-shadow refusals fire; the g2 evidence exact; the full regression suite reproduced; quantifier census exact; the reviewer re-ran the ENTIRE tower forced-fresh: 19 live PASS + 0 cached + 0 FAIL). **Findings (all swept, 77f7a3d):** F269-1 MAJOR — the check-call sanction policed only the entries argument; an arbitrary transform in the TEXT position (or pre=/forms) sanctioned itself: demonstrated paper-edit-only stale cached TOWER PASS. F269-2 MAJOR — the comprehension shape checked the generator, never the element: undeclared literal entries smuggled through (second demonstrated stale PASS). Swept together: calls sanctioned only at the exact committed shape (all 55 committed sites already conform). F269-3 MAJOR — dotted local imports silently missed by the reach walker: a package-housed helper escaped scan AND key (stale against paper and code edits; third demonstration). Swept: dotted modules resolve as path segments, module file + every __init__.py joining the reach. F269-4 — the header's surviving closure sentence re-sworn (every-surface marking obligation). F269-5 (cosmetic) — the rebind clause's non-Name binding scope documented with the fail-safe argument. Post-sweep battery: driver-only diff, cached tower **0 live + 19 cached + 0 FAIL (19/19)**; the reviewer's forced-fresh **19/19** stands on the identical member set.

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m → 266: 0M+3m+2c → 267: 1M+3m+2c → 268: 1M+2m+3c → 269: 3M+1m+1c swept. Convergence round 270 next.**

# Round 270: convergence test on the round-269 sweep — NOT CONVERGED: 1 MAJOR; the round-264 f-string exemption's assign-then-compare channel; swept; round 271 owed

**Held under attack:** the round-269 sweep in full (_call_ok vs starred/getattr/alias/filter attacks; pre=/forms honest by construction; the dotted-import fix positive; all prior probes reproduced; driver-absent-from-keys verified by key recomputation; cached tower 19/19; type_counting 12/12). **Finding (swept, f4fbb74):** F270-1 MAJOR — the blanket FormattedValue exemption (round-264 code) let `mirror = f"{paper}"` launder the paper into a plain variable invisible to every clause; a compare on the mirror was a live gate the driver never re-verifies — demonstrated as a paper-edit-only stale cached TOWER PASS. Swept: the sanction narrowed to the committed idiom exactly — f-string paper-var loads sanctioned only as DIRECT gate()/print() arguments (all nine committed sites conform and stay); assignment/return/nested/comparison positions flag at the interpolation; the false closure sentence struck. Also corrected in the record: A406's "55 committed call sites" is 53 scanned sites (all conforming) plus the driver's own two unscanned precheck calls. Post-sweep battery: driver-only diff, cached tower **0 live + 19 cached + 0 FAIL (19/19)**.

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m → 266: 0M+3m+2c → 267: 1M+3m+2c → 268: 1M+2m+3c → 269: 3M+1m+1c → 270: 1M swept. Convergence round 271 next.**

# Round 271: convergence test on the round-270 sweep — NOT CONVERGED: 1 MAJOR; the sanction's callees trusted by name (the F267-4 class in the round-270 fix); swept; round 272 owed

**Held under attack:** the round-270 positional narrowing (mirror/return/nested/keyword probes all flag; the ten committed sites are direct positional gate arguments, none a verdict path); the alternative-laundering battery flagged on every route (format, %, str, join, operator.contains, method references, bytes-mode, container membership); all prior probes reproduced; driver-absent-from-keys re-verified; cached tower 19/19. **Finding (swept, 53ad98b):** F271-1 MAJOR — `gate(f"{paper}")` was sanctioned whatever `gate` was: a rogue gate def routing detail into the verdict made the evidence f-string a live paper gate — demonstrated paper-edit-only stale cached TOWER PASS through recognized surface alone. Swept with the _norm_canonical pattern: the sanction requires every gate def byte-shape-identical to one of the three committed print-evidence shapes, no other gate binding, no print binding at all; non-canonical files get no sanction (probed both ways). Record corrected: ten evidence f-string sites (fourteen loads), not nine. Post-sweep battery: driver-only diff, cached tower **0 live + 19 cached + 0 FAIL (19/19)**.

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m → 266: 0M+3m+2c → 267: 1M+3m+2c → 268: 1M+2m+3c → 269: 3M+1m+1c → 270: 1M → 271: 1M swept. Convergence round 272 next.**

# Round 272: convergence test on the round-271 sweep — NOT CONVERGED: 1 MAJOR + 1 minor; the canon checks saw only FunctionDef bindings; swept; round 273 owed

**Held under attack:** the round-271 canon machinery on every enumerated branch (rogue FunctionDefs, Name-Store rebinds, import aliases, global/walrus/decorated cases all refuse; the 27 committed gate defs exactly 9+4+14 across three shapes, zero non-canon; all prior-round probes reproduced; driver-absent-from-keys; cached tower 19/19). **Findings (all swept, 6a6ced4):** F272-1 MAJOR — `class gate` / `async def gate` (and class/async print) bindings passed every branch of _gate_print_canonical, and a class constructor routing the sanctioned f-string into the verdict was a demonstrated paper-edit-only stale cached TOWER PASS; the check also sanctioned vacuously with no gate def at all. Swept: every binding node type refuses; found-guard added (gate-call f-strings need a canonical FunctionDef gate present). F272-2 — _norm_canonical shared the FunctionDef-only shadow scan (fail-safe, but the cited template was holed); class/async bindings of norm now refuse. Post-sweep battery: full live tower **19 live PASS + 0 cached + 0 FAIL (19/19)**, every key rotated.

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m → 266: 0M+3m+2c → 267: 1M+3m+2c → 268: 1M+2m+3c → 269: 3M+1m+1c → 270: 1M → 271: 1M → 272: 1M+1m swept. Convergence round 273 next.**

# Round 273: convergence test on the round-272 sweep — NOT CONVERGED: 1 MAJOR; the canon guards missed the raw-string binding forms; swept; round 274 owed

**Held under attack:** the round-272 fixes at the unit level (class/async gate/print refuse; vacuous sanction withheld; class/async norm refuse); zero regression across the full prior-round probe suite; the recorded post-sweep full live 19/19 battery verified genuine by independent key recomputation. **Finding (swept, 92040aa):** F273-1 MAJOR — "ANY binding node" was false: parameters, except-as, match captures, and star imports bind through plain strings invisible to Name-node walks, and the parameter shadow (paper content arriving as the sanctioned f-string argument, the rogue callee arriving as the parameter) was a demonstrated paper-edit-only stale cached TOWER PASS. Swept: paper_needles.shadow_bound enumerates the raw-string forms; both canon guards refuse when a guarded name is so bound; the overclaim re-sworn; zero such bindings in the committed reach. Post-sweep battery: full live tower **19 live PASS + 0 cached + 0 FAIL (19/19)**, every key rotated.

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m → 266: 0M+3m+2c → 267: 1M+3m+2c → 268: 1M+2m+3c → 269: 3M+1m+1c → 270: 1M → 271: 1M → 272: 1M+1m → 273: 1M swept. Convergence round 274 next.**

# Round 274: convergence test on the round-273 sweep — NOT CONVERGED: 1 MAJOR + 1 minor + 1 cosmetic; the evidence-f-string sanction RETIRED rather than patched; swept; round 275 owed

**Hash note:** the branch was rebased after rounds 264–273 were recorded; the commit hashes in those tables are stale (the round-273 sweep "92040aa" is e3cb3a7; the round-272 sweep "6a6ced4" is a13bff5). The trees, not the hashes, are the record.

**Held under attack:** every round-273 refusal at the unit level (parameter, except-as, star-import, import-as, global, match-capture, lambda-parameter, MatchClass keyword, with-as, for-target, walrus, comprehension, annotated, class-body, nonlocal); both return semantics withhold, never grant; zero raw-string bindings in the 64-file reach; precheck 64/28 reproduced; the full prior-round probe suite rebuilt and reproduced. **Findings (all lead-verified):** F274-1 MAJOR — three attribute-store routes (`gate.__code__` swap, `builtins.print` rebinding, `sys.stdout` replacement) sanctioned by both canon guards with every name binding canonical; full paper-edit-only stale cached TOWER PASS demonstrated on a 22-member mirror, three routes; the structural verdict adopted: call-time callee/sink identity is invisible to any static binding walk, so enumeration cannot close the class. F274-2 minor — the committed cache stamps match no tree in the rebased history (the rebase changed reach bytes); fail-safe (19 live). F274-3 cosmetic — PEP 695 type-parameter names outside shadow_bound (non-exploitable; 3.11 runtime).

**The sweep (3c68dff):** the sanction RETIRED — the ten evidence f-string sites (five members) replaced by plain labels, `_GATE_CANON`/`_gate_print_canonical` and the f-string sanction deleted, clause (iii) re-sworn (any f-string interpolation of a paper var flags); shadow_bound gains the 3.12 type-parameter forms and loses its stale driver reference; manifest refreshed. Precheck after the retirement clean (64/28). Post-sweep battery: full live tower **19 live PASS + 0 cached + 0 FAIL (19/19)**, every key rotated, cache re-established (049e0a4).

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m → 266: 0M+3m+2c → 267: 1M+3m+2c → 268: 1M+2m+3c → 269: 3M+1m+1c → 270: 1M → 271: 1M → 272: 1M+1m → 273: 1M → 274: 1M+1m+1c swept (the sanction retired). Convergence round 275 next.**

# Round 275: convergence test on the round-274 sweep — NOT CONVERGED: 1 MAJOR + 1 minor + 1 cosmetic; the member-side policing strategy RETIRED — members never hold paper text (owner's decision); swept; round 276 owed

**Held under attack:** every round-274 route flags at the f-string interpolation regardless of callee/sink trick; every non-f-string content path flags; the ten retired sites' needles declared with consistent mins and never verdict inputs; driver hygiene; `_TYPEPARAM_NODES` on 3.10–3.13; manifest; all 19 cache keys recomputed and matched; A411 numerics; cached battery 0 live + 19 cached (19/19). **Findings (all lead-verified):** F275-1 MAJOR — four route families in the precheck's remaining sanctioned surface (forms-call subtree in any expression position, walrus in the comprehension filter, cross-module paper var, runtime mutation of the declaration), each a demonstrated paper-edit-only stale cached TOWER PASS; zero uses in the committed reach. F275-2 minor — the round-274 record's "manifest refreshed / five members changed" was false (reach files, not members; manifest byte-identical). F275-3 cosmetic — a docstring escape warning in a function now deleted.

**The sweep (ef6c5f5; owner's decision among "structural", "four exact tightenings", "pause"):** `paper_needles.py` becomes the only reader in any member's reach, exposing `verify`/`needle`/`declared` on the declared literal with no paper text leaving it; all 28 declared reach files migrated mechanically (28 path constants + 43 read/transform assignments deleted; 41 `check` → `verify`; 140 inline compares → `needle`; zero residual paper names); the precheck collapses to five shape clauses (A)–(E) plus the live evaluation, and the twelve rounds' member-side clauses are deleted with the reads they policed; 29 probes green. Precheck 63/28 clean. Post-sweep battery: full live tower **19 live PASS + 0 cached + 0 FAIL (19/19)**, every key rotated, cache re-established (4e2e259).

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m → 266: 0M+3m+2c → 267: 1M+3m+2c → 268: 1M+2m+3c → 269: 3M+1m+1c → 270: 1M → 271: 1M → 272: 1M+1m → 273: 1M → 274: 1M+1m+1c → 275: 1M+1m+1c swept (the strategy replaced, not the clause). Convergence round 276 next.**

# Round 276: convergence test on the round-275 restructure — NOT CONVERGED: 1 MAJOR + 4 cosmetic; the sanctioned function objects and alternate import spellings breached the closure; closed exactly against the reach census; probe suite committed; round 277 owed

**Held under attack:** migration fidelity over all 28 files by scripted AST comparison (140 ↔ 140, forms and bounds matched; 41/43/28/7); every clause-(C) form flags; `declared()` copies independent; vacuous `verify` not a stale-PASS route and absent from the reach; driver hygiene; manifest; 19 keys recomputed and matched; A412 numerics; cached battery 0 live + 19 cached (19/19). **Findings (all lead-verified):** F276-1 MAJOR — `paper_needles.verify.__globals__["_TEXTCACHE"]`, `from tools.research import paper_needles as pn`, `import tools.research.paper_needles` (and the lead's siblings `cascade_floor_theory.paper_needles.PAPER_PATH`, `__import__("paper_needles")`) reach the text plainly under the round-275 clauses; stale cached PASS demonstrated, three routes. F276-2/-4 cosmetic (record wording: "three" listing four; two checkpoint commits). F276-3 cosmetic — probes uncommitted. F276-5 cosmetic — clause wording omits `declared`.

**The sweep (350de33):** the sanctioned attributes must be immediately called; no import or constant names `paper_needles` beyond the exact plain import; no attribute named `paper_needles`; no dunder attribute reach-wide except `__init__`/`__name__` — each exact against the census (4 dunders, 0 module-naming attributes/constants, 28 plain imports); the false "reachable only by computed spelling" struck at source; `precheck_probes.py` committed (51 cases, all as expected). Precheck 63/28 clean. Battery (docstring-only class; no key rotated): cached tower **0 live + 19 cached + 0 FAIL (19/19)**; TOP verifier `cascade_oneprime_interval.py` live **13/13**; probes 51/51.

**Trajectory: … → 274: 1M+1m+1c → 275: 1M+1m+1c → 276: 1M+4c swept (the sanctioned surface's coverage, now a finite censused question). Convergence round 277 next.**

# Round 277: convergence test on the round-277 sweep — NOT CONVERGED: 1 MAJOR + 2 cosmetic; the in-process reader was enclosed by nothing the member did not control; the paper is now evaluated OUT OF PROCESS; round 278 owed

**Held under attack:** every clause-(B) value-load and import spelling; the `misses` handle (no stale-PASS relevance); clause (F) census exact; probe suite exec's the committed driver, outside every reach; A413 numerics; cached tower 0 live + 19 cached; TOP 13/13; probes 51/51. **Findings (all lead-verified):** F277-1 MAJOR — `re.sub = hook` captured the whole paper before one legal `verify`; `sys.settrace`/`setprofile`/`addaudithook` handed the member the reader's frame; `sys.modules`/`globals()`/`gc` enumeration found the module without naming it; stale cached PASS demonstrated, three routes. F277-2 cosmetic — 29 plain imports in 28 files, not 28. F277-3 cosmetic — "never touch a dunder" covered attributes only.

**The sweep (321d411):** `verify`/`needle` evaluate in an isolated child process (`-I`, scrubbed environment) returning only `(ok, misses)`; the parent holds no text and no path (re-run: the `re.sub` hook captures 0 characters, the trace hook sees no text frame); child-subversion and file-reading spellings closed exactly against the census — (G) no store on or alias of an imported module except the 42 mpmath precision stores, (H) no hook/enumeration attribute, call, or import (census zero), (I) no introspection dunder as a string; disclosure re-sworn; `precheck_probes.py` 74 cases all as expected; precheck 63/28 clean; three members live PASS. Post-sweep battery: full live tower **19 live PASS + 0 cached + 0 FAIL (19/19)**, every key rotated, cache re-established (eba60b7, 2c0e16b).

**Trajectory: … → 275: 1M+1m+1c → 276: 1M+4c → 277: 1M+2c swept (the text removed from the member's process). Convergence round 278 next.**

# Round 278: convergence test on the round-278 sweep — NOT CONVERGED: 1 MAJOR + 2 cosmetic; clause (G) closed at the binding forms; and the owner's scope decision — deliberate self-subversion out of scope from round 279; round 279 owed

**Held under attack:** the `-I` child's path carries neither the script directory nor the cwd; environment inert; child evaluation = driver evaluation on all 28 declarations / 440 entries; every child failure mode exits 1; no text or path in the parent module; A414 numerics; cached tower 0 live + 19 cached; keys 19/19; probes 74/74. **Findings (all lead-verified):** F278-1 MAJOR — clause (G) rooted at Names only: a parameter, tuple element, or loop variable handed the module object to a store (`def _f(m): m.run = …; _f(subprocess)`, `(sys,)[0].executable = …`); stale cached PASS demonstrated twice. F278-2 cosmetic — `dps`/`prec` whitelisted on any root. F278-3 cosmetic — the `-I` child still imports the installation's `sitecustomize` (disclosure).

**The sweep (37773ee):** no store target with a non-Name root; no bare load of an `import`-bound module name outside an Attribute value; no `"-c"` constant — each exact against the census (0/0/0); `dps`/`prec` pinned to `mp`/`iv`; the installation-write residual disclosed; probes 84 cases all as expected; precheck 63/28 clean. **Owner's decision (recorded in `CLAUDE.md` and A415):** the precheck is a drift-detection instrument, not a sandbox; deliberate self-subversion of a member's process is out of scope from round 279 — reported as an observation, never a MAJOR/minor. Battery (driver/probe class, no key rotated): cached tower **0 live + 19 cached + 0 FAIL (19/19)**; TOP verifier live **13/13**; probes 84/84.

**Trajectory: … → 276: 1M+4c → 277: 1M+2c → 278: 1M+2c swept; the standard scoped by the owner. Convergence round 279 next, under the drift-detection standard.**

# Round 279: convergence test on the round-279 sweep under the drift-detection standard — CONVERGED: 0 MAJOR + 0 minor + 2 cosmetic; the paper-needle precheck arc STABLE; cosmetics swept editorially

**Held under attack:** every clause-(G) census recomputed (all zeros as claimed; dps/prec by root mp 40 / iv 2); the Attribute-value exemption exact both ways; every probe expectation read; suite exec's the committed driver and sits outside every reach; child evaluation = driver evaluation (28 declarations, 440 entries, 33 `g` subsets, 2 `seq` chains, 0 mismatches); no text in the parent; A415 and CLAUDE.md numerics and quantifiers; the disclosure's residual items outside the clauses; keys 19/19; cached tower 0 live + 19 cached; TOP 13/13; probes 84/84. **Cosmetics (swept, 3076c5a):** the dps/prec whitelist admitted the unused pair `iv.dps` — pinned to the three committed pairs, with a probe; the residual listed "exec/eval" although the Name calls are clause (H) — reworded; the module docstring's residual refreshed to the driver's clauses and the scope statement. **Out-of-scope observation:** a from-import re-export of a module reaches a parameter store (spawn interference); a zero-census narrow clause exists, held for the owner's discretion. Battery (docstring-only class): probes 85/85; precheck 63/28; cached tower **0 live + 19 cached + 0 FAIL (19/19)**; TOP live **13/13**.

**Trajectory: 264: 2M+2m+1c → 265: 0M+4m → 266: 0M+3m+2c → 267: 1M+3m+2c → 268: 1M+2m+3c → 269: 3M+1m+1c → 270: 1M → 271: 1M → 272: 1M+1m → 273: 1M → 274: 1M+1m+1c → 275: 1M+1m+1c → 276: 1M+4c → 277: 1M+2c → 278: 1M+2c → 279: 0M+0m+2c — CONVERGED under the owner's drift-detection standard. The A397 needle arc is stable; the research front is unblocked.**

# Round 280: hostile review of the Theorem 1bk landing (the two-prime window) — NOT CONVERGED: 0 MAJOR + 3 minors + 2 cosmetics + 1 latent observation, all lead-verified and swept (f5c2689); round 281 owed

**Held under attack:** the odd pole's sign and parity (−2⟨sinh(t/2),·⟩², both ways numerically); the pole-inclusive decomposition and the Weyl step with no interlacing; the closed-form pole column and corner (3e-16 against quadrature); the bordered Gram exactly as stated; C₃, qmax, the tail lemma (2.3102 > 2.05; h₊ increasing on [0, 700]); the W₂₃ enclosures against 40-digit mpmath; every Stage-2 substitution line (24 + 10 within the copied segments; nothing still one-prime); the window and nesting arithmetic; every pin digit-for-digit; both keys (`code_sha` = `script_sha256`, every dep sha on disk); the census (29 surfaces); Checks 7/8; the reviewer's own battery — TOP 11/11 with both checkpoints REUSED, probes 85/85, manifest recompute equal, precheck 68/29, cached tower 0 live + 20 cached (20/20), and a live re-derivation of the Stage-1 geometry identical to the checkpoint. External CC text: not obtained (as in 1bj's round). **Findings (all lead-verified):** F280-1 minor — the footer's enumerated list never received `cascade_twoprime_interval.py` (the count string, which the needles gate, was right). F280-2 minor — certified bounds printed nearest-rounded in the unsafe direction (1bk: 6.3136e-6 over the raw 6.3135876e-6 by 1.2e-11; ρ.lo, σ², the margin; inherited: four 1bj values). F280-3 cosmetic — the prime-shift kink is a jump of Tφ at the *interval* location LOG3 − a, split at the float boundary, unbudgeted by the derivative bound inside the adjacent cell (≤ 2e-16 on S against widths 5e-9; same at k0 in the one-prime instrument). F280-4 minor — "any entire trial" a universal supported by two float families. F280-5 cosmetic — an absolute ρ tolerance that could not distinguish ρ from zero (g10, both gT2). Observation — a kink inside the graded region would be silently dropped (not triggered at this cell).

**The sweep (f5c2689):** the footer list extended; every stated bound rounded in the safe direction (lower down, upper up, enclosures outward) in 1bj and 1bk; the kink-jump budget |φ|·J·D on M and (2|Tφ|·J + J²)·D on S in both Temple instruments with the graded-region assertion; the quantifier narrowed to the reconnaissance's families; relative ρ tolerances; manifest refreshed. Battery: battery 1 (kink budget, f5c2689) both Temples recomputed value-identically, 2 live + 18 cached, TOWER PASS 20/20; the lead then found the instruments' own prints nearest-rounded (F280-2's class) and swept them (a749866, `_fdir`), rotating every interval key; battery 2: both counts, Stage II-b, and both Temples recomputed (every certified value reproduced; prints now safe-direction and matching the paper digit for digit), precheck 68/29, `census: 2 live PASS + 18 cached PASS + 0 FAIL of 20`, `TOWER PASS (20/20)`.

**Trajectory: … → 279: 0M+0m+2c (the needle arc stable) → 280 (the 1bk landing): 0M+3m+2c swept. Convergence round 281 next.**

# Round 281: convergence test on the round-280 sweep — NOT CONVERGED: 0 MAJOR + 1 minor + 1 cosmetic + 1 out-of-scope observation, both findings lead-verified; the sweep's two-hour battery aborted by the owner and the PROCESS fixed instead (print-insensitive keying + provenance-verified checkpoint migration, 6fc3bc0); round 282 owed

**Held under attack:** the kink budget's mathematics (jump size against a live two-sided evaluation of the operator at the interval kink; the midpoint-rule error with one jump per cell; D from the straddling interval; signs and application on M and S; the pole flag) and an independent recompute of the two-prime kink budget from the committed fixture, digit-identical to the checkpoint; the D0 ordering; every F280-2 paper figure re-derived from the raw current-key checkpoints; the `_fdir` routes at every certified print; pre-sweep vs current checkpoint identity (relative 10⁻¹² and below); the footer census (87 + 4); F280-4's wording; F280-5's relative tolerance gating; the pins as tolerance pins; Checks 7/8; the reviewer's own battery — TOP 11/11 and member 19 13/13 in manifest mode with every checkpoint REUSED (the TOP's default full-chain mode outran the reviewer's cap: not obtained in that mode), probes 85/85, cached tower 0 live + 20 cached (20/20). External CC text: not obtained. **Findings (lead-verified):** F281-1 minor — the F280-2 class survived at three stated bounds (1bk's μ₂ enclosure top 1.990867 against the raw 1.9908673; 1bj's Stage II-b margins 2.486e-3/3.016e-3 against the raw 2.4858e-3/3.0158e-3), with the count and pole instruments' own μ₂/margin prints still nearest-rounded. F281-2 cosmetic — `_fdir` rounded the shortest repr, not the exact binary value (a sub-ulp corner no committed print hits). Observation — kinks at ≤ DEDGE or ≥ a are skipped by the budget loop (sound where reachable; unreachable otherwise).

**The sweep:** the three paper figures moved to the safe direction (7642372); the first instrument sweep (all five instruments, incl. the `_fdir` body) rotated every interval key and owed a two-hour recompute, which the owner aborted ("Abort and fix the process"). **The fix (6fc3bc0):** `ckpt_key.code_sha` ignores pure print statements (side-effect-free by construction; whitelisted formatting calls only; anything else stays in the hash), the legacy hash kept for `run_tower`'s member reach key; `ckpt_migrate.py` re-files checkpoints under new keys only on a proved chain of custody (every dep hash resolves to a git-historical version, the stored key recomputes from it, historical and working-tree code agree under the new hash) — 17 migrated, 18 stale old-key files left alone; `ckpt_key_probes.py` 19/19; the instruments re-based to the checkpointed code with the count prints re-routed as pure print edits; the pole prints and the `_fdir` exact-binary form deferred as accepted residuals. Battery: TOP 11/11 and member 19 13/13 REUSED at the migrated keys, precheck probes 85/85, tower `census: 2 live PASS + 18 cached PASS + 0 FAIL of 20`, `TOWER PASS (20/20)`. No compute state recomputed.

**Trajectory: … → 279: 0M+0m+2c (the needle arc stable) → 280 (the 1bk landing): 0M+3m+2c swept → 281: 0M+1m+1c swept, the process fixed. Convergence round 282 next (its brief also attacks the keying rule and the migration).**

# Round 282: convergence test on the round-281 sweep + hostile review of the keying process change — NOT CONVERGED: 0 MAJOR + 1 minor + 2 cosmetics + 3 out-of-scope observations, all lead-verified and swept; round 283 owed

**Held under attack:** every certified figure in 1bj/1bk re-derived from the current-key checkpoints with exact-binary rounding (all safe-direction; no stale figure on any surface); the count instruments differ from 579e977 only in print hunks, Temples/pole/core byte-identical; F281-2's corner zero reach; the kink observation; 45 strip-rule constructions (mutating calls, `key=`, starred, `**d`, walrus/lambda/await/yield, indirect calls, `file=`, `flush=f()`, `print.__call__`, computed format specs — all kept; iterator consumers, `get`, conditionals, subscripts, `mid`, `time.time()` — stripped) with the structural checks (no non-print statement's dump altered; 294 print-free files identical under both modes); the legacy hash equal to 579e977's on all 294 files; the legacy split (18 member keys unchanged, the two live keys reproduced by an independent `member_key`); the custody proof re-derived for all 17 migrated files with zero failed checks; all 18 refused files stale under the old keying; CLAUDE.md's quantifiers; the batteries — TOP 11/11 and member 19 13/13 REUSED at the migrated keys, precheck probes 85/85, ckpt_key probes 22/22, migrate dry `blocked 14, exists 17, migrate 17, skip 127, unchanged 117, unverifiable 4`, tower 0 live + 20 cached (20/20). Not obtained: the "r = 435" last-piece figure (sourced to A426's float reconnaissance; the reviewer's recompute timed out twice). **Findings (lead-verified):** F282-1 minor — the probe suite's docstring claimed a `blocked` migration case it did not contain. F282-2 cosmetic — "side-effect-free by construction" overstated a syntactic rule (iterator-consuming prints; zero reach by census). F282-3 cosmetic — the migration's working-tree fallback contradicted CLAUDE.md's "git-historical" and was not re-verifiable. Observations — A428 named `ckpt_key.py` in the live members' reach (it is in none; corrected); the keying module outside every reach means a keying change re-verifies no member live; the reconnaissance producers' hand-listed deps one level short (pre-existing).

**The sweep:** M5 (`blocked` from a real historical blob of `twoprime_recon.py`; 24 cases); the wording made precise in `ckpt_key.py` and CLAUDE.md, the stricter clause held for the owner; the working-tree fallback removed; `run_tower.py` runs the keying probes as a precheck on every invocation and fails on any unexpected case; A428 corrected. Battery: TOP 11/11 and member 19 13/13 REUSED at the migrated keys; precheck probes 85/85; ckpt_key probes 24/24; migrate dry census unchanged; tower with the new keying-probe precheck line, 0 live + 20 cached, TOWER PASS 20/20 (2439c71).

**Trajectory: … → 280 (the 1bk landing): 0M+3m+2c swept → 281: 0M+1m+1c swept, the process fixed → 282: 0M+1m+2c swept. Convergence round 283 next.**

# Round 283: convergence test on the round-282 sweep — NOT CONVERGED: 0 MAJOR + 1 minor + 3 cosmetics + 4 out-of-scope observations, all lead-verified and swept; round 284 owed

**Held under attack:** M5 live and every docstring-claimed probe case present; the syntactic-rule wording against the code and an independent iterator-consumer census of the keyed closures (5 sites, all over sized containers); the migration git-only with every migrated key derived from committed blobs; the tower's keying gate failing closed on an emptied and on a thinned suite before any member runs (0.47 s); A428's correction; CLAUDE.md's bullet clause by clause; the held items; the batteries at their recorded censuses. **Findings (lead-verified):** F283-1 minor — 1bk(vi) listed `twoprime_margin.py` among the imported, content-addressed substrates; it is in no closure, no state's deps, and not in member 20's reach (false provenance on the paper surface since the landing). F283-2 cosmetic — `_history`'s docstring still described the removed working-tree fallback. F283-3 cosmetic — the probe docstring's "print inside a lambda body" for a lambda-inside-a-print case. F283-4 cosmetic — M5 needed git history (a shallow clone failed the tower closed). Observations — a forged census line would pass an unpinned probe file (O1); a count floor under-pins a growing suite (O2); the brief understated the gate (O3); the scratch tempdir never removed (O4).

**The sweep:** the paper sentence struck and annotated (`twoprime_margin.py` committed and cited, imported by neither instrument, in no key); both docstrings aligned; M5 hermetic (synthetic historical version injected into the resolver) with M5r keeping the real-blob refusal or, on truncated history, the unverifiable refusal; the keying machinery integrity-pinned in the manifest (refresh script + run_tower fail-closed check); the probe count pinned exactly (24; a stale 25 pin failed the first run closed — the gate's own demonstration); tempdir removed. Battery: probes 24/24; tower with the pinned keying files and the exact case pin, 0 live + 20 cached (20/20); TOP 11/11 and member 19 13/13 REUSED; precheck probes 85/85; migrate dry unchanged (60c1467).

**Trajectory: … → 281: 0M+1m+1c swept, the process fixed → 282: 0M+1m+2c swept → 283: 0M+1m+3c swept. Convergence round 284 next.**

# Round 284: convergence test on the round-283 sweep — NOT CONVERGED: 0 MAJOR + 2 minors + 4 cosmetics + 3 out-of-scope observations, all lead-verified and swept; round 285 owed

**Held under attack:** the F283-1 strike and its surviving sentence (both reconnaissance files in both closures, both states' deps, member 20's reach); the F283-1 class across 1be/1bf/1bg/1bi; both docstrings claim by claim (24 cases); the hermetic M5 on a shallow clone; the keying pins (stale pin, pin-less manifest each exit 2; chain mode accepting the new key); the exact case pin (25, 23, and an inverted case each fail closed); the batteries at their recorded censuses. **Findings (lead-verified):** F284-1 minor — the pin gate checked a count, not the names (a forged manifest with one file pinned thrice passed). F284-2 minor — 1bj placed `oneprime_deflate.py` inside the keyed fixture chain; it is in no key, no closure, no reach (the F283-1 class one theorem earlier). F284-3 cosmetic — 1bh's "imported" false for `height_residue.py` (keyed through an explicit tuple). F284-4 cosmetic — the footer's descriptor for the two-prime files after the round-283 strike. F284-5 cosmetic — four probe cases absent from the docstring. F284-6 cosmetic — `rev-list` without `--full-history`. Observations — the unpinned trusted computing base (`precheck_probes.py` the next O1 candidate); manifest consumers unaffected; M5's injection masks nothing.

**The sweep:** the pin gate checks the set by name; 1bj's sentence struck and annotated (`oneprime_deflate.py` committed and consumed by gP3 at recompute time, in no key); 1bh's "imported" struck and annotated; the footer descriptor amended; the docstring completed; `--full-history`; `precheck_probes.py` pinned and run by the tower with its census pinned exactly. Battery: probes 24/24 and 85/85 inside the tower, 4 keying files pinned, 0 live + 20 cached (20/20); TOP 11/11, member 19 13/13, height member 13/13 in manifest mode; migrate dry unchanged (a704833).

**Trajectory: … → 282: 0M+1m+2c swept → 283: 0M+1m+3c swept → 284: 0M+2m+4c swept. Convergence round 285 next.**

# Round 285: convergence test on the round-284 sweep — CONVERGED: 0 MAJOR + 0 minor + 3 cosmetics + 2 out-of-scope observations; the round-280 chain STABLE; cosmetics swept editorially

**Held under attack:** the by-name pin gate (three shapes fail closed, the committed manifest passes, refresher and driver agree); the 1bj strike with every still-named file in the closure, reach, and all four keys (deps = closure exactly); the gP3 annotation true and the held deflation question closed (no certified bound depends on the data at reuse time); the 1bh annotation (17 checkpoints carry the file through the explicit tuple); the footer and every member's needles live; the probe docstring complete; `--full-history` losing nothing over 37 files; the needle-probe gate failing closed at 84/86 cases and the integrity pin catching a forged suite; the whole provenance class tabulated across 1bc–1bk with no false claim beyond F285-2; the batteries at their recorded censuses. **Findings (lead-verified):** F285-1 cosmetic — the set comparison admitted a duplicate pin entry with a false count. F285-2 cosmetic — "in no key" over-broad for `oneprime_deflate.py` (it keys its own states). F285-3 cosmetic — the exact pins undocumented for an editor and unnamed in the failure prints. Observations — `floor_probe6–9.py` gated by no member (pre-existing); four legacy provenance-less checkpoints.

**The sweep (editorial):** length equality added to the pin gate; the annotation narrowed; both suites', the refresher's, and CLAUDE.md's docstrings state the pins and the refresh obligation; the failure prints name the expected count. Battery: tower 20/20 with both precheck censuses and 4 pins, three verifiers in manifest mode, probes 24/24 and 85/85, migrate dry unchanged (2d78600).

**Trajectory: 280 (the 1bk landing): 0M+3m+2c → 281: 0M+1m+1c (the process fixed) → 282: 0M+1m+2c → 283: 0M+1m+3c → 284: 0M+2m+4c → 285: 0M+0m+3c — CONVERGED. The 1bk landing, the safe-direction sweep, the keying change and migration, and the provenance strikes are stable. Next: the mechanism theorem's landing (A425/A427), with its own hostile round.**

# Round 286: hostile review of the Theorem 1bl landing (positivity by mechanism) — NOT CONVERGED: 1 MAJOR + 5 minors + 2 cosmetics + 3 out-of-scope observations, all lead-verified and swept; round 287 owed

**Held under attack:** the domination bound (W_∞ increasing by its series against mpmath; Plancherel with both signs of r; the band normalisation and λ_k = (c/2π)|μ_k|² with no missing factor; the pole sign against 1bk; W_out to 30 digits); the tail inequality with the ε-correction to Λ_tail derived and matched line by line; the head (the pencil direction, the midpoint-free ball Cholesky, the slop term derived, Trefethen 19.3 as applied, the stadium-in-ellipse covering, the Stirling remainder and domain, M1 as a Lipschitz constant, the node grid covering both end gaps); the prolates (the tridiagonal against Xiao–Rokhlin–Yarvin and scipy's `pro_cv`, the Schur widening via Gershgorin and Haynsworth, the √2, the Bessel identity); the cells; the four numbers recomputed fresh byte-identical; the paper's decades, margin to log 4, Ω ≳ 120, the net-state marker, the footer census (88 = 88), every needle, the substrate closure and the reconnaissance files in no closure/key/reach; the member's mangle probes and g10's direction; the batteries. **Findings (lead-verified):** F286-1 MAJOR — the band-integral cells (h = 1/200, 1/400) had float boundaries and did not tile [0, 1] exactly: gaps of order 10⁻¹⁶ per boundary, an error term outside the certified chain, 10⁷ × the reported entry radii and, crudely accounted, larger than the two-prime even margin; the committed two/odd final exceeded the exactly-tiled value by 1.6×10⁻¹⁵. F286-2 minor — seven upper bounds in (iv) rounded down below the stored maxima. F286-3 minor — (v) claimed a disproof capability the one-sided bound lacks. F286-4 minor — "every quantity a ball" not earned by the float scalar assembly. F286-5 minor — the μ_k enclosure dropped the true prolate's Legendre tail beyond the truncation. F286-6 minor — python-flint an unpinned, undeclared hard dependency. F286-7/8 cosmetics — the Jacobian omitted from the per-cell bound sentence; the Sturm points unasserted within the dominance range. Observations — A433's stale keys, A425's missing ½, A427's M1/M_W relabelling (records, corrected in place).

**The sweep:** dyadic cells (1/256, 1/512; asserted a power of two) so the tiling is exact in binary; the four cells recomputed — two/even 5.7135605×10⁻¹³ and two/odd 3.9537008×10⁻¹⁰ (the reviewer's predictions reproduced), the one-prime cells unchanged, every stated bound still ≤ its final; the scalar assembly in balls at safe endpoints; the Legendre-tail term from the diagonally dominant tail block; the Sturm-point assertion; `python-flint==0.9.0` pinned and named; the paper's (iii)/(iv)/(v) re-stated with the round named at each strike; the stale old-key checkpoints removed; the migration tool recognising checkpoints already at the current keying. Battery: the four cells fresh at their rotated keys, member 21 11/11, tower 1 live + 20 cached (21/21), TOP 11/11, probes 24/24 and 85/85, migrate dry with the four new cells reported current (10f2abe).

**Trajectory: 280–285 (the 1bk chain) stable → 286 (the 1bl landing): 1M+5m+2c swept. Convergence round 287 next, with license to re-attack the whole instrument.**

# Round 287: convergence test on the round-286 sweep of the 1bl landing — NOT CONVERGED: 1 MAJOR (process: the manifest stale at HEAD) + 2 minors + 1 cosmetic + 3 out-of-scope observations, all lead-verified and swept; round 288 owed

**Held under attack:** the dyadic tiling exact in rational arithmetic (zero mismatches at 256/512 cells; the omitted term identically zero); the Gauss–Legendre roots and weights rigorous balls; the four cells fresh byte-identical to the tracked states, every stated bound ≤ its final; every (iv) figure in the safe direction; the (v) pair; the `float(` census of the assembly (upper/lower endpoints only, midpoints only as seeds or exact dyadics); the Legendre-tail derivation with the basis bounds proved for all n; the migration branch's refusals; python-flint pinned; the Jacobian; the Sturm-point assertion; the checkpoint set; g4's threshold. **Findings (lead-verified):** F287-1 MAJOR — `tower_manifest.json` pinned `ckpt_migrate.py` at its pre-sweep sha (the manifest refreshed and the tower run before the sweep's last edit), so the committed tree fails the integrity precheck and the recorded battery is not reproducible at HEAD; the refresh is one line and the tower then passes 21/21. F287-2 minor — the odd-sector tail bound summed 2n at degrees L + j instead of L + 2(j − 1), valid only for q ≤ ½ (q = 0.008–0.048 at the cells). F287-3 minor — the node radius "2×10⁻⁷⁴" against 2.03×10⁻⁷⁴. F287-4 cosmetic — two nearest floats upstream of the μ_k enclosure. Observations — A434's nearest-rounded battery figures and its even-only tail claim; a census label in the migration branch.

**The sweep:** the correct-degree tail sum; `d_tail_min`, `q_t`, `tail_widen` as ball endpoints; the paper's node radius outward; the records corrected; and the ordering enforced — edits, cells recomputed, manifest refreshed, tower on the final tree, commit. Battery: cells recomputed at the rotated keys (finals unchanged), manifest refreshed after every edit, tower on the final tree 1 live + 20 cached (21/21), member 21 11/11, TOP 11/11, probes 24/24 and 85/85, migrate dry unverifiable 4 (77875f8).

**Trajectory: 286 (the 1bl landing): 1M+5m+2c swept → 287: 1M+2m+1c swept. Convergence round 288 next.**

# Round 288: convergence test on the round-287 sweep of the 1bl landing — NOT CONVERGED: 0 MAJOR + 1 minor + 0 cosmetics + 2 out-of-scope observations, lead-verified and swept; round 289 owed

**Held under attack:** F287-1 at HEAD (every pin equal to the HEAD blob; the member-21 cache entry served at HEAD's reach key; the ordering verifiable from the commit); the correct-degree tail sum re-derived and checked numerically for every q; the node radius exact maximum; the F287-4 directions with the monotonicity argument; the four cells fresh byte-identical; every number in the block in the safe direction; the mathematics of (ii) re-attacked end to end and held; the migration branch, both probe suites, the chain, the batteries. **Finding (lead-verified):** F288-1 minor — the quadrature-error term covered the half-integral over [0, 1] once while the head uses the doubled integral; the correct term is 2×; every printed figure, every stated bound, the (v) pair and g4's threshold survive (the two-prime finals move by 5×10⁻²¹ against margins of 5.7×10⁻¹³). Observations — a label slip and the 2×-understated figures in the records.
...
**Findings.** 0 MAJOR, 2 minor, 2 cosmetic — all verified and swept (A458): F301-1 the parsed needle copies tied to the declared entries through the needle API; F301-2 clause (v)'s certified values declared as needles, parsed and pinned directionally in g5; F301-3 g1's label and the docstring; F301-4 the δ index on the two g6 values. Sweep battery: member 13/13 (fifteen needles); manifest 23; TOWER PASS 23/23 (1 live + 22 cached). **Trajectory: 298 (the 1bn landing): 0M+4m+6c → 299: 0M+3m+1c → 300: 0M+1m+2c → 301: 0M+2m+2c → 302: convergence round commissioned.**

### Round 302 — convergence test on the round-301 sweep (Theorem 1bn)
...
**Findings.** 0 MAJOR, 2 minor, 2 cosmetic, 1 observation held, 1 record observation — all verified and swept (A464): F306-1 the resolved residual attributed (a draw of the counting function's fluctuation at the cut, of order ĝ(T)² ≈ 7×10⁻⁷ of the quotient; the range 1×10⁻⁷–4×10⁻⁶ across cuts gated live at three cuts); F306-2 the bench docstring's remainder bound corrected (F(T)/(2a²T), 2.3×10⁻¹⁰ of the quotient); F306-3 twelve balls, not six; F306-4 the ratio range outward. Sweep battery: member 11/11; manifest 24; TOWER PASS 24/24 (1 live + 23 cached). **Trajectory: 304 (the 1bo landing): 0M+4m+5c → 305: 1M+2m+3c → 306: 0M+2m+2c → 307: convergence round commissioned.**

### Round 307 — convergence test on the round-306 sweep (Theorem 1bo)
...
### Round 313 — the landing of Theorems 1bp, 1bq, 1br (the three doors)

**Scope.** The landing commit (three theorem blocks after Theorem 1bo; the footer census 94 / 1i–1br with three verifiers and four substrate clauses; verifiers `cascade_linear_response.py` (member 25), `cascade_lfunction_test.py` (26), `cascade_spectrum_ladder.py` (27, top); substrates `weil_linear_response.py`, `lfun_gram.py`, `lfun_cells.py`, `lfun_zeros.py`, `weil_spectrum_ladder.py`; the census literals advanced across the tower; manifest 27). Full-tower class (a landing). Brief `brief_round313.md`. Commissioned by the owner: "Land all 3 doors and keep in mind we need to find whatever is casting that shadow" (A471).

**Findings.** 1 MAJOR, 4 minor, 7 cosmetic — all verified by the lead and swept (A472): F313-1 (MAJOR) the far-direction knife-edge search never evaluated the interval between its last doubling sample and the cap, and four certified crossings (χ₋₃, p = 2 at δ = 2.3, 2.6, 3.0, 3.5, at 0.87–0.98 of the cap) sat there where the block claimed one — the producer now scans that interval on a 64-point log grid up to the cap, the 28 cells recomputed, the block's sentence and table corrected, the census pinned; F313-2 the normalisation selftest exercised no local coefficient beyond c(2) at δ = 1.0 — a δ = 2.0 cell added and both stated; F313-3 "at no downward ε" asserted λ₁ > 0 — conditioned on positivity at ε = 0, likewise 1br(iii); F313-4 "a property of the cell, not of the arithmetic" — the one-sided conductor- and δ-dependent systematic stated and gated; F313-5 "the rest falling along the ladder" — falling in trend (gated) with a parity alternation up to 0.45 nats stated and gated; C1–C7 (the arXiv identifier, exact pins for 87 and −31.5, the top-rung clause's two cells, "5×10²–3×10⁵", the c = aT₀ coincidence at δ = 2.0 and its rejection at δ = 2.3, three classical inputs, two consistency-check labels). Battery: members 10/10, 11/11 (9/9 unchanged); manifest 27; TOWER PASS 27/27 (2 live + 25 cached). Round 314 (convergence) commissioned.

### Round 314 — convergence test on the round-313 sweep (Theorems 1bp, 1bq, 1br)
...
### Round 317 — the landing of Theorems 1bs (the count-constant theorem) and 1bt (the pole-free form is indefinite)
...
**Findings.** 2 MAJOR, 7 minor, 3 cosmetic; all twelve verified directly by the lead and accepted; swept in this commit (A482). **F1 (MAJOR)** the verifier's safely-deep filter indexed the previous rung's leakage (`pro[2*(k − 1)]` against the stated ln(1 − χ_{2k}) < −20): the census 0, 1, 4, 7, 10, 11, 11 → **0, 0, 3, 6, 9, 11, 11**, the parity strings recomputed on the stated set. **F2 (MAJOR)** the bolded "the parity fine structure of Theorem 1br(ii) lives in c_k" was contradicted by the checkpoints: the offsets o_k = c_k + d_k with d_k = F_k − ln(1 − χ_{2k}) carry 1br's parity in the formula's part (even above odd at all four cells: d_k +0.20, +0.39, +0.24, +0.06; o_k +0.27, +0.08, +0.21, +0.16, 1br's +0.22 at 3.0 reproduced) and not in the residual (c_k +0.07, −0.31, −0.03, +0.11) — the corrected statement is the stronger one, gated on the signs. **F3 (minor)** the curvature's limit is Σ_γ γ⁻² = −Ξ″(0)/(2Ξ(0)) = 0.023105, not K/2 = 0.023096. **F4 (minor)** Hypothesis D restated as the multiset equality below T_D with multiplicity; the producer's real-zero census (all K − 1 designed pairs real, the sum rule κ = Σ τ⁻² closing to 10⁻¹⁰, the located and zeta zeros below T_D equal in number) gated. **F5 (minor)** the rate O(ln T_D/T_D) withdrawn from the proof; ε at the cells stated (0.02335 → 0.00348, 0.18–0.24 of ln T_D/T_D); the bound tightened to 2R²ε. **F6 (minor)** the corollary's hypothesis restated (mass and second moment outside R(δ) → ∞ with R²ε → 0); the exterior masses beyond T_D/4, T_D/2, T_D computed and gated; (vii) names the hypothesis. **F7 (minor)** the shares of Q beyond 3T₀ with the far tail: 0.42, 0.40, 0.43, 0.44, 0.48. **F8 (minor)** the counts below γ₁ per rung (rungs 6–7 four, 8–12 four or five; the Ξ-limit's 4, 4, 4, 5, 5, 5, 5, 6) stated and gated; the title amended. **F9 (minor)** the tautological conjunct removed. **F10 (cosmetic)** nodes by mpmath. **F11 (cosmetic)** (i)/(v) read through 1bm(v)'s formula, its reduction conjectured — stated in (vii). **F12 (cosmetic)** the sign-change sequence from rung 2. Record correction: A481's "first four packed" → "first three, rung 5's fourth at the first zero's height". Convergence-test round 322 commissioned.

### Round 322 — convergence test on the round-321 sweep (Theorem 1bu)

**Scope.** The round-321 sweep commit (the block's (ii)–(iv) and (vii) rewritten; the verifier's safely-deep index, the parity gate recast, the census/ε/mass/curvature/share/below-γ₁ gates and the new needles; the producer at round 5 with its seven checkpoints recomputed; `xi_limit.curvature()`; manifest 30). Full-tower class. Brief `brief_round322.md`.

**Findings.** 0 MAJOR, 3 minor, 2 cosmetic; all five verified directly by the lead and accepted; swept in this commit (A483). **F322-1 (minor)** "D holds by the real-zero census" claimed an exact multiset equality where the census verifies equality within the dodging tolerance 0.05 (displacements up to 0.033 at δ = 1.0, exact to 10⁻¹⁴ for the low half at δ ≥ 2, up to 0.02–0.03 near the edge; ε's last digit at δ = 1.0 affected) → the displacements censused and stated, ε from the located zeros, the bound's extra term (below 10⁻⁴ at r = 3) stated and gated. **F322-2 (minor)** the bold "lives in the formula, not in the residual" overclaimed (at δ = 3.5 the residual's even−odd mean +0.108 exceeds the formula's +0.056; at δ = 2.3 the parities' mean ranks are unequal and the raw means conflate trend and parity) → restricted to the balanced cells δ = 2.6, 3.0, 3.5 with the defensible statement (d_k's sign constant, c_k's sign changing, the magnitude shared) and the means' standard errors. **F322-3 (minor)** the sum rule's residuals were the curvature's finite-difference bias (κ_FD = κ − h²(κ² − Στ⁻⁴)/2 reproduces them to 1%) → Richardson extrapolation in the producer; the unlocated pair's |τ| corrected (14876 → 14501; the six complete cells close within 3 × 10⁻¹⁷ once the second differences are taken about the exact ĝ₁(0) = 2av₀). **F322-4 (cosmetic)** the g3 label's dead `{worst_rel:.2%}` placeholder → f-string. **F322-5 (cosmetic)** the reality of the unlocated root argued (one unlocated root of a real polynomial is real) and the census's dip count stored, gated and stated. Record correction: A482's residual bound. Convergence-test round 323 commissioned.

### Round 323 — convergence test on the round-322 sweep (Theorem 1bu)

**Scope.** The round-322 sweep commit (the block's D-at-the-cells sentence with the dodging displacements, ε from the located zeros, the parity restricted to the balanced cells with standard errors, the unlocated root's reality argued and its |τ| corrected; the producer at round 7 — Richardson κ about the exact ĝ₁(0), the displacement census, the dip count — with its seven checkpoints recomputed *(the scope line first said round 6, the run discarded at the sweep; corrected at the round-323 sweep)*; the verifier's gates and needles; manifest 30). Full-tower class. Brief `brief_round323.md`.

**Findings.** 0 MAJOR, 7 minor, 1 cosmetic; all eight verified directly by the lead and accepted; swept in this commit (A484). **F323-1 (minor)** "five or six rungs per parity" false at δ = 2.6 (5/4) → "four to six", gated. **F323-2 (minor)** the verifier docstring still carried the struck "the parity lives in the formula … not in the residual" and the superseded gate descriptions → rewritten. **F323-3 (minor)** "the low half exact to 10⁻¹⁴" against stored maxima of 2.8 × 10⁻¹⁴ and a gate at 10⁻¹³ → "within 3 × 10⁻¹⁴, the double-precision floor", gated there. **F323-4 (minor)** `disp_max ≤ 0.05` could not fail (by construction) → removed. **F323-5 (minor)** "seen as a dip" credited a detector blind beyond 5 × 10⁻⁵ of a grid point; "none occurs" rests on the complete count of simple sign changes → stated so, the dip conjunct dropped. **F323-6 (minor)** the (ii) header and the cells sentence kept the exact-verification wording → "within the dodging tolerance". **F323-7 (minor)** "at every cell" rested at δ = 3.5 on +0.056 ± 0.148 → "in the point estimates (at δ = 3.5 within one standard error of zero)", c_k's sign change stated as more than two standard errors (2.4, gated). **F323-8 (cosmetic)** the duplicated "= ε =" → fixed. Record correction: this table's scope line (round 6 → round 7). Convergence-test round 324 commissioned.

### Round 324 — convergence test on the round-323 sweep (Theorem 1bu)
...
### Round 329 — the landing of Theorem 1bv (the odd sector)

**Scope.** The landing commit (one theorem block after Theorem 1bu with three proved parts — the odd form with its negative pole term, the reduction ĝ = irĜ to the even form with the multiplier γ², the odd limit shape under D_odd — and the computed complete shadow (the odd rungs at the prolate orders 4k + 2), the odd hole zeros at the nodes of the orthogonal polynomials of t²Ξ², and the odd anatomy with its Ξ-limits; the footer census 98 / 1i–1bv with the verifier, a two-instrument substrate clause and the no-new-classical-input note; verifier `cascade_odd_sector.py` (member 31, top); substrates `odd_sector.py` (keyed: seven cells and the self-test), `weil_prime_gram_odd.py`, `xi_limit.py` (extended); the census literals advanced across the tower; manifest 31) and the owner-commissioned §11 Roadmap (ten routes toward Weil's criterion with a closing falsification paragraph; prose citing gated results, no verifier of its own; Hurwitz's theorem and the Laguerre–Pólya class named in the footer's classical-inputs note as routes, not inputs). Full-tower class (a landing; every member live). Brief `brief_round329.md`. Owner's line of enquiry (A490).

**Findings.** 0 MAJOR, 6 minor, 6 cosmetic; all twelve accepted and swept (A491). Minor: F329-1 the block's 1% node gate stated at δ ≥ 2 while gated at δ ≥ 2.3 (1.26% at δ = 2.0's rung 4) — corrected, the figure now a needle; F329-2 χ_{4k+2} for the order-(4k + 2) prolate against the paper's χ_j = √λ_{2j} — χ_{2k+1} on every surface; F329-3 the sum rule's 10⁻¹⁶ gated at 10⁻¹¹ — tightened; F329-4 the verifier's docstring listed the odd Gram's two mangles as gated with no gate — `weil_prime_gram_odd_mangles.py` and the g0 conjunct (misses 3.375, 1.851); F329-5 the even anatomy limits attributed to 'Theorem 1bu's recorded values' (1bu records none) — reworded, the archimedean limits now computed directly and gated against the remainder; F329-6 the footer's keying closure misnamed (`xi_limit.py` in no checkpoint key; `weil_prime_gram.py`, `weil_spectrum_ladder.py` in every key) — named as computed. Cosmetic: F329-7 the arc's start; F329-8 the origin term's conditions; F329-9 the odd sector's carrier (the archimedean term and the constant taken together, gated); F329-10 the transform vanishes; F329-11 M's roots real and positive; F329-12 the sector qualifier's scope. Checked and held: (i)–(vii) re-derived, the Ξ-side constants by an independent quadrature, the δ = 1 cell and the self-test forced fresh bit for bit, the census, the parse tolerances, the roadmap's numbers, Hurwitz stated as a conditional.

### Round 330 — the convergence test on the round-329 sweep, and the roadmap's head item

**Scope.** The sweep commit (the twelve round-329 findings on Theorem 1bv's block, the footer's substrate clause, the verifier with the new instrument `weil_prime_gram_odd_mangles.py`, `xi_limit.py`'s direct archimedean limits, two docstrings) and the owner-commissioned head item of §11 — "The caster explained — a prime-side reason for the shape, valid at every support" — with the ten former items renumbered 2–11 and their cross-references updated. Full-tower class. Brief `brief_round330.md`.

**Findings.** 0 MAJOR, 2 minor, 5 cosmetic; all seven accepted and swept (A492). Minor: F330-1 the even sector's direct-vs-remainder gate at 1e-10 passing on the grid's own error (7e-11 at 3000 nodes, 2.3e-10 at 6000; mpmath closes the identity to 4e-26) — gated at 1e-9 with the reason stated; F330-2 the square-root item citing 1bu(vi) (the c(δ) split) for an even-sector anatomy at the cells the paper does not contain, and the sweep's 'subtracting in both' reaching the even cells — recited to 1bv(vi) at the limit, the scope matched to g5. Cosmetic: F330-3 the head item's even-sector formulas (|ĝ(γ)|²; Ĝ₁ for the odd sector); F330-4 (d)'s 'proved' on the spacing (read through 1bm(v)); F330-5 'the other ten serve' against item 3; F330-6 a dangling parenthetical and an orphaned '--'; F330-7 'line for line' against the omitted parts matrix. The same commit revises the head item from the drafting workflow's adversarial verdicts and carries the owner's 'as yet' rewordings. Checked and held: every round-329 sweep closed on every surface; the mangle copy diffed against the substrate; same_gram probed; the archimedean identity derived from the digamma integral; both import closures; the ten cross-references; the census; the parse tolerances; clause G's whitelist.

### Round 331 — the convergence test on the round-330 sweep
...
**Findings.** 0 MAJOR, 0 minor, 3 cosmetic — **converged; Theorem 1bv and the §11 roadmap with its head item stable** (A495). Cosmetic: F333-1 'growing with the degree' surviving in the g1 gate's comment; F333-2 the mechanism's provenance labels (rounds, the 2e-7); F333-3 item 8's filter without item 11 (held-observation class, taken). All three swept in the convergence commit with a full battery. Checked and held: the classification by hashing; every round-332 sweep closed on every surface (the carrier sentences grepped everywhere); F332-2's mechanism recomputed end to end; the head item's attributions; the other items' citations; the census; the gates. Trajectory: 329 (0M+6m+6c) → 330 (0M+2m+5c) → 331 (0M+1m+5c) → 332 (0M+1m+4c) → 333 (0M+0m+3c).

### Round 334 — the landing of Theorem 1bw (the excited states' exact anatomy)

**Scope.** The landing commit (one theorem block after Theorem 1bv: the full census of every designed zero pair of every safely deep rung at five cells, the factorisation ĝ_k = ĝ₁H_kC_k, the curvature law ln C_k = −Δκ_k r² + O(r⁴) with Δκ_k from the sum rules, the bi-orthogonal law of the hole polynomials under the pair weights ĝ₁²C_kC_j on the interior [0, T_D/2] — exact-C, Gaussian-recursive and the sign-flipped mangle — against the naive nodes, τ_eff, the fall of Δκ₂ across the cells, the shallow rungs' complex pairs; the footer census 99 / 1i–1bw with the verifier, the substrate clause and the no-new-classical-input note; the head item's (c) updated; verifier `cascade_rung_anatomy.py` (member 32, top); substrates `rung_anatomy.py` (keyed: the census) and `rung_laws.py` (keyed on the census: the laws); the census literals advanced across the tower; manifest 32). Full-tower class (a landing; every member live). Brief `brief_round334.md`. Owner-directed ("Hit it"; A496).

**Findings.** 2 MAJOR, 5 minor, 9 cosmetic; all sixteen verified by the lead and accepted (A497). MAJOR: F334-1 the block's headline bounds attached the producer's larger set of rungs to figures computed over the safely deep rungs (the producer's set: E 8.9 × 10⁻⁶, R 5.7 × 10⁻³, accounting 3.5 × 10⁻⁶); F334-2 the shared dodging zeros "to 10⁻¹³ at rungs 2–5 and to 10⁻⁶ at rung 8" written from the first eight zeta zeros only — by zeta zero the agreement holds within 10⁻¹² up to γ = 69.5, 65.1, 59.3, 49.8 and the difference grows toward each rung's edge to 0.08–0.18, twelve orders off as written and ungated. Minor: F334-3 1bw's T_D at tolerance 0.2 silently differed from 1bu's at 0.05 (121.4 vs 111.0 at δ = 2.3); F334-4 the pair placed by the sum rule at δ = 3, rung 3 lies inside the region, 0.048 below the sinc zero ω₇₀₂ (the census's blind spot, not 1bu's beyond-the-region branch; the verifier lacked the conjunct); F334-5 g0's accounting conjunct an identity of the construction, the k − 1 holes gated on the fine census only; F334-6 the "proved" identity omitted its constant; F334-7 "complex pairs" at rungs 9–10 gated as an unlocated count (rung 9's two shown complex by the touching at γ₁; rung 10's eight not claimed). Cosmetic: the laws checkpoints' verdict "40 digits"; "moving inward"; "located"; g6's label; Plancherel absent from the classical-inputs list; "vanishes"; "across the interior" and the gate's τ_eff range; the mass bound; the mangle conjunct vacuous on None. **Sweep (the convergence-test commit).** The block regenerated whole from the template with the bounds over the safely deep rungs and the producer's extension in its own sentence; the shared-dodging and located-count sentences regenerated character for character from the census and gated (g0b new); T₁ defined with the relation to 1bu's T_D as a needle (2.9–8.5% over the five cells); the placed pair's neighbour named and the verifier's placed-pair conjunct (beyond the region, or within one census step of a sinc zero with j ≥ K); the direct census gates (k − 1 holes with the coarse and fine censuses agreeing to 10⁻¹², one dodging zero per zeta zero below the edge, T_D < T₁); the normalised identity; rung 9's touching computed and gated; the laws producer's verdict corrected and its five checkpoints recomputed under the new keys; Plancherel appended to the footer's list. Trajectory: 334 (2M+5m+9c) → 335 pending.

### Round 335 — the convergence test on the round-334 sweep of Theorem 1bw

**Scope.** The sweep commit (Theorem 1bw's block regenerated whole: the bounds over the safely deep rungs with the producer's extension separate; T₁ defined and related to 1bu's T_D; the shared dodging zeros by zeta zero; the placed pair's neighbour; the normalised identity; rung 9's touching; "never outward", "located or placed", "falls", "to r = 40"; the footer's classical-inputs list with Plancherel and the 1bw note reworded; verifier `cascade_rung_anatomy.py` at eleven gates — g0b new, the placed-pair conjunct, the direct census gates, the mangle conjunct; producer `rung_laws.py`'s verdict string with the five laws checkpoints recomputed; manifest 32). Full-tower class. Brief `brief_round335.md`. Convergence test on round 334's sixteen findings (A497).

**Findings.** 0 MAJOR, 3 minor, 8 cosmetic; all eleven verified by the lead and accepted (A498). Minor: F335-1 τ_eff's stated range "between 1.17 and 1.42 T₁" a floor (the minimum 1.1754 rounds to 1.18), ungated as a needle, g3's "(observed …)" a literal; F335-2 "never outward with k" gated nowhere beyond each edge inside T₁ (the claim holds at all five cells); F335-3 "within 10⁻¹² up to γ = 69.5, …" naming the first exceedances rather than the last heights within the bound. Cosmetic: the Plancherel entry inside the 1bu-landing group; the verifier's docstring on rung 10 and "vanishing"; the laws producer's "[0, T_D/2]"; the T_D clause's grammar; the freed sum's variable (z⁻², the ground state's dodging zeros, not γ⁻²); "growing toward the rung's edge" (the maximum two zeta zeros short at rungs 3 and 5); the generator's unconditional "below". Out of scope (record): A497's "ties at four cells" (three among the safely deep rungs; corrected). **Sweep.** The τ_eff range a generated needle parsed in g7 with g3's label computed; the edges gated non-increasing over the safely deep rungs at every cell; "below γ = …"; the largest displacement's position gated above 0.9 of the edge; Plancherel after the 1bv marker; the sentence restructured; z⁻²; "below"/"above" by sign; the docstrings. Trajectory: 334 (2M+5m+9c) → 335 (0M+3m+8c) → 336 pending.

### Round 336 — the convergence test on the round-335 sweep of Theorem 1bw

**Scope.** The sweep commit (Theorem 1bw's block regenerated: T_k's sentence restructured with the edges never outward over the safely deep rungs at every cell and 1bu's T_D in its own clause; the shared dodging zeros "below γ = …" with the largest difference's position; τ_eff's observed range as a needle; the freed sum over the ground state's dodging zeros z⁻²; the footer's Plancherel entry after the 1bv marker; verifier `cascade_rung_anatomy.py` at eleven gates with nineteen needles — g0's monotonicity conjunct, g0b's position conjunct, g3's computed label, the by-sign neighbour; `rung_laws.py`'s docstring; manifest 32). Full-tower class. Brief `brief_round336.md`. Convergence test on round 335's eleven findings (A498).

**Findings.** 0 MAJOR, 0 minor, 3 cosmetic — **converged** (A499). Cosmetic: the shared-zeros sentence's missing connective before the seven maxima; "below γ = 49.8" where 49.7738 rounds up (now "below the zeta zeros γ = …"); the verifier's docstring "50-70". All three swept in the convergence commit under a full battery (member 11/11, chain parent 10/10, TOWER PASS 32/32). Trajectory: 334 (2M+5m+9c) → 335 (0M+3m+8c) → 336 (0M+0m+3c). Theorem 1bw stable.

### Round 337 — the landing of Theorem 1bx (the law of Δκ_k derived: the exterior's re-balayage)

**Scope.** The landing commit (one theorem block after Theorem 1bw: the outer counting deficit's R⁻³ moment as Δκ_k; the balayage law proved under Hypothesis B from the Green function of 1bm's doubly slit plane — ln C_k in closed form, Δκ_k = −Σ_h(1 + σ_h²)/(2T_w²), the deficit profile (2/π)arccos(T_w/R), the r⁴ coefficient; Hypothesis B verified at every safely deep rung of the five cells with one wall per rung — the interior profile at 0.3, 0.5, 0.7 T_w, the deficit profile's wall, the r = 40 ratio, the pre-edge deficit; rung 2's wall at T₁ and T₁ at two horizons; the walls' brackets; the wall's law named as what remains; Theorem 1bw's (iv)/(v) pointed at 1bx; the head item's (c) and Done-when; the footer census 100 / 1i–1bx with the verifier, the no-new-substrate clause and the classical-inputs note; verifier `cascade_rung_balayage.py` (member 33, top), computing from 1bw's and 1bu's checkpoints; every census literal advanced across the tower; manifest 33). Full-tower class (a landing; every member live). Brief `brief_round337.md`. Owner-directed ("Derive the law"; A500).

**Findings.** 0 MAJOR, 2 minor, 5 cosmetic; all seven verified by the lead and accepted (A501). Minor: F337-1 "the wall moves inward with k" false at δ = 3.5 (rung 2's wall inside rungs 3–6's; ungated); F337-2 "the deficit already present at T₁ is the profile's" contradicted at δ = 3.5's rungs 2–5 (no deficit at T₁ against the profile's 0.16–0.20) with a one-signed lag at 34 of 40 rungs the block reported as agreement, and a per-rung tolerance that could not fail at 14 rungs. Cosmetic: rung 2's wall "is" T₁ where δ = 3.5 is 4.8% off; "between T_k and T₁" at 37 of 40; the Green identity's constant; the substrate sentence (1bu's ladder checkpoints omitted, the 6700-zero list 1bm's); g1's one-sided residual. **Sweep.** Monotonicity from rung 3 stated and gated with rung 2's placement; the onset lag stated with its signed means and gated (g5 rewritten); the fit window's reason; "within 1.3% at δ ≤ 3 and 4.8% at δ = 3.5"; the between-count a needle; V's constant; the substrate sentences; the absolute residual. Trajectory: 337 (0M+2m+5c) → 338 pending.

### Round 338 — the convergence test on the round-337 sweep of Theorem 1bx

**Scope.** The sweep commit (Theorem 1bx's block regenerated: the wall non-increasing from rung 3 with rung 2's placement at δ = 3.5; the onset lag near the wall stated as a one-signed late onset with its signed means and the δ = 3.5 rungs without deficit; the profile-fit window's reason; rung 2's wall within 1.3%/4.8%; the between-count needle; the Green identity's constant; the substrate sentences in the block and the footer; verifier `cascade_rung_balayage.py` at nine gates with nine needles — g1's monotonicity conjuncts and absolute residual, g5 rewritten; manifest 33). Full-tower class. Brief `brief_round338.md`. Convergence test on round 337's seven findings (A501).

**Findings.** 0 MAJOR, 1 minor, 7 cosmetic; all eight verified by the lead and accepted (A502). Minor: F338-1 (v)'s list of Hypothesis B's gated confirmations still carried "the pre-edge deficit" (F337-2's struck claim in summary) with "the onset lag" appended — the one gate twice. Cosmetic: the verifier's comment attributing the 6700-zero list to 1bu; an ASCII minus; the lag statistics' shape (the mean over all forty attached to the 34; the absolute maximum; the far heights unnamed); the δ = 3.5 "no deficit" tolerance stated per hole where rung 5 met it by 0.006; two placements exact but ungated (rung 7 below rung 2 at δ = 3.5; the exceptions' identities); "neither edge itself" per rung; "the next clause". **Sweep.** The parenthetical's three confirmations with the lag as B's one departure; the comment; U+2212; the statistics named as computed (signed, over the forty, the four heights pooled); "under a quarter of a zero" with the set gated exactly; rung 7 gated and the exceptions regenerated from the walls; "neither edge's law"; "stated below". Trajectory: 337 (0M+2m+5c) → 338 (0M+1m+7c) → 339 pending.

### Round 339 — the convergence test on the round-338 sweep of Theorem 1bx

**Scope.** The sweep commit (Theorem 1bx's block regenerated: (v)'s confirmations and the lag as B's departure; the lag statistics named as computed; the δ = 3.5 rungs' "under a quarter of a zero"; rung 7's placement; the exceptions' identities as generated text; "neither edge's law"; "stated below"; verifier `cascade_rung_balayage.py` at nine gates — g5's signed statistics and exact set, g1's rung-7 conjunct and exception generator, the parses; the comment; manifest 33). Full-tower class. Brief `brief_round339.md`. Convergence test on round 338's eight findings (A502).

**Findings.** 0 MAJOR, 4 minor, 3 cosmetic; all seven verified by the lead and accepted (A503). Minor: F339-1 "the lag gone by 2T₁" gated only through the pooled far mean; F339-2 the quarter-zero criterion selects twelve rungs across the cells where the block named δ = 3.5's four and the gate saw one cell; F339-3 the exceptions generator renders a non-contiguous set as a range; F339-4 (v)'s "every safely deep rung" over-reaching for the profile's wall (thirty rungs). Cosmetic: the window's reason placing the lag below 1.5T₁; the verifier's docstring and comment ("neither edge is the wall", "within 0.2 at every rung"); two summaries of B without the departure. **Sweep.** The 2T₁ height gated over the rungs with two or more holes and stated with its mean; the twelve rungs named cell by cell from the census, generated and parsed, the count and shape gated; the generator fails closed on anything but a cell's contiguous range; (v)'s quantifiers split; the window's wording; the docstring; "three confirmations and one departure"; the head item's qualifier. Trajectory: 337 (0M+2m+5c) → 338 (0M+1m+7c) → 339 (0M+4m+3c) → 340 pending.

### Round 340 — the convergence test on the round-339 sweep of Theorem 1bx

**Scope.** The sweep commit (Theorem 1bx's block regenerated: the lag's 2T₁ statement over the rungs with two or more holes; the twelve quarter-zero rungs named cell by cell; the profile-window wording; (v)'s quantifiers; "three confirmations and one departure"; the head item's "up to a late onset near the wall"; verifier `cascade_rung_balayage.py` at nine gates — g5's 2T₁ conjunct and the twelve-rung set, the fail-closed generator, the parses; the docstring; manifest 33). Full-tower class. Brief `brief_round340.md`. Convergence test on round 339's seven findings (A503).

**Findings.** 0 MAJOR, 3 minor, 3 cosmetic; all six verified by the lead and accepted (A504). Minor: F340-1 the lag statistics switching population by height (the one-hole rungs excluded at 2T₁ only; the pooled figure the forty's inside the 35's clause); F340-2 five of the twelve "no deficit at T₁" rungs the profile's own prediction (rung 2 of every cell, the wall at T₁); F340-3 the window's reason contradicted by the block's own lag statistic. Cosmetic: the aside's staircase reason failing at the eleven-hole rungs; the verifier's comment; g5's "every cell" not in its conjunct. **Sweep.** One population at every height (the 35 rungs with two or more holes) with the one-hole rungs reported apart; the seven lag members named against the profile's prediction and the five stated as B's confirmation, both gated; the window aside removed; the comment; the five gated as rung 2 of every cell. Trajectory: 337 (0M+2m+5c) → 338 (0M+1m+7c) → 339 (0M+4m+3c) → 340 (0M+3m+3c) → 341 pending.

### Round 341 — the convergence test on the round-340 sweep of Theorem 1bx
...
### Round 342 — the landing of Theorem 1by (the wall's law: the unlocking functional, the continuum law, the displacement law)

**Scope.** The landing commit (one theorem block after Theorem 1bx: the displacement law — the Weil form's stationarity in zero coordinates, proved, its dodged-zero form d_j = (M_j − λH_j)/(γ_jĝ′(γ_j)²), verified at every dodged zero of eleven rungs at δ = 2 and 2.3 with the Gram and polished eigenvectors recomputed in-process; the unlocking functional F₁(T) + 4Σ_h ln(1/σ_h(T)) — 1bm(v)'s finite-δ exponent plus 1bx's hole-pair cost — whose minimiser is a zeta zero, the ground state's edge at the cells and the rung's wall between the census edge and 1bx's sharp wall at 35 of 40, and whose minimum is the rung's exponent within 0.45 nats at the forty rungs; the continuum law u ln(2/u) = (2m/π)e^{−δ} from 1bm(iv)'s reduced problem, the admissibility balance, within 3.3% rms of the sharp wall; the soft onset's 4% quantified, not derived; Theorem 1bx's (iv)/(v) pointed at 1by; the head item's (c) and Done-when; the footer census 101 / 1i–1by with the verifier, the substrate clause and the classical-inputs note; verifier `cascade_wall_law.py` (member 34, top); every census literal advanced across the tower; manifest 34). Full-tower class (a landing; every member live). Brief `brief_round342.md`. Owner-directed ("Hit what remains"; A506).

**Findings.** 1 MAJOR, 6 minor, 4 cosmetic; all eleven verified by the lead and accepted (A507). Major: F342-1 the sharp wall's 4% excess over T_u attributed to the soft onset on four surfaces — the excess grows with the hole count (correlation +0.52 with m, −0.21 with 1bx's onset shortfall; the functional's wall descending 1.5–2.2 times faster per hole than the sharp wall at four of five cells). Minor: F342-2 the functional is Theorem 1bu(iv)'s k-level formula and T_u its stored edge (39 of 40), uncited, with a symbol clash; F342-3 the minimiser's identity decided by margins under 0.02 nats at 11 of 40 rungs and at two ground states; F342-4 "proved" lacked the concavity step; F342-5 the continuum law compared only with the sharp wall; F342-6 "exact" for a residual floored by the disclosed tail approximation; F342-7 "at the bottom of a rung the displacement is λ's own" contradicted by the ground states' bottoms. Cosmetic: "rms deviation from 1"; the five exceptions unnamed; the slope wording; the existence clause. **Sweep.** The attribution struck and retracted on all four surfaces; the m-dependence stated with slopes, ranges and both correlations, gated; "derived" re-scoped to the minimum's flatness (the half-nat window stated and gated); the Done-when re-aimed at the excess's law; the 1bu coincidence and the symbol clash stated, the 39-of-40 gated; the margins and window gated; concavity argued and witnessed; the continuum law against T_u with its descent, gated; "exact" retracted, the residual's tail-proxy correlation gated; the bottoms gated; the four cosmetics. Trajectory: 342 (1M+6m+4c) → 343 pending.

### Round 343 — the convergence test on the round-342 sweep of Theorem 1by

**Scope.** The sweep commit (Theorem 1by's block regenerated: the sharp wall's excess as the hole count's, with slopes and correlations, "not the soft onset's"; the flatness of the minimum; the coincidence with 1bu(iv)'s k-level formula and the symbol clash; the margins; concavity; the continuum law against T_u; "exact" retracted to the tail approximation; the field at the bottoms; the exceptions sided; the four cosmetics; Theorem 1bx's pointer, the head item and the Done-when; verifier `cascade_wall_law.py` at nine gates with the new conjuncts and eleven needles; manifest 34). Full-tower class. Brief `brief_round343.md`. Convergence test on round 342's eleven findings (A507).

**Findings.** 0 MAJOR, 5 minor, 5 cosmetic; all ten verified by the lead and accepted (A508); no round-342 disposition reopened. Minor: F343-1 the two negative correlations stated as signs though within about one standard error of zero at n = 40; F343-2 the exponent residual grows with the hole count (+0.028 nats per hole, correlation +0.63), so "under half a nat" is the censused range's; F343-3 "agree only at one hole" false at δ = 2 and 2.6; F343-4 the continuum descent nearer the sharp wall's than the functional's at δ = 2; F343-5 the domain endpoint T = h_max a cusp the concavity argument does not exclude (excluded by computation, 16 nats). Cosmetic: "about a tenth" (an eighth in the median, up to a quarter); the 1bu exception unnamed; the slopes' fit unlabelled; the head item's "derived" not re-scoped; "61 points per spacing". **Sweep.** The standard errors stated and gated; the residual's growth stated and gated; the one-hole agreement counted; the four-per-hole reading of the continuum law with the nearer-wall count gated; the endpoint's gap stated and gated; the five cosmetics. Trajectory: 342 (1M+6m+4c) → 343 (0M+5m+5c) → 344 pending.

### Round 344 — the convergence test on the round-343 sweep of Theorem 1by

**Scope.** The sweep commit (Theorem 1by's block regenerated: the correlations with their standard errors; the exponent residual's growth with the hole count; the one-hole agreement at three of five cells; the continuum law's four-per-hole reading and its nearer wall; the domain endpoint's cusp excluded by computation; the five cosmetics; the head item's re-scoping; verifier `cascade_wall_law.py` at nine gates with the new conjuncts and twelve needles; manifest 34). Full-tower class. Brief `brief_round344.md`. Convergence test on round 343's ten findings (A508).

**Findings.** 0 MAJOR, 3 minor, 6 cosmetic; all nine verified by the lead and accepted (A509); no round-343 disposition reopened. Minor: F344-1 g4's new label printing its placeholder; F344-2 "3.7 standard errors" a t-statistic, not r over the stated null standard error (3.16); F344-3 the "Proved" ledger not re-scoped for the endpoint h_max. Cosmetic: two implied g2 conjuncts; a lower bound rounded to nearest; "that grid"; the flatness phrase by halves; "agree" without a threshold; the half-nat scope only in (iv). **Sweep.** The label formatted; "at t = 3.7" throughout; "a zeta zero or the domain's endpoint h_max" with the exclusion under Computed; the implied conjuncts dropped; "at least 16.1" with a floor parse; the grid named; the flatness phrase completed on both surfaces; "within 2% at 3 of the five cells", gated; "over the censused range" in the title and (v). Trajectory: 342 (1M+6m+4c) → 343 (0M+5m+5c) → 344 (0M+3m+6c) → 345 pending.

### Round 345 — the convergence test on the round-344 sweep of Theorem 1by
...
**Findings.** 0 MAJOR, 3 minor, 6 cosmetic; all nine verified by the lead and accepted (A511); no round-345 disposition reopened. Minor: F346-1 "thousands of times λH_j near every edge" false at eight of the eleven rungs (37–500 there); F346-2 the docstring's continuum formula missing the e^δ weight and naming the wrong extremum (from the landing); F346-3 "descends faster" universal where δ = 2 is the exception. Cosmetic: two restored conjuncts implied by the T_u/T_w bounds; a contentless g0 conjunct; the holes' 0.22 bound ungated; "4% on average" and "resolves the other way" ungated; "or its neighbour" at a cell three zeros off; two parses on literals. **Sweep.** The per-rung field maxima computed, the three above a thousand named and the eight's range gated, the sentence rewritten; the docstring formula corrected; "at four of the five cells"; the implied conjuncts dropped; the g0 witness made to evaluate the function; the holes' bound, the mean excess and the runner-up identity gated; "a zero within three of it"; named constants parsed against. Trajectory: 342 (1M+6m+4c) → 343 (0M+5m+5c) → 344 (0M+3m+6c) → 345 (1M+1m+5c) → 346 (0M+3m+6c) → 347 pending.

### Round 347 — the convergence test on the round-346 sweep of Theorem 1by

**Scope.** The sweep commit (verifier `cascade_wall_law.py`: g0's u ln(2/u) witness, g2's holes-bound, mean-excess and runner-up conjuncts with the two implied ones dropped, g5's per-rung field maxima, the named constants, the docstring's continuum formula; Theorem 1by's block regenerated: the per-rung field maxima sentence, the holes' bound as a needle, "at four of the five cells", "a zero within three of it", the mean excess in the needle; fifteen needles; manifest 34). Full-tower class. Brief `brief_round347.md`. Convergence test on round 346's nine findings (A511).

**Findings.** 0 MAJOR, 0 minor, 5 cosmetic — **converged** (A512). Cosmetic: the value half of the g0 witness still an identity; four duplicate or implied conjuncts; the agreement threshold as a split anchor and a misnamed constant; 1bx's pointer "lies below" universal where three rungs lie above; "thousands" at δ = 2.3's rung 2 (1185). All five swept in the convergence commit under a full battery (member 9/9, parents 9/9 and 11/11, TOWER PASS 34/34): the grid argmax witness; the duplicates dropped; the anchor removed and NSUB; "at 37 of the 40 rungs"; "over a thousand". Trajectory: 342 (1M+6m+4c) → 343 (0M+5m+5c) → 344 (0M+3m+6c) → 345 (1M+1m+5c) → 346 (0M+3m+6c) → 347 (0M+0m+5c). Theorem 1by stable.

### Round 348 — the landing of Theorem 1bz (the gap's law: the two walls' leakage budget, the share's law, the geometric-mean law)

**Scope.** The landing commit (one theorem block after Theorem 1by: the exterior reads the sharp wall — Hypothesis B's constant at T_w matched by the exterior envelope lift with the estimator ĝ(γ)² + ĝ(γ̄)², and by the deficit's log-moment from the census sets, the harmonic measure's log-mean ln((1 + s_h)T) witnessed; the leakage-budget identity cost(T_w) − cost(T_u) = U − ln(1/s_ext) + ε between Theorem 1by's reading of the exponent at T_u and Hypothesis B's reading of the exterior leakage at T_w, ε within 0.21 nats rms at the forty rungs and T_w recovered from T_u, U and the measured share within 0.9% rms at three or more holes; the share's law s_ext = S₁(T₁/T_u) — the rung's leakage profile the ground state's attached at T_u — placing T_w from the ground state alone within 0.9% rms at the thirty rungs with three or more holes and not at one or two; the leading law T_w = s_ext^{1/(4m)}√(T_uT_u(1)) derived from the identity in the continuum (F₁′ = ln(T/2T₀) witnessed), 1bx's geometric mean explained; the zone's anatomy; Theorem 1by's (v)/(vi) and 1bx's (iv) pointed at 1bz; the head item's (c) and Done-when; the footer census 102 / 1i–1bz with the verifier, the producer's substrate clause and the classical-inputs note; the keyed producer `leakage_profiles.py` with its five committed checkpoints; verifier `cascade_gap_law.py` (member 35, top); every census literal advanced across the tower; manifest 35). Full-tower class (a landing; every member live). Brief `brief_round348.md`. Owner-directed ("Derive the gap"; A513).

**Findings.** 1 MAJOR, 7 minor, 5 cosmetic; all thirteen verified by the lead and accepted (A514). MAJOR: F348-1 the exterior lift's agreement with Hypothesis B's constant specific to the window [T₁, 1.5T₁] — the near quarter a third of a nat low, the far windows a sixth high, undisclosed. Minor: B's exterior reading missing ln S₁(1) (the ground leaks 2–4% below its edge); the residuals' sign convention; the boxed law's derivation using ΔT = 4m before invoking it; "two to four by T₁" (0.48–4.39); "a fifth of a nat" at one hole (0.01–0.13); "under one percent" as a per-rung claim; the producer's closure of three files. Cosmetic: the per-rung versus per-zero phase mean; "confirmed by a second moment" for a systematic offset; the docstring's third of a nat; seven duplicate or implied conjuncts; 1by's title pointer and the geometric mean's naming. **Sweep.** (i) rewritten around the whole-exterior reading (+0.01 ± 0.11 nats at T_w, −1.09 ± 1.72 at T_u) with the sub-windows disclosed and gated; the share relative to the ground state's own (ŝ = s_ext/S₁(1)) throughout; the readings as formula − r; the law derived in order with its four forms gated; the counts, the one-hole residuals, "rms", the closure; fifteen needles; manifest 35. Trajectory: 348 (1M+7m+5c) → 349 pending.

### Round 349 — the convergence test on the round-348 sweep of Theorem 1bz

**Scope.** The sweep commit (verifier `cascade_gap_law.py`: g1 rebuilt on the whole-exterior reading with the three sub-window lifts and the log-moment's count above T_w; ŝ = s_ext/S₁(1) in g2–g4; the one-hole residuals; the law's four forms and ΔT/(4m); the profile's count range, the pooled phase; the duplicates dropped; Theorem 1bz's block regenerated: (i) rewritten, (ii) with ŝ and the residual convention, (iii) the one-hole residuals, (iv) the ordered derivation and S_LAW, (v) the count range, (vi) the closure; Theorem 1by's title pointer; the head item's "rms"; the footer's closure; fifteen needles; manifest 35). Full-tower class. Brief `brief_round349.md`. Convergence test on round 348's thirteen findings (A514).

**Findings.** 0 MAJOR, 2 minor, 4 cosmetic; all six verified by the lead and accepted (A515); no round-348 disposition reopened. Minor: F349-1 "T_u is T_k or a zero above it" against 1by(iv)'s three rungs one zero below; F349-2 the whole-exterior headline undisclosed as the leakage-weighted balance of a near quarter (a wall 2.8% below T_w on its own) and a far field (1.2% above). Cosmetic: "to second order" for a first-order step; an implied conjunct; the residual convention defined late; the docstring's count and mean conflated. **Sweep.** The near-quarter and far-field readings and the walls they name stated as needle S_WIN and gated; the 1by(iv) wording; "to first order in ΔT/T_u(1)"; the convention named at first use; the conjunct dropped; the docstring. Sixteen needles; manifest 35. Trajectory: 348 (1M+7m+5c) → 349 (0M+2m+4c) → 350 pending.

### Round 350 — the convergence test on the round-349 sweep of Theorem 1bz

**Scope.** The sweep commit (verifier `cascade_gap_law.py`: g1's leakage-window reading — the near quarter and the field beyond with the walls each names — gated; g3's implied conjunct dropped; the docstring's 1by(iv) wording and the log-moment's count; Theorem 1bz's block regenerated: (i) the convention named at first use and the balance stated (S_WIN), (iv) the 1by(iv) wording and "to first order in ΔT/T_u(1)"; sixteen needles; manifest 35). Full-tower class. Brief `brief_round350.md`. Convergence test on round 349's six findings (A515).

**Findings.** 0 MAJOR, 2 minor, 2 cosmetic; all four verified by the lead and accepted (A516); no round-349 disposition reopened. Minor: F350-1 "39 of the 40" attached to the leakage-weighted balance, which is an identity (the mediant of the two window ratios) — what holds at 39 is the near reading above the far; F350-2 the window walls' offsets without their scatter (the near wall's 4.7% rms exceeding its 2.8% offset). Cosmetic: S_LIFT against the named convention; "to 1.5%" as if a bound. **Sweep.** The identity named and the count re-attached on the block, needle and gate label; the scatters stated and gated; the envelope lifts restated as the constant less the lift; "on average and 4% at worst" restored; a parse-back defect of the sweep caught by g6 before the recorded run. Sixteen needles; manifest 35. Trajectory: 348 (1M+7m+5c) → 349 (0M+2m+4c) → 350 (0M+2m+2c) → 351 pending.

### Round 351 — the convergence test on the round-350 sweep of Theorem 1bz
...
**Findings.** 0 MAJOR, 0 minor, 2 cosmetic — **converged** (A520). Cosmetic: F354-1 the count conjunct `len(gap1) == 5` implied by the member's assert on the five one-hole rungs (the class of F349-4); F354-2 the aggregation gloss naming the lift for the mean of the log of its square. Both swept in the convergence commit under a full battery (member 9/9, parents 9/9 and 9/9, TOWER PASS 35/35): the conjunct dropped, the two range conjuncts and the g6 parse carrying "gated here"; "each rung's figure the mean of ln(A_k²/A₁²) over a window's zeta zeros". Trajectory: 348 (1M+7m+5c) → 349 (0M+2m+4c) → 350 (0M+2m+2c) → 351 (0M+2m+2c) → 352 (0M+1m+2c) → 353 (0M+1m+3c) → 354 (0M+0m+2c). Theorem 1bz stable.

### Round 355 — the landing of Theorem 1ca (the smooth count in the unlocking functional: the split, the smooth wall's law with the count constant, the leftover's decay, the exterior tail by the explicit formula)

**Scope.** The landing commit (one theorem block after Theorem 1bz: the exact split F_k = F_k^s + Osc with Theorem 1bs's smooth count and constant; the smooth wall's stationarity in closed form and its reduction T ln(2T₀/T) = 4m + 7/2, within 1.9% rms of T_u at the 40 rungs against 3.9% for 1by's law; the leftover's bound from Littlewood's S and S₁, O(ln T/√T) at the wall, with its consequences for the minimum's value and the minimiser; the exterior tail beyond a Gaussian cut by Weil's explicit formula — the smooth count's integral of the transform squared less the prime shells, the leftover the Stirling remainder bounded by Binet's formula, the identity within 4.4 × 10⁻⁶ at the ground states and 2.7 × 10⁻¹¹ at the 40 rungs, the smooth count overestimating the exterior leakage by a factor 1.9–2.9; verifier `cascade_smooth_functional.py`, member 36, nine gates, twelve needles; the footer's census 103 and 1i–1ca; Theorem 1bz's (vi) and the head item pointed at 1ca; 44 members' census literals advanced). Full-tower class (a landing). Brief `brief_round355.md`.

**Findings.** 2 MAJOR, 5 minor, 3 cosmetic; all ten verified by the lead and accepted (A522). MAJOR: F355-1 the title's "at every rung" false at ten of the forty (the rms 1.9%, the largest 6.3%); F355-2 the minimum's closed form with +(4m + 7/2)²/(4T₀) where the sign is minus, the verifier gating the wrong form at the sign error's footprint. Minor: the stationarity equation's missing factor 4 on the piece below γ₁; the leftover's extrema from a grid that missed the cusp minima; the other critical point misplaced at the 25 rungs with a hole above γ₁; an expansion's O(Δ/T) for O(√(Δ/T)); the "support's edge" clause uncomputed. Cosmetic: the convexity implication's margin; the cut on a zeta zero undisclosed; the docstring's 2.2%, S = O(ln t) attributed to Littlewood, the fidelity gate at the ground states only, the proof prose's dropped 4. **Sweep.** The title's quantifier; the sign, the remainder's order, g2's gate and S_MIN; the 4 restored; the extrema exact (the minima at the cusps, the maxima by the analytic derivative), S_OSC and S_BOUND regenerated; the critical point at the domain's cusp; O(√(Δ/T)); the shells of the smallest primes; the condition-free convexity argument with the bracket's margin gated; the cut's zero disclosed; the four small items. Twelve needles; manifest 36.

### Round 356 — the convergence test on the round-355 sweep of Theorem 1ca

**Scope.** The sweep commit (verifier `cascade_smooth_functional.py`: the closed form's sign and g2's gate, the exact extrema of the leftover and g3, the convexity margin over the bracket in g1, the fidelity gate over the 45 states, the docstring; Theorem 1ca's block regenerated: the title's quantifier, the stationarity's factor 4, the closed form's sign and remainders, the condition-free uniqueness argument, the critical point's location, O(√(Δ/T)), the proof prose's 4s, the attributions, the cut on a zeta zero, the shells of the smallest primes; twelve needles; manifest 36). Full-tower class. Brief `brief_round356.md`. Convergence test on round 355's ten findings (A522).

**Findings.** 0 MAJOR, 2 minor, 1 cosmetic; all three verified by the lead and accepted (A523); no round-355 disposition reopened. Minor: F356-1 the closed form's remainder O(Σh²/T₀) for O(Σh²/T₀²), the g2 label's attribution; F356-2 the uniqueness argument's monotonicity asserted with the piece below γ₁ written as O(T⁻³) where it is of the cusp's order at γ₁ (coefficient 4N₀(γ₁) = −1.70 against 7/2) and rises against the rest, "exactly two zeros" without its condition. Cosmetic: F356-3 the von Mangoldt attribution's residue in the title, (vi) and Check 7. **Sweep.** The remainder at its order; T·G with the piece below γ₁ exact, its monotonicity computed and gated at the 45 states, the two zeros tied to the condition; von Mangoldt named. Twelve needles (unchanged); manifest 36; conjunct census 124 → 125. Trajectory: 355 (2M+5m+3c) → 356 (0M+2m+1c) → 357 pending.

### Round 357 — the convergence test on the round-356 sweep of Theorem 1ca

**Scope.** The sweep commit (verifier `cascade_smooth_functional.py`: the monotonicity of T·G computed and gated at the 45 states in g1 with the piece below γ₁ exact, the g2 labels' attributions, the docstring; Theorem 1ca's block regenerated: the remainder's order, the uniqueness argument with the piece below γ₁ explicit and the condition on the two zeros, von Mangoldt named in the title, (vi) and Check 7; twelve needles unchanged; manifest 36), and the lead's fix commit 359a0ef9 (A524: `TG`'s quadrature of the piece below γ₁ at the cusp — a garbage value under an IntegrationWarning at the grid's first point, the gated monotonicity unaffected — moved into `below5` with the interval split near the cusp; a g0 witness against a 60-digit quadrature; conjunct census 125 → 126). Full-tower class. Brief `brief_round357.md`. Convergence test on round 356's three findings (A523) and review of the fix.

**Findings.** 0 MAJOR, 3 minor, 1 cosmetic; all four verified by the lead and accepted (A525); no round-356 disposition reopened; the fix verified independently. Minor: F357-1 (vi) listing the uniqueness under "Proved" while (ii) reports its monotonicity as computed — the reviewer's state-independent criterion R(T) < 1 adopted and closed analytically for T ≥ 1.51γ₁ by the elementary bound (8/7)[1 − (x − 1)^{5/2}/(x^{3/2}(x + 2))], computed below it (at most 0.69); F357-2 the two-zeros condition 4m + 7/2 < 2T₀/e stated as exact where it is leading-order (the bracket raises the minimum: −1.48 against −2.28 at δ = 2's ground state); F357-3 the ground state's ε remainder O(γ₁²/T₀²) for O(γ₁²/T₀³) (36.5/T³). Cosmetic: F357-4 the g0 witness on the split branch only. **Sweep.** (ii) rewritten with the criterion, the bound and the computed interval; the minimum of F_k^s′ computed negative at the 45 states (−0.35 or less, gated at −0.3); the ε remainder; (vi) reclassified; the witness on both branches, epsrel 10⁻¹²; two new needles (S_UNIQ, S_FMIN; fourteen); manifest 36; conjunct census 126 → 142 (g0 10 → 15, g1 15 → 19, g6 52 → 59). Trajectory: 355 (2M+5m+3c) → 356 (0M+2m+1c) → 357 (0M+3m+1c) → 358 pending.

### Round 358 — the convergence test on the round-357 sweep of Theorem 1ca

**Scope.** The sweep commit (verifier `cascade_smooth_functional.py`: the uniqueness criterion R(T) computed on 2000 points over (γ₁, 1.508γ₁] with its elementary bound, the 60-digit witnesses of the weight integral, the bound's form, the log-derivative identity, N₀ ≥ −1 and the crossing x*, the minimum of F^s′ at the 45 states, the third witness point of `below5` and its epsrel 10⁻¹², the docstring; Theorem 1ca's block regenerated: (ii)'s uniqueness for every hole set, the two-zeros sentence, the ε remainder, (vi); fourteen needles, two new; manifest 36). Full-tower class. Brief `brief_round358.md`. Convergence test on round 357's four findings (A525).

**Findings.** 0 MAJOR, 1 minor, 4 cosmetic, 1 process; all verified by the lead and accepted (A526); no round-357 disposition reopened. (The first launch was terminated by a model rate limit before any finding; the relaunch found the scratchpad wiped by a container rollback and reconstructed the round from the records.) Minor: F358-1 "far above the minimum in value" an uncomputed universal, false for some hole sets (a single hole at 67.3 at δ = 2 puts the endpoint 0.06 nats below the stationary minimum), true at the 45 states by at least 16 nats. Cosmetic: F358-2 the two-zeros clause without F′ → +∞ at the cusp; F358-3 the computed interval named 1.51γ₁ for 1.508γ₁; F358-4 the label's and docstring's "leading order" residue; F358-5 the docstring's gate summary. Process: F358-6 the brief and cited review scripts lost with the scratchpad. **Sweep.** "The stationary minimum is unique"; the endpoint's value and the local maximum computed at the 45 states and gated (a new needle S_GLOB, fifteen); the cusp coefficient 7/2 + 4N₀(γ₁) = 1.80 stated and witnessed; 1.508γ₁; the label and docstring; `dbelow` given the split form near the cusp with a witness (a lead-noticed warning in the dry run); the brief carried in the launch prompt. Manifest 36; conjunct census 142 → 150 (g0 15 → 17, g1 19 → 22, g6 59 → 62). Trajectory: 355 (2M+5m+3c) → 356 (0M+2m+1c) → 357 (0M+3m+1c) → 358 (0M+1m+4c+1p) → 359 pending.

### Round 359 — the convergence test on the round-358 sweep of Theorem 1ca

**Scope.** The sweep commit (verifier `cascade_smooth_functional.py`: F^s at the domain's endpoint and the local maximum above the cusp at the 45 states, gated; the cusp coefficient of F^s′ witnessed; `dbelow` in the split form near the cusp with its witness; S_UNIQ at 1.508γ₁; the label and docstring; Theorem 1ca's block regenerated: the stationary minimum, the two-zeros clause with F′ → +∞ at both ends, the least value of F_k^s at the 45 states, (vi); fifteen needles, one new; manifest 36). Full-tower class. Brief `brief_round359.md` (its text also inside the launch prompt). Convergence test on round 358's six findings (A526).

**Findings.** 0 MAJOR, 0 minor, 4 cosmetic; all verified by the lead and accepted (A527); no round-358 disposition reopened — **converged: the round-358 sweep is stable.** Cosmetic: F359-1 the docstring's "at least 16 nats" for 15.983 and the g1 label's bound figures printed to nearest; F359-2 the equality point γ₁√x* on the bound's side, and the computed grid's end 1.507984γ₁ named 1.508γ₁; F359-3 the verifier's "1.51 gamma_1" residue; F359-4 "T^s lies in (2T₀/e, 2T₀)" with no conjunct or scope marker. **Sweep (round 360, batching the cosmetics with the five items from the owner's Lean pilot, A527):** R < 1 on (γ₁, ∞) proved by the split bound R₂ = 0.327A + 0.673R̄ enclosed in interval arithmetic on [1, 2.28] (the computed R a cross-check); F^s′(2T₀) > 0 for every hole set once 2T₀ > 17.01, so T^s ∈ (2T₀/e, 2T₀) exactly when F^s′(2T₀/e) < 0, computed and gated (a sixteenth needle S_BRK); (iii)'s third term in its by-parts form with the O(ln T/√T) conclusion from |S₁| ≤ C ln t, and its consequences' two computed inputs named; (iv)'s identity over t_ρ; the E_arch bound with Binet on r ≥ 8, the shoulder factor 1 + 8Δ/T and the cut's tail below T − 3Δ, the Binet bound against the smooth count's integral gated at the 65 windows (S_LEFT extended); the labels' bounds on the safe side. census: 150 → 178 over 74 ok-statements (g0 17 → 31, g1 22 → 26, g5 18 → 20, g6 62 → 70; g2, g3, g4 unchanged) by the AST census against 2233579. Trajectory: 355 (2M+5m+3c) → 356 (0M+2m+1c) → 357 (0M+3m+1c) → 358 (0M+1m+4c+1p) → 359 (0M+0m+4c, converged) → 360 pending.

### Round 360 — the round-360 sweep of Theorem 1ca (the round-359 cosmetics and the Lean-derived items)

**Scope.** The sweep commit (verifier `cascade_smooth_functional.py`: the split bound R₂ and its interval enclosure, the bracket's F^s′ at 2T₀/e and 2T₀ and T_u inside it, the Binet bound against the smooth count's integral, the labels on the safe side, the docstring; Theorem 1ca's block regenerated: (ii)'s uniqueness passage and bracket sentence, (iii)'s bound and consequences, (iv)'s identity and E_arch bound, (vi); sixteen needles, one new; manifest 36). Full-tower class. Brief `brief_round360.md` (its text also inside the launch prompt). Convergence test on round 359's four cosmetics (A527) and first review of the Lean-derived items.

**Findings.** 0 MAJOR, 1 minor, 4 cosmetic; all verified by the lead and accepted (A528); no round-359 disposition reopened; the split bound and its enclosure held under attack (reproduced in mpmath and in python-flint ball arithmetic). Minor: F360-1 the bracket sentence's converse false ("T^s in (2T₀/e, 2T₀) exactly when F^s′(2T₀/e) < 0"; a hole at 34.0 at δ = 2 has F^s′(2T₀/e) = +0.32 with T^s = 84.6 inside). Cosmetic: F360-2 the g5 label's window count 65 for 105; F360-3 the docstring's "1.0x" unfilled; F360-4 six label bounds printed to nearest; F360-5 "two computed inputs" undercounting T^s inside the bracket. **Sweep (round 361):** F^s′ = ln(T/2T₀) + RHS(T) > 0 on all of [2T₀, ∞) for every hole set once 2T₀ > 17.01, so T^s < 2T₀ for every hole set (proved) and T^s > 2T₀/e when F^s′(2T₀/e) < 0 (computed); the consequences' inputs recounted in (iii), (vi) and the docstring; 105 windows; 0.897; `fl`/`ce` on the six labels. Census unchanged at 178 over 74. Trajectory: 355 (2M+5m+3c) → 356 (0M+2m+1c) → 357 (0M+3m+1c) → 358 (0M+1m+4c+1p) → 359 (0M+0m+4c, converged) → 360 (0M+1m+4c) → 361 pending.

### Round 361 — the convergence test on the round-361 sweep of Theorem 1ca

**Scope.** The sweep commit (verifier `cascade_smooth_functional.py`: the docstring's bracket and consequences sentences, the g5 label's window count, six label bounds on the safe side; Theorem 1ca's block regenerated: (ii)'s bracket sentence — T^s < 2T₀ for every hole set proved via F^s′ > 0 on [2T₀, ∞), T^s > 2T₀/e computed — (iii)'s and (vi)'s computed inputs; sixteen needles unchanged; manifest 36). Full-tower class. Brief `brief_round361.md` (its text also inside the launch prompt). Convergence test on round 360's five findings (A528).

**Findings.** 0 MAJOR, 0 minor, 3 cosmetic; all verified by the lead and accepted (A529); no round-360 disposition reopened — **converged: the round-361 sweep is stable and the Theorem 1ca arc (rounds 355–361) is closed.** Cosmetic, batched to the next substantive round on 1ca: F361-1 four bound-worded needle figures rounded to nearest in the unsafe direction (2.1% for 2.12%, 8% for 8.0015%, 4.4 × 10⁻⁶ for 4.418, 1.4 × 10⁻⁴ for 1.432) and the label/needle range conventions disagreeing on one quantity; F361-2 the shoulder clause's regime Δ ≤ T/100 excluding the δ = 2 windows (Δ/T₁ = 1.14%; Δ ≤ T/80 would cover every cell); F361-3 the (iii) sentence's "at the 45 states and the 40 rungs" true only distributively (T_u(1) = 419.86 > 2T₀ = 416.14 at δ = 3.5). Trajectory: 355 (2M+5m+3c) → 356 (0M+2m+1c) → 357 (0M+3m+1c) → 358 (0M+1m+4c+1p) → 359 (0M+0m+4c, converged) → 360 (0M+1m+4c) → 361 (0M+0m+3c, converged).

### Round 362 — the §11 sweep (items 5–6 restated after the Lean pilot's rounds 24–26, the owner's commission) and the round-361 cosmetics on Theorem 1ca

**Scope.** The sweep commit (paper: §11 items 5–6 — the zero-free reduction (a′) + (b) ⇒ RH with D one sufficient condition for (a′), the pilot's `rh_of_prime_side`/`hypConv_of_D` named as corroboration; the shape mechanism for (b) ruled out at the cells via the Pólya-class zero in (0, 2π/a] and δ > 4π/γ₁ = 0.889; Theorem 1ca's block — four needle figures rounded up, the shoulder regime Δ ≤ T/80, the (iii) sentence's scope; verifier `cascade_smooth_functional.py` — the parse-backs by `ce` and `_r1u`, the g0 witness at Δ = T/80, the g1 label's range at nearest; sixteen needles; manifest 36). Full-tower class. Brief `brief_round362.md` (its text also inside the launch prompt). First review of the §11 edits (A530, A531) and convergence test on round 361's three cosmetics (A529).

**Findings.** 0 MAJOR, 1 minor, 5 cosmetic; all verified by the lead and accepted (A532). The §11 reduction and the shape obstruction held under attack; `PrimeSide.lean`'s description found exact. Minor: F362-1 item 6's "the mechanism for (b) there must be tied to the form" an unproved universal (a tent convolved with a box at δ = 1 is real-rooted, not concave, first zero 5π > γ₁: the Laguerre–Pólya closure survives the argument). Cosmetic: F362-2 S_WALL's "within 0.13% rms" for 0.13206 and the docstring's 1.9%; F362-3 the g5 label's 4.4e-06 / 1.4e-04 at nearest; F362-4 three wording imprecisions in the reduction (M only in the cosine basis; 1bu(ii)'s verification of D at seven cells within the tolerance, the rate not proved; D a witness for (a′) only); F362-5 the footer's roadmap parenthetical stale; F362-6 three pilot figures in the paper's text against the header's standard. **Sweep (round 363):** "whatever carries (b) there is not concavity" with the surviving shape route and the form-tied routes named (the pilot's rounds 27–28 — realification not lowering Q, the monotone zero flow, the pinned zeros above the zeta zeros — folded into the attributed parenthetical after "None has been tried" proved false against them); the wording sharpened; the footer parenthetical; the pilot's figures replaced by qualitative, attributed statements; 0.14% with its parse by `ce`; the g5 label by `_r1u`. Census unchanged at 178 over 74. Trajectory of the §11/1ca-cosmetics arc: 362 (0M+1m+5c) → 363 pending.

### Round 363 — the convergence test on the round-363 sweep (§11 items 5–6, the footer's parenthetical, Theorem 1ca's S_WALL)

**Scope.** The sweep commit (paper: §11 item 6's closing of the shape obstruction, the reduction's wording, item 5's closing, the footer's roadmap parenthetical, the block's "within 0.14% rms"; verifier `cascade_smooth_functional.py`: S_WALL's parse by `ce`, the g5 label's two figures by `_r1u` (moved above g0), the docstring's 2.0% and 0.14%; sixteen needles; manifest 36). Full-tower class. Brief `brief_round363.md` (its text also inside the launch prompt). Convergence test on round 362's six findings (A532).

**Findings.** 0 MAJOR, 1 minor, 4 cosmetic; all verified by the lead and accepted (A533); every round-362 disposition held. Minor: F363-1 item 6's heading "Ruled out for (b) at the cells: a shape mechanism" still carried the universal F362-1 struck from its conclusion. Cosmetic: F363-2 the tent/box example's "width" against the paper's full-width convention; F363-3 the flow clause's quantifier beyond the pilot's tracked zeros below 60; F363-4 item 1's closing step attributed to item 6 without its tails clause; F363-5 six latent nearest roundings in a parse and the g5 label (safe today). **Sweep (round 364):** "concavity, Pólya's shape mechanism"; "a tent on [−0.3, 0.3] convolved with a box on [−0.2, 0.2]"; "the zeros it tracked … below a fixed height"; "by item 5's finite step at each support, or by item 6's with vanishing tails"; the parse by `ce`, the label by `_r1u` and `math.ceil`. Census unchanged at 178 over 74. Trajectory of the §11 arc: 362 (0M+1m+5c) → 363 (0M+1m+4c) → 364 pending.

### Round 364 — the convergence test on the round-364 sweep (§11 items 1 and 6; the verifier's latent roundings)

**Scope.** The sweep commit (paper: §11 item 6's "Ruled out" heading, the tent/box example's intervals, the flow clause's scope; item 1's closing step; verifier `cascade_smooth_functional.py`: S_MIN's parse by `ce`, five g5 label figures by `_r1u` and the pole exponent by `math.ceil`; sixteen needles unchanged; manifest 36). Full-tower class. Brief `brief_round364.md` (its text also inside the launch prompt). Convergence test on round 363's five findings (A533).

**Findings.** 0 MAJOR, 0 minor, 2 cosmetic; all verified by the lead and accepted (A534); every round-363 disposition held — **converged: the round-364 sweep is stable and the §11 arc (rounds 362–364) is closed.** Cosmetic, batched to the next substantive round on §11: F364-1 item 1's item-5 route needs T_D(δ) unbounded, which §11 states nowhere (1bu(ii) bundles the tail condition into D; §11 item 6 separates it); F364-2 item 6's 2π/a argument stated for nonnegative g and applied to the ground state, whose sign is unstated (nonnegativity unused). Trajectory of the §11 arc: 362 (0M+1m+5c) → 363 (0M+1m+4c) → 364 (0M+0m+2c, converged).

### Round 365 — the §11 sweep after the Lean pilot's rounds 43–62 (item 6's parenthetical extended with the (b)-free chain and the strip/L² form of (a′); the round-364 cosmetics; item 1's item-5 clause; the footer's routes parenthetical)

**Scope.** The sweep commits 84252dd and its round-63 amendment 4c445cd (paper only: §11 item 6's attributed parenthetical — `theoremD`/`topGS_cross`, Hurwitz for closed sets and ζ ≠ 0 on (0, 1) proved, `rh_of_hypConv_top` from (a′) alone for the top ground state, `rh_of_close_top` from L² closeness o(e^{−ba}/√a) on the strip |Im z| < ½ with Riemann's truncated Φ the kernel family, the pilot's angle measurement reported qualitatively and its RH-strength judgement kept; the amendment after the pilot's round 63 landed mid-battery — Riemann's formula proved on |Im z| < 1, then (round 64, a third sweep commit) on all of ℂ, `rh_of_close_RPhi` from the closeness to Φ alone; the wall sentence; F364-1's T_D clause in item 1; F364-2's nonnegativity parenthetical; the held note's asymptotic; the footer's routes parenthetical; item 6 rewrapped at 76). Prose-only class. Brief `brief_round365.md` (its text also inside the launch prompt). First review of the edit (A535) and convergence test on round 364's two cosmetics (A534).

**Findings.** 0 MAJOR, 2 minor, 2 cosmetic; all verified by the lead and accepted (A536); every clause of the pilot parenthetical checked against the Lean statements, the rewrap shown word-preserving, F364-1/F364-2 verified. Minor: F365-1 the footer's new routes clause classified Riemann's integral formula for Ξ over Φ as not an input while Theorem 1au's moment instrument consumes it (the ½ calibration and the coefficient route; 1ax's deformation); F365-2 item 6's "reformulation" asserts an equivalence the pilot proves in one direction only and calls presumable. Cosmetic: F365-3 the strip weakening named after `rh_of_close_top` instead of `rh_of_hypConvStrip_top`; F365-4 "the probes on which Q attains its minimum" for the Rayleigh quotient's minimisers. An out-of-scope observation on the pilot's README (the formula's constant 2 against its own theorem's 4). **Sweep (round 366):** the formula appended to the classical-inputs census and removed from the routes clause; "a reduction of the hypothesis to one rate — presumably a reformulation, not a proof"; the two names; "the Rayleigh quotient of Q"; the held note adopted ("which fixes the ground state's sign"). Trajectory of the §11/pilot arc: 365 (0M+2m+2c) → 366 pending.

### Round 366 — the convergence test on the round-366 sweep (§11 item 6's parenthetical; the footer's census)

**Scope.** The sweep commits a8fac02, 87ea956 and c8499ee (paper only: the footer's routes clause and the appended census entry for Riemann's integral formula; item 6's "a reduction of the hypothesis to one rate — presumably a reformulation, not a proof", `rh_of_hypConvStrip_top`/`rh_of_close_top`, "the Rayleigh quotient of Q", "which fixes the ground state's sign"; item 6 rewrapped at 76). Prose-only class. Brief `brief_round366.md` (its text also inside the launch prompt). Convergence test on round 365's four findings (A536).

**Findings.** 0 MAJOR, 0 minor, 3 cosmetic; all verified by the lead and accepted (A537); every round-365 disposition held — **converged: the round-366 sweep is stable and the §11/pilot arc (rounds 365–366) is closed.** Cosmetic, batched to the next substantive round on §11: F366-1 the census entry names one consuming verifier where Theorem 1aw's `cascade_floor_meter.py` consumes the same identity; F366-2 "named in 1ax's de Bruijn–Newman deformation" names the kernel, not the formula; F366-3 item 6's kernel-family clause omits "over [−a, a]" and Φ's decay (A535's text). Two out-of-scope observations (the README's constant; the records' pre-round-65 line pointers). Trajectory of the §11/pilot arc: 365 (0M+2m+2c) → 366 (0M+0m+3c, converged).

### Round 367 — the working note `riemann-strip-target-note.md` (the strip target's theoretical attempt; the owner's commission after A537)
...
### Round 377 — the working note's new §3.4: the derivation of the pilot's constant 1/(16π)

**Scope.** Commit db95c80 (the note only: §3.4 added; header, §3.3 and §4 updated; the three batched round-376 cosmetics applied). No tower obligation. Brief `brief_round377.md` (its text also inside the launch prompt).

**Findings.** 0 MAJOR, 4 minor, 7 cosmetic; all verified by the lead and accepted (A549); the mathematics of §3.4 held under the reviewer's independent recomputation (the paper's explicit balayage density integrated directly). Minor: F377-1 "30 digits" for a 19-digit quadrature; F377-2 a spliced status quotation; F377-3 the moment identity attributed to round 62 (it is round 40); F377-4 the count constant named as a discreteness effect on the moment (it contributes zero; through the wall shift it is second order and — the lead's correction of the reviewer's sign — of the wrong sign, 13%). Cosmetic: F377-5 the quarter-exponent gloss; F377-6 README formatting; F377-7 two κ(a); F377-8 round 73 uncited; F377-9 "because"; F377-10 the cross-check's two objects; F377-11 the ephemeral script path. Swept in fc25dcb. Convergence test: round 378.

### Round 378 — the convergence test on the round-377 sweep of the working note

**Scope.** The sweep commit fc25dcb (the note only: the thirteen edits of A549). No tower obligation. Brief `brief_round378.md` (its text also inside the launch prompt). Convergence test on round 377's eleven findings (A549).

**Findings.** 0 MAJOR, 3 minor, 4 cosmetic; all verified by the lead and accepted (A550); the mathematics of §3.4 held under a second independent recomputation; nine of round 377's dispositions realised, two sweep sentences found newly wrong. Minor: F378-1 the cross-check's objects (the paper's curvatures are the prime-side Gram's, the same object as the pilot's — the sweep's attribution to the zero-side model wrong); F378-2 "13%" compares coefficients one order apart (true ratio 1.8% at δ = 2, falling); F378-3 a sign-fluctuating ripple named as the home of a steady correction. Cosmetic: F378-4 Q's digit count; F378-5 the angle list's source; F378-6 the header's "30 digits"; F378-7 the Gaussian law at the wall as extrapolation. Swept in 3300392. Convergence test: round 379.

### Round 379 — the convergence test on the round-378 sweep of the working note, with the round-74 citation
...
### Round 381 — the Theorem 1bm(iv) landing: the balayage density in closed form and the admissibility proof

**Scope.** Commits 502f996 (the paper: 1bm(iv)'s closed form and proof, net-state markers at (iv) and (vi), the footer's classical inputs) and bb317a5 (the working note's two net-state clauses). Prose-only class; battery owed and run: the tower precheck (36/36), `cascade_slack_law.py` (13/13), `cascade_smooth_functional.py` (9/9), by the lead and independently by the reviewer. Brief `brief_round381.md` (its text also inside the launch prompt).

**Findings.** 0 MAJOR, 1 minor, 2 cosmetic; all verified by the lead and accepted (A553); every proof step recomputed symbolically and at 40 digits; no over-claim; no conjectural status changed. Minor: F381-1 the pilot's check script over-described ("checks it at 30 digits" for a print script without assertion). Cosmetic: F381-2 the second elementary integral unattributed; F381-3 the closed form absent from (vi)'s proved items. Swept in ceeec08; the pilot's rounds 76 (the Lean formalisation) and 77 (the arccosh deficit) cited in 27aa22f. Convergence test: round 382.

### Round 382 — the convergence test on the round-381 sweep of Theorem 1bm(iv), with the round-76/77 follow-up

**Scope.** Commits ceeec08 (the sweep) and 27aa22f (the pilot's rounds 76–77 cited; the arccosh deficit). Prose-only class; battery owed and run on 27aa22f: the tower precheck (36/36), `cascade_slack_law.py` (13/13), `cascade_smooth_functional.py` (9/9), by the lead and independently by the reviewer. Brief `brief_round382.md` (its text also inside the launch prompt). Convergence test on round 381's three findings (A553).

**Findings.** 0 MAJOR, 0 minor, 2 cosmetic; both verified by the lead and accepted (A554); every round-381 disposition realised; the proof held a second time; the round-76 and round-77 citations exact — **converged: the 1bm(iv) closed-form landing is stable (rounds 381–382).** Cosmetic, applied editorially in e74b4cf: F382-1 the π/4 moment's derivation; F382-2 the block's Check-7 list. Battery on e74b4cf (lead, 20:45–20:58 UTC, `scratchpad/r382/*_final.log`): `run_tower.py` — manifest integrity 36 members verified, 4 keying files pinned; paper-needle precheck 105 reach files scanned, 45 declared surfaces verified live; ckpt_key probes 24/24; precheck probes 85/85; census 0 live PASS + 36 cached PASS + 0 FAIL of 36; TOWER PASS (36/36). `cascade_slack_law.py` — ALL GATES PASS (13/13). `cascade_smooth_functional.py` — ALL GATES PASS (9/9), 25 REUSED, 0 recomputed.

### Round 383 — the working note's §5: the prime-side ledger at the cells, and its instrument

**Scope.** Commits fd1c5f8 (§5 and `tools/research/strip_note/ledger.py`), 96967ec and 31e9476 (the pilot's rounds 78–80 cited), aaf53d9 (the 120-digit δ = 3 validation). The paper and every verifier unchanged; no battery owed (the tower prechecks re-run by the reviewer, 36/36). Brief `brief_round383.md` (its text also inside the launch prompt).

**Findings.** 1 MAJOR, 6 minor, 6 cosmetic; all verified by the lead and accepted (A555); the instrument reproduced byte-for-byte and its pieces verified against every Gram entry. MAJOR: F383-1 §5.3(iv)'s "the projection roughly triples the energy" — false, from a normalisation slip (√(2∫Φ) for √(2∫Φ²)) in an uncommitted session script; the projection changes the energy by under 1%. Minor: F383-2 the zero-side validation uncommitted; F383-3 1bl miscalled a Temple certificate; F383-4 1bm(v) for 1bm(vii), the offset's sign and ℓ ≠ −ln λ₁; F383-5 c(δ)'s share of the offsets; F383-6 "three parts in 10⁶" at δ = 3; F383-7 the even sector and the Fejér–Riesz class. Cosmetic: F383-8 the (1 − η) factor; F383-9 instrument and log housekeeping; F383-10 "every line"; F383-11 the heat-flow accuracy sentence; F383-12 the δ = 3 table's constant row; F383-13 the e^{−2πn} law's owner. Swept (the note, the instrument, the new `zeroside.py`). Convergence test: round 384.

### Round 384 — the convergence test on the round-383 sweep of the working note's §5 and its instruments

**Scope.** Commits 263828a (the sweep) and e1e0828 (the zero-side log). The paper and every verifier unchanged; no battery owed (the tower prechecks re-run by the reviewer, 36/36). Brief `brief_round384.md` (its text also inside the launch prompt). Convergence test on round 383's thirteen findings (A555).

**Findings.** 0 MAJOR, 2 minor, 5 cosmetic; all verified by the lead and accepted (A556); the corrected §5.3(iv) and the instruments verified independently. Minor: F384-1 the F383-4 sweep's growth term pointed the wrong way; F384-2 "within 1% at ln 4" (3.9%). Cosmetic: F384-3 the δ = 3 log not regenerated; F384-4 the zero-side runtime; F384-5 "the paper's residuals" for the same quantity on another instrument; F384-6 the tail quadrature's resolution; F384-7 the JSON's storage precision. Swept (the note, both instruments, both logs and JSONs regenerated; the pilot's rounds 81–82 cited). Convergence test: round 385.

### Round 385 — the convergence test on the round-384 sweep of the working note's §5 and its instruments

**Scope.** Commits 784db69 (the sweep) and d3121ad (the regenerated δ = 3 artefacts; round 83 cited). The paper and every verifier unchanged; no battery owed (the tower prechecks re-run by the reviewer, 36/36). Brief `brief_round385.md` (its text also inside the launch prompt). Convergence test on round 384's seven findings (A556).

**Findings.** 0 MAJOR, 2 minor, 6 cosmetic; all verified by the lead and accepted (A557); every round-384 disposition realised; the JSON recombination verified to 10⁻²³. Minor: F385-1 the zero-side display broken by the precision change (its log unreproducible); F385-2 the tail caveat's "up to 6.5%" not a maximum. Cosmetic: F385-3 round 81's dating; F385-4 the header's accounting; F385-5 §5.4's closing sentence (now the Hellmann–Feynman identity); F385-6 round 77's percentages; F385-7 "on the positive form"; F385-8 the header's trichotomy. Swept (the note, `zeroside.py`; its log regenerated in a follow-up commit). Convergence test: round 386.
