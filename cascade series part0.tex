...
the maxima are structurally stable (they exist for any value of the
...
= 1.0990\times 10^{-120}
\]
is the unique scalar built from all four distinguished sphere areas under
the constraints of the recurrence. $I$ is the unique order-0 (global)
cascade invariant; it depends only on the four distinguished dimensions
and is immune to subleading corrections from inter-layer structure. The
...
they combine uniquely with the cascade's two natural maxima, and evaluates
the result.
...
1 & 7 & $\pi^4/3$ & Area maximum (natural zero of $p$) \\
...
values of the recurrence constant. Two are maxima: $d_V = 5$ (volume
maximum) and $d_0 = 7$ (area maximum). These four exhaust the Gamma
...
\begin{proof}
(i)~At a stationary point, the recurrence ratio equals unity: no net
output is produced. The sphere area at a stationary point is the
maximum---it measures the recurrence's capacity, not its output. It serves
as a reference scale. Two stationary points (volume and area) give two
references. The relationship between two references is their ratio:
$\Omega_{d_V}/\Omega_{d_0}$.
...
Scale dimensions cannot be content: a stationary point marks the
recurrence's capacity, which exists for any $K$. Including it as a
...
classification imposed by hand. The maxima are scale dimensions because
...
unique dimensionless scalar relating the cascade's two maxima.
\end{theorem}

\begin{proof}
By Corollary~\ref{cor:primary}, sphere area is the unique independent
cascade quantity at each level. The ratio of sphere areas at the two
maxima is therefore the unique dimensionless relationship between them.
Evaluation:
$\Omega_5 = \pi^3$,\;
$\Omega_7 = \pi^4/3$,\;
ratio $= 3/\pi$.
...
\begin{remark}
$3/\pi < 1$ because $d_V < d_0$: the volume maximum is interior to the
area maximum, and sphere areas decrease inward from their peak. The
cascade's geometric capacity at the volume maximum is $3/\pi\approx 95.5\%$
of its capacity at the area maximum. The deficit is the cost of the two
recurrence steps between $d_0 = 7$ and $d_V = 5$, each governed by the
same $\sqrt{\pi}$ forced by orthogonality
(Theorem~\ref{thm:sqrtpi}).
\end{remark}
...
Completeness is not an axiom; it is a consequence of Theorem~\ref{thm:tower}.
The Gamma function provides exactly four distinguished dimensions. An
invariant that uses $k < 4$ depends on an arbitrary selection of $k$
from~4, introducing a choice not present in the recurrence. The invariant
that uses all four introduces no choice: $\Omega_5$, $\Omega_7$,
$\Omega_{19}$, $\Omega_{217}$.
...
\emph{(5)~Ratio for scale.} The scale dimensions (maxima) combine as a
ratio (Theorem~\ref{thm:cs}): $\Omega_5/\Omega_7$.

\emph{(6)~Scale exponent.} By Lemma~\ref{lem:bilinear}, the product
$\Omega_{19}\times\Omega_{217}$ contains $\Omega_{d_0}^{\,2}$ at
exponent exactly~2 (first-order recurrence, two factors). The scale
ratio replaces $\Omega_{d_0}$ with $\Omega_{d_V}$ in each factor,
giving exponent~2 on $\Omega_{d_V}/\Omega_{d_0}$.

The direction $\Omega_5/\Omega_7 < 1$ (not inverted) is forced: the
complete invariant must satisfy $I \leq I_0$, because incorporating the
scale structure refines the content---it cannot create content beyond
what the partial invariant provides. Since $\Omega_5 < \Omega_7$ (the
volume maximum is interior to the area maximum, and sphere areas decrease
below their peak), $(\Omega_5/\Omega_7)^2 < 1$ and $I < I_0$. The
inverse $(\Omega_7/\Omega_5)^2 > 1$ would give $I > I_0$, violating
$I \leq I_0$.
...
The agreement is $0.1\%$, fifty times smaller than the $1\sigma$
...
The full invariant ($0.1\%$) dominates the partial invariant ($10\%$). The
volume-ratio variant ($1.7\%$) confirms that sphere areas, not volumes, are
primary (Corollary~\ref{cor:primary}): using derived quantities instead of
primary quantities costs an order of magnitude in precision. No alternative
primitive set comes within an order of magnitude of $10^{-120}$.
...
function: the locations and sphere areas of its maxima and thresholds.
...
The order-0 invariant $I$ matches a physical constant to $0.1\%$. The
order-1 and order-2 predictions in the companion papers match to
$1$--$3\%$. This is expected: higher-order invariants depend on finer
geometric structure and accumulate subleading corrections from inter-layer
coupling. The order-0 invariant is immune to these corrections because it
depends only on the four distinguished dimensions, which are global
...
11 & Maxima structurally stable; thresholds sensitive & Thms 8.2, 8.3 \\
12 & Content--scale separation from stability & Thm 8.4 \\
13 & Content: product $\Omega_{19}\times\Omega_{217}$ & Thm 8.5 \\
14 & Scale: ratio $\Omega_5/\Omega_7 = 3/\pi$; exponent~2 & Thms 8.6, 8.7; Lem 8.8 \\
15 & $I = 9\,\Omega_{19}\,\Omega_{217}/\pi^2$; unique & Thm 8.8 \\
16 & $I = 1.0990\times 10^{-120}$ & Computation \\
17 & $120\approx\pi e^{2\sqrt{\pi}}(2\sqrt{\pi}-1)/\ln 10$ & Thm 9.1 \\
\bottomrule
\end{tabular}
\end{center}
...
Maxima exist for all $K$; thresholds shift by orders of
magnitude.\;$\checkmark$
