...
$\Omega_{d_V} \leq \Omega_{d_0}$, with equality only if $d_V = d_0$.
\end{lemma}

\begin{proof}
$d_0$ is the sphere-area maximum: $\Omega_{d_0}\geq\Omega_d$ for all $d$.
In particular $\Omega_{d_0}\geq\Omega_{d_V}$. Equality requires
$d_V = d_0$, which fails for the slicing recurrence because
$V_d = \Omega_{d-1}/d$ shifts the volume peak below the area peak
(dividing a unimodal sequence by a strictly increasing sequence shifts the
maximum to a lower index).
\end{proof}
...
\emph{(6)~Scale exponent and direction.} By Lemma~\ref{lem:bilinear}, the
product $\Omega_{19}\times\Omega_{217}$ contains $\Omega_{d_0}^{\,2}$ at
exponent exactly~2 (first-order recurrence, two factors). The scale
ratio replaces $\Omega_{d_0}$ with $\Omega_{d_V}$ in each factor,
giving exponent~2 on $\Omega_{d_V}/\Omega_{d_0}$. The direction
$\Omega_{d_V}/\Omega_{d_0} < 1$ is forced by the scale ordering
(Lemma~\ref{lem:ordering}): $d_0$ is the area maximum, so no sphere area
exceeds $\Omega_{d_0}$.
