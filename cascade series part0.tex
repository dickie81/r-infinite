...
cascade: the volume maximum ($d_V = 5$), the boundary-area peak
($d_0 = 7$: the unique ball whose boundary has maximum area, equivalently
the first integer in the decay regime), and the two thresholds
($d_1 = 19$, $d_2 = 217$). No fifth exists.
...
($V_d^{\,n}$ satisfies $V_{d+1}^{\,n}/V_d^{\,n} = K^n\cdot R(d)^n$), or
...
two factors (one per threshold, exponent~1 each by constraint~3). By
Lemma~\ref{lem:bilinear}, each factor, expressed via the recurrence from
$d_0$, contributes one power of $\Omega_{d_0}$. Two factors, two powers:
the product $\Omega_{19}\times\Omega_{217}$ contains
$\Omega_{d_0}^{\,2}$ at exponent exactly~2. The scale correction replaces
$\Omega_{d_0}$ with $\Omega_{d_V}$ in each factor, giving exponent~2 on
$\Omega_{d_V}/\Omega_{d_0}$. The exponent is not chosen; it is counted.
The direction $\Omega_{d_V}/\Omega_{d_0} < 1$ is forced by the scale
ordering (Lemma~\ref{lem:ordering}).
...
11 & Equilibria structurally stable; thresholds sensitive & Thms 8.2, 8.3 \\
